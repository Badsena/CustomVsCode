...
export async function activate(context: vscode.ExtensionContext) {
	...
	const startFolderAccessMonitor = (projectPath: string): void => {
		...
		const psScript = [
			...
			'  } catch {}',
			'}',
			'$editorNames = @("code","cursor","notepad","notepad++","sublime_text","atom","idea64","webstorm64","pycharm64","rider64","gedit","kate","vim","nvim","wordpad","write")',
			'try {',
			...
			'      if ($editorNames -contains $nameKey) {',
			'        if ($cmd.ToLower() -like "*amypo*workspace*" -or $cmd.ToLower() -like "*$fn*") {',
			'          $violations.Add("$($wp.Name)|$($wp.ProcessId)|CmdLine")',
			'        }',
			'      }',
			'    } catch {}',
			'  }',
