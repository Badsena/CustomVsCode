...
export function createTextInputActions(clipboardService, logService) {
    return [
        toAction({ id: 'undo', label: localize(3805, null), run: () => getActiveDocument().execCommand('undo') }),
        toAction({ id: 'redo', label: localize(3806, null), run: () => getActiveDocument().execCommand('redo') }),
        new Separator(),
        toAction({
            id: 'editor.action.clipboardCutAction', label: localize(3807, null), run: () => {
                logService.trace('TextInputActionsProvider#cut');
                getActiveDocument().execCommand('cut');
            }
        }),
        toAction({
            id: 'editor.action.clipboardCopyAction', label: localize(3808, null), run: () => {
        ...
        new Separator(),
        toAction({ id: 'editor.action.selectAll', label: localize(3810, null), run: () => getActiveDocument().execCommand('selectAll') })
    ];
}
