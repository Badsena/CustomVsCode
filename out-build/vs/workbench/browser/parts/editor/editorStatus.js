...
const nlsSingleSelectionRange = localize(4373, null);
const nlsSingleSelection = localize(4374, null);
const nlsMultiSelectionRange = localize(4375, null);
const nlsMultiSelection = localize(4376, null);
const nlsEOLLF = localize(4377, null);
const nlsEOLCRLF = localize(4378, null);
let EditorStatus = class EditorStatus extends Disposable {
    ...
    async showIndentationPicker() {
        ...
        if (!activeTextEditorControl) {
            return this.quickInputService.pick([{ label: localize(4379, null) }]);
        }
        if (this.editorService.activeEditor?.isReadonly()) {
            return this.quickInputService.pick([{ label: localize(4380, null) }]);
        }
        const picks = [
        ...
        picks.splice(3, 0, { type: 'separator', label: localize(4381, null) });
        picks.unshift({ type: 'separator', label: localize(4382, null) });
        const action = await this.quickInputService.pick(picks, { placeHolder: localize(4383, null), matchOnDetail: true });
        return action?.run();
    }
    updateTabFocusModeElement(visible) {
        if (visible) {
            if (!this.tabFocusModeElement.value) {
                const text = localize(4384, null);
                this.tabFocusModeElement.value = this.statusbarService.addEntry({
                    name: localize(4385, null),
                    text,
                    ariaLabel: text,
                    tooltip: localize(4386, null),
    ...
    updateInputModeElement(inputMode) {
        if (inputMode === 'overtype') {
            if (!this.inputModeElement.value) {
                const text = localize(4387, null);
                const name = localize(4388, null);
    ...
    updateColumnSelectionModeElement(visible) {
        if (visible) {
            if (!this.columnSelectionModeElement.value) {
                const text = localize(4389, null);
                this.columnSelectionModeElement.value = this.statusbarService.addEntry({
                    name: localize(4390, null),
                    text,
                    ariaLabel: text,
                    tooltip: localize(4391, null),
    ...
    updateSelectionElement(text) {
        ...
        const props = {
            name: localize(4392, null),
            text,
            ariaLabel: text,
            tooltip: localize(4393, null),
    ...
    updateIndentationElement(text) {
        ...
        const props = {
            name: localize(4394, null),
            text,
            ariaLabel: text,
            tooltip: localize(4395, null),
    ...
    updateEncodingElement(text) {
        ...
        const props = {
            name: localize(4396, null),
            text,
            ariaLabel: text,
            tooltip: localize(4397, null),
    ...
    updateEOLElement(text) {
        ...
        const props = {
            name: localize(4398, null),
            text,
            ariaLabel: text,
            tooltip: localize(4399, null),
    ...
    updateLanguageIdElement(text) {
        ...
        const props = {
            name: localize(4400, null),
            text,
            ariaLabel: text,
            tooltip: localize(4401, null),
    ...
    updateMetadataElement(text) {
        ...
        const props = {
            name: localize(4402, null),
            text,
            ariaLabel: text,
            tooltip: localize(4403, null)
        };
            ...
            if (model) {
                ...
                update.indentation = (modelOpts.insertSpaces
                    ? modelOpts.tabSize === modelOpts.indentSize
                        ? localize(4404, null, modelOpts.indentSize)
                        : localize(4405, null, modelOpts.indentSize, modelOpts.tabSize)
                    : localize(4406, null, modelOpts.tabSize));
            }
        }
        ...
        if (this.hasToUpdateStatus(previousMarker, this.currentMarker)) {
            if (this.currentMarker) {
                ...
                if (!this.statusBarEntryAccessor.value) {
                    this.statusBarEntryAccessor.value = this.statusbarService.addEntry({ name: localize(4407, null), text, ariaLabel: text }, 'statusbar.currentProblem', 0 /* StatusbarAlignment.LEFT */);
                }
                else {
                    this.statusBarEntryAccessor.value.update({ name: localize(4408, null), text, ariaLabel: text });
                }
            }
            else {
...
export class ChangeLanguageAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: localize2(4437, 'Change Language Mode'),
            f1: true,
            keybinding: {
            ...
            metadata: {
                description: localize(4409, null),
                args: [
                    {
                        name: localize(4410, null),
    ...
    async run(accessor, languageMode) {
        ...
        if (!activeTextEditorControl) {
            await quickInputService.pick([{ label: localize(4411, null) }]);
            return;
        }
        ...
        const picks = languages
            ...
            if (currentLanguageName === languageName) {
                description = localize(4412, null, languageId);
            }
            else {
                description = localize(4413, null, languageId);
            }
            return {
                id: languageId,
                label: languageName,
        ...
        picks.unshift({ type: 'separator', label: localize(4414, null) });
        ...
        if (hasLanguageSupport && resource) {
            ...
            if (galleryService.isEnabled()) {
                galleryAction = toAction({
                    ...
                    label: localize(4415, null, ext),
                    run: () => commandService.executeCommand('workbench.extensions.action.showExtensionsForLanguage', ext)
                });
                picks.unshift(galleryAction);
            }
            configureLanguageSettings = { label: localize(4416, null, currentLanguageName) };
            picks.unshift(configureLanguageSettings);
            configureLanguageAssociations = { label: localize(4417, null, ext) };
            picks.unshift(configureLanguageAssociations);
        }
        // Offer to "Auto Detect", but only if the document is not empty.
        const autoDetectLanguage = { label: localize(4418, null) };
        if (textModel && textModel.getValueLength() > 0) {
            picks.unshift(autoDetectLanguage);
        }
        const pick = typeof languageMode === 'string' ? { label: languageMode } : await quickInputService.pick(picks, { placeHolder: localize(4419, null), matchOnDescription: true });
        if (!pick) {
            return;
        }
    ...
    configureFileAssociation(resource, languageService, quickInputService, configurationService) {
        ...
        const picks = languages.map(({ languageName, languageId }) => {
            return {
                ...
                label: languageName,
                iconClasses: getIconClassesForLanguageId(languageId),
                description: (languageId === currentAssociation) ? localize(4420, null) : undefined
            };
        });
        setTimeout(async () => {
            const language = await quickInputService.pick(picks, { placeHolder: localize(4421, null, extension || base) });
            if (language) {
...
export class ChangeEOLAction extends Action2 {
    constructor() {
        super({
            ...
            title: localize2(4438, 'Change End of Line Sequence'),
            f1: true
        });
    }
    async run(accessor) {
        ...
        const activeTextEditorControl = getCodeEditor(editorService.activeTextEditorControl);
        if (!activeTextEditorControl) {
            await quickInputService.pick([{ label: localize(4422, null) }]);
            return;
        }
        if (editorService.activeEditor?.isReadonly()) {
            await quickInputService.pick([{ label: localize(4423, null) }]);
            return;
        }
        ...
        const eol = await quickInputService.pick(EOLOptions, { placeHolder: localize(4424, null), activeItem: EOLOptions[selectedIndex] });
        if (eol) {
...
export class ChangeEncodingAction extends Action2 {
    constructor() {
        super({
            ...
            title: localize2(4439, 'Change File Encoding'),
            f1: true
        });
    }
    async run(accessor) {
        ...
        const activeTextEditorControl = getCodeEditor(editorService.activeTextEditorControl);
        if (!activeTextEditorControl) {
            await quickInputService.pick([{ label: localize(4425, null) }]);
            return;
        }
        const activeEditorPane = editorService.activeEditorPane;
        if (!activeEditorPane) {
            await quickInputService.pick([{ label: localize(4426, null) }]);
            return;
        }
        const encodingSupport = toEditorWithEncodingSupport(activeEditorPane.input);
        if (!encodingSupport) {
            await quickInputService.pick([{ label: localize(4427, null) }]);
            return;
        }
        const saveWithEncodingPick = { label: localize(4428, null) };
        const reopenWithEncodingPick = { label: localize(4429, null) };
        ...
        if (encodingSupport instanceof UntitledTextEditorInput) {
            ...
            action = await quickInputService.pick([reopenWithEncodingPick, saveWithEncodingPick], { placeHolder: localize(4430, null), matchOnDetail: true });
        }
        if (!action) {
            return;
        }
        ...
        if (guessedEncoding && configuredEncoding !== guessedEncoding && SUPPORTED_ENCODINGS[guessedEncoding]) {
            ...
            picks.unshift({ id: guessedEncoding, label: SUPPORTED_ENCODINGS[guessedEncoding].labelLong, description: localize(4431, null) });
        }
        const encoding = await quickInputService.pick(picks, {
            placeHolder: isReopenWithEncoding ? localize(4432, null) : localize(4433, null),
        ...
        if (typeof encoding.id !== 'undefined' && activeEncodingSupport) {
            ...
            if (isReopenWithEncoding && editorService.activeEditorPane.input.isDirty()) {
                const { confirmed } = await dialogService.confirm({
                    message: localize(4434, null),
                    detail: localize(4435, null),
                    primaryButton: localize(4436, null)
                });
                if (!confirmed) {
                    return;
                }
