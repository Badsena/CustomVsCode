...
let GlobalCompositeBar = class GlobalCompositeBar extends Disposable {
    ...
    static { this.ACCOUNTS_ICON = registerIcon('accounts-view-bar-icon', Codicon.account, localize(4453, null)); }
            ...
            actionViewItemProvider: (action, options) => {
                ...
                if (action.id === ACCOUNTS_ACTIVITY_ID) {
                    return this.instantiationService.createInstance(AccountsActivityActionViewItem, this.contextMenuActionsProvider, {
                        ...
                        actions.unshift(...[
                            toAction({ id: 'hideAccounts', label: localize(4454, null), run: () => setAccountsActionVisible(storageService, false) }),
                            new Separator()
                        ]);
                    });
                }
    ...
    getContextMenuActions() {
        return [toAction({ id: 'toggleAccountsVisibility', label: localize(4456, null), checked: this.accountsVisibilityPreference, run: () => this.accountsVisibilityPreference = !this.accountsVisibilityPreference })];
    }
...
let AccountsActivityActionViewItem = class AccountsActivityActionViewItem extends AbstractGlobalActivityActionViewItem {
    ...
    async resolveMainMenuActions(accountsMenu, disposables) {
        ...
        if (!this.initialized) {
            const noAccountsAvailableAction = disposables.add(new Action('noAccountsAvailable', localize(4458, null), undefined, false));
            ...
            for (const providerId of registeredProviders) {
                ...
                if (!accounts) {
                    if (this.problematicProviders.has(providerId)) {
                        const providerUnavailableAction = disposables.add(new Action('providerUnavailable', localize(4459, null, provider.label), undefined, false));
            ...
            for (const providerId of dynamicProviders) {
                ...
                const manageDynamicAuthProvidersAction = toAction({
                    ...
                    label: localize(4463, null),
                    enabled: true,
                    run: () => this.commandService.executeCommand('workbench.action.removeDynamicAuthenticationProviders')
                });
                if (!accounts) {
                    if (this.problematicProviders.has(providerId)) {
                        const providerUnavailableAction = disposables.add(new Action('providerUnavailable', localize(4464, null, provider.label), undefined, false));
                        menus.push(providerUnavailableAction);
                        // try again in the background so that if the failure was intermittent, we can resolve it on the next showing of the menu
                        try {
                            await this.addAccountsFromProvider(providerId);
...
let GlobalActivityActionViewItem = class GlobalActivityActionViewItem extends AbstractGlobalActivityActionViewItem {
    ...
    computeTitle() {
        return this.userDataProfileService.currentProfile.isDefault ? super.computeTitle() : localize(4468, null, this.userDataProfileService.currentProfile.name);
    }
};
...
function simpleActivityContextMenuActions(storageService, isAccount) {
    ...
    if (isAccount) {
        currentElementContextMenuActions.push(toAction({ id: 'hideAccounts', label: localize(4469, null), run: () => setAccountsActionVisible(storageService, false) }), new Separator());
    }
    return [
        ...currentElementContextMenuActions,
        toAction({ id: 'toggle.hideAccounts', label: localize(4470, null), checked: isAccountsActionVisible(storageService), run: () => setAccountsActionVisible(storageService, !isAccountsActionVisible(storageService)) }),
        toAction({ id: 'toggle.hideManage', label: localize(4471, null), checked: true, enabled: false, run: () => { throw new Error('"Manage" can not be hidden'); } })
    ];
}
