...
export class NotificationsAlerts extends Disposable {
    ...
    doTriggerAriaAlert(notification) {
        ...
        if (notification.severity === Severity.Error) {
            alertText = localize(4497, null, notification.message.linkedText.toString());
        }
        else if (notification.severity === Severity.Warning) {
            alertText = localize(4498, null, notification.message.linkedText.toString());
        }
        else {
            alertText = localize(4499, null, notification.message.linkedText.toString());
        }
        alert(alertText);
    }
}
