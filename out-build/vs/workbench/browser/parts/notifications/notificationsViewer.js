        ...
        for (const node of message.linkedText.nodes) {
            if (typeof node === 'string') {
                ...
                if (!title && node.href.startsWith('command:')) {
                    title = localize(4542, null, node.href.substr('command:'.length));
                }
                else if (!title) {
                    title = node.href;
                }
                        ...
                        getActions() {
                            ...
                            if (isNotificationSource(source)) {
                                ...
                                actions.push(toAction({
                                    id: source.id,
                                    label: isSourceFiltered ? localize(4544, null, source.label) : localize(4545, null, source.label),
...
let NotificationTemplateRenderer = class NotificationTemplateRenderer extends Disposable {
    ...
    renderSource(notification, sourceCustomHover) {
        if (notification.expanded && notification.source) {
            this.template.source.textContent = localize(4546, null, notification.source);
