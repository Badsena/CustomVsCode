...
let NotificationAccessibilityProvider = class NotificationAccessibilityProvider {
    ...
    getAriaLabel(element) {
        ...
        if (this._configurationService.getValue('accessibility.verbosity.notification')) {
            accessibleViewHint = keybinding ? localize(4520, null, keybinding) : localize(4521, null);
        }
        if (!element.source) {
            return withSeverityPrefix(accessibleViewHint ? localize(4522, null, element.message.raw, accessibleViewHint) : localize(4523, null, element.message.raw), element.severity);
        }
        return withSeverityPrefix(accessibleViewHint ? localize(4524, null, element.message.raw, element.source, accessibleViewHint) : localize(4525, null, element.message.raw, element.source), element.severity);
    }
    getWidgetAriaLabel() {
        return this._options.widgetAriaLabel ?? localize(4526, null);
    }
    getRole() {
