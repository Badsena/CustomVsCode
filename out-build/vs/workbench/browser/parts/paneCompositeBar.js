...
let PaneCompositeBar = class PaneCompositeBar extends Disposable {
    ...
    getContextMenuActionsForComposite(compositeId) {
        ...
        actions.push(new SubmenuAction('moveToMenu', localize(4547, null), moveActions));
        // Reset Location
        if (defaultLocation !== currentLocation) {
            actions.push(toAction({
                id: 'resetLocationAction', label: localize(4548, null), run: () => {
            ...
            if (viewContainerModel.allViewDescriptors.length === 1) {
                ...
                if (defaultContainer !== viewContainer) {
                    actions.push(toAction({
                        id: 'resetLocationAction', label: localize(4549, null), run: () => {
    ...
    createMoveAction(viewContainer, newLocation, defaultLocation) {
        return toAction({
            ...
            label: newLocation === 1 /* ViewContainerLocation.Panel */ ? localize(4550, null) : newLocation === 0 /* ViewContainerLocation.Sidebar */ ? localize(4551, null) : localize(4552, null),
            run: () => {
                let index;
