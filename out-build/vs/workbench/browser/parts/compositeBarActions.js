...
let CompositeBarActionViewItem = class CompositeBarActionViewItem extends BaseActionViewItem {
    ...
    computeTitle() {
        ...
        let title = this.keybindingLabel ? localize(3922, null, this.compositeBarActionItem.name, this.keybindingLabel) : this.compositeBarActionItem.name;
...
let CompositeOverflowActivityActionViewItem = class CompositeOverflowActivityActionViewItem extends CompositeBarActionViewItem {
    ...
    getActions() {
        return this.getOverflowingComposites().map(composite => {
            ...
            if (suffix) {
                action.label = localize(3924, null, composite.name, suffix);
            }
            else {
...
let CompositeActionViewItem = class CompositeActionViewItem extends CompositeBarActionViewItem {
    ...
    showContextMenu(container) {
        ...
        if (isPinned) {
            this.toggleCompositePinnedAction.label = localize(3925, null, this.compositeBarActionItem.name);
            this.toggleCompositePinnedAction.checked = false;
            this.toggleCompositePinnedAction.enabled = this.compositeBar.getPinnedCompositeIds().length > 1;
        }
        else {
            this.toggleCompositePinnedAction.label = localize(3926, null, this.compositeBarActionItem.name);
        ...
        if (isBadgeEnabled) {
            this.toggleCompositeBadgeAction.label = localize(3927, null);
        }
        else {
            this.toggleCompositeBadgeAction.label = localize(3928, null);
        }
...
export class ToggleCompositePinnedAction extends Action {
    constructor(activity, compositeBar) {
        super('show.toggleCompositePinned', activity ? activity.name : localize(3929, null));
...
export class ToggleCompositeBadgeAction extends Action {
    constructor(compositeBarActionItem, compositeBar) {
        super('show.toggleCompositeBadge', compositeBarActionItem ? compositeBarActionItem.name : localize(3930, null));
