...
const noDataProviderMessage = localize(4657, null);
...
let AbstractTreeView = class AbstractTreeView extends Disposable {
    ...
    initializeShowCollapseAllAction(startingValue = false) {
        if (!this.collapseAllContext) {
            this.collapseAllContextKey = new RawContextKey(`treeView.${this.id}.enableCollapseAll`, startingValue, localize(4658, null, this.id));
    ...
    initializeShowRefreshAction(startingValue = false) {
        if (!this.refreshContext) {
            this.refreshContextKey = new RawContextKey(`treeView.${this.id}.enableRefresh`, startingValue, localize(4659, null, this.id));
    ...
    initializeCollapseAllToggle() {
        if (!this.collapseAllToggleContext) {
            this.collapseAllToggleContextKey = new RawContextKey(`treeView.${this.id}.toggleCollapseAll`, false, localize(4662, null, this.id));
    ...
    constructor(notificationService, getSelectedResources) {
        ...
        this._register(this.onDidRun(e => {
            if (e.error && !isCancellationError(e.error)) {
                notificationService.error(localize(4663, null, e.error.message, e.action.id));
            }
        }));
    }
