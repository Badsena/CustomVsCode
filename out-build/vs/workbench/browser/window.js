...
let BaseWindow = class BaseWindow extends Disposable {
    ...
    static async confirmOnShutdown(accessor, reason) {
        ...
        const message = reason === 2 /* ShutdownReason.QUIT */ ?
            (isMacintosh ? localize(4682, null) : localize(4683, null)) :
            localize(4684, null);
        const primaryButton = reason === 2 /* ShutdownReason.QUIT */ ?
            (isMacintosh ? localize(4685, null) : localize(4686, null)) :
            localize(4687, null);
        const res = await dialogService.confirm({
            message,
            primaryButton,
            checkbox: {
                label: localize(4688, null)
            }
        });
    ...
    onWillShutdown() {
        ...
        Event.toPromise(Event.any(Event.once(new DomEmitter(mainWindow.document.body, EventType.KEY_DOWN, true).event), Event.once(new DomEmitter(mainWindow.document.body, EventType.MOUSE_DOWN, true).event))).then(async () => {
            ...
            await this.dialogService.prompt({
                type: Severity.Error,
                message: localize(4689, null),
                detail: localize(4690, null),
                buttons: [
                    {
                        label: localize(4691, null),
            ...
            openExternal: async (href) => {
                    ...
                    if (isSafari) {
                        ...
                        if (!opened) {
                            await this.dialogService.prompt({
                                ...
                                message: localize(4692, null),
                                custom: {
                                    markdownDetails: [{ markdown: new MarkdownString(localize(4693, null, 'https://aka.ms/allow-vscode-popup'), true) }]
                                },
                                buttons: [
                                    {
                                        label: localize(4694, null),
                ...
                // handling explicitly to prevent the workbench from going down.
                    ...
                    const showProtocolUrlOpenedDialog = async () => {
                        ...
                        const buttons = [
                            {
                                label: localize(4695, null),
                                run: () => invokeProtocolHandler()
                            }
                        ];
                        if (downloadUrl !== undefined) {
                            detail = localize(4696, null, this.productService.nameLong, this.productService.nameLong);
                            buttons.push({
                                label: localize(4697, null),
                                run: async () => {
                            ...
                            detail = localize(4698, null, this.productService.nameLong, this.productService.nameLong);
                        }
