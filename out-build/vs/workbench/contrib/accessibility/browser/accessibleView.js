...
let AccessibleView = class AccessibleView extends Disposable {
    ...
    configureKeybindings(unassigned) {
        ...
        quickPick.title = localize(5386, null);
        quickPick.placeholder = localize(5387, null);
        quickPick.show();
    ...
    _convertTokensToSymbols(tokens, symbols) {
        ...
        for (const token of tokens) {
            ...
            if (label) {
                symbols.push({ markdownToParse: label, label: localize(5388, null, token.type, label), ariaLabel: localize(5389, null, token.type, label), firstListItem });
    ...
    disableHint() {
        ...
        alert(localize(5390, null, this._currentProvider.verbositySettingKey));
    }
    ...
    _render(provider, container, showAccessibleViewHelp, updatedContent) {
        ...
        this._getTextModel(stableUri).then((model) => {
            ...
            if (verbose && !showAccessibleViewHelp && hasActions) {
                actionsHint = provider.options.position ? localize(5391, null) : localize(5392, null);
            }
            let ariaLabel = provider.options.type === "help" /* AccessibleViewType.Help */ ? localize(5393, null) : localize(5394, null);
            this._title.textContent = ariaLabel;
            if (actionsHint && provider.options.type === "view" /* AccessibleViewType.View */) {
                ariaLabel = localize(5395, null, actionsHint);
            }
            else if (actionsHint) {
                ariaLabel = localize(5396, null, actionsHint);
            }
    ...
    _updateToolbar(providedActions, type) {
        this._toolbar.setAriaLabel(type === "help" /* AccessibleViewType.Help */ ? localize(5397, null) : localize(5398, null));
    ...
    _accessibleViewHelpDialogContent(providerHasSymbols) {
        ...
        const toolbarHint = localize(5399, null);
        const chatHints = this._getChatHints();
        let hint = localize(5400, null);
        if (navigationHint) {
            hint += ' - ' + navigationHint + '\n';
        }
        if (goToSymbolHint) {
    ...
    _getChatHints() {
        ...
        return [localize(5401, null, '<keybinding:workbench.action.chat.insertCodeBlock>'),
            localize(5402, null, '<keybinding:workbench.action.chat.insertIntoNewFile>'),
            localize(5403, null, '<keybinding:workbench.action.chat.runInTerminal>')].join('\n');
    }
    _navigationHint() {
        return localize(5404, null, `<keybinding:${"editor.action.accessibleViewNext" /* AccessibilityCommandId.ShowNext */}>`, `<keybinding:${"editor.action.accessibleViewPrevious" /* AccessibilityCommandId.ShowPrevious */}>`);
    }
    _disableVerbosityHint(provider) {
        if (provider.options.type === "help" /* AccessibleViewType.Help */ && this._verbosityEnabled()) {
            return localize(5405, null, `<keybinding:${"editor.action.accessibleViewDisableHint" /* AccessibilityCommandId.DisableVerbosityHint */}>`);
        }
        return '';
    }
    _goToSymbolHint(providerHasSymbols) {
        ...
        return localize(5406, null, `<keybinding:${"editor.action.accessibleViewGoToSymbol" /* AccessibilityCommandId.GoToSymbol */}>`);
    }
    _configureUnassignedKbHint() {
        ...
        return localize(5407, null, keybindingToConfigureQuickPick);
    }
    _configureAssignedKbHint() {
        ...
        return localize(5408, null, keybindingToConfigureQuickPick);
    }
    ...
    _exitDialogHint(provider) {
        return this._verbosityEnabled() && !provider.options.position ? localize(5409, null) : '';
    }
    _readMoreHint(provider) {
        return provider.options.readMoreUrl ? localize(5410, null, `<keybinding:${"editor.action.accessibilityHelpOpenHelpLink" /* AccessibilityCommandId.AccessibilityHelpOpenHelpLink */}>`) : '';
    }
};
...
let AccessibleViewService = class AccessibleViewService extends Disposable {
    ...
    getOpenAriaHint(verbositySettingKey) {
        ...
        let hint = null;
        if (keybinding) {
            hint = localize(5411, null, keybinding);
        }
        else {
            hint = localize(5412, null);
        }
        return hint;
    }
    disableHint() {
...
let AccessibleViewSymbolQuickPick = class AccessibleViewSymbolQuickPick {
    ...
    show(provider) {
        ...
        const quickPick = disposables.add(this._quickInputService.createQuickPick());
        quickPick.placeholder = localize(5413, null);
        quickPick.title = localize(5414, null);
        const picks = [];
