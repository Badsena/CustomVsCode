...
let LanguageStatus = class LanguageStatus {
    ...
    _update() {
        ...
        if (model.combined.length === 0) {
            ...
            const props = {
                name: localize(11853, null),
                ariaLabel: localize(11854, null, ariaLabels.join(', next: ')),
    ...
    _renderStatus(container, status, showSeverity, isPinned, store) {
        ...
        const actionLabel = isPinned ? localize(11855, null) : localize(11856, null);
    ...
    static _accessibilityInformation(status) {
        ...
        if (status.detail) {
            return { label: localize(11857, null, textValue, status.detail) };
        }
        else {
            return { label: localize(11858, null, textValue) };
        }
    }
    // ---
    static _asStatusbarEntry(item) {
        ...
        return {
            name: localize(11859, null, item.name),
...
export class ResetAction extends Action2 {
    constructor() {
        super({
            ...
            title: localize2(11860, "Reset Language Status Interaction Counter"),
