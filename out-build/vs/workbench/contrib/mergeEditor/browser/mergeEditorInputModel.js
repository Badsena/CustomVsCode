    ...
    async confirmClose(inputModels) {
        ...
        if (someDirty) {
            ...
            const message = isMany
                ? localize(12621, null, inputModels.length)
                : localize(12622, null, basename(inputModels[0].model.resultTextModel.uri));
    ...
    async save(options) {
        ...
        (async () => {
            const { confirmed } = await this.dialogService.confirm({
                message: localize(12630, null),
                detail: localize(12631, null),
                primaryButton: localize(12632, null)
            });
            if (confirmed) {
                await this.accept();
...
let WorkspaceMergeEditorModeFactory = class WorkspaceMergeEditorModeFactory {
    ...
    static { this.FILE_SAVED_SOURCE = SaveSourceRegistry.registerSource('merge-editor.source', localize(12633, null)); }
    async createInputModel(args) {
        const store = new DisposableStore();
...
let WorkspaceMergeEditorInputModel = class WorkspaceMergeEditorInputModel extends EditorModel {
    ...
    */
    ...
    async confirmClose(inputModels) {
        ...
        if (someDirty) {
            const message = isMany
                ? localize(12634, null, inputModels.length)
                : localize(12635, null, basename(inputModels[0].resultUri));
            ...
            const { confirmed, checkboxChecked } = await this._dialogService.confirm({
                message: isMany
                    ? localize(12643, null, inputModels.length)
                    : localize(12644, null, basename(inputModels[0].resultUri)),
                detail: someUnhandledConflicts ?
                    isMany
                        ? localize(12645, null)
                        : localize(12646, null)
                ...
                primaryButton: someUnhandledConflicts
                    ? localize(12647, null)
                    : localize(12648, null),
                checkbox: { label: localize(12649, null) }
            });
            if (checkboxChecked) {
