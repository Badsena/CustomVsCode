    ...
    constructor(inputNumber, viewModel, instantiationService, contextMenuService, configurationService) {
        ...
        this._register(autorunOpts({ debugName: `input${this.inputNumber}: update labels & text model` }, reader => {
            ...
            const title = this.inputNumber === 1
                ? vm.model.input1.title || localize(12691, null)
                : vm.model.input2.title || localize(12692, null);
    ...
    getContextMenuActions() {
        ...
        return [
            this.baseRange.input1Diffs.length > 0
                ? action('mergeEditor.acceptInput1', localize(12693, null, this.model.input1.title), state.toggle(1), state.includesInput1)
                : undefined,
            this.baseRange.input2Diffs.length > 0
                ? action('mergeEditor.acceptInput2', localize(12694, null, this.model.input2.title), state.toggle(2), state.includesInput2)
                : undefined,
            this.baseRange.isConflicting
                ? setFields(action('mergeEditor.acceptBoth', localize(12695, null), state.withInputValue(1, !both).withInputValue(2, !both), both), { enabled: this.baseRange.canBeCombined })
                : undefined,
            new Separator(),
            this.baseRange.isConflicting
                ? setFields(action('mergeEditor.swap', localize(12696, null), state.swap(), false), { enabled: !state.kind && (!both || this.baseRange.isOrderRelevant) })
                : undefined,
            setFields(new Action('mergeEditor.markAsHandled', localize(12697, null), undefined, true, () => {
                transaction((tx) => {
...
export class MergeConflictGutterItemView extends Disposable {
    constructor(item, target, contextMenuService) {
        ...
        this._register(autorun(reader => {
            ...
            const iconMap = {
                [0 /* InputState.excluded */]: { icon: undefined, checked: false, title: localize(12698, null) },
                [3 /* InputState.unrecognized */]: { icon: Codicon.circleFilled, checked: false, title: localize(12699, null) },
                [1 /* InputState.first */]: { icon: Codicon.check, checked: true, title: localize(12700, null) },
                [2 /* InputState.second */]: { icon: Codicon.checkAll, checked: true, title: localize(12701, null) },
            };
