    ...
    constructor(viewModel, modifiedBaseRange) {
        ...
        this.resultItems = derived(this, reader => {
            ...
            if (state.kind === ModifiedBaseRangeStateKind.unrecognized) {
                result.push({
                    text: localize(12678, null),
                    tooltip: localize(12679, null),
                });
            }
            else if (state.kind === ModifiedBaseRangeStateKind.base) {
                result.push({
                    text: localize(12680, null),
                    tooltip: localize(12681, null),
                });
            }
            else {
                const labels = [];
            ...
            const stateToggles = [];
            if (state.includesInput1) {
                stateToggles.push(command(localize(12682, null, model.input1.title), async () => {
                    transaction((tx) => {
                ...
                }, localize(12683, null, model.input1.title)));
            }
            if (state.includesInput2) {
                stateToggles.push(command(localize(12684, null, model.input2.title), async () => {
                    transaction((tx) => {
                ...
                }, localize(12685, null, model.input2.title)));
            }
            ...
            if (state.kind === ModifiedBaseRangeStateKind.unrecognized) {
                result.push(command(localize(12686, null), async () => {
                    transaction((tx) => {
        ...
        return derived(reader => {
            ...
            if (state.kind !== ModifiedBaseRangeStateKind.unrecognized && !state.isInputIncluded(inputNumber)) {
                if (!state.isInputIncluded(otherInputNumber) || !this.viewModel.shouldUseAppendInsteadOfAccept.read(reader)) {
                    result.push(command(localize(12667, null, inputData.title), async () => {
                        transaction((tx) => {
                    ...
                    result.push(command(localize(12672, null, inputData.title), async () => {
                        transaction((tx) => {
                            model.setState(modifiedBaseRange, state.withInputValue(inputNumber, true, false), inputNumber, tx);
                            model.telemetry.reportAcceptInvoked(inputNumber, state.includesInput(otherInputNumber));
                        });
                    }, localize(12673, null, inputData.title)));
                    if (modifiedBaseRange.canBeCombined) {
                        result.push(command(localize(12674, null, inputData.title), async () => {
                            transaction((tx) => {
                ...
                if (!model.isInputHandled(modifiedBaseRange, inputNumber).read(reader)) {
                    result.push(command(localize(12676, null), async () => {
                        transaction((tx) => {
