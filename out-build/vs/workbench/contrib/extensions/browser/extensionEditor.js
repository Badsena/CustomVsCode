...
class VersionWidget extends ExtensionWithDifferentGalleryVersionWidget {
    constructor(container, hoverService) {
        ...
        this._register(hoverService.setupManagedHover(getDefaultHoverDelegate('mouse'), this.element, localize(10465, null)));
        this.render();
    }
    render() {
...
let ExtensionEditor = class ExtensionEditor extends EditorPane {
    ...
    createEditor(parent) {
        ...
        this._register(this.hoverService.setupManagedHover(getDefaultHoverDelegate('mouse'), name, localize(10466, null)));
        const versionWidget = new VersionWidget(title, this.hoverService);
        const preview = append(title, $('span.preview'));
        this._register(this.hoverService.setupManagedHover(getDefaultHoverDelegate('mouse'), preview, localize(10467, null)));
        preview.textContent = localize(10468, null);
        const builtin = append(title, $('span.builtin'));
        builtin.textContent = localize(10469, null);
    ...
    renderNavbar(extension, manifest, template, preserveFocus) {
        ...
        template.navbar.push("readme" /* ExtensionEditorTab.Readme */, localize(10470, null), localize(10471, null));
        if (manifest) {
            template.navbar.push("features" /* ExtensionEditorTab.Features */, localize(10472, null), localize(10473, null));
        }
        if (extension.hasChangelog()) {
            template.navbar.push("changelog" /* ExtensionEditorTab.Changelog */, localize(10474, null), localize(10475, null));
        }
        if (extension.dependencies.length) {
            template.navbar.push("dependencies" /* ExtensionEditorTab.Dependencies */, localize(10476, null), localize(10477, null));
        }
        if (manifest && manifest.extensionPack?.length && !this.shallRenderAsExtensionPack(manifest)) {
            template.navbar.push("extensionPack" /* ExtensionEditorTab.ExtensionPack */, localize(10478, null), localize(10479, null));
        }
        if (this.options?.tab) {
    ...
    async openDetails(extension, contentContainer, token) {
        ...
        if (manifest && manifest.extensionPack?.length && this.shallRenderAsExtensionPack(manifest)) {
            ...
            activeElement = await this.openMarkdown(extension, this.extensionReadme.get(), localize(10480, null), contentContainer, 0 /* WebviewIndex.Readme */, localize(10481, null), token);
        }
    ...
    async openExtensionPackReadme(extension, manifest, container, token) {
        ...
        extensionPackHeader.textContent = localize(10482, null, manifest.extensionPack.length);
        const extensionPackContent = append(extensionPack, $('div', { class: 'extension-pack-content' }));
        extensionPackContent.setAttribute('tabindex', '0');
        const readmeContent = append(extensionPackReadme, $('div.readme-content'));
        await Promise.all([
            this.renderExtensionPack(manifest, extensionPackContent, token),
            this.openMarkdown(extension, this.extensionReadme.get(), localize(10483, null), readmeContent, 0 /* WebviewIndex.Readme */, localize(10484, null), token),
        ]);
    ...
    async openChangelog(extension, contentContainer, token) {
        const activeElement = await this.openMarkdown(extension, this.extensionChangelog.get(), localize(10485, null), contentContainer, 1 /* WebviewIndex.Changelog */, localize(10486, null), token);
        return activeElement;
    }
    ...
    openExtensionDependencies(extension, contentContainer, token) {
        ...
        if (arrays.isFalsyOrEmpty(extension.dependencies)) {
            append(contentContainer, $('p.nocontent')).textContent = localize(10487, null);
...
let AdditionalDetailsWidget = class AdditionalDetailsWidget extends Disposable {
    ...
    renderCategories(container, extension) {
        if (extension.categories.length) {
            ...
            append(categoriesContainer, $('.additional-details-title', undefined, localize(10488, null)));
    ...
    renderExtensionResources(container, extension) {
        ...
        if (extension.repository) {
            try {
                resources.push([localize(10489, null), ThemeIcon.fromId(Codicon.repo.id), URI.parse(extension.repository)]);
            }
            catch (error) { /* Ignore */ }
        }
        if (extension.supportUrl) {
            try {
                resources.push([localize(10490, null), ThemeIcon.fromId(Codicon.issues.id), URI.parse(extension.supportUrl)]);
            }
            catch (error) { /* Ignore */ }
        }
        if (extension.licenseUrl) {
            try {
                resources.push([localize(10491, null), ThemeIcon.fromId(Codicon.linkExternal.id), URI.parse(extension.licenseUrl)]);
            }
            catch (error) { /* Ignore */ }
        }
        ...
        if (extension.url) {
            resources.push([localize(10492, null), ThemeIcon.fromId(Codicon.linkExternal.id), URI.parse(extension.url)]);
        }
        if (resources.length || extension.publisherSponsorLink) {
            const extensionResourcesContainer = append(container, $('.resources-container.additional-details-element'));
            append(extensionResourcesContainer, $('.additional-details-title', undefined, localize(10493, null)));
    ...
    renderInstallInfo(container, extension) {
        ...
        append(installInfoContainer, $('.additional-details-title', undefined, localize(10494, null)));
        const installInfo = append(installInfoContainer, $('.more-info'));
        append(installInfo, $('.more-info-entry', undefined, $('div.more-info-entry-name', undefined, localize(10495, null)), $('code', undefined, extension.identifier.id)));
        if (extension.type !== 0 /* ExtensionType.System */) {
            append(installInfo, $('.more-info-entry', undefined, $('div.more-info-entry-name', undefined, localize(10496, null)), $('code', undefined, extension.manifest.version)));
        }
        if (extension.installedTimestamp) {
            append(installInfo, $('.more-info-entry', undefined, $('div.more-info-entry-name', undefined, localize(10497, null)), $('div', {
        ...
        if (!extension.isBuiltin && extension.source !== 'gallery') {
            const element = $('div', undefined, extension.source === 'vsix' ? localize(10498, null) : localize(10499, null));
            append(installInfo, $('.more-info-entry', undefined, $('div.more-info-entry-name', undefined, localize(10500, null)), element));
        ...
        if (extension.size) {
            ...
            append(installInfo, $('.more-info-entry', undefined, $('div.more-info-entry-name', { title: localize(10501, null) }, localize(10502, null)), element));
        ...
        this.getCacheLocation(extension).then(cacheLocation => {
            ...
            computeSize(cacheLocation, this.fileService).then(cacheSize => {
                ...
                append(installInfo, $('.more-info-entry', undefined, $('div.more-info-entry-name', { title: localize(10503, null) }, localize(10504, null)), element));
                if (isNative && extension.location.scheme === Schemas.file) {
                    element.classList.add('link');
                    element.tabIndex = 0;
                    element.setAttribute('role', 'link');
    ...
    renderMarketplaceInfo(container, extension) {
        ...
        append(moreInfoContainer, $('.additional-details-title', undefined, localize(10505, null)));
        const moreInfo = append(moreInfoContainer, $('.more-info'));
        if (gallery) {
            if (!extension.local) {
                append(moreInfo, $('.more-info-entry', undefined, $('div.more-info-entry-name', undefined, localize(10506, null)), $('code', undefined, extension.identifier.id)));
                append(moreInfo, $('.more-info-entry', undefined, $('div.more-info-entry-name', undefined, localize(10507, null)), $('code', undefined, gallery.version)));
            }
            append(moreInfo, $('.more-info-entry', undefined, $('div.more-info-entry-name', undefined, localize(10508, null)), $('div', {
                'title': new Date(gallery.releaseDate).toString()
            }, fromNow(gallery.releaseDate, true, true, true))), $('.more-info-entry', undefined, $('div.more-info-entry-name', undefined, localize(10509, null)), $('div', {
