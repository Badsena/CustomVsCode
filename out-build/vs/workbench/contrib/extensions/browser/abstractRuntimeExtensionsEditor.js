    ...
    createEditor(parent) {
        ...
        const renderer = {
            ...
            renderElement: (element, index, data) => {
                ...
                if (activationTimes) {
                    ...
                    if (activationEvent === '*') {
                        title = nls.localize(10445, null, activationId);
                        ...
                        if (fileNameOrGlob.indexOf('*') >= 0 || fileNameOrGlob.indexOf('?') >= 0) {
                            title = nls.localize(10446, null, fileNameOrGlob, activationId);
                            ...
                            title = nls.localize(10447, null, fileNameOrGlob, activationId);
                        ...
                        title = nls.localize(10448, null, glob, activationId);
                        ...
                        title = nls.localize(10449, null, activationId);
                        ...
                        title = nls.localize(10450, null, language, activationId);
                    }
                    else {
                        title = nls.localize(10451, null, activationEvent, activationId);
                ...
                if (this._getUnresponsiveProfile(element.description.identifier)) {
                    ...
                    const extensionHostFreezTitle = nls.localize(10453, null);
                ...
                if (isNonEmptyArray(element.status.runtimeErrors)) {
                    const el = $('span', undefined, ...renderLabelWithIcons(`$(bug) ${nls.localize(10454, null, element.status.runtimeErrors.length)}`));
                    data.msgContainer.appendChild(el);
                }
                ...
                for (const feature of features) {
                    ...
                    if (accessData) {
                        ...
                        if (accessData?.accessTimes.length > 0) {
                            const element = $('span', undefined, `${nls.localize(10455, null, feature.label, accessData.accessTimes.length)}${accessData.current ? nls.localize(10456, null, accessData.current.accessTimes.length) : ''}`);
                            if (accessData.current) {
                                const title = nls.localize(10457, null, fromNow(accessData.current.lastAccessed, true, true));
        ...
        this._list = this._instantiationService.createInstance((WorkbenchList), 'RuntimeExtensions', parent, delegate, [renderer], {
            ...
            accessibilityProvider: new class {
                getWidgetAriaLabel() {
                    return nls.localize(10458, null);
                }
        ...
        this._register(this._list.onContextMenu((e) => {
            ...
            const actions = [];
            actions.push(new Action('runtimeExtensionsEditor.action.copyId', nls.localize(10459, null, e.element.description.identifier.value), undefined, true, () => {
            ...
            if (e.element.marketplaceInfo) {
                actions.push(new Action('runtimeExtensionsEditor.action.disableWorkspace', nls.localize(10460, null), undefined, true, () => this._extensionsWorkbenchService.setEnablement(e.element.marketplaceInfo, 11 /* EnablementState.DisabledWorkspace */)));
                actions.push(new Action('runtimeExtensionsEditor.action.disable', nls.localize(10461, null), undefined, true, () => this._extensionsWorkbenchService.setEnablement(e.element.marketplaceInfo, 10 /* EnablementState.DisabledGlobally */)));
            }
            actions.push(new Separator());
...
export class ShowRuntimeExtensionsAction extends Action2 {
    constructor() {
        super({
            ...
            title: nls.localize2(10462, "Show Running Extensions"),
