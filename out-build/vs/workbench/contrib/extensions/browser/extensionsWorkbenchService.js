        ...
        if (this.gallery) {
            ...
            if (!cache) {
                if (this.gallery.assets.manifest) {
                    ...
                    this.logService.error(nls.localize(10996, null), this.identifier.id);
                }
            }
            return cache;
        }
...
let ExtensionsWorkbenchService = class ExtensionsWorkbenchService extends Disposable {
    ...
    async updateAutoUpdateForAllExtensions(isAutoUpdateEnabled) {
        ...
        const result = await this.dialogService.confirm({
            title: nls.localize(10997, null),
            message: isAutoUpdateEnabled
                ? nls.localize(10998, null)
                : nls.localize(10999, null),
            detail: nls.localize(11000, null),
        });
        if (!result.confirmed) {
            return;
        }
    ...
    computeExtensionsNotifications() {
        ...
        if (disallowedExtensions.length) {
            computedNotificiations.push({
                message: this.configurationService.inspect(AllowedExtensionsConfigKey).policy
                    ? nls.localize(11002, null)
                    : nls.localize(11003, null),
        ...
        if (invalidExtensions.length) {
            if (invalidExtensions.some(e => e.local && e.local.manifest.engines?.vscode &&
                ...
                computedNotificiations.push({
                    message: nls.localize(11004, null),
                    severity: Severity.Warning,
                ...
                computedNotificiations.push({
                    message: nls.localize(11005, null),
                    severity: Severity.Warning,
                    extensions: invalidExtensions,
        ...
        if (!this.configurationService.getValue(AutoRestartConfigurationKey)) {
            ...
            if (restartRequiredExtensions.length) {
                ...
                computedNotificiations.push({
                    message: needsReload
                        ? nls.localize(11006, null)
                        : nls.localize(11007, null),
                    severity: Severity.Info,
                    extensions: restartRequiredExtensions,
                    query: '@restartrequired',
                    action: {
                        label: needsReload
                            ? nls.localize(11008, null)
                            : nls.localize(11009, null),
                        run: () => {
                            if (needsReload) {
        ...
        if (deprecatedExtensions.length) {
            computedNotificiations.push({
                message: nls.localize(11010, null),
                severity: Severity.Warning,
    ...
    async updateRunningExtensions(message = nls.localize(11011, null), auto = false) {
        const toAdd = [];
        const toRemove = [];
            ...
            if (await this.extensionService.stopExtensionHosts(message, auto)) {
                ...
                if (auto) {
                    this.notificationService.notify({
                        severity: Severity.Info,
                        message: nls.localize(11012, null),
    ...
    getRuntimeState(extension) {
        ...
        const reloadActionLabel = reloadAction === "reloadWindow" /* ExtensionRuntimeActionType.ReloadWindow */ ? nls.localize(11013, null) : nls.localize(11014, null);
        if (isUninstalled) {
            ...
            if (!canRemoveRunningExtension && isSameExtensionRunning && !runningExtension.isUnderDevelopment) {
                return { action: reloadAction, reason: nls.localize(11015, null, reloadActionLabel) };
            }
            return undefined;
        }
        if (extension.local) {
            ...
            if (runningExtension) {
                if (isEnabled) {
                    ...
                    if (isSameExtensionRunning) {
                        ...
                        if (!runningExtension.isUnderDevelopment && (extension.version !== runningExtension.version || extension.local.targetPlatform !== runningExtension.targetPlatform)) {
                            ...
                            if (productUpdateVersion
                                ...
                                if (state.type === "available for download" /* StateType.AvailableForDownload */) {
                                    return { action: "downloadUpdate" /* ExtensionRuntimeActionType.DownloadUpdate */, reason: nls.localize(11016, null, this.productService.nameLong) };
                                }
                                if (state.type === "downloaded" /* StateType.Downloaded */) {
                                    return { action: "applyUpdate" /* ExtensionRuntimeActionType.ApplyUpdate */, reason: nls.localize(11017, null, this.productService.nameLong) };
                                }
                                if (state.type === "ready" /* StateType.Ready */) {
                                    return { action: "quitAndInstall" /* ExtensionRuntimeActionType.QuitAndInstall */, reason: nls.localize(11018, null, this.productService.nameLong) };
                                }
                                return undefined;
                            }
                            return { action: reloadAction, reason: nls.localize(11019, null, reloadActionLabel) };
                        }
                        if (this.extensionsServers.length > 1) {
                            ...
                            if (extensionInOtherServer) {
                                ...
                                if (runningExtensionServer === this.extensionManagementServerService.remoteExtensionManagementServer && this.extensionManifestPropertiesService.prefersExecuteOnUI(extension.local.manifest) && extensionInOtherServer.server === this.extensionManagementServerService.localExtensionManagementServer) {
                                    return { action: reloadAction, reason: nls.localize(11020, null, reloadActionLabel) };
                                }
                                ...
                                if (runningExtensionServer === this.extensionManagementServerService.localExtensionManagementServer && this.extensionManifestPropertiesService.prefersExecuteOnWorkspace(extension.local.manifest) && extensionInOtherServer.server === this.extensionManagementServerService.remoteExtensionManagementServer) {
                                    return { action: reloadAction, reason: nls.localize(11021, null, reloadActionLabel, this.extensionManagementServerService.remoteExtensionManagementServer?.label) };
                                }
                            }
                        }
                    }
                        ...
                        if (extension.server === this.extensionManagementServerService.localExtensionManagementServer && runningExtensionServer === this.extensionManagementServerService.remoteExtensionManagementServer) {
                            ...
                            if (this.extensionManifestPropertiesService.prefersExecuteOnUI(extension.local.manifest)) {
                                return { action: reloadAction, reason: nls.localize(11022, null, reloadActionLabel) };
                            }
                        }
                        if (extension.server === this.extensionManagementServerService.remoteExtensionManagementServer && runningExtensionServer === this.extensionManagementServerService.localExtensionManagementServer) {
                            ...
                            if (this.extensionManifestPropertiesService.prefersExecuteOnWorkspace(extension.local.manifest)) {
                                return { action: reloadAction, reason: nls.localize(11023, null, reloadActionLabel) };
                            }
                        }
                    }
                    return undefined;
                    ...
                    if (isSameExtensionRunning) {
                        return { action: reloadAction, reason: nls.localize(11024, null, reloadActionLabel) };
                    }
                }
                return undefined;
            }
            // Extension is not running
                ...
                if (isEnabled && !this.extensionService.canAddExtension(toExtensionDescription(extension.local))) {
                    return { action: reloadAction, reason: nls.localize(11025, null, reloadActionLabel) };
                }
                ...
                if (otherServer && extension.enablementState === 1 /* EnablementState.DisabledByExtensionKind */) {
                    ...
                    if (extensionInOtherServer && extensionInOtherServer.local && this.extensionEnablementService.isEnabled(extensionInOtherServer.local)) {
                        return { action: reloadAction, reason: nls.localize(11026, null, reloadActionLabel) };
                    }
                }
            }
        }
    ...
    async downloadVSIX(extensionId, versionKind) {
        ...
        if (!galleryExtension) {
            throw new Error(nls.localize(11027, null, extensionId));
        }
        ...
        for (const targetPlatform of version?.targetPlatforms ?? galleryExtension.allTargetPlatforms) {
            if (targetPlatform !== "unknown" /* TargetPlatform.UNKNOWN */ && targetPlatform !== "universal" /* TargetPlatform.UNIVERSAL */) {
                options.push({
                    label: targetPlatform === "undefined" /* TargetPlatform.UNDEFINED */ ? nls.localize(11028, null) : TargetPlatformToString(targetPlatform),
                    id: targetPlatform
                });
            }
        }
        if (options.length > 1) {
            const message = nls.localize(11029, null);
        ...
        const result = await this.fileDialogService.showOpenDialog({
            title: nls.localize(11030, null),
            canSelectFiles: false,
            canSelectFolders: true,
            canSelectMany: false,
            openLabel: nls.localize(11031, null),
        });
        if (!result?.[0]) {
            return;
        }
        this.progressService.withProgress({ location: 15 /* ProgressLocation.Notification */ }, async (progress) => {
            try {
                progress.report({ message: nls.localize(11032, null) });
                ...
                this.notificationService.info(nls.localize(11033, null));
            }
            catch (error) {
                this.notificationService.error(nls.localize(11034, null, getErrorMessage(error)));
            }
        });
    }
    async pickVersionToDownload(extensionId) {
        ...
        if (!allVersions.length) {
            await this.dialogService.info(nls.localize(11035, null));
            return;
        }
        const picks = allVersions.map((v, i) => {
            return {
                id: v.version,
                label: v.version,
                description: `${fromNow(new Date(Date.parse(v.date)), true)}${v.isPreReleaseVersion ? ` (${nls.localize(11036, null)})` : ''}`,
                ariaLabel: `${v.isPreReleaseVersion ? 'Pre-Release version' : 'Release version'} ${v.version}`,
                data: v,
            };
        });
        const pick = await this.quickInputService.pick(picks, {
            placeHolder: nls.localize(11037, null),
            matchOnDetail: true
        });
        return pick?.data;
    }
    ...
    async shouldRequireConsentToUpdate(extension) {
        ...
        if (extension.local.identifier.uuid && extension.local.identifier.uuid !== extension.gallery.identifier.uuid) {
            return nls.localize(11038, null);
        }
        ...
        return nls.localize(11039, null, extension.displayName);
    }
    ...
    async canInstall(extension) {
        if (!(extension instanceof Extension)) {
            return new MarkdownString().appendText(nls.localize(11040, null));
        }
        if (extension.isMalicious) {
            return new MarkdownString().appendText(nls.localize(11041, null));
        }
        if (extension.deprecationInfo?.disallowInstall) {
            return new MarkdownString().appendText(nls.localize(11042, null));
        }
        if (extension.gallery) {
            if (!extension.gallery.isSigned && shouldRequireRepositorySignatureFor(extension.private, await this.extensionGalleryManifestService.getExtensionGalleryManifest())) {
                return new MarkdownString().appendText(nls.localize(11043, null));
            }
            ...
            return localResult ?? remoteResult ?? webResult ?? new MarkdownString().appendText(nls.localize(11044, null, extension.displayName ?? extension.identifier.id));
        }
        if (extension.resourceExtension && await this.extensionManagementService.canInstall(extension.resourceExtension) === true) {
            return true;
        }
        return new MarkdownString().appendText(nls.localize(11045, null, extension.displayName ?? extension.identifier.id));
    }
    async install(arg, installOptions = {}, progressLocation) {
        ...
        if (arg instanceof URI) {
            ...
            if (extension?.isMalicious) {
                throw new Error(nls.localize(11046, null));
            }
            if (gallery) {
            ...
            if (!servers || servers.length) {
                if (!installable) {
                    if (!gallery) {
                        ...
                        const reportIssueMessage = reportIssueUri ? nls.localize(11047, null, reportIssueUri.toString()) : '';
                        if (installOptions.version) {
                            const message = nls.localize(11048, null, id, installOptions.version);
                            ...
                            const message = nls.localize(11049, null, id);
                            throw new ExtensionManagementError(reportIssueMessage ? `${message} ${reportIssueMessage}` : message, "NotFound" /* ExtensionManagementErrorCode.NotFound */);
                        }
                    }
                    installable = gallery;
            ...
            if (installOptions.justification) {
                ...
                buttons.push({
                    label: isString(installOptions.justification) || !installOptions.justification.action
                        ? nls.localize(11050, null)
                        : nls.localize(11051, null, installOptions.justification.action), run: () => true
                });
                if (!extension) {
                    buttons.push({ label: nls.localize(11052, null), run: () => { this.open(extension); return false; } });
                }
                const result = await this.dialogService.prompt({
                    title: nls.localize(11053, null),
                    message: extension ? nls.localize(11054, null, extension.displayName, extension.publisherDisplayName) : nls.localize(11055, null),
                    detail: isString(installOptions.justification) ? installOptions.justification : installOptions.justification.reason,
                    cancelButton: true,
                    buttons,
                    checkbox: syncCheck ? {
                        label: nls.localize(11056, null),
                        checked: true,
                    } : undefined,
                });
                if (!result.result) {
        ...
        if (!extension) {
            throw new Error(nls.localize(11057, null));
        }
        if (installOptions.enable) {
            if (extension.enablementState === 11 /* EnablementState.DisabledWorkspace */ || extension.enablementState === 10 /* EnablementState.DisabledGlobally */) {
                if (installOptions.justification) {
                    const result = await this.dialogService.confirm({
                        title: nls.localize(11058, null),
                        message: nls.localize(11059, null, extension.displayName),
                        detail: isString(installOptions.justification) ? installOptions.justification : installOptions.justification.reason,
                        primaryButton: isString(installOptions.justification) ? nls.localize(11060, null) : nls.localize(11061, null, installOptions.justification.action),
                    });
                    if (!result.confirmed) {
                        throw new CancellationError();
                    }
    ...
    async installInServer(extension, server, installOptions) {
        await this.doInstall(extension, async () => {
            ...
            if (!isTargetPlatformCompatible(local.targetPlatform, [local.targetPlatform], targetPlatform)) {
                throw new Error(nls.localize(11062, null, extension.identifier.id));
            }
    ...
    async uninstall(e) {
        ...
        if (extension.local.isApplicationScoped && this.userDataProfilesService.profiles.length > 1) {
            const { confirmed } = await this.dialogService.confirm({
                title: nls.localize(11063, null),
                type: Severity.Info,
                message: nls.localize(11064, null, extension.displayName),
                primaryButton: nls.localize(11065, null)
            });
            if (!confirmed) {
                throw new CancellationError();
            }
    ...
    getErrorMessageForUninstallingAnExtensionWithDependents(extension, dependents) {
        if (dependents.length === 1) {
            return nls.localize(11069, null, extension.displayName, dependents[0].manifest.displayName);
        }
        if (dependents.length === 2) {
            return nls.localize(11070, null, extension.displayName, dependents[0].manifest.displayName, dependents[1].manifest.displayName);
        }
        return nls.localize(11071, null, extension.displayName, dependents[0].manifest.displayName, dependents[1].manifest.displayName);
    }
    ...
    doInstall(extension, installTask, progressLocation) {
        const title = extension ? nls.localize(11072, null, extension.displayName) : nls.localize(11073, null);
        return this.withProgress({
    ...
    getErrorMessageForDisablingAnExtensionWithDependents(extension, dependents) {
        if (dependents.length === 1) {
            return nls.localize(11076, null, extension.displayName, dependents[0].displayName);
        }
        if (dependents.length === 2) {
            return nls.localize(11077, null, extension.displayName, dependents[0].displayName, dependents[1].displayName);
        }
        return nls.localize(11078, null, extension.displayName, dependents[0].displayName, dependents[1].displayName);
    }
