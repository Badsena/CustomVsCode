...
let PromptExtensionInstallFailureAction = class PromptExtensionInstallFailureAction extends Action {
    ...
    async run() {
        ...
        if (this.error.name === "Unsupported" /* ExtensionManagementErrorCode.Unsupported */) {
            const productName = isWeb ? localize(10704, null, this.productService.nameLong) : this.productService.nameLong;
            const message = localize(10705, null, this.extension.displayName || this.extension.identifier.id, productName);
            const { confirmed } = await this.dialogService.confirm({
                type: Severity.Info,
                message,
                primaryButton: localize(10706, null),
                cancelButton: localize(10707, null)
            });
            if (confirmed) {
        ...
        if ("ReleaseVersionNotFound" /* ExtensionManagementErrorCode.ReleaseVersionNotFound */ === this.error.name) {
            await this.dialogService.prompt({
                ...
                cancelButton: localize(10709, null)
            });
            return;
        }
        ...
        if ("PackageNotSigned" /* ExtensionManagementErrorCode.PackageNotSigned */ === this.error.name) {
            await this.dialogService.prompt({
                type: 'error',
                message: localize(10710, null, this.extension.displayName),
                detail: getErrorMessage(this.error),
                buttons: [{
                        label: localize(10711, null),
                        run: () => {
        ...
        if ("SignatureVerificationFailed" /* ExtensionManagementErrorCode.SignatureVerificationFailed */ === this.error.name) {
            await this.dialogService.prompt({
                type: 'error',
                message: localize(10712, null, this.extension.displayName, this.productService.nameLong),
                detail: getErrorMessage(this.error),
                buttons: [{
                        label: localize(10713, null),
        ...
        if ("SignatureVerificationInternal" /* ExtensionManagementErrorCode.SignatureVerificationInternal */ === this.error.name) {
            await this.dialogService.prompt({
                type: 'error',
                message: localize(10715, null, this.extension.displayName, this.productService.nameLong),
                detail: getErrorMessage(this.error),
                buttons: [{
                        label: localize(10716, null),
                        run: () => this.openerService.open('https://code.visualstudio.com/docs/editor/extension-marketplace#_the-extension-signature-cannot-be-verified-by-vs-code')
                    }, {
                        label: localize(10717, null),
                        run: () => this.workbenchIssueService.openReporter({
                            issueTitle: localize(10718, null, this.extension.displayName),
                            issueBody: localize(10719, null)
                        })
                    }, {
                        label: localize(10720, null),
                        run: () => {
                            const installAction = this.instantiationService.createInstance(InstallAction, { ...this.options, donotVerifySignature: true, });
                            installAction.extension = this.extension;
                            return installAction.run();
        ...
        const operationMessage = this.installOperation === 3 /* InstallOperation.Update */ ? localize(10721, null, this.extension.displayName || this.extension.identifier.id)
            : localize(10722, null, this.extension.displayName || this.extension.identifier.id);
        let additionalMessage;
        const promptChoices = [];
        const downloadUrl = await this.getDownloadUrl();
        if (downloadUrl) {
            additionalMessage = localize(10723, null, createCommandUri(showWindowLogActionId).toString());
            promptChoices.push({
                label: localize(10724, null),
                run: () => this.openerService.open(downloadUrl).then(() => {
                    this.notificationService.prompt(Severity.Info, localize(10725, null, this.extension.identifier.id), [{
                            label: localize(10726, null),
...
let InstallAction = class InstallAction extends ExtensionAction {
    ...
    constructor(options, extensionsWorkbenchService, instantiationService, runtimeExtensionService, workbenchThemeService, labelService, dialogService, preferencesService, telemetryService, contextService, allowedExtensionsService, extensionGalleryManifestService) {
        super('extensions.install', localize(10727, null), InstallAction_1.CLASS, false);
    ...
    async run() {
        ...
        if (this.extension.gallery && !this.extension.gallery.isSigned && shouldRequireRepositorySignatureFor(this.extension.private, await this.extensionGalleryManifestService.getExtensionGalleryManifest())) {
            const { result } = await this.dialogService.prompt({
                ...
                message: localize(10728, null, this.extension.displayName),
                detail: localize(10729, null),
                buttons: [
                    {
                        label: localize(10730, null),
                        run: () => {
        ...
        if (this.extension.deprecationInfo) {
            let detail = localize(10731, null);
            ...
            const buttons = [
                {
                    label: localize(10732, null),
                    run: () => DeprecationChoice.InstallAnyway
                }
            ];
            if (this.extension.deprecationInfo.extension) {
                detail = localize(10733, null, this.extension.deprecationInfo.extension.displayName);
                const alternateExtension = this.extension.deprecationInfo.extension;
                buttons.push({
                    label: localize(10734, null, this.extension.deprecationInfo.extension.displayName),
                    run: async () => {
            ...
            const { result } = await this.dialogService.prompt({
                type: Severity.Warning,
                message: localize(10737, null, this.extension.displayName),
        ...
        alert(localize(10738, null, this.extension.displayName));
        /* __GDPR__
        ...
        */
        ...
        if (extension?.local) {
            alert(localize(10739, null, this.extension.displayName));
    ...
    getLabel(primary) {
        if (this.extension?.isWorkspaceScoped && this.extension.resourceExtension && this.contextService.isInsideWorkspace(this.extension.resourceExtension.location)) {
            return localize(10740, null);
        }
        /* install pre-release version */
        if (this.options.installPreReleaseVersion && this.extension?.hasPreReleaseVersion) {
            return primary ? localize(10741, null) : localize(10742, null);
        }
        /* install released version that has a pre release version */
        if (this.extension?.hasPreReleaseVersion) {
            return primary ? localize(10743, null) : localize(10744, null);
        }
        return localize(10745, null);
    }
};
...
export class InstallingLabelAction extends ExtensionAction {
    static { this.LABEL = localize(10746, null); }
...
let InstallInOtherServerAction = class InstallInOtherServerAction extends ExtensionAction {
    ...
    static { this.INSTALL_LABEL = localize(10747, null); }
    static { this.INSTALLING_LABEL = localize(10748, null); }
    ...
    async run() {
        ...
        alert(localize(10749, null, this.extension.displayName));
...
let RemoteInstallAction = class RemoteInstallAction extends InstallInOtherServerAction {
    ...
    getInstallLabel() {
        return this.extensionManagementServerService.remoteExtensionManagementServer
            ? localize(10750, null, this.extensionManagementServerService.remoteExtensionManagementServer.label)
...
let UninstallAction = class UninstallAction extends ExtensionAction {
    ...
    static { this.UninstallLabel = localize(10753, null); }
    static { this.UninstallingLabel = localize(10754, null); }
    ...
    update() {
        ...
        this.label = this.extension.local?.isApplicationScoped && this.userDataProfilesService.profiles.length > 1 ? localize(10755, null) : UninstallAction_1.UninstallLabel;
    ...
    async run() {
        ...
        alert(localize(10756, null, this.extension.displayName));
        try {
            await this.extensionsWorkbenchService.uninstall(this.extension);
            alert(localize(10757, null, this.extension.displayName));
        }
        catch (error) {
...
let UpdateAction = class UpdateAction extends ExtensionAction {
    ...
    static { this.EnabledClass = `${this.LABEL_ACTION_CLASS} update`; }
    ...
    constructor(verbose, extensionsWorkbenchService, dialogService, openerService, instantiationService) {
        super(`extensions.update`, localize(10758, null), UpdateAction_1.DisabledClass, false);
    ...
    update() {
        this.updateThrottler.queue(() => this.computeAndUpdateEnablement());
        if (this.extension) {
            this.label = this.verbose ? localize(10759, null, this.extension.latestVersion) : localize(10760, null);
        }
    }
    async computeAndUpdateEnablement() {
        this.enabled = false;
    ...
    async run() {
        ...
        if (consent) {
            const { result } = await this.dialogService.prompt({
                type: 'warning',
                title: localize(10761, null, this.extension.displayName),
                message: localize(10762, null, consent),
                buttons: [{
                        label: localize(10763, null),
                        run: () => 'update'
                    }, {
                        label: localize(10764, null),
                        run: () => 'review'
                    }, {
                        label: localize(10765, null),
                        run: () => 'cancel'
                    }]
            });
            if (result === 'cancel') {
        ...
        try {
            alert(localize(10766, null, this.extension.displayName, this.extension.latestVersion));
            await this.extensionsWorkbenchService.install(this.extension, installOptions);
            alert(localize(10767, null, this.extension.displayName, this.extension.latestVersion));
        }
        catch (err) {
...
let ToggleAutoUpdateForExtensionAction = class ToggleAutoUpdateForExtensionAction extends ExtensionAction {
    ...
    static { this.LABEL = localize2(10878, "Auto Update"); }
    ...
    async run() {
        ...
        if (enableAutoUpdate) {
            alert(localize(10768, null, this.extension.displayName));
        }
        else {
            alert(localize(10769, null, this.extension.displayName));
        }
    }
};
...
let ToggleAutoUpdatesForPublisherAction = class ToggleAutoUpdatesForPublisherAction extends ExtensionAction {
    ...
    static { this.LABEL = localize(10770, null); }
    ...
    async run() {
        ...
        alert(localize(10771, null, this.extension.publisherDisplayName));
        ...
        if (enableAutoUpdate) {
            alert(localize(10772, null, this.extension.displayName));
        }
        else {
            alert(localize(10773, null, this.extension.displayName));
        }
    }
};
...
let MigrateDeprecatedExtensionAction = class MigrateDeprecatedExtensionAction extends ExtensionAction {
    ...
    constructor(small, extensionsWorkbenchService) {
        super('extensionsAction.migrateDeprecatedExtension', localize(10774, null), MigrateDeprecatedExtensionAction_1.DisabledClass, false);
        this.small = small;
        this.extensionsWorkbenchService = extensionsWorkbenchService;
        this.update();
    }
    update() {
        ...
        this.tooltip = localize(10775, null, this.extension.deprecationInfo.extension.displayName);
        this.label = this.small ? localize(10776, null) : this.tooltip;
    }
    async run() {
        if (!this.extension?.deprecationInfo?.extension) {
            return;
...
let ManageExtensionAction = class ManageExtensionAction extends DropDownExtensionAction {
    ...
    constructor(instantiationService, extensionService, contextKeyService) {
        ...
        this.tooltip = localize(10777, null);
        this.update();
    }
...
export class ExtensionEditorManageExtensionAction extends DropDownExtensionAction {
    constructor(contextKeyService, instantiationService) {
        ...
        this.contextKeyService = contextKeyService;
        this.tooltip = localize(10778, null);
    }
    update() { }
    async run() {
        const actionGroups = [];
...
let TogglePreReleaseExtensionAction = class TogglePreReleaseExtensionAction extends ExtensionAction {
    ...
    static { this.LABEL = localize(10779, null); }
    static { this.EnabledClass = `${ExtensionAction.LABEL_ACTION_CLASS} prominent pre-release`; }
    static { this.DisabledClass = `${this.EnabledClass} hide`; }
    ...
    update() {
        ...
        if (this.extension.preRelease) {
            this.label = localize(10780, null);
            this.tooltip = localize(10781, null);
        }
        else {
            this.label = localize(10782, null);
            this.tooltip = localize(10783, null);
        }
    }
    async run() {
        if (!this.extension) {
...
let InstallAnotherVersionAction = class InstallAnotherVersionAction extends ExtensionAction {
    ...
    static { this.LABEL = localize(10784, null); }
    ...
    async run() {
        ...
        if (!allVersions.length) {
            await this.dialogService.info(localize(10785, null));
            return;
        }
        const picks = allVersions.map((v, i) => {
            return {
                id: v.version,
                label: v.version,
                description: `${fromNow(new Date(Date.parse(v.date)), true)}${v.isPreReleaseVersion ? ` (${localize(10786, null)})` : ''}${v.version === this.extension?.local?.manifest.version ? ` (${localize(10787, null)})` : ''}`,
                ariaLabel: `${v.isPreReleaseVersion ? 'Pre-Release version' : 'Release version'} ${v.version}`,
                isPreReleaseVersion: v.isPreReleaseVersion
            };
        });
        const pick = await this.quickInputService.pick(picks, {
            placeHolder: localize(10788, null),
            matchOnDetail: true
        });
        if (pick) {
...
let EnableForWorkspaceAction = class EnableForWorkspaceAction extends ExtensionAction {
    ...
    static { this.LABEL = localize(10789, null); }
    constructor(extensionsWorkbenchService, extensionEnablementService) {
        ...
        this.extensionsWorkbenchService = extensionsWorkbenchService;
        this.extensionEnablementService = extensionEnablementService;
        this.tooltip = localize(10790, null);
        this.update();
    }
    update() {
        this.enabled = false;
...
let DisableForWorkspaceAction = class DisableForWorkspaceAction extends ExtensionAction {
    ...
    static { this.LABEL = localize(10793, null); }
    constructor(workspaceContextService, extensionsWorkbenchService, extensionEnablementService, extensionService) {
        ...
        this.extensionEnablementService = extensionEnablementService;
        this.extensionService = extensionService;
        this.tooltip = localize(10794, null);
        this.update();
...
let DisableGloballyAction = class DisableGloballyAction extends ExtensionAction {
    ...
    static { this.LABEL = localize(10795, null); }
    constructor(extensionsWorkbenchService, extensionEnablementService, extensionService) {
        ...
        this.extensionEnablementService = extensionEnablementService;
        this.extensionService = extensionService;
        this.tooltip = localize(10796, null);
        this.update();
        this._register(this.extensionService.onDidChangeExtensions(() => this.update()));
    }
    update() {
...
let ExtensionRuntimeStateAction = class ExtensionRuntimeStateAction extends ExtensionAction {
    ...
    static { this.EnabledClass = `${ExtensionAction.LABEL_ACTION_CLASS} prominent reload`; }
    static { this.DisabledClass = `${this.EnabledClass} disabled`; }
    ...
    update() {
        ...
        this.label = runtimeState.action === "reloadWindow" /* ExtensionRuntimeActionType.ReloadWindow */ ? localize(10797, null)
            : runtimeState.action === "restartExtensions" /* ExtensionRuntimeActionType.RestartExtensions */ ? localize(10798, null)
                : runtimeState.action === "quitAndInstall" /* ExtensionRuntimeActionType.QuitAndInstall */ ? localize(10799, null)
                    : runtimeState.action === "applyUpdate" /* ExtensionRuntimeActionType.ApplyUpdate */ || runtimeState.action === "downloadUpdate" /* ExtensionRuntimeActionType.DownloadUpdate */ ? localize(10800, null, this.productService.nameShort) : '';
    }
    async run() {
        const runtimeState = this.extension?.runtimeState;
...
function getQuickPickEntries(themes, currentTheme, extension, showCurrentTheme) {
    ...
    if (showCurrentTheme) {
        picks.push({ type: 'separator', label: localize(10801, null) });
...
let SetColorThemeAction = class SetColorThemeAction extends ExtensionAction {
    ...
    static { this.TITLE = localize2(10879, 'Set Color Theme'); }
    ...
    async run({ showCurrentTheme, ignoreFocusLost } = { showCurrentTheme: false, ignoreFocusLost: false }) {
        ...
        const pickedTheme = await this.quickInputService.pick(picks, {
            placeHolder: localize(10802, null),
...
let SetFileIconThemeAction = class SetFileIconThemeAction extends ExtensionAction {
    ...
    static { this.TITLE = localize2(10880, 'Set File Icon Theme'); }
    static { this.EnabledClass = `${ExtensionAction.LABEL_ACTION_CLASS} theme`; }
    static { this.DisabledClass = `${this.EnabledClass} disabled`; }
    constructor(extensionService, workbenchThemeService, quickInputService, extensionEnablementService) {
    ...
    async run({ showCurrentTheme, ignoreFocusLost } = { showCurrentTheme: false, ignoreFocusLost: false }) {
        ...
        const pickedTheme = await this.quickInputService.pick(picks, {
            placeHolder: localize(10803, null),
...
let SetProductIconThemeAction = class SetProductIconThemeAction extends ExtensionAction {
    ...
    static { this.TITLE = localize2(10881, 'Set Product Icon Theme'); }
    static { this.EnabledClass = `${ExtensionAction.LABEL_ACTION_CLASS} theme`; }
    static { this.DisabledClass = `${this.EnabledClass} disabled`; }
    constructor(extensionService, workbenchThemeService, quickInputService, extensionEnablementService) {
    ...
    async run({ showCurrentTheme, ignoreFocusLost } = { showCurrentTheme: false, ignoreFocusLost: false }) {
        ...
        const pickedTheme = await this.quickInputService.pick(picks, {
            placeHolder: localize(10804, null),
...
let SetLanguageAction = class SetLanguageAction extends ExtensionAction {
    ...
    static { this.TITLE = localize2(10882, 'Set Display Language'); }
...
let ClearLanguageAction = class ClearLanguageAction extends ExtensionAction {
    ...
    static { this.TITLE = localize2(10883, 'Clear Display Language'); }
    static { this.EnabledClass = `${ExtensionAction.LABEL_ACTION_CLASS} language`; }
    static { this.DisabledClass = `${this.EnabledClass} disabled`; }
...
let ShowRecommendedExtensionAction = class ShowRecommendedExtensionAction extends Action {
    ...
    static { this.LABEL = localize(10805, null); }
...
let InstallRecommendedExtensionAction = class InstallRecommendedExtensionAction extends Action {
    ...
    static { this.LABEL = localize(10806, null); }
...
let IgnoreExtensionRecommendationAction = class IgnoreExtensionRecommendationAction extends Action {
    ...
    constructor(extension, extensionRecommendationsManagementService) {
        ...
        this.tooltip = localize(10807, null);
        this.enabled = true;
    }
    run() {
...
let UndoIgnoreExtensionRecommendationAction = class UndoIgnoreExtensionRecommendationAction extends Action {
    ...
    constructor(extension, extensionRecommendationsManagementService) {
        ...
        this.tooltip = localize(10808, null);
        this.enabled = true;
    }
    run() {
...
let AbstractConfigureRecommendedExtensionsAction = class AbstractConfigureRecommendedExtensionsAction extends Action {
    ...
    openExtensionsFile(extensionsFileResource) {
        ...
        })), error => Promise.reject(new Error(localize(10809, null, error))));
    }
...
let ConfigureWorkspaceRecommendedExtensionsAction = class ConfigureWorkspaceRecommendedExtensionsAction extends AbstractConfigureRecommendedExtensionsAction {
    ...
    static { this.LABEL = localize(10810, null); }
...
let ConfigureWorkspaceFolderRecommendedExtensionsAction = class ConfigureWorkspaceFolderRecommendedExtensionsAction extends AbstractConfigureRecommendedExtensionsAction {
    ...
    static { this.LABEL = localize(10811, null); }
    ...
    computeLabel() {
        ...
        if (currentEnablementState !== null) {
            ...
            if (!currentlyEnabled && enabled) {
                return canAddExtension() ? localize(10815, null) : null;
            }
            if (currentlyEnabled && !enabled) {
                return canRemoveExtension() ? localize(10816, null) : null;
            }
        }
        return null;
    }
...
let ToggleSyncExtensionAction = class ToggleSyncExtensionAction extends DropDownExtensionAction {
    ...
    update() {
        ...
        if (this.extension) {
            ...
            this.tooltip = isIgnored ? localize(10817, null) : localize(10818, null);
        }
    }
    async run() {
        return super.run([
            [
                new Action('extensions.syncignore', this.extensionsWorkbenchService.isExtensionIgnoredToSync(this.extension) ? localize(10819, null) : localize(10820, null), undefined, true, () => this.extensionsWorkbenchService.toggleExtensionIgnoredToSync(this.extension))
            ]
        ]);
    }
};
...
let ExtensionStatusAction = class ExtensionStatusAction extends ExtensionAction {
    ...
    async computeAndUpdateStatus() {
        ...
        if (this.extension.isMalicious) {
            this.updateStatus({ icon: warningIcon, message: new MarkdownString(localize(10821, null)) }, true);
            return;
        }
        if (this.extension.state === 3 /* ExtensionState.Uninstalled */ && this.extension.gallery && !this.extension.gallery.isSigned && shouldRequireRepositorySignatureFor(this.extension.private, await this.extensionGalleryManifestService.getExtensionGalleryManifest())) {
            this.updateStatus({ icon: warningIcon, message: new MarkdownString(localize(10822, null)) }, true);
            return;
        }
        if (this.extension.deprecationInfo) {
            if (this.extension.deprecationInfo.extension) {
                ...
                this.updateStatus({ icon: warningIcon, message: new MarkdownString(localize(10823, null, link)) }, true);
            }
            else if (this.extension.deprecationInfo.settings) {
                const link = `[${localize(10824, null)}](${createCommandUri('workbench.action.openSettings', this.extension.deprecationInfo.settings.map(setting => `@id:${setting}`).join(' '))}})`;
                this.updateStatus({ icon: warningIcon, message: new MarkdownString(localize(10825, null, link)) }, true);
            }
            else {
                const message = new MarkdownString(localize(10826, null));
        ...
        if (this.extension.missingFromGallery) {
            this.updateStatus({ icon: warningIcon, message: new MarkdownString(localize(10827, null)) }, true);
            return;
        }
        if (this.extensionsWorkbenchService.canSetLanguage(this.extension)) {
            return;
        ...
        if (this.extension.outdated) {
            ...
            if (message) {
                ...
                markdown.appendMarkdown(localize(10828, null, this.extension.hasChangelog()
        ...
        if (this.extension.enablementState === 7 /* EnablementState.DisabledByAllowlist */) {
            ...
            if (result !== true) {
                this.updateStatus({ icon: warningIcon, message: new MarkdownString(localize(10829, null, result.value)) }, true);
                return;
            }
        }
        // Extension is disabled by environment
        if (this.extension.enablementState === 2 /* EnablementState.DisabledByEnvironment */) {
            this.updateStatus({ message: new MarkdownString(localize(10830, null)) }, true);
            return;
        }
        // Extension is enabled by environment
        if (this.extension.enablementState === 3 /* EnablementState.EnabledByEnvironment */) {
            this.updateStatus({ message: new MarkdownString(localize(10831, null)) }, true);
            return;
        }
        ...
        if (this.extension.enablementState === 5 /* EnablementState.DisabledByVirtualWorkspace */) {
            ...
            this.updateStatus({ icon: infoIcon, message: new MarkdownString(details ? escapeMarkdownSyntaxTokens(details) : localize(10832, null)) }, true);
            return;
        }
        ...
        if (isVirtualWorkspace(this.contextService.getWorkspace())) {
            ...
            if (virtualSupportType === 'limited' || details) {
                this.updateStatus({ icon: warningIcon, message: new MarkdownString(details ? escapeMarkdownSyntaxTokens(details) : localize(10833, null)) }, true);
                return;
            }
        }
        // Unification
        if (this.extension.enablementState === 9 /* EnablementState.DisabledByUnification */) {
            this.updateStatus({ icon: infoIcon, message: new MarkdownString(localize(10834, null, '`chat.extensionUnification.enabled`')) }, true);
            return;
        }
        if (!this.workspaceTrustService.isWorkspaceTrusted() &&
            ...
            this.updateStatus({ icon: trustIcon, message: new MarkdownString(untrustedDetails ? escapeMarkdownSyntaxTokens(untrustedDetails) : localize(10835, null)) }, true);
            return;
        }
        ...
        if (this.workspaceTrustEnablementService.isWorkspaceTrustEnabled() && !this.workspaceTrustService.isWorkspaceTrusted()) {
            ...
            if (untrustedSupportType === 'limited' || untrustedDetails) {
                this.enabled = true;
                this.updateStatus({ icon: trustIcon, message: new MarkdownString(untrustedDetails ? escapeMarkdownSyntaxTokens(untrustedDetails) : localize(10836, null)) }, true);
                return;
            }
        }
                ...
                if (this.extensionManagementServerService.localExtensionManagementServer === this.extension.server) {
                    if (this.extensionManifestPropertiesService.prefersExecuteOnWorkspace(this.extension.local.manifest)) {
                        if (this.extensionManagementServerService.remoteExtensionManagementServer) {
                            message = new MarkdownString(`${localize(10837, null, this.extensionManagementServerService.remoteExtensionManagementServer.label)} [${localize(10838, null)}](https://code.visualstudio.com/api/advanced-topics/remote-extensions#architecture-and-extension-kinds)`);
                        }
                    }
                }
                // Extension on Remote Server
                    ...
                    if (this.extensionManifestPropertiesService.prefersExecuteOnUI(this.extension.local.manifest)) {
                        if (this.extensionManagementServerService.localExtensionManagementServer) {
                            message = new MarkdownString(`${localize(10839, null, this.extensionManagementServerService.remoteExtensionManagementServer.label)} [${localize(10840, null)}](https://code.visualstudio.com/api/advanced-topics/remote-extensions#architecture-and-extension-kinds)`);
                        }
                        else if (isWeb) {
                            message = new MarkdownString(`${localize(10841, null, this.productService.nameLong)} [${localize(10842, null)}](https://code.visualstudio.com/api/advanced-topics/remote-extensions#architecture-and-extension-kinds)`);
                        }
                    }
                }
                // Extension on Web Server
                else if (this.extensionManagementServerService.webExtensionManagementServer === this.extension.server) {
                    message = new MarkdownString(`${localize(10843, null, this.productService.nameLong)} [${localize(10844, null)}](https://code.visualstudio.com/api/advanced-topics/remote-extensions#architecture-and-extension-kinds)`);
                }
                if (message) {
                    this.updateStatus({ icon: warningIcon, message }, true);
                }
        ...
        for (const feature of features) {
            ...
            const manageAccessLink = `[${localize(10845, null)}](${createCommandUri('extension.open', this.extension.identifier.id, "features" /* ExtensionEditorTab.Features */, false, feature.id)})`;
        ...
        if (this.extensionManagementServerService.remoteExtensionManagementServer) {
            if (isLanguagePackExtension(this.extension.local.manifest)) {
                if (!this.extensionsWorkbenchService.installed.some(e => areSameExtensions(e.identifier, this.extension.identifier) && e.server !== this.extension.server)) {
                    const message = this.extension.server === this.extensionManagementServerService.localExtensionManagementServer
                        ? new MarkdownString(localize(10846, null, this.extensionManagementServerService.remoteExtensionManagementServer.label))
                        : new MarkdownString(localize(10847, null));
            ...
            if (this.extension.server === this.extensionManagementServerService.localExtensionManagementServer && runningExtensionServer === this.extensionManagementServerService.remoteExtensionManagementServer) {
                if (this.extensionManifestPropertiesService.prefersExecuteOnWorkspace(this.extension.local.manifest)) {
                    this.updateStatus({ icon: infoIcon, message: new MarkdownString(`${localize(10848, null)} [${localize(10849, null)}](https://code.visualstudio.com/api/advanced-topics/remote-extensions#architecture-and-extension-kinds)`) }, true);
                }
                return;
            }
            if (this.extension.server === this.extensionManagementServerService.remoteExtensionManagementServer && runningExtensionServer === this.extensionManagementServerService.localExtensionManagementServer) {
                if (this.extensionManifestPropertiesService.prefersExecuteOnUI(this.extension.local.manifest)) {
                    this.updateStatus({ icon: infoIcon, message: new MarkdownString(`${localize(10850, null)} [${localize(10851, null)}](https://code.visualstudio.com/api/advanced-topics/remote-extensions#architecture-and-extension-kinds)`) }, true);
                }
                return;
            }
            if (this.extension.server === this.extensionManagementServerService.remoteExtensionManagementServer && runningExtensionServer === this.extensionManagementServerService.webExtensionManagementServer) {
                if (this.extensionManifestPropertiesService.canExecuteOnWeb(this.extension.local.manifest)) {
                    this.updateStatus({ icon: infoIcon, message: new MarkdownString(`${localize(10852, null)} [${localize(10853, null)}](https://code.visualstudio.com/api/advanced-topics/remote-extensions#architecture-and-extension-kinds)`) }, true);
                }
                return;
            }
        }
        ...
        if (this.extension.enablementState === 8 /* EnablementState.DisabledByExtensionDependency */) {
            this.updateStatus({
                icon: warningIcon,
                message: new MarkdownString(localize(10854, null))
                    .appendMarkdown(`&nbsp;[${localize(10855, null)}](${createCommandUri('extension.open', this.extension.identifier.id, "dependencies" /* ExtensionEditorTab.Dependencies */)})`)
            }, true);
            return;
        }
        ...
        if (!this.extension.isWorkspaceScoped && isEnabled && isRunning) {
            if (this.extension.enablementState === 13 /* EnablementState.EnabledWorkspace */) {
                this.updateStatus({ message: new MarkdownString(localize(10856, null)) }, true);
                return;
            }
            if (this.extensionManagementServerService.localExtensionManagementServer && this.extensionManagementServerService.remoteExtensionManagementServer) {
                if (this.extension.server === this.extensionManagementServerService.remoteExtensionManagementServer) {
                    this.updateStatus({ message: new MarkdownString(localize(10857, null, this.extension.server.label)) }, true);
                    return;
                }
            }
        ...
        if (!isEnabled && !isRunning) {
            if (this.extension.enablementState === 10 /* EnablementState.DisabledGlobally */) {
                this.updateStatus({ message: new MarkdownString(localize(10858, null)) }, true);
                return;
            }
            if (this.extension.enablementState === 11 /* EnablementState.DisabledWorkspace */) {
                this.updateStatus({ message: new MarkdownString(localize(10859, null)) }, true);
                return;
            }
        }
    }
...
let InstallSpecificVersionOfExtensionAction = class InstallSpecificVersionOfExtensionAction extends Action {
    ...
    static { this.LABEL = localize(10860, null); }
    ...
    async run() {
        const extensionPick = await this.quickInputService.pick(this.getExtensionEntries(), { placeHolder: localize(10861, null), matchOnDetail: true });
...
let AbstractInstallExtensionsInServerAction = class AbstractInstallExtensionsInServerAction extends Action {
    ...
    async selectAndInstallExtensions() {
        ...
        if (localExtensionsToInstall.length) {
            ...
            quickPick.placeholder = localize(10862, null);
        ...
        if (selectedItems.length) {
            ...
            if (localExtensionsToInstall.length) {
                await this.progressService.withProgress({
                    ...
                    title: localize(10864, null)
                }, () => this.installExtensions(localExtensionsToInstall));
                this.notificationService.info(localize(10865, null));
            }
        }
    }
};
...
let InstallLocalExtensionsInRemoteAction = class InstallLocalExtensionsInRemoteAction extends AbstractInstallExtensionsInServerAction {
    ...
    get label() {
        if (this.extensionManagementServerService && this.extensionManagementServerService.remoteExtensionManagementServer) {
            return localize(10866, null, this.extensionManagementServerService.remoteExtensionManagementServer.label);
        }
        return '';
    }
    getQuickPickTitle() {
        return localize(10867, null, this.extensionManagementServerService.remoteExtensionManagementServer.label);
    }
...
let InstallRemoteExtensionsInLocalAction = class InstallRemoteExtensionsInLocalAction extends AbstractInstallExtensionsInServerAction {
    ...
    get label() {
        return localize(10868, null);
    }
    getQuickPickTitle() {
        return localize(10869, null);
    }
    getExtensionsToInstall(local) {
...
registerColor('extensionButton.separator', buttonSeparator, localize(10874, null));
