...
let RuntimeStatusMarkdownRenderer = class RuntimeStatusMarkdownRenderer extends Disposable {
    ...
    createElement(manifest, disposables) {
        ...
        if (this.extensionService.extensions.some(extension => ExtensionIdentifier.equals(extension.identifier, extensionId))) {
            ...
            data.appendMarkdown(`### ${localize(10514, null)}\n\n`);
        ...
        for (const feature of features) {
            ...
            if (accessData) {
                this.renderMarkdown(new MarkdownString(`\n ### ${localize(10515, null, feature.label)}\n\n`), container, disposables);
                if (accessData.accessTimes.length) {
                    const description = append(container, $('.feature-chart-description', undefined, localize(10516, null, accessData?.accessTimes.length, feature.accessDataLabel ?? feature.label)));
...
let ExtensionFeaturesTab = class ExtensionFeaturesTab extends Themable {
    ...
    create() {
        ...
        if (features.length === 0) {
            append($('.no-features'), this.domNode).textContent = localize(10520, null);
            return;
        }
...
let ExtensionFeatureItemRenderer = class ExtensionFeatureItemRenderer {
    ...
    renderTemplate(container) {
        ...
        disabledElement.textContent = localize(10522, null);
...
let ExtensionFeatureView = class ExtensionFeatureView extends Disposable {
        ...
        if (this.feature.access.canToggle) {
            ...
            this._register(button.onDidClick(async () => {
                ...
                const confirmationResult = await this.dialogService.confirm({
                    title: localize(10523, null, this.feature.label),
                    message: enabled
                        ? localize(10524, null, this.manifest.displayName ?? this.extensionId.value, this.feature.label)
                        : localize(10525, null, this.manifest.displayName ?? this.extensionId.value, this.feature.label),
                    custom: true,
                    primaryButton: enabled ? localize(10526, null) : localize(10527, null),
                    cancelButton: localize(10528, null),
                });
    ...
    updateButtonLabel(button) {
        button.label = this.extensionFeaturesManagementService.isEnabled(this.extensionId, this.feature.id) ? localize(10529, null) : localize(10530, null);
    }
