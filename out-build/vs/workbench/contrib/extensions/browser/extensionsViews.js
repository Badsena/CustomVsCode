...
let ExtensionsListView = class ExtensionsListView extends AbstractExtensionsListView {
    ...
    async query(query, options, token) {
        ...
        if (ExtensionsListView_1.isSearchRecentlyPublishedQuery(query.value)) {
    ...
    filterInstalledExtensions(local, runningExtensions, query, options) {
        ...
        const matchingText = (e) => (!e.isBuiltin || e.identifier.id.toLowerCase() === 'amypo.amypo-question') && (e.name.toLowerCase().indexOf(value) > -1 || e.displayName.toLowerCase().indexOf(value) > -1 || e.description.toLowerCase().indexOf(value) > -1)
            && this.filterExtensionByCategory(e, includedCategories, excludedCategories);
        let result;
    ...
    async queryGallery(query, options, token) {
        ...
        if (!text) {
            ...
            try {
                ...
                if (localExtensions.length) {
                    ...
                    return { model: new PagedModel(localExtensions), disposables: new DisposableStore(), message: { text: localize(10957, null, message.text), severity: message.severity } };
                }
                throw error;
            }
        }
        ...
        try {
            ...
            if (localExtensions.length) {
                const message = this.getMessage(error);
                return { model: new PagedModel(localExtensions), disposables: new DisposableStore(), message: { text: localize(10958, null, message.text), severity: message.severity } };
            }
            throw error;
        }
    }
        ...
        if (this.bodyTemplate) {
            ...
            if (this.isBodyVisible()) {
                if (message) {
                    ...
                    this.bodyTemplate.messageBox.textContent = localize(10959, null);
                }
    ...
    getMessage(error) {
        if (this.isOfflineError(error)) {
            return { text: localize(10960, null), severity: Severity.Warning };
        }
        else {
            return { text: localize(10961, null, getErrorMessage(error)), severity: Severity.Error };
        }
    }
...
export class DefaultRecommendedExtensionsView extends ExtensionsListView {
    ...
    renderBody(container) {
        ...
        this._register(Event.filter(this.extensionsWorkbenchService.onChange, e => e?.state === 1 /* ExtensionState.Installed */ || e?.state === 3 /* ExtensionState.Uninstalled */)(() => {
