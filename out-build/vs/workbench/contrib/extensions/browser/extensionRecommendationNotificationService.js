...
let ExtensionRecommendationNotificationService = class ExtensionRecommendationNotificationService extends Disposable {
    ...
    async promptImportantExtensionsInstallNotification(extensionRecommendations) {
        ...
        return this.promptRecommendationsNotification({ ...extensionRecommendations, extensions }, {
            ...
            onDidNeverShowRecommendedExtensionsAgain: (extensions) => {
                ...
                this.notificationService.prompt(Severity.Info, localize(10531, null), [{
                        label: localize(10532, null),
                        run: () => this.setIgnoreRecommendationsConfig(true)
                    }, {
                        label: localize(10533, null),
    ...
    async promptWorkspaceRecommendations(recommendations) {
        ...
        await this.promptRecommendationsNotification({ extensions: recommendations, source: 2 /* RecommendationSource.WORKSPACE */, name: localize(10534, null) }, {
    ...
    async promptRecommendationsNotification({ extensions: extensionIds, source, name, searchValue }, recommendationsNotificationActions) {
        ...
        if (extensions.length === 1) {
            extensionsMessage = localize(10535, null, extensions[0].displayName, extensions[0].publisherDisplayName);
        }
        else {
            ...
            if (publishers.length > 2) {
                extensionsMessage = localize(10536, null, publishers[0], publishers[1]);
            }
            else if (publishers.length === 2) {
                extensionsMessage = localize(10537, null, publishers[0], publishers[1]);
            }
            else {
                extensionsMessage = localize(10538, null, publishers[0]);
            }
        }
        let message = localize(10539, null, extensionsMessage, name);
        if (source === 3 /* RecommendationSource.EXE */) {
            message = localize(10540, null, name, extensionsMessage);
        }
        if (!searchValue) {
        ...
        const donotShowAgainLabel = source === 2 /* RecommendationSource.WORKSPACE */ ? localize(10541, null)
            : extensions.length > 1 ? localize(10542, null) : localize(10543, null);
        ...
        return createCancelablePromise(async (token) => {
            ...
            choices.push({
                ...
                menu: this.userDataSyncEnablementService.isEnabled() && this.userDataSyncEnablementService.isResourceEnabled("extensions" /* SyncResource.Extensions */) ? [{
                        label: localize(10545, null),
                        run: () => installExtensions(true)
                    }] : undefined,
            });
            choices.push(...[{
                    label: localize(10546, null),
                    run: async () => {
