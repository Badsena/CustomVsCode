...
import { IExtensionsWorkbenchService, VIEWLET_ID, CloseExtensionDetailsOnViewChangeKey, INSTALL_EXTENSION_FROM_VSIX_COMMAND_ID, /* WORKSPACE_RECOMMENDATIONS_VIEW_ID, OUTDATED_EXTENSIONS_VIEW_ID, */ CONTEXT_HAS_GALLERY, AutoCheckUpdatesConfigurationKey, extensionsSearchActionsMenu, AutoRestartConfigurationKey, SearchMcpServersContext, SearchAgentPluginsContext, DefaultViewsContext, CONTEXT_EXTENSIONS_GALLERY_STATUS } from '../common/extensions.js';
...
const REMOTE_CATEGORY = localize2(10951, "Remote");
let ExtensionsViewletViewsContribution = class ExtensionsViewletViewsContribution extends Disposable {
    ...
    createDefaultExtensionsViewDescriptors() {
        ...
        for (const server of servers) {
            const getInstalledViewName = () => getViewName(localize(10930, null), server);
                ...
                this._register(registerAction2(class InstallLocalExtensionsInRemoteAction2 extends Action2 {
                    constructor() {
                        super({
                            ...
                            get title() {
                                return localize2(10952, "Install Local Extensions in '{0}'...", server.label);
                            },
            ...
            this._register(registerAction2(class InstallRemoteExtensionsInLocalAction2 extends Action2 {
                constructor() {
                    super({
                        ...
                        title: localize2(10953, 'Install Remote Extensions Locally...'),
                        category: REMOTE_CATEGORY,
                        f1: true
                    });
                }
         ...
         * Always shown with curated extension recommendations
         */
        viewDescriptors.push({
            id: 'extensions.recommendedList',
            name: localize2(10954, "Recommended"),
    ...
    createSearchExtensionsViewDescriptors() {
        ...
        viewDescriptors.push({
            ...
            name: localize2(10955, "Marketplace"),
        ...
        viewDescriptors.push({
            ...
            name: localize2(10956, "Installed"),
...
let ExtensionsViewPaneContainer = class ExtensionsViewPaneContainer extends ViewPaneContainer {
    ...
    create(parent) {
        ...
        const placeholder = localize(10931, null);
    ...
    renderNotificaiton() {
        ...
        if (status && (query === this.searchBox?.getValue() || !this.searchMarketplaceExtensionsContextKey.get())) {
            ...
            const showAction = append(messageText, $('span.message-text-action', {
                'tabindex': '0',
                'role': 'button',
                'aria-label': `${status.message}. ${localize(10932, null)}`
            }, localize(10933, null)));
    ...
    alertSearchResult(count, viewId) {
        ...
        switch (count) {
            ...
            case 1:
                if (view) {
                    alert(localize(10934, null, view.name.value));
                }
                else {
                    alert(localize(10935, null));
                }
                break;
            default:
                if (view) {
                    alert(localize(10936, null, count, view.name.value));
                }
                else {
                    alert(localize(10937, null, count));
                }
                break;
        }
    }
    ...
    onError(err) {
        ...
        if (/ECONNREFUSED/.test(message)) {
            const error = createErrorWithActions(localize(10938, null), [
                new Action('open user settings', localize(10939, null), undefined, true, () => this.preferencesService.openUserSettings())
            ]);
    ...
    onServiceChange() {
        ...
        if (!badge) {
            ...
            if (newBadgeNumber > 0) {
                let msg = '';
                if (outdated) {
                    msg += outdated === 1 ? localize(10940, null, outdated) : localize(10941, null, outdated);
                }
                if (outdated > 0 && actionRequired.length > 0) {
                    msg += ', ';
                }
                if (actionRequired.length) {
                    msg += actionRequired.length === 1 ? localize(10942, null, actionRequired.length) : localize(10943, null, actionRequired.length);
                }
            ...
            if (maliciousExtensions.length) {
                ...
                for (const [extension, link] of maliciousExtensions) {
                    ...
                    if (shouldRestartExtensions || shouldReloadWindow) {
                        buttons.push({
                            label: shouldRestartExtensions ? localize(10944, null) : localize(10945, null),
                            run: () => shouldRestartExtensions ? this.extensionsWorkbenchService.updateRunningExtensions() : this.hostService.reload()
                        });
                    }
                    if (link) {
                        buttons.push({
                            label: localize(10946, null),
                            run: () => this.commandService.executeCommand('vscode.open', URI.parse(link))
                        });
                    }
                    this.notificationService.prompt(Severity.Warning, localize(10947, null, extension.manifest.displayName || extension.identifier.id), buttons, {
                        sticky: true,
    ...
    async updateBadge() {
        ...
        switch (status) {
            case "requiresSignIn" /* ExtensionGalleryManifestStatus.RequiresSignIn */:
                badge = new NumberBadge(1, () => localize(10948, null));
                break;
            case "accessDenied" /* ExtensionGalleryManifestStatus.AccessDenied */:
                badge = new WarningBadge(() => localize(10949, null));
                break;
        }
        if (badge) {
            this.badgeHandle.value = this.activityService.showViewContainerActivity(VIEWLET_ID, { badge });
        ...
        if (status === "requiresSignIn" /* ExtensionGalleryManifestStatus.RequiresSignIn */) {
            const badge = new NumberBadge(1, () => localize(10950, null));
