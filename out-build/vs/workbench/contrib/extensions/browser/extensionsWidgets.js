...
let InstallCountWidget = InstallCountWidget_1 = class InstallCountWidget extends ExtensionWidget {
    ...
    render() {
        ...
        if (!this.small) {
            this.disposables.add(this.hoverService.setupManagedHover(getDefaultHoverDelegate('mouse'), this.container, localize(10963, null)));
        }
    }
...
let RatingsWidget = class RatingsWidget extends ExtensionWidget {
    ...
    render() {
        ...
        if (this.small) {
            ...
            this.containerHover.update(localize(10964, null, rating));
...
let PublisherWidget = class PublisherWidget extends ExtensionWidget {
    ...
    render() {
        ...
        if (this.small) {
            ...
            this.containerHover = this.disposables.add(this.hoverService.setupManagedHover(getDefaultHoverDelegate('mouse'), this.element, localize(10965, null, this.extension.publisherDisplayName)));
            append(this.element, publisherDisplayName);
            if (this.extension.publisherDomain?.verified) {
                append(this.element, verifiedPublisher);
                ...
                this.containerHover.update(localize(10966, null, this.extension.publisherDomain.link));
...
let SponsorWidget = class SponsorWidget extends ExtensionWidget {
    ...
    render() {
        ...
        const label = $('span', undefined, localize(10967, null));
...
let RemoteBadgeWidget = class RemoteBadgeWidget extends ExtensionWidget {
    ...
    render() {
        ...
        if (this.tooltip && this.extensionManagementServerService.remoteExtensionManagementServer) {
            tooltip = localize(10968, null, this.extensionManagementServerService.remoteExtensionManagementServer.label);
        }
...
let ExtensionKindIndicatorWidget = class ExtensionKindIndicatorWidget extends ExtensionWidget {
    ...
    render() {
        ...
        if (this.extension?.private) {
            ...
            if (!this.small) {
                append(this.element, $('span.private-extension-label', undefined, localize(10969, null)));
            }
            return;
        }
        if (!this.small) {
        ...
        if (workspaceFolder && this.extension.isWorkspaceScoped) {
            this.element.textContent = localize(10970, null);
            ...
            this.element.textContent = localize(10971, null);
        }
    }
};
...
let SyncIgnoredWidget = class SyncIgnoredWidget extends ExtensionWidget {
    ...
    render() {
        ...
        if (this.extension && this.extension.state === 1 /* ExtensionState.Installed */ && this.userDataSyncEnablementService.isEnabled() && this.extensionsWorkbenchService.isExtensionIgnoredToSync(this.extension)) {
            ...
            this.disposables.add(this.hoverService.setupManagedHover(getDefaultHoverDelegate('mouse'), element, localize(10972, null)));
...
let ExtensionRestartRequiredWidget = class ExtensionRestartRequiredWidget extends ExtensionWidget {
    ...
    render() {
        ...
        if (runtimeState && /restart|reload/i.test(reason)) {
            ...
            append(this.container, $('span.extension-restart-required-label', undefined, localize(10973, null)));
    ...
    render() {
        ...
        if (this.extensionViewState.filters.featureId && this.extension.state === 1 /* ExtensionState.Installed */) {
            ...
            if (feature?.icon && accessData) {
                ...
                featureAccessTimeElement.textContent = localize(10974, null, accessData.accessTimes.length);
...
let ExtensionHoverWidget = ExtensionHoverWidget_1 = class ExtensionHoverWidget extends ExtensionWidget {
    ...
    getHoverMarkdown() {
        ...
        if (this.extension.private) {
            markdown.appendMarkdown(`$(${privateExtensionIcon.id}) ${localize(10975, null)}`);
            addSeparator = true;
        }
        if (this.extension.state === 1 /* ExtensionState.Installed */) {
            ...
            if (this.extension.publisherSponsorLink) {
                ...
                markdown.appendMarkdown(`$(${sponsorIcon.id}) [${localize(10976, null)}](${this.extension.publisherSponsorLink})`);
                addSeparator = true;
            }
        }
        if (addSeparator) {
        ...
        if (location) {
            if (this.extension.isWorkspaceScoped && this.contextService.isInsideWorkspace(location)) {
                markdown.appendMarkdown(localize(10977, null));
            }
            else {
                markdown.appendMarkdown(localize(10978, null));
            }
            markdown.appendText(`\n`);
        }
        ...
        if (this.extension.publisherDomain?.verified) {
            ...
            const publisherVerifiedTooltip = localize(10979, null, `[${URI.parse(this.extension.publisherDomain.link).authority}](${this.extension.publisherDomain.link})`);
        ...
        if (this.extension.outdated) {
            markdown.appendMarkdown(localize(10980, null));
        ...
        if (extensionRuntimeStatus || extensionFeaturesAccessData.size || extensionStatus.length || runtimeState || recommendationMessage || preReleaseMessage) {
            ...
            if (extensionRuntimeStatus) {
                if (extensionRuntimeStatus.activationTimes) {
                    ...
                    markdown.appendMarkdown(`${localize(10981, null)}${extensionRuntimeStatus.activationTimes.activationReason.startup ? ` (${localize(10982, null)})` : ''}: \`${activationTime}ms\``);
                    markdown.appendText(`\n`);
                }
                if (extensionRuntimeStatus.runtimeErrors.length || extensionRuntimeStatus.messages.length) {
                    ...
                    const errorsLink = extensionRuntimeStatus.runtimeErrors.length ? `[${extensionRuntimeStatus.runtimeErrors.length === 1 ? localize(10983, null) : localize(10984, null, extensionRuntimeStatus.runtimeErrors.length)}](${createCommandUri('extension.open', this.extension.identifier.id, "features" /* ExtensionEditorTab.Features */)})` : undefined;
                    const messageLink = extensionRuntimeStatus.messages.length ? `[${extensionRuntimeStatus.messages.length === 1 ? localize(10985, null) : localize(10986, null, extensionRuntimeStatus.messages.length)}](${createCommandUri('extension.open', this.extension.identifier.id, "features" /* ExtensionEditorTab.Features */)})` : undefined;
            ...
            if (extensionFeaturesAccessData.size) {
                ...
                for (const [featureId, accessData] of extensionFeaturesAccessData) {
                    if (accessData?.accessTimes.length) {
                        ...
                        if (feature) {
                            markdown.appendMarkdown(localize(10987, null, feature.label));
                            markdown.appendMarkdown(`: [${localize(10988, null, accessData.accessTimes.length, feature.accessDataLabel ?? feature.label)}](${createCommandUri('extension.open', this.extension.identifier.id, "features" /* ExtensionEditorTab.Features */)})`);
                            markdown.appendText(`\n`);
                        }
                    }
                }
    ...
    static getPreReleaseMessage(extension) {
        ...
        const preReleaseVersionLink = `[${localize(10989, null)}](${createCommandUri('workbench.extensions.action.showPreReleaseVersion', extension.identifier.id)})`;
        return localize(10990, null, preReleaseVersionLink);
    }
};
...
let ExtensionRecommendationWidget = class ExtensionRecommendationWidget extends ExtensionWidget {
    ...
    getRecommendationStatus() {
        ...
        if (extRecommendations[this.extension.identifier.id.toLowerCase()]) {
            ...
            return { icon: undefined, message: localize(10991, null) };
        }
        return undefined;
    }
};
...
export const extensionRatingIconColor = registerColor('extensionIcon.starForeground', { light: '#DF6100', dark: '#FF8E00', hcDark: '#FF8E00', hcLight: textLinkForeground }, localize(10992, null), false);
export const extensionPreReleaseIconColor = registerColor('extensionIcon.preReleaseForeground', { dark: '#1d9271', light: '#1d9271', hcDark: '#1d9271', hcLight: textLinkForeground }, localize(10993, null), false);
export const extensionSponsorIconColor = registerColor('extensionIcon.sponsorForeground', { light: '#B51E78', dark: '#D758B3', hcDark: null, hcLight: '#B51E78' }, localize(10994, null), false);
export const extensionPrivateBadgeBackground = registerColor('extensionIcon.privateForeground', { dark: '#ffffff60', light: '#00000060', hcDark: '#ffffff60', hcLight: '#00000060' }, localize(10995, null));
