...
let KeymapExtensions = class KeymapExtensions extends Disposable {
    ...
    promptForDisablingOtherKeymaps(newKeymap, oldKeymaps) {
        ...
        this.notificationService.prompt(Severity.Info, localize(11092, null, oldKeymaps.map(k => `'${k.local.manifest.displayName}'`).join(', ')), [{
                label: localize(11093, null),
                run: () => onPrompt(true)
            }, {
                label: localize(11094, null),
                run: () => onPrompt(false)
            }]);
    }
};
