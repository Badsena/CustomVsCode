...
let ExtensionHostProfileService = class ExtensionHostProfileService extends Disposable {
    ...
    updateProfilingStatusBarIndicator(visible) {
        ...
        if (visible) {
            const indicator = {
                name: nls.localize(11119, null),
                text: nls.localize(11120, null),
                showProgress: true,
                ariaLabel: nls.localize(11121, null),
                tooltip: nls.localize(11122, null),
                command: 'workbench.action.extensionHostProfiler.stop'
            };
            const timeStarted = Date.now();
            const handle = disposableWindowInterval(mainWindow, () => {
                this.profilingStatusBarIndicator?.update({ ...indicator, text: nls.localize(11123, null, Math.round((new Date().getTime() - timeStarted) / 1000)), });
            }, 1000);
    ...
    async startProfiling() {
        ...
        if (inspectPorts.length === 0) {
            return this._dialogService.confirm({
                type: 'info',
                message: nls.localize(11124, null),
                detail: nls.localize(11125, null, this._productService.nameLong),
                primaryButton: nls.localize(11126, null)
            }).then(res => {
                if (res.confirmed) {
