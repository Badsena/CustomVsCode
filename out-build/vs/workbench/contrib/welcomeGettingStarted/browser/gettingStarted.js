...
let GettingStartedPage = class GettingStartedPage extends EditorPane {
    ...
    constructor(group, commandService, productService, keybindingService, gettingStartedService, configurationService, telemetryService, languageService, fileService, openerService, themeService, storageService, extensionService, instantiationService, notificationService, groupsService, contextService, quickInputService, hostService, webviewService, workspaceContextService, accessibilityService, markdownRendererService) {
        ...
        this.container = $('.gettingStartedContainer', {
            role: 'document',
            tabindex: 0,
            'aria-label': localize(17332, null)
        });
            ...
            if (category.id === this.currentWalkthrough?.id) {
                ...
                badgeelements.forEach(badgeelement => {
                    if (step.done) {
                        ...
                        badgeelement.setAttribute('aria-label', localize(17333, null, step.title));
                    }
                    else {
                        ...
                        badgeelement.setAttribute('aria-label', localize(17334, null, step.title));
                    }
                });
                if (step.done) {
                    status(localize(17335, null, step.title));
                }
            }
    ...
    async openWalkthroughSelector() {
        ...
        })), { canPickMany: false, matchOnDescription: true, matchOnDetail: true, title: localize(17336, null) });
        if (selection) {
    ...
    async buildMediaComponent(stepId, forceRebuild = false) {
        ...
        if (stepToExpand.media.type === 'image') {
            ...
            const altText = media.altText ? media.altText : localize(17337, null, stepToExpand.title);
    ...
    provideScreenReaderUpdate() {
        if (this.configurationService.getValue("accessibility.verbosity.walkthrough" /* AccessibilityVerbositySettingId.Walkthrough */)) {
            ...
            return kbLabel ? localize(17338, null, kbLabel) : localize(17339, null);
        }
        return '';
    }
    ...
    createEditor(parent) {
        ...
        const prevButton = $('button.prev-button.button-link', { 'x-dispatch': 'scrollPrev' }, $('span.scroll-button.codicon.codicon-chevron-left'), $('span.moreText', {}, localize(17340, null)));
    ...
    async buildCategoriesSlide(preserveFocus) {
        ...
        const showOnStartupCheckbox = new Toggle({
            ...
            title: localize(17341, null),
            ...defaultToggleStyles
        });
        showOnStartupCheckbox.domNode.id = 'showOnStartup';
        const showOnStartupLabel = $('label.caption', { for: 'showOnStartup' }, localize(17342, null));
        ...
        const header = $('.header', {}, $('h1.product-name.caption', {}, this.productService.nameLong), $('p.subtitle.description', {}, localize(17343, null)));
        const leftColumn = $('.categories-column.categories-column-left', {});
        const rightColumn = $('.categories-column.categories-column-right', {});
        const startList = this.buildStartList();
        const slogan = $('.getting-started-slogan', {
            style: 'margin-top: 24px; font-size: 1.25em; line-height: 1.4; color: var(--vscode-descriptionForeground); opacity: 0.9; padding: 0 20px;'
        }, $('div', { style: 'font-weight: 600; margin-bottom: 8px;' }, localize(17344, null)), $('div', { style: 'font-size: 0.9em; opacity: 0.8;' }, localize(17345, null)));
        ...
        const layoutRecentList = () => {
            if (this.container.classList.contains('noWalkthroughs')) {
                ...
                reset(rightColumn, slogan);
            }
            else {
                reset(leftColumn, startList.getDomElement(), slogan);
            }
        };
...
...
    ...
    */
    ...
    buildGettingStartedWalkthroughsList() {
        const renderGetttingStaredWalkthrough = (category) => {
            ...
            if (category.newEntry) {
                reset(newBadge, $('.new-category', {}, localize(17347, null)));
            }
            else if (category.newItems) {
                reset(newBadge, $('.new-items', {}, localize(17348, null)));
            }
            ...
            return $('button.getting-started-category' + (category.isFeatured && this.showFeaturedWalkthrough ? '.featured' : ''), {
                ...
                'title': localize(17349, null),
                'role': 'button',
                'aria-label': localize(17350, null),
        ...
        const gettingStartedList = this.gettingStartedList = new GettingStartedIndexList({
            title: localize(17351, null),
            klass: 'getting-started',
            limit: 5,
            footer: $('span.button-link.see-all-walkthroughs', { 'x-dispatch': 'seeAllWalkthroughs', 'tabindex': 0 }, localize(17352, null)),
    ...
    updateCategoryProgress() {
        ...
        this.window.document.querySelectorAll('.category-progress').forEach(element => {
            ...
            if (stats.stepsTotal === stats.stepsComplete) {
                bar.title = localize(17353, null, stats.stepsComplete);
            }
            else {
                bar.title = localize(17354, null, stats.stepsComplete, stats.stepsTotal);
            }
        });
    }
        ...
        for (const linkedText of text) {
            if (linkedText.nodes.length === 1 && typeof linkedText.nodes[0] !== 'string') {
                ...
                if (isCommand) {
                    ...
                    if (keybinding) {
                        const shortcutMessage = $('span.shortcut-message', {}, localize(17355, null));
    ...
    buildCategorySlide(categoryID, selectedStep, preserveFocus) {
            ...
            reset(stepListContainer, ...renderedSteps
                ...
                const codicon = $('.codicon' + (step.done ? '.complete' + ThemeIcon.asCSSSelector(gettingStartedCheckedCodicon) : ThemeIcon.asCSSSelector(gettingStartedUncheckedCodicon)), {
                    ...
                    'aria-label': step.done
                        ? localize(17356, null, step.title)
                        : localize(17357, null, step.title),
                });
                ...
                if (step.media.type === 'image') {
                    stepDescription.appendChild($('.image-description', { 'aria-label': localize(17358, null, step.media.altText) }));
                }
                else if (step.media.type === 'video') {
                    stepDescription.appendChild($('.video-description', { 'aria-label': localize(17359, null, step.media.altText) }));
                }
        ...
        const stepsContainer = $('.getting-started-detail-container', { 'role': 'list' }, stepListContainer, $('.done-next-container', {}, $('button.button-link.all-done', { 'x-dispatch': 'allDone' }, $('span.codicon.codicon-check-all'), localize(17360, null)), ...(showNextCategory
            ? [$('button.button-link.next', { 'x-dispatch': 'nextSection' }, localize(17361, null), $('span.codicon.codicon-arrow-right'))]
            : [])));
    ...
    buildTelemetryFooter(parent) {
        const privacyStatementCopy = localize(17362, null);
        const privacyStatementButton = `[${privacyStatementCopy}](command:workbench.action.openPrivacyStatementUrl)`;
        const optOutCopy = localize(17363, null);
        const optOutButton = `[${optOutCopy}](command:settings.filterByTelemetry)`;
        const text = localize(17364, null, this.productService.nameShort, privacyStatementButton, optOutButton);
    ...
    setSlide(toEnable, firstLaunch = false) {
        ...
        if (toEnable === 'categories') {
            ...
            moreTextElement.textContent = firstLaunch ? localize(17365, null) : localize(17366, null);
            // eslint-disable-next-line no-restricted-syntax
