...
let TableColumnResizeQuickPick = class TableColumnResizeQuickPick extends Disposable {
    ...
    async show() {
        ...
        const column = await this._quickInputService.pick(items, { placeHolder: localize(11871, null) });
        if (!column) {
            return;
        }
        const value = await this._quickInputService.input({
            placeHolder: localize(11872, null),
            prompt: localize(11873, null, column.label),
    ...
    async _validateColumnResizeValue(input) {
        ...
        if (input && !Number.isInteger(percentage)) {
            return localize(11874, null);
        }
        else if (percentage < 0 || percentage > 100) {
            return localize(11875, null);
        }
        return null;
    }
};
