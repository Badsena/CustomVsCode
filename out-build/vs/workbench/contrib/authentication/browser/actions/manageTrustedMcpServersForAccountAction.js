...
export class ManageTrustedMcpServersForAccountAction extends Action2 {
    constructor() {
        super({
            ...
            title: localize2(5501, "Manage Trusted MCP Servers For Account"),
            category: localize2(5502, "Accounts"),
            f1: true,
...
let ManageTrustedMcpServersForAccountActionImpl = class ManageTrustedMcpServersForAccountActionImpl {
    ...
    async _resolveAccountQuery(providerId, accountLabel) {
        ...
        const pick = await this._quickInputService.pick(accounts, {
            placeHolder: localize(5491, null),
    ...
    async _getItems(accountQuery) {
        ...
        if (!filteredMcpServers.length) {
            this._dialogService.info(localize(5492, null));
            return [];
        }
        ...
        return [
            ...
            { type: 'separator', label: localize(5493, null) },
    ...
    _toQuickPickItem(mcpServer) {
        ...
        const description = lastUsed
            ? localize(5494, null, fromNow(lastUsed, true))
            : localize(5495, null);
        let tooltip;
        let disabled;
        if (mcpServer.trusted) {
            tooltip = localize(5496, null);
            disabled = true;
        }
        return {
    ...
    _createQuickPick(accountQuery) {
        ...
        quickPick.customLabel = localize(5498, null);
        quickPick.customButtonSecondary = true;
        quickPick.title = localize(5499, null);
        quickPick.placeholder = localize(5500, null);
