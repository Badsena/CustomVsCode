...
export class RemoveDynamicAuthenticationProvidersAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: localize2(5476, 'Remove Dynamic Authentication Providers'),
            category: localize2(5477, 'Authentication'),
            f1: true
        });
    }
    async run(accessor) {
        ...
        if (interactedProviders.length === 0) {
            await dialogService.info(localize(5468, null), localize(5469, null));
            return;
        }
        const items = interactedProviders.map(provider => ({
            label: provider.label,
            description: localize(5470, null, provider.clientId),
            provider
        }));
        const selected = await quickInputService.pick(items, {
            placeHolder: localize(5471, null),
            canPickMany: true
        });
        ...
        const message = selected.length === 1
            ? localize(5472, null, providerNames)
            : localize(5473, null, selected.length, providerNames);
        const result = await dialogService.confirm({
            message,
            detail: localize(5474, null),
            primaryButton: localize(5475, null),
            type: 'warning'
        });
        if (!result.confirmed) {
            return;
