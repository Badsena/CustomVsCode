    ...
    async run(accessor, { providerId, accountLabel }) {
        ...
        const { confirmed } = await dialogService.confirm({
            ...
            message: accountUsages.length
                ? localize(5504, null, accountLabel, accountUsages.map(usage => usage.extensionName).join('\n'))
                : localize(5505, null, accountLabel),
            primaryButton: localize(5506, null)
        });
        if (confirmed) {
