...
export class ManageAccountsAction extends Action2 {
    constructor() {
        super({
            ...
            title: localize2(5466, "Manage Accounts"),
            category: localize2(5467, "Accounts"),
            f1: true
        });
    }
    run(accessor) {
...
let ManageAccountsActionImpl = class ManageAccountsActionImpl {
    ...
    async run() {
        const placeHolder = localize(5459, null);
        const accounts = await this.listAccounts();
        if (!accounts.length) {
            await this.quickInputService.pick([{ label: localize(5460, null) }], { placeHolder });
            return;
        }
