...
export const shieldIcon = registerIcon('workspace-trust-banner', Codicon.shield, localize(17653, null));
const checkListIcon = registerIcon('workspace-trust-editor-check', Codicon.check, localize(17654, null));
const xListIcon = registerIcon('workspace-trust-editor-cross', Codicon.x, localize(17655, null));
const folderPickerIcon = registerIcon('workspace-trust-editor-folder-picker', Codicon.folder, localize(17656, null));
const editIcon = registerIcon('workspace-trust-editor-edit-folder', Codicon.edit, localize(17657, null));
const removeIcon = registerIcon('workspace-trust-editor-remove-folder', Codicon.close, localize(17658, null));
let WorkspaceTrustedUrisTable = class WorkspaceTrustedUrisTable extends Disposable {
    constructor(container, instantiationService, workspaceService, workspaceTrustManagementService, uriService, labelService, fileDialogService) {
            ...
            accessibilityProvider: {
                getAriaLabel: (item) => {
                    ...
                    if (hostLabel === undefined || hostLabel.length === 0) {
                        return localize(17661, null, this.labelService.getUriLabel(item.uri));
                    }
                    return localize(17662, null, this.labelService.getUriLabel(item.uri), hostLabel);
                },
                getWidgetAriaLabel: () => localize(17663, null)
            },
            identityProvider: {
                getId(element) {
        ...
        const addButton = this._register(buttonBar.addButton({ title: localize(17664, null), ...defaultButtonStyles }));
        addButton.label = localize(17665, null);
        this._register(addButton.onDidClick(async () => {
            const uri = await this.fileDialogService.showOpenDialog({
                ...
                openLabel: localize(17666, null),
                title: localize(17667, null)
            });
            if (uri) {
        ...
        if (item.uri.scheme === 'vscode-vfs') {
            ...
            if (segments.length === 0 && path.startsWith(posix.sep)) {
                return {
                    ...
                    content: localize(17670, null, getHostLabel(this.labelService, item))
                };
            }
            if (segments.length === 1) {
                return {
                    type: 2 /* MessageType.WARNING */,
                    content: localize(17671, null, segments[0], getHostLabel(this.labelService, item))
                };
            }
            if (segments.length > 2) {
                return {
                    type: 3 /* MessageType.ERROR */,
                    content: localize(17672, null, path)
                };
            }
        }
        return null;
    ...
    async edit(item, usePickerIfPossible) {
        ...
        if (canUseOpenDialog && usePickerIfPossible) {
            const uri = await this.fileDialogService.showOpenDialog({
                ...
                canSelectMany: false,
                defaultUri: item.uri,
                openLabel: localize(17673, null),
                title: localize(17674, null)
            });
            if (uri) {
...
function getHostLabel(labelService, item) {
    return item.uri.authority ? labelService.getHostLabel(item.uri.scheme, item.uri.authority) : localize(17678, null);
}
...
let WorkspaceTrustEditor = class WorkspaceTrustEditor extends EditorPane {
    ...
    getHeaderTitleText(trusted) {
        if (trusted) {
            if (this.workspaceTrustManagementService.isWorkspaceTrustForced()) {
                return localize(17679, null);
            }
            switch (this.workspaceService.getWorkbenchState()) {
                case 1 /* WorkbenchState.EMPTY */:
                    return localize(17680, null);
                case 2 /* WorkbenchState.FOLDER */:
                    return localize(17681, null);
                case 3 /* WorkbenchState.WORKSPACE */:
                    return localize(17682, null);
            }
        }
        return localize(17683, null);
    }
    ...
    getFeaturesHeaderText(trusted) {
        ...
        switch (this.workspaceService.getWorkbenchState()) {
            case 1 /* WorkbenchState.EMPTY */: {
                title = trusted ? localize(17684, null) : localize(17685, null);
                subTitle = trusted ? localize(17686, null) :
                    localize(17687, null);
                break;
            }
            case 2 /* WorkbenchState.FOLDER */: {
                title = trusted ? localize(17688, null) : localize(17689, null);
                subTitle = trusted ? localize(17690, null) :
                    localize(17691, null);
                break;
            }
            case 3 /* WorkbenchState.WORKSPACE */: {
                title = trusted ? localize(17692, null) : localize(17693, null);
                subTitle = trusted ? localize(17694, null) :
                    localize(17695, null);
                break;
            }
        }
    ...
    async render() {
        ...
        headerDescriptionText.innerText = isWorkspaceTrusted ?
            localize(17696, null) :
            localize(17697, null, this.productService.nameShort);
        const headerDescriptionActions = append(this.headerDescription, $('div'));
        const headerDescriptionActionsText = localize(17698, null, `command:workbench.trust.configure`);
        ...
        this.rootElement.setAttribute('aria-label', `${localize(17699, null)}:  ${this.headerContainer.innerText}`);
        // Settings
    ...
    createConfigurationElement(parent) {
        ...
        configurationTitle.innerText = localize(17700, null);
    ...
    async renderAffectedFeatures(numSettings, numExtensions) {
        ...
        const untrustedContainerItems = this.workspaceService.getWorkbenchState() === 1 /* WorkbenchState.EMPTY */ ?
            [
                localize(17708, null),
                localize(17709, null),
                fixBadLocalizedLinks(localize(17710, null, numExtensions, `command:${LIST_WORKSPACE_UNSUPPORTED_EXTENSIONS_COMMAND_ID}`))
            ] :
            [
                localize(17711, null),
                localize(17712, null),
                fixBadLocalizedLinks(numSettings ? localize(17713, null, numSettings, 'command:settings.filterUntrusted') : localize(17714, null)),
                fixBadLocalizedLinks(localize(17715, null, numExtensions, `command:${LIST_WORKSPACE_UNSUPPORTED_EXTENSIONS_COMMAND_ID}`))
            ];
    ...
    createButtonRow(parent, buttonInfo, enabled) {
        ...
        for (const { action, keybinding } of buttonInfo) {
            ...
            button.element.ariaLabel = action.label + ', ' + localize(17716, null, keybinding.getAriaLabel());
    ...
    addTrustButtonToElement(parent) {
        const trustAction = this.rerenderDisposables.add(new Action('workspace.trust.button.action.grant', localize(17717, null), undefined, true, async () => {
        ...
        if (this.workspaceTrustManagementService.canSetParentFolderTrust()) {
            ...
            trustMessageElement.innerText = localize(17718, null, name);
            const trustParentAction = this.rerenderDisposables.add(new Action('workspace.trust.button.action.grantParent', localize(17719, null), undefined, true, async () => {
    ...
    addDontTrustButtonToElement(parent) {
        this.createButtonRow(parent, [{
                action: this.rerenderDisposables.add(new Action('workspace.trust.button.action.deny', localize(17720, null), undefined, true, async () => {
    ...
    addTrustedTextToElement(parent) {
        ...
        if (!this.workspaceTrustManagementService.isWorkspaceTrustForced()) {
            textElement.innerText = this.workspaceService.getWorkbenchState() === 3 /* WorkbenchState.WORKSPACE */ ? localize(17721, null) : localize(17722, null);
        }
        else {
            textElement.innerText = localize(17723, null);
        }
    }
