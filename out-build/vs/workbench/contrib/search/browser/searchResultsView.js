...
let TextSearchResultRenderer = class TextSearchResultRenderer extends Disposable {
    ...
    async renderElement(node, index, templateData) {
        if (isPlainTextSearchHeading(node.element)) {
            templateData.label.setLabel(nls.localize(14625, null));
            ...
            const localizedLabel = nls.localize(14626, null);
...
let FolderMatchRenderer = class FolderMatchRenderer extends Disposable {
    ...
    renderCompressedElements(node, index, templateData) {
        ...
        if (folder.resource) {
            ...
            templateData.label.setLabel(nls.localize(14627, null));
        }
    ...
    renderElement(node, index, templateData) {
        ...
        if (folderMatch.resource) {
            ...
            templateData.label.setLabel(nls.localize(14628, null));
        }
    ...
    renderFolderDetails(folder, templateData) {
        ...
        templateData.badge.setTitleFormat(count > 1 ? nls.localize(14629, null, count) : nls.localize(14630, null, count));
...
let FileMatchRenderer = class FileMatchRenderer extends Disposable {
    ...
    renderElement(node, index, templateData) {
        ...
        templateData.badge.setCount(count);
        templateData.badge.setTitleFormat(count > 1 ? nls.localize(14631, null, count) : nls.localize(14632, null, count));
...
let MatchRenderer = class MatchRenderer extends Disposable {
    ...
    getMatchTitle(match, showLineNumbers) {
        ...
        const lineNumStr = showLineNumbers ?
            nls.localize(14633, null, startLine, numLines) + ' ' :
            '';
        const numLinesStr = numLines > 0 ?
            '+ ' + nls.localize(14634, null, numLines) :
            '';
...
let SearchAccessibilityProvider = class SearchAccessibilityProvider {
    ...
    getWidgetAriaLabel() {
        return nls.localize(14635, null);
    }
    getAriaLabel(element) {
        if (isSearchTreeFolderMatch(element)) {
            const count = element.allDownstreamFileMatches().reduce((total, current) => total + current.count(), 0);
            return element.resource ?
                nls.localize(14636, null, count, element.name()) :
                nls.localize(14637, null, count);
        }
        if (isSearchTreeFileMatch(element)) {
            const path = this.labelService.getUriLabel(element.resource, { relative: true }) || element.resource.fsPath;
            return nls.localize(14638, null, element.count(), element.name(), paths.dirname(path));
        }
        if (isSearchTreeMatch(element)) {
            ...
            if (replace) {
                return nls.localize(14639, null, matchText, range.startColumn, matchString, match.replaceString);
            }
            return nls.localize(14640, null, matchText, range.startColumn, matchString);
        }
        return null;
    }
};
