...
let McpServer = McpServer_1 = class McpServer extends Disposable {
    ...
    showInteractiveError(cnx, error, debug) {
        if (cnx.definition.sandboxEnabled) {
            if (!this.showSandboxConfigSuggestionFromPotentialBlocks(cnx, this._potentialSandboxBlocks)) {
                this._notificationService.warn(localize(12543, null, cnx.definition.label, error.message));
            }
            return;
        }
        if (error.code === 'ENOENT' && cnx.launchDefinition.type === 1 /* McpServerTransportType.Stdio */) {
            ...
            const options = [{
                    label: localize(12544, null),
                    run: () => this.showOutput(),
                }];
            if (cnx.definition.devMode?.debug?.type === 'debugpy' && debug) {
                this._notificationService.prompt(Severity.Error, localize(12545, null, cnx.launchDefinition.command, cnx.definition.label), [...options, {
                        label: localize(12546, null),
            ...
            if (docsLink) {
                options.push({
                    label: localize(12547, null, cnx.launchDefinition.command),
                    run: () => this._openerService.open(URI.parse(docsLink)),
                });
            }
            this._notificationService.prompt(Severity.Error, localize(12548, null, cnx.launchDefinition.command, cnx.definition.label), options);
        }
        else {
            this._notificationService.warn(localize(12549, null, cnx.definition.label, error.message));
        }
    }
    ...
    _confirmAndApplySandboxConfigSuggestion(cnx, potentialBlocks, suggestion) {
        ...
        void this._dialogService.confirm({
            type: 'warning',
            message: localize(12550, null, cnx.definition.label),
            detail: suggestion.message,
            primaryButton: localize(12551, null),
            cancelButton: localize(12552, null),
        }).then(async (result) => {
            if (!result.confirmed) {
                return;
            }
            if (!mcpResource || configTarget === undefined) {
                this._notificationService.warn(localize(12553, null, cnx.definition.label));
                return;
            }
            try {
                ...
                if (updated) {
                    this._removePotentialSandboxBlocks(potentialBlocks);
                    this._notificationService.info(localize(12554, null, cnx.definition.label));
                }
            }
            catch (e) {
                this._notificationService.error(localize(12555, null, cnx.definition.label, e instanceof Error ? e.message : String(e)));
            }
        }).finally(() => {
            this._isSandboxSuggestionDialogVisible = false;
        });
    ...
    async _getValidatedTools(tools) {
        ...
        for (const [i, result] of validations.entries()) {
            if ('error' in result) {
                error += localize(12556, null, tools[i].name) + '\n';
...
function warnInvalidTools(instaService, serverName, errorText) {
    instaService.invokeFunction((accessor) => {
        ...
        notificationService.notify({
            ...
            message: localize(12557, null, serverName),
            actions: {
                primary: [{
                        class: undefined,
                        enabled: true,
