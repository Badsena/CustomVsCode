...
let McpSamplingService = class McpSamplingService extends Disposable {
    ...
    async _getMatchingModel(opts) {
        ...
        if (model === 0 /* ModelMatch.UnsureAllowedDuringChat */) {
            ...
            const retry = await this._showContextual(opts.isDuringToolCall, localize(12529, null, opts.server.definition.label), localize(12530, null, opts.server.definition.label), this.allowButtons(opts.server, 'allowedDuringChat'));
            if (retry) {
                return this._getMatchingModel(opts);
            }
            ...
            const retry = await this._showContextual(opts.isDuringToolCall, localize(12531, null, opts.server.definition.label), localize(12532, null, opts.server.definition.label), this.allowButtons(opts.server, 'allowedOutsideChat'));
            if (retry) {
                return this._getMatchingModel(opts);
            }
            throw McpError.notAllowed();
            ...
            const newlyPickedModels = opts.isDuringToolCall
                ...
                : await this._notify(localize(12533, null, opts.server.definition.label), {
                    [localize(12534, null)]: () => this._commandService.executeCommand("workbench.mcp.configureSamplingModels" /* McpCommandIds.ConfigureSamplingModels */, opts.server),
                    [localize(12535, null)]: () => Promise.resolve(undefined),
                });
