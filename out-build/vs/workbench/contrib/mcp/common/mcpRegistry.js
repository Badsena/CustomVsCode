...
let McpRegistry = class McpRegistry extends Disposable {
    ...
    async _checkTrust(collection, definition, { trustNonceBearer, interaction, promptType = 'only-new', autoTrustChanges = false, errorOnUserInteraction = false, }) {
        if (collection.scope === 1 /* StorageScope.WORKSPACE */ && !this._workspaceTrustManagementService.isWorkspaceTrusted()) {
            ...
            else if (!await this._workspaceTrustRequestService.requestWorkspaceTrust({ message: localize(12515, null) })) {
                return false;
            }
        }
    ...
    async _promptForTrustOpenDialog(definitions) {
        function labelFor(r) {
            ...
            if (r.collection.source instanceof ExtensionIdentifier) {
                labelWithOrigin += ` (${localize(12516, null, r.collection.source.value)})`;
            }
        ...
        if (definitions.length === 1) {
            ...
            const { result } = await this._dialogService.prompt({
                message: localize(12517, null, def.definition.label),
                custom: {
                    icon: Codicon.shield,
                    markdownDetails: [{
                            markdown: new MarkdownString(localize(12518, null, labelFor(def))),
                            actionHandler: () => {
                ...
                buttons: [
                    { label: localize(12519, null), run: () => true },
                    { label: localize(12520, null), run: () => false }
                ],
            });
        ...
        const { result } = await this._dialogService.prompt({
            message: localize(12521, null, definitions.length),
            custom: {
                icon: Codicon.shield,
                markdownDetails: [{
                        markdown: new MarkdownString(localize(12522, null, list)),
                        actionHandler: (uri) => {
            ...
            buttons: [
                { label: localize(12523, null), run: () => 'all' },
                { label: localize(12524, null), run: () => 'pick' },
                { label: localize(12525, null), run: () => 'none' },
            ],
        });
        if (result === undefined) {
            return undefined;
    ...
    async resolveConnection(opts) {
        ...
        try {
            ...
            this._notificationService.notify({
                ...
                message: localize(12526, null, definition.label, String(e)),
                actions: {
