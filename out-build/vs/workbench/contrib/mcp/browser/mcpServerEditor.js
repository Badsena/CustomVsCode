...
let McpServerEditor = class McpServerEditor extends EditorPane {
    ...
    createEditor(parent) {
        ...
        this._register(this.hoverService.setupManagedHover(getDefaultHoverDelegate('mouse'), name, localize(12379, null)));
    ...
    renderNavbar(extension, template, preserveFocus) {
        ...
        if (extension.readmeUrl || extension.gallery?.readme) {
            template.navbar.push("readme" /* McpServerEditorTab.Readme */, localize(12380, null), localize(12381, null));
        }
        if (extension.gallery || extension.local?.manifest) {
            template.navbar.push("manifest" /* McpServerEditorTab.Manifest */, localize(12382, null), localize(12383, null));
        }
        if (extension.config) {
            template.navbar.push("configuration" /* McpServerEditorTab.Configuration */, localize(12384, null), localize(12385, null));
        }
        this.transientDisposables.add(this.mcpWorkbenchService.onChange(e => {
            if (e === extension) {
                if (e.config && !template.navbar.has("configuration" /* McpServerEditorTab.Configuration */)) {
                    template.navbar.push("configuration" /* McpServerEditorTab.Configuration */, localize(12386, null), localize(12387, null), extension.readmeUrl ? 1 : 0);
                }
                if (!e.config && template.navbar.has("configuration" /* McpServerEditorTab.Configuration */)) {
    ...
    async openDetails(extension, template, token) {
        ...
        const activeElement = await this.openMarkdown(extension, this.mcpServerReadme.get(), localize(12388, null), readmeContainer, 0 /* WebviewIndex.Readme */, localize(12389, null), token);
    ...
    async openManifest(mcpServer, parent, token) {
        ...
        try {
            ...
            noManifestMessage.textContent = localize(12390, null);
        }
        const scrollableContent = new DomScrollableElement(content, {});
        const layout = () => scrollableContent.scanDomNode();
        this.contentDisposables.add(toDisposable(arrays.insert(this.layoutParticipants, { layout })));
    ...
    renderConfigurationDetails(container, mcpServer) {
        ...
        typeLabel.textContent = localize(12393, null);
        ...
        if (config.type === "stdio" /* McpServerType.LOCAL */) {
            ...
            commandLabel.textContent = localize(12394, null);
            ...
            if (config.args && config.args.length > 0) {
                ...
                argsLabel.textContent = localize(12395, null);
            ...
            urlLabel.textContent = localize(12396, null);
    ...
    renderManifestDetails(container, manifest) {
        ...
        if (manifest.packages && manifest.packages.length > 0) {
            ...
            append(container, $('.manifest-section', undefined, $('.manifest-section-title', undefined, localize(12397, null))));
            for (const [packageType, packages] of packagesByType) {
                ...
                for (let i = 0; i < packages.length; i++) {
                    ...
                    append(packagesGrid, $('.package-detail', undefined, $('.detail-label', undefined, localize(12398, null)), $('.detail-value', undefined, pkg.identifier)));
                    if (pkg.packageArguments && pkg.packageArguments.length > 0) {
                        ...
                        append(packagesGrid, $('.package-detail', undefined, $('.detail-label', undefined, localize(12399, null)), $('code.detail-value', undefined, argStrings.join(' '))));
                    }
                    if (pkg.runtimeArguments && pkg.runtimeArguments.length > 0) {
                        ...
                        append(packagesGrid, $('.package-detail', undefined, $('.detail-label', undefined, localize(12400, null)), $('code.detail-value', undefined, argStrings.join(' '))));
                    }
                    if (pkg.environmentVariables && pkg.environmentVariables.length > 0) {
                        const envStrings = pkg.environmentVariables.map((envVar) => `${envVar.name}=${envVar.value ?? ''}`);
                        append(packagesGrid, $('.package-detail', undefined, $('.detail-label', undefined, localize(12401, null)), $('code.detail-value', undefined, envStrings.join(' '))));
                    }
        ...
        if (manifest.remotes && manifest.remotes.length > 0) {
            const packageSection = append(container, $('.package-section', undefined, $('.package-section-title', undefined, localize(12402, null).toLocaleUpperCase())));
            for (const remote of manifest.remotes) {
                const packagesGrid = append(packageSection, $('.package-details'));
                append(packagesGrid, $('.package-detail', undefined, $('.detail-label', undefined, localize(12403, null)), $('.detail-value', undefined, remote.url)));
                if (remote.type) {
                    append(packagesGrid, $('.package-detail', undefined, $('.detail-label', undefined, localize(12404, null)), $('.detail-value', undefined, remote.type)));
                }
                if (remote.headers && remote.headers.length > 0) {
                    const headerStrings = remote.headers.map((header) => `${header.name}: ${header.value ?? ''}`);
                    append(packagesGrid, $('.package-detail', undefined, $('.detail-label', undefined, localize(12405, null)), $('.detail-value', undefined, headerStrings.join(', '))));
                }
            }
        }
    }
...
let AdditionalDetailsWidget = class AdditionalDetailsWidget extends Disposable {
    ...
    renderTags(container, extension) {
        if (extension.gallery?.topics?.length) {
            ...
            append(categoriesContainer, $('.additional-details-title', undefined, localize(12406, null)));
    ...
    async renderExtensionResources(container, extension) {
        ...
        if (extension.repository) {
            try {
                resources.push([localize(12407, null), ThemeIcon.fromId(Codicon.repo.id), URI.parse(extension.repository)]);
            }
            catch (error) { /* Ignore */ }
        }
        if (manifest) {
            ...
            if (supportUri) {
                try {
                    resources.push([localize(12408, null), ThemeIcon.fromId(Codicon.commentDiscussion.id), URI.parse(supportUri)]);
                }
                catch (error) { /* Ignore */ }
            }
        }
        if (resources.length) {
            ...
            append(extensionResourcesContainer, $('.additional-details-title', undefined, localize(12409, null)));
    ...
    renderInstallInfo(container, extension) {
        ...
        append(installInfoContainer, $('.additional-details-title', undefined, localize(12410, null)));
        const installInfo = append(installInfoContainer, $('.more-info'));
        append(installInfo, $('.more-info-entry', undefined, $('div.more-info-entry-name', undefined, localize(12411, null)), $('code', undefined, extension.name)));
        if (extension.version) {
            append(installInfo, $('.more-info-entry', undefined, $('div.more-info-entry-name', undefined, localize(12412, null)), $('code', undefined, extension.version)));
        }
    }
    renderMarketplaceInfo(container, extension) {
        const gallery = extension.gallery;
        const moreInfoContainer = append(container, $('.more-info-container.additional-details-element'));
        append(moreInfoContainer, $('.additional-details-title', undefined, localize(12413, null)));
        const moreInfo = append(moreInfoContainer, $('.more-info'));
        if (gallery) {
            if (!extension.local) {
                append(moreInfo, $('.more-info-entry', undefined, $('div.more-info-entry-name', undefined, localize(12414, null)), $('code', undefined, extension.name)));
                if (gallery.version) {
                    append(moreInfo, $('.more-info-entry', undefined, $('div.more-info-entry-name', undefined, localize(12415, null)), $('code', undefined, gallery.version)));
                }
            }
            if (gallery.lastUpdated) {
                append(moreInfo, $('.more-info-entry', undefined, $('div.more-info-entry-name', undefined, localize(12416, null)), $('div', {
                    'title': new Date(gallery.lastUpdated).toString()
                }, fromNow(gallery.lastUpdated, true, true, true))));
            }
            if (gallery.publishDate) {
                append(moreInfo, $('.more-info-entry', undefined, $('div.more-info-entry-name', undefined, localize(12417, null)), $('div', {
