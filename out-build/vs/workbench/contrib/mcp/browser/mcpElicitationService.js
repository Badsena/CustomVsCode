...
const noneItem = { id: undefined, label: localize(12260, null), description: localize(12261, null), alwaysShow: true };
...
let McpElicitationService = class McpElicitationService {
    ...
    async _elicitForm(server, context, elicitation, token) {
        ...
        const value = await new Promise(resolve => {
            ...
            const handle = this._notificationService.notify({
                ...
                source: localize(12262, null, server.definition.label),
                severity: Severity.Info,
                actions: {
                    primary: [store.add(new Action('mcp.elicit.give', localize(12263, null), undefined, true, () => resolve(this._doElicitForm(elicitation, token))))],
                    secondary: [store.add(new Action('mcp.elicit.cancel', localize(12264, null), undefined, true, () => resolve({ action: 'decline' })))],
                }
            });
    ...
    async _elicitUrl(server, context, elicitation, token) {
        ...
        const value = await new Promise(resolve => {
            ...
            if (chatModel instanceof ChatModel) {
                const request = chatModel.getRequests().at(-1);
                if (request) {
                    const part = new ChatElicitationRequestPart(localize(12265, null), new MarkdownString().appendText(elicitation.message)
                        .appendMarkdown('\n\n' + localize(12266, null))
                        .appendCodeblock('', elicitation.url), localize(12267, null, server.definition.label), localize(12268, null, URI.parse(elicitation.url).authority), localize(12269, null), async () => {
                ...
                const handle = this._notificationService.notify({
                    message: elicitation.message + ' ' + localize(12270, null, elicitation.url),
                    source: localize(12271, null, server.definition.label),
                    severity: Severity.Info,
                    actions: {
                        primary: [store.add(new Action('mcp.elicit.url.open2', localize(12272, null), undefined, true, () => resolve(this._doElicitUrl(elicitation, token))))],
                        secondary: [store.add(new Action('mcp.elicit.cancel', localize(12273, null), undefined, true, () => resolve({ action: 'decline' })))],
                    }
                });
                store.add(handle.onDidClose(() => resolve({ action: 'cancel' })));
                store.add(token.onCancellationRequested(() => resolve({ action: 'cancel' })));
    ...
    _getFieldPlaceholder(schema, required) {
        ...
        if (!required) {
            placeholder = placeholder ? `${placeholder} (${localize(12274, null)})` : localize(12275, null);
        }
        return placeholder;
    }
    ...
    async _handleInputField(quickPick, schema, required, store, token) {
        ...
        const updateItems = () => {
            ...
            if (quickPick.value) {
                ...
                quickPick.validationMessage = '';
                if (schema.default) {
                    items.push({ id: '$default', label: `${schema.default}`, description: localize(12276, null) });
                }
            }
    ...
    _validateString(value, schema) {
        if (schema.minLength && value.length < schema.minLength) {
            return { isValid: false, message: localize(12277, null, schema.minLength) };
        }
        if (schema.maxLength && value.length > schema.maxLength) {
            return { isValid: false, message: localize(12278, null, schema.maxLength) };
        }
        if (schema.format) {
        ...
        switch (format) {
            case 'email':
                return value.includes('@')
                    ? { isValid: true }
                    : { isValid: false, message: localize(12279, null) };
            case 'uri':
                if (URL.canParse(value)) {
                    return { isValid: true };
                }
                else {
                    return { isValid: false, message: localize(12280, null) };
                }
            case 'date': {
                const dateRegex = /^\d{4}-\d{2}-\d{2}$/;
                if (!dateRegex.test(value)) {
                    return { isValid: false, message: localize(12281, null) };
                }
                const date = new Date(value);
                return !isNaN(date.getTime())
                    ? { isValid: true }
                    : { isValid: false, message: localize(12282, null) };
            }
            case 'date-time': {
                const dateTime = new Date(value);
                return !isNaN(dateTime.getTime())
                    ? { isValid: true }
                    : { isValid: false, message: localize(12283, null) };
            }
            default:
                return { isValid: true };
        }
    ...
    _validateNumber(value, schema) {
        ...
        if (isNaN(parsed)) {
            return { isValid: false, message: localize(12284, null) };
        }
        if (schema.type === 'integer' && !Number.isInteger(parsed)) {
            return { isValid: false, message: localize(12285, null) };
        }
        if (schema.minimum !== undefined && parsed < schema.minimum) {
            return { isValid: false, message: localize(12286, null, schema.minimum) };
        }
        if (schema.maximum !== undefined && parsed > schema.maximum) {
            return { isValid: false, message: localize(12287, null, schema.maximum) };
        }
        ...
        for (const [propertyName, schema] of properties) {
            ...
            if (schema.type === 'boolean') {
                questions.push({
                    ...
                    options: [
                        { id: 'true', label: localize(12288, null), value: 'true' },
                        { id: 'false', label: localize(12289, null), value: 'false' },
                    ],
