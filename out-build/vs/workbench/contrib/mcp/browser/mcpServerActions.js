...
let InstallAction = class InstallAction extends McpServerAction {
    ...
    constructor(open, mcpWorkbenchService, telemetryService, mcpService) {
        super('extensions.install', localize(12339, null), InstallAction_1.CLASS, false);
        this.open = open;
    ...
    async run() {
        ...
        if (this.open) {
            ...
            alert(localize(12340, null, this.mcpServer.label));
        }
...
let InstallInWorkspaceAction = class InstallInWorkspaceAction extends McpServerAction {
    ...
    constructor(open, mcpWorkbenchService, workspaceService, quickInputService, telemetryService, mcpService) {
        super('extensions.installWorkspace', localize(12341, null), InstallAction.CLASS, false);
        this.open = open;
        this.mcpWorkbenchService = mcpWorkbenchService;
    ...
    async run() {
        ...
        if (this.open) {
            ...
            alert(localize(12342, null, this.mcpServer.label));
        }
    ...
    async getConfigurationTarget() {
        ...
        for (const folder of this.workspaceService.getWorkspace().folders) {
            options.push({ target: folder, label: folder.name, description: localize(12343, null) });
        }
        if (this.workspaceService.getWorkbenchState() === 3 /* WorkbenchState.WORKSPACE */) {
            if (options.length > 0) {
                options.push({ type: 'separator' });
            }
            options.push({ target: 5 /* ConfigurationTarget.WORKSPACE */, label: localize(12344, null) });
        }
        if (options.length === 1) {
            return options[0].target;
        }
        const targetPick = await this.quickInputService.pick(options, {
            title: localize(12345, null),
        });
...
let InstallInRemoteAction = class InstallInRemoteAction extends McpServerAction {
    ...
    constructor(open, mcpWorkbenchService, environmentService, telemetryService, labelService, mcpService) {
        super('extensions.installRemote', localize(12346, null), InstallAction.CLASS, false);
        this.open = open;
        this.mcpWorkbenchService = mcpWorkbenchService;
        ...
        this.label = localize(12347, null, remoteLabel);
        this.update();
    }
    update() {
        this.enabled = false;
    ...
    async run() {
        ...
        if (this.open) {
            this.mcpWorkbenchService.open(this.mcpServer);
            alert(localize(12348, null, this.mcpServer.label));
        }
...
export class InstallingLabelAction extends McpServerAction {
    static { this.LABEL = localize(12349, null); }
...
let UninstallAction = class UninstallAction extends McpServerAction {
    ...
    constructor(mcpWorkbenchService) {
        super('extensions.uninstall', localize(12350, null), UninstallAction_1.CLASS, false);
        this.mcpWorkbenchService = mcpWorkbenchService;
        this.update();
    }
    update() {
        ...
        this.enabled = true;
        this.label = localize(12351, null);
    }
    async run() {
        if (!this.mcpServer) {
            return;
...
let EnableMcpServerGloballyAction = class EnableMcpServerGloballyAction extends McpServerAction {
    ...
    constructor(mcpService) {
        super(EnableMcpServerGloballyAction_1.ID, localize(12352, null), McpServerAction.LABEL_ACTION_CLASS);
        this.mcpService = mcpService;
        this.tooltip = localize(12353, null);
        this.update();
    }
    update() {
        this.enabled = false;
...
let EnableMcpServerForWorkspaceAction = class EnableMcpServerForWorkspaceAction extends McpServerAction {
    ...
    constructor(mcpService, workspaceService) {
        super(EnableMcpServerForWorkspaceAction_1.ID, localize(12354, null), McpServerAction.LABEL_ACTION_CLASS);
        this.mcpService = mcpService;
        this.workspaceService = workspaceService;
        this.tooltip = localize(12355, null);
        this.update();
    }
    update() {
        this.enabled = false;
...
let DisableMcpServerGloballyAction = class DisableMcpServerGloballyAction extends McpServerAction {
    ...
    constructor(mcpService) {
        super(DisableMcpServerGloballyAction_1.ID, localize(12356, null), McpServerAction.LABEL_ACTION_CLASS);
        this.mcpService = mcpService;
        this.tooltip = localize(12357, null);
        this.update();
    }
    update() {
        this.enabled = false;
...
let DisableMcpServerForWorkspaceAction = class DisableMcpServerForWorkspaceAction extends McpServerAction {
    ...
    constructor(mcpService, workspaceService) {
        super(DisableMcpServerForWorkspaceAction_1.ID, localize(12358, null), McpServerAction.LABEL_ACTION_CLASS);
        this.mcpService = mcpService;
        this.workspaceService = workspaceService;
        this.tooltip = localize(12359, null);
        this.update();
    }
    update() {
        this.enabled = false;
...
let ManageMcpServerAction = class ManageMcpServerAction extends DropDownAction {
    ...
    constructor(isEditorAction, instantiationService) {
        ...
        this.tooltip = localize(12360, null);
        this.update();
    }
    async run() {
...
let StartServerAction = class StartServerAction extends McpServerAction {
    ...
    static { this.HIDE = `${this.CLASS} hide`; }
    constructor(mcpService) {
        super('extensions.start', localize(12361, null), StartServerAction_1.CLASS, false);
        this.mcpService = mcpService;
        this.update();
    }
    update() {
        ...
        this.enabled = true;
        this.label = localize(12362, null);
    }
    async run() {
        const server = this.getServer();
        if (!server) {
...
let StopServerAction = class StopServerAction extends McpServerAction {
    ...
    static { this.HIDE = `${this.CLASS} hide`; }
    constructor(mcpService) {
        super('extensions.stop', localize(12363, null), StopServerAction_1.CLASS, false);
        this.mcpService = mcpService;
        this.update();
    }
    update() {
        ...
        this.enabled = true;
        this.label = localize(12364, null);
    }
    async run() {
        const server = this.getServer();
        if (!server) {
...
let RestartServerAction = class RestartServerAction extends McpServerAction {
    ...
    static { this.HIDE = `${this.CLASS} hide`; }
    constructor(mcpService) {
        super('extensions.restart', localize(12365, null), RestartServerAction_1.CLASS, false);
        this.mcpService = mcpService;
        this.update();
    }
    update() {
        ...
        this.enabled = true;
        this.label = localize(12366, null);
    }
    async run() {
        const server = this.getServer();
        if (!server) {
...
let AuthServerAction = class AuthServerAction extends McpServerAction {
    ...
    static { this.HIDE = `${this.CLASS} hide`; }
    static { this.SIGN_OUT = localize(12367, null); }
    static { this.DISCONNECT = localize(12368, null); }
    constructor(mcpService, _authenticationQueryService, _authenticationService) {
        super('extensions.restart', localize(12369, null), RestartServerAction.CLASS, false);
        this.mcpService = mcpService;
...
let ShowServerOutputAction = class ShowServerOutputAction extends McpServerAction {
    ...
    static { this.HIDE = `${this.CLASS} hide`; }
    constructor(mcpService) {
        super('extensions.output', localize(12370, null), ShowServerOutputAction_1.CLASS, false);
        this.mcpService = mcpService;
        this.update();
    }
    update() {
        ...
        this.enabled = true;
        this.label = localize(12371, null);
    }
    async run() {
        const server = this.getServer();
        if (!server) {
...
let ShowServerConfigurationAction = class ShowServerConfigurationAction extends McpServerAction {
    ...
    static { this.HIDE = `${this.CLASS} hide`; }
    constructor(mcpWorkbenchService) {
        super('extensions.config', localize(12372, null), ShowServerConfigurationAction_1.CLASS, false);
        this.mcpWorkbenchService = mcpWorkbenchService;
        this.update();
    }
    update() {
...
let ShowServerJsonConfigurationAction = class ShowServerJsonConfigurationAction extends McpServerAction {
    ...
    constructor(mcpService, mcpRegistry, editorService) {
        super('extensions.jsonConfig', localize(12373, null), ShowServerJsonConfigurationAction_1.CLASS, false);
        this.mcpService = mcpService;
...
let ConfigureModelAccessAction = class ConfigureModelAccessAction extends McpServerAction {
    ...
    constructor(mcpService, commandService) {
        super('extensions.config', localize(12374, null), ConfigureModelAccessAction_1.CLASS, false);
        this.mcpService = mcpService;
    ...
    update() {
        ...
        this.enabled = true;
        this.label = localize(12375, null);
    }
    async run() {
        const server = this.getServer();
        if (!server) {
...
let ShowSamplingRequestsAction = class ShowSamplingRequestsAction extends McpServerAction {
    ...
    constructor(mcpService, samplingService, editorService) {
        super('extensions.config', localize(12376, null), ShowSamplingRequestsAction_1.CLASS, false);
        this.mcpService = mcpService;
    ...
    async run() {
        ...
        this.editorService.openEditor({
            ...
            label: localize(12377, null, server.definition.label),
        });
    }
    getServer() {
        if (!this.mcpServer) {
...
let BrowseResourcesAction = class BrowseResourcesAction extends McpServerAction {
    ...
    static { this.HIDE = `${this.CLASS} hide`; }
    constructor(mcpService, commandService) {
        super('extensions.config', localize(12378, null), BrowseResourcesAction_1.CLASS, false);
        this.mcpService = mcpService;
        this.commandService = commandService;
        this.update();
    }
