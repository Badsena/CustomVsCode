...
let McpPromptArgumentPick = class McpPromptArgumentPick extends Disposable {
    ...
    async createArgs(token) {
        ...
        for (let i = 0; i < prompt.arguments.length; i++) {
            ...
            quickPick.step = i + 1;
            quickPick.placeholder = arg.required ? arg.description : `${arg.description || ''} (${localize(12316, null)})`;
            quickPick.title = localize(12317, null, arg.title || arg.name);
        ...
        quickPick.value = '';
        quickPick.placeholder = localize(12318, null);
        quickPick.busy = true;
        return args;
    }
    async _getArg(arg, didRestoreState, argsSoFar, token) {
        ...
        store.add(autorun(reader => {
            ...
            let items = [];
            items.push({ id: 'insert-text', label: localize(12322, null), iconClass: ThemeIcon.asClassName(Codicon.textSize), action: 'text', alwaysShow: true });
            items.push({ id: 'run-command', label: localize(12323, null), description: localize(12324, null), iconClass: ThemeIcon.asClassName(Codicon.terminal), action: 'command', alwaysShow: true });
            let busy = false;
            ...
            const value = await new Promise(resolve => {
                ...
                store.add(quickPick.onDidAccept(() => {
                    ...
                    if (!quickPick.value && arg.required && (!item || item.action === 'text' || item.action === 'command')) {
                        quickPick.validationMessage = localize(12325, null);
                    }
                    else if (!item) {
        ...
        return derived(reader => {
            ...
            if (selection && selection.model && selection.range && !selection.range.isEmpty()) {
                ...
                const description = lineCount === 1
                    ? localize(12327, null, selection.range.startLineNumber)
                    : localize(12328, null, lineCount);
                items.push({
                    id: 'selected-text',
                    label: localize(12329, null),
                    description,
                    selectedText,
