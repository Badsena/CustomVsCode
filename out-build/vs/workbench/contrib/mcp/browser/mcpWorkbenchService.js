...
let McpWorkbenchService = class McpWorkbenchService extends Disposable {
    ...
    getEnabledLocalMcpServers() {
        ...
        for (const server of userRemote) {
            ...
            if (existing) {
                this.logService.warn(localize(12447, null, server.name, server.mcpResource.path, existing.mcpResource.path));
            }
        ...
        for (const server of workspace) {
            const existing = result.get(server.name);
            if (existing) {
                this.logService.warn(localize(12448, null, server.name, server.mcpResource.path, existing.mcpResource.path));
            }
            result.set(server.name, server);
        }
    ...
    canInstall(mcpServer) {
        if (!(mcpServer instanceof McpWorkbenchServer)) {
            return new MarkdownString().appendText(localize(12449, null));
        }
        if (mcpServer.gallery) {
        ...
        return new MarkdownString().appendText(localize(12450, null, mcpServer.label));
    }
    ...
    getUserMcpConfigPath(mcpResource) {
        return {
            ...
            label: localize(12451, null, this.productService.nameShort),
    ...
    getRuntimeStatus(mcpServer) {
        ...
        if (enablement === 0 /* ContributionEnablementState.DisabledProfile */) {
            return {
                ...
                message: {
                    ...
                    text: new MarkdownString(localize(12452, null))
                }
            };
        }
        if (enablement === 1 /* ContributionEnablementState.DisabledWorkspace */) {
            return {
                ...
                message: {
                    severity: Severity.Info,
                    text: new MarkdownString(localize(12453, null))
                }
            };
        }
        return undefined;
        ...
        if (accessValue === "none" /* McpAccessValue.None */) {
            return {
                ...
                message: {
                    ...
                    text: new MarkdownString(localize(12454, null, settingsCommandLink))
                }
            };
        }
        if (accessValue === "registry" /* McpAccessValue.Registry */) {
            if (!mcpServer.gallery) {
                return {
                    ...
                    message: {
                        severity: Severity.Warning,
                        text: new MarkdownString(localize(12455, null, settingsCommandLink))
                    }
                };
            }
            ...
            if (remoteUrl && !mcpServer.gallery.configuration.remotes?.some(remote => remote.url === remoteUrl)) {
                return {
                    ...
                    message: {
                        severity: Severity.Warning,
                        text: new MarkdownString(localize(12456, null, settingsCommandLink))
                    }
                };
            }
        }
