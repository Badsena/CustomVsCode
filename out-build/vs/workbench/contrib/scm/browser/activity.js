...
let SCMActiveRepositoryController = class SCMActiveRepositoryController extends Disposable {
    ...
    _updateActivityCountBadge(count, store) {
        ...
        const badge = new NumberBadge(count, num => localize(14160, null, num));
    ...
    _updateStatusBar(activeRepository, commands, store) {
        ...
        for (let index = 0; index < commands.length; index++) {
            ...
            const statusbarEntry = {
                name: localize(14161, null) + (genericCommandName ? ` ${genericCommandName}` : ''),
                text: command.title,
                ariaLabel: tooltip,
                tooltip,
...
let SCMActiveResourceContextKeyController = class SCMActiveResourceContextKeyController extends Disposable {
    constructor(editorGroupsService, scmService, uriIdentityService) {
        ...
        const activeResourceHasChangesContextKey = new RawContextKey('scmActiveResourceHasChanges', false, localize(14163, null));
        const activeResourceRepositoryContextKey = new RawContextKey('scmActiveResourceRepository', undefined, localize(14164, null));
