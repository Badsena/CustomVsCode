...
const SHOW_ALL = nls.localize(15028, null);
export const configureTaskIcon = registerIcon('tasks-list-configure', Codicon.gear, nls.localize(15029, null));
const removeTaskIcon = registerIcon('tasks-remove', Codicon.close, nls.localize(15030, null));
...
let TaskQuickPick = TaskQuickPick_1 = class TaskQuickPick extends Disposable {
    ...
    _createTaskEntry(task, extraButtons = []) {
        const buttons = [
            { iconClass: ThemeIcon.asClassName(configureTaskIcon), tooltip: nls.localize(15031, null) },
            ...extraButtons
        ];
    ...
    _createTypeEntries(entries, types) {
        entries.push({ type: 'separator', label: nls.localize(15032, null) });
        types.forEach(type => {
            entries.push({ label: `$(folder) ${type}`, task: type, ariaLabel: nls.localize(15033, null, type) });
        });
    ...
    async getTopLevelEntries(defaultEntry) {
        ...
        if (recentTasks.length > 0) {
            const removeRecentButton = {
                ...
                tooltip: nls.localize(15034, null)
            };
            this._createEntriesForGroup(this._topLevelEntries, recentTasks, nls.localize(15035, null), [removeRecentButton]);
        }
        if (configuredTasks.length > 0) {
            if (dedupedConfiguredTasks.length > 0) {
                this._createEntriesForGroup(this._topLevelEntries, dedupedConfiguredTasks, nls.localize(15036, null));
            }
        }
        if (defaultEntry && (configuredTasks.length === 0)) {
            this._topLevelEntries.push({ type: 'separator', label: nls.localize(15037, null) });
    ...
    async handleSettingOption(selectedType) {
        const { confirmed } = await this._dialogService.confirm({
            ...
            message: nls.localize(15038, null, selectedType),
            cancelButton: nls.localize(15039, null)
        });
        if (confirmed) {
            await this._configurationService.updateValue(`${selectedType}.autoDetect`, 'on');
            await new Promise(resolve => setTimeout(() => resolve(), 100));
            return this.show(nls.localize(15040, null), undefined, selectedType);
        }
        return undefined;
    }
    ...
    static getSettingEntry(configurationService, type) {
        if (configurationService.getValue(`${type}.autoDetect`) === 'off') {
            return {
                label: nls.localize(15041, null, type[0].toUpperCase() + type.slice(1), type),
                task: null,
                settingType: type,
                alwaysShow: true
            };
    ...
    async _getEntriesForProvider(type) {
        ...
        if (tasks.length > 0) {
            ...
            taskQuickPickEntries = [{
                    label: nls.localize(15043, null, type),
                    task: null,
                    alwaysShow: true
                }];
        }
    ...
    async _toTask(task) {
        ...
        if (!resolvedTask) {
            this._notificationService.error(nls.localize(15044, null, task.type));
        }
        return resolvedTask;
    }
};
