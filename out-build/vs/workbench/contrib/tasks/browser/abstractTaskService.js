...
(function (ConfigureTaskAction) {
    ...
    ConfigureTaskAction.TEXT = nls.localize2(14969, "Configure Task");
...
let AbstractTaskService = class AbstractTaskService extends Disposable {
    ...
    static { this.OutputChannelLabel = nls.localize(14856, null); }
    ...
    constructor(_configurationService, _markerService, _outputService, _paneCompositeService, _viewsService, _commandService, _editorService, _fileService, _contextService, _telemetryService, _textFileService, _modelService, _extensionService, _quickInputService, _configurationResolverService, _terminalService, _terminalGroupService, _storageService, _progressService, _openerService, _dialogService, _notificationService, _contextKeyService, _environmentService, _terminalProfileResolverService, _pathService, _textModelResolverService, _preferencesService, _viewDescriptorService, _workspaceTrustRequestService, _workspaceTrustManagementService, _logService, _themeService, _lifecycleService, remoteAgentService, _instantiationService, _chatService, _chatAgentService, _hostService) {
        ...
        this._configurationResolverService.contributeVariable('defaultBuildTask', async () => {
            ...
            if (tasks && tasks.length > 0) {
                entry = await this._showQuickPick(tasks, nls.localize(14857, null));
            }
        ...
        this._register(this.onDidStateChange(async (e) => {
            this._log(nls.localize(14858, null, e.kind), true);
            switch (e.kind) {
    ...
    _attemptTaskReconnection() {
        if (this._lifecycleService.startupKind !== 3 /* StartupKind.ReloadedWindow */) {
            this._log(nls.localize(14859, null), true);
            this._tasksReconnected = true;
            this._storageService.remove(AbstractTaskService_1.PersistentTasks_Key, 1 /* StorageScope.WORKSPACE */);
        }
        if (!this._configurationService.getValue("task.reconnection" /* TaskSettingId.Reconnection */) || this._tasksReconnected) {
            this._log(nls.localize(14860, null, this._configurationService.getValue("task.reconnection" /* TaskSettingId.Reconnection */), this._tasksReconnected), true);
            this._tasksReconnected = true;
            return;
        }
        this._log(nls.localize(14861, null), true);
        this.getWorkspaceTasks(4 /* TaskRunSource.Reconnect */).then(async () => {
            this._tasksReconnected = await this._reconnectTasks();
            this._log(nls.localize(14862, null), true);
            this._onDidReconnectToTasks.fire();
        });
    }
    async _handleLongRunningTaskCompletion(event, durationMs) {
        ...
        const message = taskLabel
            ? nls.localize(14863, null, taskLabel, durationText)
            : nls.localize(14864, null, durationText);
    ...
    _formatTaskDuration(durationMs) {
        ...
        if (minutes > 0) {
            return seconds > 0
                ? nls.localize(14865, null, minutes, seconds)
                : nls.localize(14866, null, minutes);
        }
        return nls.localize(14867, null, seconds);
    }
    async _reconnectTasks() {
        const tasks = await this.getSavedTasks('persistent');
        if (!tasks.length) {
            this._log(nls.localize(14868, null), true);
            return true;
        }
        const taskLabels = tasks.map(task => task._label).join(', ');
        this._log(nls.localize(14869, null, taskLabels), true);
        ...
        CommandsRegistry.registerCommand({
            ...
            metadata: {
                ...
                args: [{
                        name: 'args',
                        isOptional: true,
                        description: nls.localize(14870, null),
                        schema: {
                            anyOf: [
                                {
                                    type: 'string',
                                    description: nls.localize(14871, null)
                                },
                                {
                                    type: 'object',
                                    properties: {
                                        type: {
                                            type: 'string',
                                            description: nls.localize(14872, null)
                                        },
                                        task: {
                                            type: 'string',
                                            description: nls.localize(14873, null)
                                        }
                                    }
                                }
                            ]
    ...
    _showOutput(runSource = 1 /* TaskRunSource.User */, userRequested, errorMessage) {
        if (!VirtualWorkspaceContext.getValue(this._contextKeyService) && ((runSource === 1 /* TaskRunSource.User */) || (runSource === 3 /* TaskRunSource.ConfigurationChange */))) {
            if (userRequested) {
                ...
                actions.push({
                    label: nls.localize(14875, null),
                    run: () => {
                        this._outputService.showChannel(this._outputChannel.id, true);
                    }
                });
                if (chatEnabled && actions.length > 1) {
                    this._notificationService.prompt(Severity.Warning, nls.localize(14876, null), actions);
                }
                else {
                    this._notificationService.prompt(Severity.Warning, nls.localize(14877, null), actions);
                }
            }
        }
    }
    ...
    async getTask(folder, identifier, compareId = false, type = undefined) {
        ...
        if (this.ignoredWorkspaceFolders.some(ignored => ignored.name === name)) {
            return Promise.reject(new Error(nls.localize(14878, null, name)));
        }
    ...
    async tryResolveTask(configuringTask) {
        ...
        if (!matchingProvider) {
            if (matchingProviderUnavailable) {
                this._log(nls.localize(14879, null, configuringTask.configures.type));
            }
            return;
        }
    ...
    _getPersistentTasks() {
        if (this._persistentTasks) {
            this._log(nls.localize(14880, null, this._persistentTasks.size), true);
    ...
    async getSavedTasks(type) {
        ...
        const tasks = [];
        this._log(nls.localize(14881, null), true);
        ...
        for (const entry of storedTasks.entries()) {
            try {
                ...
                this._log(nls.localize(14882, null, key, task, folderInfo.folder), true);
                addTaskToMap(folderInfo.isWorkspaceFile ? workspaceToTaskMap : folderToTasksMap, folderInfo.folder, task);
            }
            catch (error) {
                this._log(nls.localize(14883, null, error), true);
            }
        }
        ...
        for (const key of storedTasks.keys()) {
            if (readTasksMap.has(key)) {
                ...
                this._log(nls.localize(14884, null, key), true);
            }
            else {
                this._log(nls.localize(14885, null, key), true);
            }
        }
        return tasks;
    }
    ...
    removePersistentTask(key) {
        this._log(nls.localize(14886, null, key), true);
    ...
    async _setPersistentTask(task) {
        ...
        if (!InMemoryTask.is(task) && key) {
            ...
            this._log(nls.localize(14887, null, key), true);
    ...
    _savePersistentTasks() {
        ...
        this._log(nls.localize(14888, null, keys.join(', ')), true);
        ...
        if (!runnable || !runnable.task) {
            if (test) {
                if (this.schemaVersion === 1 /* JsonSchemaVersion.V0_1_0 */) {
                    throw new TaskError(Severity.Info, nls.localize(14889, null), 3 /* TaskErrors.NoTestTask */);
                }
                else {
                    throw new TaskError(Severity.Info, nls.localize(14890, null), 3 /* TaskErrors.NoTestTask */);
                }
            }
            else {
                if (this.schemaVersion === 1 /* JsonSchemaVersion.V0_1_0 */) {
                    throw new TaskError(Severity.Info, nls.localize(14891, null), 2 /* TaskErrors.NoBuildTask */);
                }
                else {
                    throw new TaskError(Severity.Info, nls.localize(14892, null), 2 /* TaskErrors.NoBuildTask */);
                }
            }
        }
    ...
    async run(task, options, runSource = 0 /* TaskRunSource.System */) {
        ...
        if (!task) {
            throw new TaskError(Severity.Info, nls.localize(14893, null), 5 /* TaskErrors.TaskNotFound */);
        }
    ...
    async _attachProblemMatcher(task) {
        ...
        entries.unshift({ type: 'separator', label: nls.localize(14894, null) });
        let taskType;
        if (CustomTask.is(task)) {
            const configProperties = task._source.config.element;
        ...
        entries.unshift({ label: nls.localize(14895, null), matcher: undefined }, { label: nls.localize(14896, null), matcher: undefined, never: true }, { label: nls.localize(14897, null, taskType), matcher: undefined, setting: taskType }, { label: nls.localize(14898, null), matcher: undefined, learnMore: true });
        const problemMatcher = await this._quickInputService.pick(entries, { placeHolder: nls.localize(14899, null) });
        if (!problemMatcher) {
            return task;
        }
    ...
    async customize(task, properties, openConfig) {
        ...
        if (configuration.hasParseErrors) {
            this._notificationService.warn(nls.localize(14900, null));
            return Promise.resolve(undefined);
        }
    ...
    _createRunnableTask(tasks, group) {
        ...
        if (workspaceTasks.length > 0) {
            if (workspaceTasks.length > 1) {
                this._log(nls.localize(14902, null));
            }
    ...
    async _saveBeforeRun() {
        ...
        if (saveBeforeRunTaskConfig === SaveBeforeRunConfigOptions.Never) {
            ...
            const { confirmed } = await this._dialogService.confirm({
                message: nls.localize(14903, null),
                detail: nls.localize(14904, null),
                primaryButton: nls.localize(14905, null),
                cancelButton: nls.localize(14906, null),
            });
            if (!confirmed) {
                return false;
            }
    ...
    async _handleExecuteResult(executeResult, runSource) {
        ...
        if (executeResult.kind === 2 /* TaskExecuteKind.Active */) {
            ...
            if (active && active.same) {
                ...
                throw new TaskError(Severity.Warning, nls.localize(14907, null), 1 /* TaskErrors.RunningTask */);
            }
        }
    ...
    _handleInstancePolicy(task, policy) {
        ...
        switch (policy) {
            ...
            case "warn" /* InstancePolicy.warn */:
                this._notificationService.warn(nls.localize(14908, null));
                break;
            case "prompt" /* InstancePolicy.prompt */:
            default:
                this._showQuickPick(this._taskSystem.getActiveTasks().filter(t => task._id === t._id), nls.localize(14909, null), {
                    label: nls.localize(14910, null),
                    task: undefined
    ...
    async _restart(task) {
        ...
        if (isTaskRunning) {
            ...
            if (!response.success) {
                this._notificationService.warn(nls.localize(14911, null, Types.isString(task) ? task : task.configurationProperties.name));
                return;
            }
        }
        ...
        try {
            ...
            if (updatedTask) {
                ...
                if (!success || (typeof success.exitCode === 'number' && success.exitCode !== 0)) {
                    ...
                    this._notificationService.warn(nls.localize(14912, null, task.configurationProperties.name));
                }
            }
        }
        catch {
                        ...
                        raceTimeout(provider.provideTasks(validTypes).then((taskSet) => {
                            ...
                            for (const task of taskSet.tasks) {
                                if (task.type !== this._providerTypes.get(handle)) {
                                    this._log(nls.localize(14913, null, this._providerTypes.get(handle), task.type));
                ...
                if (configurations || legacyTaskConfigurations) {
                    ...
                    const unUsedConfigurationPromises = unUsedConfigurationsAsArray.map(async (value) => {
                        ...
                        if (requiredTaskProviderUnavailable) {
                            this._log(nls.localize(14914, null, configuringTask.configures.type));
                        }
                        else if (!waitToActivate) {
                            this._log(nls.localize(14915, null, configuringTask.configures.type, JSON.stringify(configuringTask._source.config.element, undefined, 4)));
                        }
                    });
    ...
    async _computeWorkspaceFolderTasks(workspaceFolder, runSource = 1 /* TaskRunSource.User */) {
        ...
        if (problemReporter.status.isFatal()) {
            problemReporter.fatal(nls.localize(14916, null));
    ...
    _testParseExternalConfig(config, location) {
        ...
        if (parseErrors) {
            ...
            if (isAffected) {
                this._log(nls.localize(14917, null, location));
                this._showOutput(undefined, undefined, nls.localize(14918, null, location));
    ...
    async _computeWorkspaceFileTasks(workspaceFolder, runSource = 1 /* TaskRunSource.User */) {
        ...
        const configuration = this._testParseExternalConfig(workspaceFileConfig.config, nls.localize(14919, null));
        ...
        if (engine === ExecutionEngine.Process) {
            this._notificationService.warn(nls.localize(14920, null));
            return this._emptyWorkspaceTaskResults(workspaceFolder);
        }
    ...
    async _computeUserTasks(workspaceFolder, runSource = 1 /* TaskRunSource.User */) {
        ...
        const configuration = this._testParseExternalConfig(userTasksConfig.config, nls.localize(14921, null));
        const customizedTasks = {
            byIdentifier: Object.create(null)
        };
        const custom = [];
        ...
        const engine = configuration.config ? TaskConfig.ExecutionEngine.from(configuration.config) : ExecutionEngine.Terminal;
        if (engine === ExecutionEngine.Process) {
            this._notificationService.warn(nls.localize(14922, null));
            return this._emptyWorkspaceTaskResults(workspaceFolder);
        }
        return { workspaceFolder, set: { tasks: custom }, configurations: customizedTasks, hasErrors: configuration.hasParseErrors };
    }
    ...
    async _computeTasksForSingleConfig(workspaceFolder, config, runSource, custom, customized, source, isRecentTask = false) {
        ...
        if (problemReporter.status.isFatal()) {
            problemReporter.fatal(nls.localize(14923, null));
            return hasErrors;
        }
        if (parseResult.configured && parseResult.configured.length > 0) {
            for (const task of parseResult.configured) {
        ...
        if (this._contextService.getWorkbenchState() === 2 /* WorkbenchState.FOLDER */) {
            ...
            for (const workspaceFolder of this._contextService.getWorkspace().folders) {
                if (schemaVersion === this._computeJsonSchemaVersion(workspaceFolder)) {
                    ...
                    this._log(nls.localize(14924, null, workspaceFolder.uri.fsPath));
                }
            }
        }
    ...
    _getConfiguration(workspaceFolder, source) {
        ...
        if (parseErrors) {
            ...
            if (isAffected) {
                this._log(nls.localize(14925, null));
                this._showOutput(undefined, undefined, nls.localize(14926, null));
        ...
        if (err instanceof TaskError) {
            ...
            if (needsConfig || needsTerminate) {
                this._notificationService.prompt(buildError.severity, buildError.message, [{
                        label: needsConfig ? ConfigureTaskAction.TEXT.value : nls.localize(14927, null),
                        run: () => {
                            if (needsConfig) {
                                this._runConfigureTasks();
                            }
            ...
            this._notificationService.error(nls.localize(14928, null));
        }
        if (showOutput) {
    ...
    async _createTaskQuickPickEntries(tasks, group = false, sort = false, selectedEntry, includeRecents = true) {
        ...
        function fillEntries(entries, tasks, groupLabel) {
            ...
            for (const task of tasks) {
                ...
                entry.buttons = [{ iconClass: ThemeIcon.asClassName(configureTaskIcon), tooltip: nls.localize(14929, null) }];
        ...
        if (group) {
            ...
            if (tasks.length === 1) {
                ...
                if (includeRecents) {
                    fillEntries(entries, recent, nls.localize(14930, null));
                }
                configured = configured.sort((a, b) => sorter.compare(a, b));
                fillEntries(entries, configured, nls.localize(14931, null));
                detected = detected.sort((a, b) => sorter.compare(a, b));
                fillEntries(entries, detected, nls.localize(14932, null));
            }
        }
        else {
            if (sort) {
    ...
    _showIgnoredFoldersMessage() {
        ...
        this._notificationService.prompt(Severity.Info, nls.localize(14933, null, this.ignoredWorkspaceFolders.map(f => f.name).join(', ')), [{
                label: nls.localize(14934, null),
                isSecondary: true,
                run: () => {
    ...
    async _trust() {
        ...
        if (!this._workspaceTrustManagementService.isWorkspaceTrusted()) {
            return (await this._workspaceTrustRequestService.requestWorkspaceTrust({
                message: nls.localize(14935, null)
            })) === true;
        }
        return true;
    }
    ...
    _doRunTaskCommand(tasks, type, name) {
        ...
        const placeholder = nls.localize(14936, null);
        this._showIgnoredFoldersMessage().then(() => {
            if (this._configurationService.getValue(USE_SLOW_PICKER)) {
                ...
                this._showQuickPick(tasks ? tasks : taskResult.tasks, placeholder, {
                    label: '$(plus) ' + nls.localize(14937, null),
                    task: null
                ...
                this._showTwoLevelQuickPick(placeholder, {
                    label: '$(plus) ' + nls.localize(14938, null),
                    task: null
                }, type, name).
                    then(pickThen);
            }
    ...
    _runBuildCommand() {
        ...
        return this._runTaskGroupCommand(TaskGroup.Build, {
            fetching: nls.localize(14939, null),
            select: nls.localize(14940, null),
            notFoundConfigure: nls.localize(14941, null)
        }, this._runConfigureDefaultBuildTask, this._build);
    }
    _runTestCommand() {
        return this._runTaskGroupCommand(TaskGroup.Test, {
            fetching: nls.localize(14942, null),
            select: nls.localize(14943, null),
            notFoundConfigure: nls.localize(14944, null)
    ...
    _runTerminateCommand(arg) {
        ...
        const runQuickPick = (promise) => {
            this._showQuickPick(promise || this.getActiveTasks(), nls.localize(14945, null), {
                label: nls.localize(14946, null),
                task: undefined
            }, false, true, undefined, [{
                    label: nls.localize(14947, null),
                    id: 'terminateAll',
                    task: undefined
                }]).then(entry => {
                ...
                if (active) {
                    this._terminateAll().then((responses) => {
                        ...
                        if (response.code && response.code === 3 /* TerminateResponseCode.ProcessNotFound */) {
                            this._notificationService.error(nls.localize(14948, null));
                        }
                        else {
                            this._notificationService.error(nls.localize(14949, null));
                        }
                    });
                }
            });
    ...
    async _runRestartTaskCommand(arg) {
        ...
        if (this.inTerminal()) {
            ...
            const entry = await this._showQuickPick(activeTasks, nls.localize(14950, null), {
                label: nls.localize(14951, null),
                task: null
            }, false, true);
            if (entry && entry.task) {
    ...
    async _runRerunAllRunningTasksCommand() {
        const activeTasks = await this.getActiveTasks();
        if (activeTasks.length === 0) {
            this._notificationService.info(nls.localize(14952, null));
            return;
        }
    ...
    _openTaskFile(resource, taskSource) {
        ...
        this._fileService.stat(resource).then((stat) => stat, () => undefined).then(async (stat) => {
            ...
            let content;
            if (!tasksExistInFile) {
                const pickTemplateResult = await this._quickInputService.pick(getTaskTemplates(), { placeHolder: nls.localize(14953, null) });
    ...
    getTaskDescription(task) {
        ...
        if (task._source.kind === TaskSourceKind.User) {
            description = nls.localize(14954, null);
        }
    ...
    async _runConfigureTasks() {
        ...
        const createLabel = nls.localize(14955, null);
        const openLabel = nls.localize(14956, null);
        ...
        this._quickInputService.pick(entriesWithSettings, { placeHolder: nls.localize(14957, null) }, cancellationToken).
                ...
                this._showIgnoredFoldersMessage().then(async () => {
                    ...
                    for (const task of tasks) {
                        if (task === defaultBuildTask) {
                            const label = nls.localize(14958, null, TaskQuickPick.getTaskLabelWithIcon(task, task.getQualifiedLabel()));
                            selectedTask = task;
                    ...
                    const tokenSource = new CancellationTokenSource();
                    const cancellationToken = tokenSource.token;
                    this._quickInputService.pick(entries, { placeHolder: nls.localize(14959, null) }, cancellationToken).
                        then(async (entry) => {
                        if (cancellationToken.isCancellationRequested) {
                            // canceled when there's only one task
                            const task = (await entries)[0];
                    ...
                    this._quickInputService.pick(entries, {
                        placeHolder: nls.localize(14960, null)
                    }).
                        then((entry) => {
                        const task = entry && Object.hasOwn(entry, 'task') ? entry.task : undefined;
                        if ((task === undefined) || (task === null)) {
    ...
    _runConfigureDefaultTestTask() {
        if (this.schemaVersion === 2 /* JsonSchemaVersion.V2_0_0 */) {
            this.tasks().then((tasks => {
                ...
                if (selectedTask) {
                    selectedEntry = {
                        label: nls.localize(14961, null, selectedTask.getQualifiedLabel()),
                        task: selectedTask,
                        detail: this._showDetail() ? selectedTask.configurationProperties.detail : undefined
                    };
                }
                this._showIgnoredFoldersMessage().then(() => {
                    this._showQuickPick(tasks, nls.localize(14962, null), undefined, true, false, selectedEntry).then((entry) => {
                        const task = entry && Object.hasOwn(entry, 'task') ? entry.task : undefined;
                        if (!task) {
                            return;
                        }
    ...
    async runShowTasks() {
        ...
        if (activeTasks.length === 1) {
            ...
            this._showQuickPick(activeTasksPromise, nls.localize(14963, null), {
                label: nls.localize(14964, null),
                task: null
    ...
    async _upgrade() {
        ...
        this._notificationService.prompt(Severity.Warning, fileDiffs.length === 1 ?
            nls.localize(14965, null)
            : nls.localize(14966, null), [{
                label: fileDiffs.length === 1 ? nls.localize(14967, null) : nls.localize(14968, null),
                run: async () => {
