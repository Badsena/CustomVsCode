...
export class TerminalTaskSystem extends Disposable {
    ...
    constructor(_terminalService, _terminalGroupService, _outputService, _paneCompositeService, _viewsService, _markerService, _modelService, _configurationResolverService, _contextService, _environmentService, _outputChannelId, _fileService, _terminalProfileResolverService, _pathService, _viewDescriptorService, _logService, _notificationService, contextKeyService, instantiationService, taskSystemInfoResolver, _taskLookup) {
        ...
        this._terminalTabActions = [{ id: RerunForActiveTerminalCommandId, label: nls.localize(15058, null), icon: rerunTaskIcon }];
    ...
    run(task, resolver, trigger = Triggers.command) {
        ...
        try {
            ...
            if (error instanceof TaskError) {
                ...
                throw new TaskError(Severity.Error, nls.localize(15059, null), 7 /* TaskErrors.UnknownError */);
            }
        }
    }
    ...
    _showTaskLoadErrors(task) {
        if (task.taskLoadMessages && task.taskLoadMessages.length > 0) {
            ...
            this._notificationService.prompt(Severity.Warning, nls.localize(15060, null, task._label), [{
                    label: openOutput,
    ...
    _showDependencyCycleMessage(task) {
        this._log(nls.localize(15061, null, task._label));
        this._showOutput();
    }
            ...
            if (task.configurationProperties.dependsOn) {
                ...
                for (const dependency of task.configurationProperties.dependsOn) {
                    ...
                    if (dependencyTask) {
                        ...
                        this._log(nls.localize(15062, null, Types.isString(dependency.task) ? dependency.task : JSON.stringify(dependency.task, undefined, 0), dependency.uri.toString()));
                        this._showOutput();
                    }
                }
            }
    ...
    async _executeInTerminal(task, trigger, resolver, workspaceFolder) {
        ...
        if (task.configurationProperties.isBackground) {
            ...
            if ((problemMatchers.length > 0) && !watchingProblemMatcher.isWatching()) {
                this._appendOutput(nls.localize(15063, null, task._label));
                this._showOutput();
            }
        ...
        if (isShellCommand) {
            ...
            if (task.command.presentation && task.command.presentation.echo) {
                if (needsFolderQualification && workspaceFolder) {
                    ...
                    shellLaunchConfig.initialText = this.taskShellIntegrationStartSequence(cwd) + formatMessageForTerminal(nls.localize(15064, null, folder, commandLine), { excludeLeadingNewLine: true }) + this.getTaskShellIntegrationOutputSequence(commandLineInfo);



                }
                else {
                    shellLaunchConfig.initialText = this.taskShellIntegrationStartSequence(cwd) + formatMessageForTerminal(nls.localize(15065, null, commandLine), { excludeLeadingNewLine: true }) + this.getTaskShellIntegrationOutputSequence(commandLineInfo);
            ...
            if (task.command.presentation && task.command.presentation.echo) {
                ...
                if (needsFolderQualification && workspaceFolder) {
                    shellLaunchConfig.initialText = this.taskShellIntegrationStartSequence(cwd) + formatMessageForTerminal(nls.localize(15066, null, workspaceFolder.name, `${shellLaunchConfig.executable} ${getArgsToEcho(shellLaunchConfig.args)}`), { excludeLeadingNewLine: true }) + this.getTaskShellIntegrationOutputSequence(undefined);



                }
                else {
                    shellLaunchConfig.initialText = this.taskShellIntegrationStartSequence(cwd) + formatMessageForTerminal(nls.localize(15067, null, `${shellLaunchConfig.executable} ${getArgsToEcho(shellLaunchConfig.args)}`), { excludeLeadingNewLine: true }) + this.getTaskShellIntegrationOutputSequence(undefined);
    ...
    async _createTerminal(task, resolver, workspaceFolder) {
        ...
        if (task.command.runtime === RuntimeType.CustomExecution) {
            this._currentTask.shellLaunchConfig = launchConfigs = {
                ...
                initialText: task.command.presentation && task.command.presentation.echo ? formatMessageForTerminal(nls.localize(15068, null, task._label), { excludeLeadingNewLine: true }) : undefined,
            ...
            if (launchConfigs === undefined) {
                return [undefined, new TaskError(Severity.Error, nls.localize(15069, null), 7 /* TaskErrors.UnknownError */)];
            }
        }
    ...
    async _resolveMatchers(resolver, values) {
        ...
        for (const value of values) {
            ...
            if (!matcher) {
                this._appendOutput(nls.localize(15070, null));
                continue;
            }
    ...
    if ((presentationOptions.close === undefined) || (presentationOptions.close === false)) {
        if ((presentationOptions.reveal !== RevealKind.Never) || !configurationProperties.isBackground || (presentationOptions.close === false)) {
            if (presentationOptions.panel === PanelKind.New) {
                return taskShellIntegrationWaitOnExitSequence(nls.localize(15071, null));
            }
            else if (presentationOptions.showReuseMessage) {
                return taskShellIntegrationWaitOnExitSequence(nls.localize(15072, null));
            }
            else {
                return true;
            }
