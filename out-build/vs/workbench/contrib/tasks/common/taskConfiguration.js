    ...
    function from(options, context) {
        ...
        if (options.cwd !== undefined) {
            if (Types.isString(options.cwd)) {
                ...
                context.taskLoadIssues.push(nls.localize(15282, null, options.cwd));
            }
        }
        ...
        if (config.args !== undefined) {
            ...
            for (const arg of config.args) {
                ...
                if (converted !== undefined) {
                    ...
                    context.taskLoadIssues.push(nls.localize(15283, null, arg ? JSON.stringify(arg, undefined, 4) : 'undefined'));
                }
            }
        }
        if (config.options !== undefined) {
            ...
            if (result.options && result.options.shell === undefined && ShellConfiguration.is(config.isShellCommand)) {
                ...
                if (context.engine !== Tasks.ExecutionEngine.Terminal) {
                    context.taskLoadIssues.push(nls.localize(15284, null));
                }
            }
        }
...
(function (ProblemMatcherConverter) {
    function namedFrom(declares, context) {
        ...
        declares.forEach((value) => {
            ...
            if (isNamedProblemMatcher(namedProblemMatcher)) {
                ...
                context.problemReporter.error(nls.localize(15285, null, JSON.stringify(value, undefined, 4)));
            }
        });
        return result;
    }
    ...
    function from(config, context) {
        ...
        if (kind === ProblemMatcherKind.Unknown) {
            const error = nls.localize(15286, null, JSON.stringify(config, null, 4));
    ...
    function resolveProblemMatcher(value, context) {
        if (Types.isString(value)) {
            ...
            return { errors: [nls.localize(15287, null, value)] };
        }
        else {
            const json = value;
...
(function (ConfiguringTask) {
    ...
    function from(external, context, index, source, registry) {
        ...
        if (!type && !customize) {
            context.problemReporter.error(nls.localize(15288, null, JSON.stringify(external, null, 4)));
            return undefined;
        }
        const typeDeclaration = type ? registry?.get?.(type) || TaskDefinitionRegistry.get(type) : undefined;
        if (!typeDeclaration) {
            const message = nls.localize(15289, null, type);
        ...
        if (identifier === undefined) {
            context.problemReporter.error(nls.localize(15290, null, JSON.stringify(external, undefined, 0)));
            return undefined;
        }
        ...
        if (taskIdentifier === undefined) {
            context.problemReporter.error(nls.localize(15291, null, JSON.stringify(external, undefined, 0)));
            return undefined;
        }
        const configElement = {
...
(function (CustomTask) {
    function from(external, context, index, source) {
        ...
        if (type !== Tasks.CUSTOMIZED_TASK_TYPE && type !== 'shell' && type !== 'process') {
            context.problemReporter.error(nls.localize(15292, null, JSON.stringify(external, null, 4)));
            return undefined;
        }
        ...
        if (!taskName) {
            context.problemReporter.error(nls.localize(15293, null, JSON.stringify(external, null, 4)));
            return undefined;
        }
        let taskSource;
        switch (source) {
...
(function (TaskParser) {
    ...
    function from(externals, globals, context, source, registry) {
        ...
        for (let index = 0; index < externals.length; index++) {
            ...
            if (typeNotSupported) {
                context.problemReporter.info(nls.localize(15294, null, external.type));
                continue;
            }
                ...
                if (customTask) {
                    ...
                    if (schema2_0_0) {
                        if ((customTask.command === undefined || customTask.command.name === undefined) && (customTask.configurationProperties.dependsOn === undefined || customTask.configurationProperties.dependsOn.length === 0)) {
                            context.problemReporter.error(nls.localize(15295, null, customTask.configurationProperties.name, JSON.stringify(external, undefined, 4)));
                            continue;
                        }
                    }
                    else {
                        if (customTask.command === undefined || customTask.command.name === undefined) {
                            context.problemReporter.warn(nls.localize(15296, null, customTask.configurationProperties.name, JSON.stringify(external, undefined, 4)));
                            continue;
                        }
                    }
...
class ConfigurationParser {
    ...
    createTaskRunnerConfiguration(fileConfig, context, source) {
        ...
        if (context.schemaVersion === 2 /* Tasks.JsonSchemaVersion.V2_0_0 */ && globalTasks && globalTasks.length > 0 && externalGlobalTasks && externalGlobalTasks.length > 0) {
            ...
            context.problemReporter.error(nls.localize(15297, null, taskContent.join('\n')));
        }
