...
KeybindingsRegistry.registerCommandAndKeybindingRule({
    ...
    handler: async (accessor, resource) => {
        ...
        if (uri && fileService.hasProvider(uri)) {
            ...
            const editorLabel = nls.localize(11328, null, name, name);
            try {
...
async function doSaveEditors(accessor, editors, options) {
    ...
    try {
        ...
        if (!isCancellationError(error)) {
            const actions = [toAction({ id: 'workbench.action.files.saveEditors', label: nls.localize(11329, null), run: () => instantiationService.invokeFunction(accessor => doSaveEditors(accessor, editors, options)) })];
            ...
            if (editorsToRevert.length > 0) {
                actions.push(toAction({ id: 'workbench.action.files.revertEditors', label: editorsToRevert.length > 1 ? nls.localize(11330, null) : nls.localize(11331, null), run: () => editorService.revert(editorsToRevert) }));
            }
            notificationService.notify({
                id: editors.map(({ editor }) => hash(editor.resource?.toString())).join(), // ensure unique notification ID per set of editor
                severity: Severity.Error,
                message: nls.localize(11332, null, editors.map(({ editor }) => editor.getName()).join(', '), toErrorMessage(error, false)),
...
CommandsRegistry.registerCommand({
    ...
    handler: async (accessor) => {
        ...
        try {
            ...
            const notificationService = accessor.get(INotificationService);
            notificationService.error(nls.localize(11333, null, editors.map(({ editor }) => editor.getName()).join(', '), toErrorMessage(error, false)));
        }
    }
});
CommandsRegistry.registerCommand({
...
CommandsRegistry.registerCommand({
    ...
    handler: async (accessor, args) => {
        ...
        const fileService = accessor.get(IFileService);
        const createFileLocalized = nls.localize(11334, null);
