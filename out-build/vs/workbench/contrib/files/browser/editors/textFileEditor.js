    ...
    async handleSetInputError(error, input, options) {
        ...
        if (error.fileOperationResult === 0 /* FileOperationResult.FILE_IS_DIRECTORY */) {
            ...
            actions.push(toAction({
                id: 'workbench.files.action.openFolder', label: localize(11168, null), run: async () => {
            ...
            if (this.contextService.isInsideWorkspace(input.preferredResource)) {
                actions.push(toAction({
                    id: 'workbench.files.action.reveal', label: localize(11169, null), run: async () => {
            ...
            throw createEditorOpenError(localize(11170, null), actions, { forceMessage: true });
        }
        // Handle case where a file is too large to open without confirmation
        if (error.fileOperationResult === 7 /* FileOperationResult.FILE_TOO_LARGE */) {
            let message;
            if (error instanceof TooLargeFileOperationError) {
                message = localize(11171, null, ByteSize.formatSize(error.size));
            }
            else {
                message = localize(11172, null);
            }
        ...
        if (error.fileOperationResult === 1 /* FileOperationResult.FILE_NOT_FOUND */ &&
            ...
            const fileNotFoundError = createEditorOpenError(new FileOperationError(localize(11173, null), 1 /* FileOperationResult.FILE_NOT_FOUND */), [
                toAction({
                    id: 'workbench.files.action.createMissingFile', label: localize(11174, null), run: async () => {
