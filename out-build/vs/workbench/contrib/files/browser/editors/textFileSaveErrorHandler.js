...
const conflictEditorHelp = localize(11175, null);
    ...
    onSaveError(error, model, options) {
        ...
        if (fileOperationError.fileOperationResult === 3 /* FileOperationResult.FILE_MODIFIED_SINCE */) {
            ...
            // Otherwise show the message that will lead the user into the save conflict editor.
                ...
                message = localize(11176, null, basename(resource));
        ...
        // Any other save error
            ...
            if (isWriteLocked) {
                if (triedToUnlock && canSaveElevated) {
                    message = isWindows ? localize(11177, null, basename(resource)) : localize(11178, null, basename(resource));
                }
                else {
                    message = localize(11179, null, basename(resource));
                }
            }
            else if (canSaveElevated && isPermissionDenied) {
                message = isWindows ? localize(11180, null, basename(resource)) : localize(11181, null, basename(resource));
            }
            else {
                message = localize(11182, null, basename(resource), toErrorMessage(error, false));
            }
        }
...
let ResolveConflictLearnMoreAction = class ResolveConflictLearnMoreAction extends Action {
    constructor(openerService) {
        super('workbench.files.action.resolveConflictLearnMore', localize(11183, null));
...
let DoNotShowResolveConflictLearnMoreAction = class DoNotShowResolveConflictLearnMoreAction extends Action {
    constructor(storageService) {
        super('workbench.files.action.resolveConflictLearnMoreDoNotShowAgain', localize(11184, null));
        this.storageService = storageService;
    }
...
let ResolveSaveConflictAction = class ResolveSaveConflictAction extends Action {
    constructor(model, editorService, notificationService, instantiationService, productService) {
        super('workbench.files.action.resolveConflict', localize(11185, null));
        this.model = model;
        this.editorService = editorService;
        this.notificationService = notificationService;
        this.instantiationService = instantiationService;
    ...
    async run() {
        if (!this.model.isDisposed()) {
            ...
            const editorLabel = localize(11186, null, name, name, this.productService.nameLong);
...
class SaveModelElevatedAction extends Action {
    constructor(model, options, triedToUnlock) {
        super('workbench.files.action.saveModelElevated', triedToUnlock ? isWindows ? localize(11187, null) : localize(11188, null) : isWindows ? localize(11189, null) : localize(11190, null));
        this.model = model;
...
class RetrySaveModelAction extends Action {
    constructor(model, options) {
        super('workbench.files.action.saveModel', localize(11191, null));
        this.model = model;
        this.options = options;
    }
    async run() {
...
class RevertModelAction extends Action {
    constructor(model) {
        super('workbench.files.action.revertModel', localize(11192, null));
        this.model = model;
    }
    async run() {
        if (!this.model.isDisposed()) {
...
class UnlockModelAction extends Action {
    constructor(model, options) {
        super('workbench.files.action.unlock', localize(11193, null));
        this.model = model;
        this.options = options;
    }
    async run() {
...
class SaveModelIgnoreModifiedSinceAction extends Action {
    constructor(model, options) {
        super('workbench.files.action.saveIgnoreModifiedSince', localize(11194, null));
        this.model = model;
        this.options = options;
    }
    async run() {
...
let ConfigureSaveConflictAction = class ConfigureSaveConflictAction extends Action {
    constructor(preferencesService) {
        super('workbench.files.action.configureSaveConflict', localize(11195, null));
