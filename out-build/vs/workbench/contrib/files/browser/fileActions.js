...
export const NEW_FILE_LABEL = nls.localize2(11308, "New File...");
export const NEW_FOLDER_COMMAND_ID = 'explorer.newFolder';
export const NEW_FOLDER_LABEL = nls.localize2(11309, "New Folder...");
export const TRIGGER_RENAME_LABEL = nls.localize(11233, null);
export const MOVE_FILE_TO_TRASH_LABEL = nls.localize(11234, null);
export const COPY_FILE_LABEL = nls.localize(11235, null);
export const PASTE_FILE_LABEL = nls.localize(11236, null);
export const FileCopiedContext = new RawContextKey('fileCopied', false);
export const DOWNLOAD_COMMAND_ID = 'explorer.download';
export const DOWNLOAD_LABEL = nls.localize(11237, null);
export const UPLOAD_COMMAND_ID = 'explorer.upload';
export const UPLOAD_LABEL = nls.localize(11238, null);
...
async function deleteFiles(explorerService, workingCopyFileService, dialogService, configurationService, filesConfigurationService, elements, useTrash, skipConfirm = false, ignoreIfNotExists = false) {
    let primaryButton;
    if (useTrash) {
        primaryButton = isWindows ? nls.localize(11239, null) : nls.localize(11240, null);
    }
    else {
        primaryButton = nls.localize(11241, null);
    }
    // Handle dirty
    ...
    if (dirtyWorkingCopies.size) {
        ...
        if (distinctElements.length > 1) {
            message = nls.localize(11242, null);
        }
        else if (distinctElements[0].isDirectory) {
            if (dirtyWorkingCopies.size === 1) {
                message = nls.localize(11243, null, distinctElements[0].name);
            }
            else {
                message = nls.localize(11244, null, distinctElements[0].name, dirtyWorkingCopies.size);
            }
        }
        else {
            message = nls.localize(11245, null, distinctElements[0].name);
        }
        const response = await dialogService.confirm({
            type: 'warning',
            message,
            detail: nls.localize(11246, null),
            primaryButton
        });
    ...
    if (!skipConfirm) {
        ...
        if (readonlyResources.length) {
            ...
            if (readonlyResources.length > 1) {
                message = nls.localize(11247, null);
            }
            else if (readonlyResources[0].isDirectory) {
                message = nls.localize(11248, null, distinctElements[0].name);
            }
            else {
                message = nls.localize(11249, null, distinctElements[0].name);
            }
            const response = await dialogService.confirm({
                type: 'warning',
                message,
                detail: nls.localize(11250, null),
                primaryButton: nls.localize(11251, null)
            });
            if (!response.confirmed) {
                return;
            }
    ...
    const deleteDetail = distinctElements.some(e => e.isDirectory) ? nls.localize(11252, null) :
        distinctElements.length > 1 ? nls.localize(11253, null) : nls.localize(11254, null);
    ...
    // Confirm for moving to trash
        ...
        detail += detail ? '\n' : '';
        if (isWindows) {
            detail += distinctElements.length > 1 ? nls.localize(11255, null) : nls.localize(11256, null);
        }
        else {
            detail += distinctElements.length > 1 ? nls.localize(11257, null) : nls.localize(11258, null);
        }
    ...
    try {
        ...
        const options = {
            undoLabel: distinctElements.length > 1 ? nls.localize(11260, null, distinctElements.length) : nls.localize(11261, null, distinctElements[0].name),
            progressLabel: distinctElements.length > 1 ? nls.localize(11262, null, distinctElements.length) : nls.localize(11263, null, distinctElements[0].name),
        };
        ...
        let primaryButton;
        if (useTrash) {
            errorMessage = isWindows ? nls.localize(11264, null) : nls.localize(11265, null);
            detailMessage = deleteDetail;
            primaryButton = nls.localize(11266, null);
        }
        else {
            errorMessage = toErrorMessage(error, false);
            primaryButton = nls.localize(11267, null);
        }
...
function getMoveToTrashMessage(distinctElements) {
    if (containsBothDirectoryAndFile(distinctElements)) {
        return {
            message: nls.localize(11268, null, distinctElements.length),
    ...
    if (distinctElements.length > 1) {
        if (distinctElements[0].isDirectory) {
            return {
                message: nls.localize(11269, null, distinctElements.length),
                detail: getFileNamesMessage(distinctElements.map(e => e.resource))
            };
        }
        return {
            message: nls.localize(11270, null, distinctElements.length),
            detail: getFileNamesMessage(distinctElements.map(e => e.resource))
        };
    }
    if (distinctElements[0].isDirectory && !distinctElements[0].isSymbolicLink) {
        return { message: nls.localize(11271, null, distinctElements[0].name), detail: '' };
    }
    return { message: nls.localize(11272, null, distinctElements[0].name), detail: '' };
}
function getDeleteMessage(distinctElements) {
    if (containsBothDirectoryAndFile(distinctElements)) {
        return {
            message: nls.localize(11273, null, distinctElements.length),
            detail: getFileNamesMessage(distinctElements.map(e => e.resource))
        };
    }
    if (distinctElements.length > 1) {
        if (distinctElements[0].isDirectory) {
            return {
                message: nls.localize(11274, null, distinctElements.length),
                detail: getFileNamesMessage(distinctElements.map(e => e.resource))
            };
        }
        return {
            message: nls.localize(11275, null, distinctElements.length),
            detail: getFileNamesMessage(distinctElements.map(e => e.resource))
        };
    }
    if (distinctElements[0].isDirectory) {
        return { message: nls.localize(11276, null, distinctElements[0].name), detail: '' };
    }
    return { message: nls.localize(11277, null, distinctElements[0].name), detail: '' };
}
...
async function askForOverwrite(fileService, dialogService, targetResource) {
    ...
    const { confirmed } = await dialogService.confirm({
        ...
        message: nls.localize(11278, null, basename(targetResource.path)),
        primaryButton: nls.localize(11279, null)
    });
    return confirmed;
}
// Global Compare with
export class GlobalCompareResourcesAction extends Action2 {
    ...
    static { this.LABEL = nls.localize2(11310, "Compare Active File With..."); }
    constructor() {
        super({
            ...
            metadata: {
                description: nls.localize2(11311, "Opens a picker to select a file to diff with the active editor.")
            }
        });
    }
    async run(accessor) {
...
export class ToggleAutoSaveAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: nls.localize2(11312, "Toggle Auto Save"),
            f1: true,
            category: Categories.File,
            metadata: { description: nls.localize2(11313, "Toggle the ability to save files automatically after typing") }
        });
    }
    run(accessor) {
...
export class SaveAllInGroupAction extends BaseSaveAllAction {
    ...
    static { this.LABEL = nls.localize(11280, null); }
    get class() {
...
let CloseGroupAction = class CloseGroupAction extends Action {
    ...
    static { this.LABEL = nls.localize(11281, null); }
...
export class FocusFilesExplorer extends Action2 {
    ...
    static { this.LABEL = nls.localize2(11314, "Focus on Files Explorer"); }
    constructor() {
        super({
            ...
            category: Categories.File,
            metadata: {
                description: nls.localize2(11315, "Moves focus to the file explorer view container.")
            }
        });
    }
    async run(accessor) {
...
export class ShowActiveFileInExplorer extends Action2 {
    ...
    static { this.LABEL = nls.localize2(11316, "Reveal Active File in Explorer View"); }
    constructor() {
        super({
            ...
            category: Categories.File,
            metadata: {
                description: nls.localize2(11317, "Reveals and selects the active file within the explorer view.")
            }
        });
    }
    async run(accessor) {
...
export class OpenActiveFileInEmptyWorkspace extends Action2 {
    ...
    static { this.LABEL = nls.localize2(11318, "Open Active Editor in New Empty Workspace"); }
    constructor() {
        super({
            ...
            metadata: {
                description: nls.localize2(11319, "Opens the active editor in a new window with no folders open.")
            }
        });
    }
    async run(accessor) {
        ...
        if (fileResource && fileService.hasProvider(fileResource)) {
            ...
            dialogService.error(nls.localize(11282, null));
        }
    }
}
export function validateFileName(pathService, item, name, os) {
    ...
    if (!name || name.length === 0 || /^\s+$/.test(name)) {
        return {
            content: nls.localize(11283, null),
    ...
    if (name[0] === '/' || name[0] === '\\') {
        return {
            content: nls.localize(11284, null),
            severity: Severity.Error
        };
    }
    ...
    if (name !== item.name) {
        ...
        if (child && child !== item) {
            return {
                content: nls.localize(11285, null, name),
                severity: Severity.Error
            };
        }
    }
    ...
    if (names.some(folderName => !pathService.hasValidBasename(item.resource, os, folderName))) {
        ...
        return {
            content: nls.localize(11286, null, trimLongName(escapedName)),
            severity: Severity.Error
        };
    }
    if (names.some(name => /^\s|\s$/.test(name))) {
        return {
            content: nls.localize(11287, null),
...
export class CompareNewUntitledTextFilesAction extends Action2 {
    ...
    static { this.LABEL = nls.localize2(11320, "Compare New Untitled Text Files"); }
    constructor() {
        super({
            ...
            category: Categories.File,
            metadata: {
                description: nls.localize2(11321, "Opens a new diff editor with two untitled files.")
            }
        });
    }
    async run(accessor) {
...
export class CompareWithClipboardAction extends Action2 {
    ...
    static { this.LABEL = nls.localize2(11322, "Compare Active File with Clipboard"); }
    ...
    constructor() {
        super({
            ...
            metadata: {
                description: nls.localize2(11323, "Opens a new diff editor to compare the active file with the contents of the clipboard.")
            }
        });
    }
    async run(accessor) {
        ...
        if (resource && (fileService.hasProvider(resource) || resource.scheme === Schemas.untitled)) {
            ...
            const editorLabel = nls.localize(11288, null, name);
            await editorService.openEditor({
    ...
    const onSuccess = async (value) => {
        try {
            ...
            await explorerService.applyBulkEdit([new ResourceFileEdit(undefined, resourceToCreate, { folder: isFolder })], {
                undoLabel: nls.localize(11290, null, value),
                progressLabel: nls.localize(11291, null, value),
                ...
                if (stat.resource.toString() !== targetResource.toString()) {
                    try {
                        await explorerService.applyBulkEdit([new ResourceFileEdit(stat.resource, targetResource)], {
                            ...
                            undoLabel: nls.localize(11292, null, stat.name, value),
                            progressLabel: nls.localize(11293, null, stat.name, value),
                        });
                        await refreshIfSeparator(value, explorerService);
                    }
                    catch (e) {
...
export const pasteFileHandler = async (accessor, fileList) => {
    ...
    if (confirmPasteNative && toPaste.files.length >= 1) {
        const message = toPaste.files.length > 1 ?
            nls.localize(11294, null, toPaste.files.length) :
            nls.localize(11295, null, basename(toPaste.type === 'paths' ? toPaste.files[0].fsPath : toPaste.files[0].name));
        ...
        const confirmation = await dialogService.confirm({
            ...
            detail,
            checkbox: {
                label: nls.localize(11296, null)
            },
            primaryButton: nls.localize(11297, null)
        });
        if (!confirmation.confirmed) {
            return;
        }
    ...
    try {
        ...
        if (toPaste.type === 'paths') { // Pasting from files on disk
            ...
            const sourceTargetPairs = coalesce(await Promise.all(toPaste.files.map(async (fileToPaste) => {
                if (element.resource.toString() !== fileToPaste.toString() && resources.isEqualOrParent(element.resource, fileToPaste)) {
                    throw new Error(nls.localize(11298, null));
                }
            ...
            if (sourceTargetPairs.length >= 1) {
                ...
                if (pasteShouldMove) {
                    ...
                    const options = {
                        confirmBeforeUndo: configurationService.getValue().explorer.confirmUndo === "verbose" /* UndoConfirmLevel.Verbose */,
                        progressLabel: sourceTargetPairs.length > 1 ? nls.localize(11299, null, sourceTargetPairs.length)
                            : nls.localize(11300, null, resources.basenameOrAuthority(sourceTargetPairs[0].target)),
                        undoLabel: sourceTargetPairs.length > 1 ? nls.localize(11301, null, sourceTargetPairs.length)
                            : nls.localize(11302, null, resources.basenameOrAuthority(sourceTargetPairs[0].target))
                    };
                    await explorerService.applyBulkEdit(resourceFileEdits, options);
                }
                else {
        ...
        notificationService.error(toErrorMessage(new Error(nls.localize(11303, null, getErrorMessage(e))), false));
    }
    finally {
        if (pasteShouldMove) {
    ...
    async function applyCopyResourceEdit(targets, resourceFileEdits) {
        ...
        const options = {
            ...
            progressLabel: targets.length > 1 ? nls.localize(11304, null, targets.length)
                : nls.localize(11305, null, resources.basenameOrAuthority(targets[0])),
            undoLabel: targets.length > 1 ? nls.localize(11306, null, targets.length)
                : nls.localize(11307, null, resources.basenameOrAuthority(targets[0]))
        };
        await explorerService.applyBulkEdit(resourceFileEdits, options);
    }
};
...
export class SetActiveEditorReadonlyInSession extends BaseSetActiveEditorReadonlyInSession {
    ...
    static { this.LABEL = nls.localize2(11324, "Set Active Editor Read-only in Session"); }
    constructor() {
...
export class SetActiveEditorWriteableInSession extends BaseSetActiveEditorReadonlyInSession {
    ...
    static { this.LABEL = nls.localize2(11325, "Set Active Editor Writeable in Session"); }
    constructor() {
...
export class ToggleActiveEditorReadonlyInSession extends BaseSetActiveEditorReadonlyInSession {
    ...
    static { this.LABEL = nls.localize2(11326, "Toggle Active Editor Read-only in Session"); }
    constructor() {
...
export class ResetActiveEditorReadonlyInSession extends BaseSetActiveEditorReadonlyInSession {
    ...
    static { this.LABEL = nls.localize2(11327, "Reset Active Editor Read-only in Session"); }
    constructor() {
