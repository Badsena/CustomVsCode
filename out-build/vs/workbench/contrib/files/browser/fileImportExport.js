            ...
            return uploadLimiter.queue(async () => {
                ...
                if (target && entry.name && target.getChild(entry.name)) {
                    ...
                    await this.explorerService.applyBulkEdit([new ResourceFileEdit(joinPath(target.resource, entry.name), undefined, { recursive: true, folder: target.getChild(entry.name)?.isDirectory })], {
                        undoLabel: localize(11342, null, entry.name),
                        progressLabel: localize(11343, null, entry.name),
                    });
                    if (token.isCancellationRequested) {
                        return;
                    }
        ...
        const reportProgress = (fileSize, bytesUploaded) => {
            ...
            if (fileSize < ByteSize.MB) {
                if (operation.filesTotal === 1) {
                    ...
                    message = localize(11344, null, operation.filesUploaded, operation.filesTotal, ByteSize.formatSize(bytesUploadedPerSecond));
                }
            }
            // Large file
            else {
                message = localize(11345, null, entry.name, ByteSize.formatSize(fileBytesUploaded), ByteSize.formatSize(fileSize), ByteSize.formatSize(bytesUploadedPerSecond));
            }
        ...
        if (folders.length > 0 && target.isRoot) {
            ...
            if (folders.some(folder => workspaceFolderSchemas.indexOf(folder.uri.scheme) >= 0)) {
                ...
                message = folders.length > 1 ?
                    localize(11351, null) :
                    localize(11352, null, basename(folders[0].uri));
            }
            else {
                message = folders.length > 1 ?
                    localize(11353, null) :
                    localize(11354, null, basename(folders[0].uri));
            }
    ...
    async importResources(target, resources, token) {
        if (resources && resources.length > 0) {
            ...
            if (inaccessibleFileCount > 0) {
                this.notificationService.error(inaccessibleFileCount > 1 ? localize(11355, null) : localize(11356, null));
            }
            ...
            await this.explorerService.applyBulkEdit(resourceFileEdits, {
                undoLabel: resourcesFiltered.length === 1 ?
                    localize(11357, null, basename(resourcesFiltered[0])) :
                    localize(11358, null, resourcesFiltered.length),
                progressLabel: resourcesFiltered.length === 1 ?
                    localize(11359, null, basename(resourcesFiltered[0])) :
                    localize(11360, null, resourcesFiltered.length),
...
let FileDownload = class FileDownload {
    ...
    reportProgress(name, fileSize, bytesDownloaded, operation) {
        ...
        if (fileSize < ByteSize.MB) {
            if (operation.filesTotal === 1) {
                ...
                message = localize(11362, null, operation.filesDownloaded, operation.filesTotal, ByteSize.formatSize(bytesDownloadedPerSecond));
            }
        }
        // Large file
        else {
            message = localize(11363, null, name, ByteSize.formatSize(operation.fileBytesDownloaded), ByteSize.formatSize(fileSize), ByteSize.formatSize(bytesDownloadedPerSecond));
        }
        // Report progress but limit to update only once per second
        operation.progressScheduler.work({ message });
    }
    async doDownloadNative(explorerItem, progress, cts) {
        ...
        const destination = await this.fileDialogService.showSaveDialog({
            ...
            saveLabel: localize(11364, null),
            title: localize(11365, null),
            defaultUri
        });
        if (destination) {
            ...
            await this.explorerService.applyBulkEdit([new ResourceFileEdit(explorerItem.resource, destination, { overwrite: true, copy: true })], {
                undoLabel: localize(11366, null, explorerItem.name),
                progressLabel: localize(11367, null, explorerItem.name),
                progressLocation: 10 /* ProgressLocation.Window */
            });
        }
        else {
...
export function getFileOverwriteConfirm(name) {
    return {
        message: localize(11368, null, name),
        detail: localize(11369, null),
        primaryButton: localize(11370, null),
        type: 'warning'
    };
}
export function getMultipleFilesOverwriteConfirm(files) {
    if (files.length > 1) {
        return {
            message: localize(11371, null, files.length),
            detail: getFileNamesMessage(files) + '\n' + localize(11372, null),
            primaryButton: localize(11373, null),
            type: 'warning'
        };
    }
