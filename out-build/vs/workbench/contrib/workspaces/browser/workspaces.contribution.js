...
let WorkspacesFinderContribution = class WorkspacesFinderContribution extends Disposable {
    ...
    doHandleWorkspaceFiles(folder, workspaces) {
        ...
        if (workspaces.length === 1) {
            ...
            this.notificationService.prompt(Severity.Info, localize(17726, null, workspaceFile, 'https://go.microsoft.com/fwlink/?linkid=2025315'), [{



                    label: localize(17727, null),
        ...
        // Prompt to select a workspace from many
            ...
            this.notificationService.prompt(Severity.Info, localize(17728, null, 'https://go.microsoft.com/fwlink/?linkid=2025315'), [{



                    label: localize(17729, null),
                    run: () => {
                        this.quickInputService.pick(workspaces.map(workspace => ({ label: workspace })), { placeHolder: localize(17730, null) }).then(pick => {
                            if (pick) {
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: localize2(17732, "Open Workspace"),
            f1: false,
            menu: {
    ...
    async run(accessor, uri) {
        ...
        if (contextService.getWorkbenchState() === 3 /* WorkbenchState.WORKSPACE */) {
            ...
            if (workspaceConfiguration && isEqual(workspaceConfiguration, uri)) {
                notificationService.info(localize(17731, null));
