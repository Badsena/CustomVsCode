...
let SimpleSettingRenderer = class SimpleSettingRenderer {
    ...
    viewInSettingsMessage(settingId, alreadyDisplayed) {
        if (alreadyDisplayed) {
            return nls.localize(11959, null);
        }
        else {
            const displayName = settingKeyToDisplayFormat(settingId);
            return nls.localize(11960, null, displayName.category, displayName.label);
        }
    }
    restorePreviousSettingMessage(settingId) {
        const displayName = settingKeyToDisplayFormat(settingId);
        return nls.localize(11961, null, displayName.category, displayName.label);
    }
    ...
    booleanSettingMessage(setting, booleanValue) {
        ...
        if (this.isAlreadySet(setting, booleanValue)) {
            if (booleanValue) {
                return nls.localize(11962, null, displayName.category, displayName.label);
            }
            else {
                return nls.localize(11963, null, displayName.category, displayName.label);
            }
        }
        if (booleanValue) {
            return nls.localize(11964, null, displayName.category, displayName.label);
        }
        else {
            return nls.localize(11965, null, displayName.category, displayName.label);
        }
    }
    stringSettingMessage(setting, stringValue) {
        const displayName = settingKeyToDisplayFormat(setting.key);
        if (this.isAlreadySet(setting, stringValue)) {
            return nls.localize(11966, null, displayName.category, displayName.label, stringValue);
        }
        return nls.localize(11967, null, displayName.category, displayName.label, stringValue);
    }
    numberSettingMessage(setting, numberValue) {
        const displayName = settingKeyToDisplayFormat(setting.key);
        if (this.isAlreadySet(setting, numberValue)) {
            return nls.localize(11968, null, displayName.category, displayName.label, numberValue);
        }
        return nls.localize(11969, null, displayName.category, displayName.label, numberValue);
    }
    renderSetting(setting, newValue) {
        const href = this.settingToUriString(setting.key, newValue);
        const title = nls.localize(11970, null);
    ...
    getActions(uri) {
        ...
        actions.push({
            ...
            enabled: true,
            id: 'copySettingId',
            tooltip: nls.localize(11971, null),
            label: nls.localize(11972, null),
            run: () => {
