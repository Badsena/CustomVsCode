...
let TroubleshootIssueService = class TroubleshootIssueService extends Disposable {
    ...
    async start() {
        ...
        const res = await this.dialogService.confirm({
            message: localize(11814, null),
            detail: localize(11815, null, this.productService.nameLong),
            primaryButton: localize(11816, null),
            custom: true
        });
        if (!res.confirmed) {
            return;
    ...
    async reproduceIssueWithExtensionsDisabled() {
        ...
        const result = await this.askToReproduceIssue(localize(11817, null));
        if (result === 'good') {
            const profile = this.userDataProfilesService.profiles.find(p => p.id === this.state.profile) ?? this.userDataProfilesService.defaultProfile;
    ...
    async reproduceIssueWithEmptyProfile() {
        ...
        const result = await this.askToReproduceIssue(localize(11818, null));
        if (result === 'stop') {
            await this.stop();
        }
        if (result === 'good') {
            await this.askToReportIssue(localize(11819, null));
        }
        if (result === 'bad') {
            await this.askToReportIssue(localize(11820, null, this.productService.nameLong));
        }
    }
    ...
    askToReproduceIssue(message) {
        return new Promise((c, e) => {
            const goodPrompt = {
                label: localize(11821, null),
                run: () => c('good')
            };
            const badPrompt = {
                label: localize(11822, null),
                run: () => c('bad')
            };
            const stop = {
                label: localize(11823, null),
                run: () => c('stop')
            };
        ...
        if (this.productService.quality === 'stable') {
            ...
            if (res === 'good') {
                await this.dialogService.prompt({
                    type: Severity.Info,
                    message: localize(11824, null),
                    detail: localize(11825, null, this.productService.nameLong),
                    custom: true
                });
                return;
            }
    ...
    async askToReproduceIssueWithInsiders() {
        const confirmRes = await this.dialogService.confirm({
            type: 'info',
            message: localize(11826, null),
            primaryButton: localize(11827, null, this.productService.nameLong),
            cancelButton: localize(11828, null),
            detail: localize(11829, null, this.productService.nameLong),
            custom: {
        ...
        const res = await this.dialogService.prompt({
            type: 'info',
            message: localize(11830, null),
            buttons: [{
                    label: localize(11831, null),
                    run: () => 'good'
                }, {
                    label: localize(11832, null),
                    run: () => 'bad'
                }],
            cancelButton: {
                label: localize(11833, null),
                run: () => 'stop'
            },
            detail: localize(11834, null, this.productService.nameLong),
            custom: {
                disableCloseAction: true,
            }
        });
...
registerAction2(class TroubleshootIssueAction extends Action2 {
    constructor() {
        super({
            ...
            title: localize2(11835, 'Troubleshoot Issue...'),
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: localize2(11836, 'Stop Troubleshoot Issue'),
            category: Categories.Help,
            f1: true,
