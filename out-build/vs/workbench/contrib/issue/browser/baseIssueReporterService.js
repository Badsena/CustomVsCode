...
let BaseIssueReporterService = class BaseIssueReporterService extends Disposable {
    constructor(disableExtensions, data, os, product, window, isWeb, issueFormService, themeService, fileService, fileDialogService, contextMenuService, authenticationService, openerService) {
        ...
        this.createAction = this._register(new Action('issueReporter.create', localize(11730, null), undefined, true, async () => {
            this.delayedSubmit.trigger(async () => {
                this.createIssue(true); // create issue
            });
        }));
        this.previewAction = this._register(new Action('issueReporter.preview', localize(11731, null), undefined, true, async () => {
            this.delayedSubmit.trigger(async () => {
                this.createIssue(false); // preview issue
            });
        }));
        this.privateAction = this._register(new Action('issueReporter.privateCreate', localize(11732, null), undefined, true, async () => {
            this.delayedSubmit.trigger(async () => {
    ...
    updateInternalFilingNote(container) {
        ...
        filingNote.textContent = escape(localize(11733, null));
    }
    updatePublicGithubButton(container) {
        ...
        if (!this.acknowledged && this.needsUpdate) { // * old version and hasn't ack'd
            ...
            this.publicGithubButton.label = localize(11734, null);
            ...
            this.publicGithubButton.label = localize(11735, null);
            ...
            this.publicGithubButton.label = localize(11736, null);
            this.publicGithubButton.enabled = true;
        }
            ...
            this.publicGithubButton.label = localize(11737, null);
            this.publicGithubButton.enabled = true;
        }
    ...
    updateInternalGithubButton(container) {
        ...
        if (this.data.githubAccessToken && this.data.privateUri) {
            ...
            this.internalGithubButton.label = localize(11738, null);
    ...
    updateExtensionSelector(extensions) {
        ...
        if (extensionsSelector) {
            ...
            reset(extensionsSelector, this.makeOption('', localize(11739, null), true), ...extensionOptions.map(extension => makeOption(extension, selectedExtension)));
    ...
    setEventHandlers() {
            ...
            showInfo.addEventListener('click', (e) => {
                ...
                if (label) {
                    ...
                    if (info && info.classList.contains('hidden')) {
                        show(info);
                        label.textContent = localize(11740, null);
                    }
                    else {
                        hide(info);
                        label.textContent = localize(11741, null);
                    }
                }
            });
        }
        this.addEventListener('issue-source', 'change', (e) => {
            ...
            if (value === IssueSource.VSCode) {
                descriptionTextArea.placeholder = localize(11742, null);
            }
            else if (value === IssueSource.Extension) {
                descriptionTextArea.placeholder = localize(11743, null);
            }
            else if (value === IssueSource.Marketplace) {
                descriptionTextArea.placeholder = localize(11744, null);
            }
            else {
                descriptionTextArea.placeholder = localize(11745, null);
            }
    ...
    displaySearchResults(results) {
        ...
        if (results.length) {
            ...
            issuesText.textContent = localize(11746, null);
            ...
            for (let i = 0; i < this.numberOfSearchResultsDisplayed; i++) {
                ...
                if (issue.state) {
                    ...
                    issueStateLabel.textContent = issue.state === 'open' ? localize(11747, null) : localize(11748, null);
                    issueState.title = issue.state === 'open' ? localize(11749, null) : localize(11750, null);
    ...
    setUpTypes() {
        ...
        reset(typeSelect, makeOption(0 /* IssueType.Bug */, localize(11751, null)), makeOption(2 /* IssueType.FeatureRequest */, localize(11752, null)), makeOption(1 /* IssueType.PerformanceIssue */, localize(11753, null)));
    ...
    setSourceOptions() {
        ...
        sourceSelect.append(this.makeOption('', localize(11754, null), true));
        sourceSelect.append(this.makeOption(IssueSource.VSCode, localize(11755, null), false));
        sourceSelect.append(this.makeOption(IssueSource.Extension, localize(11756, null), false));
        if (this.product.reportMarketplaceIssueUrl) {
            sourceSelect.append(this.makeOption(IssueSource.Marketplace, localize(11757, null), false));
        }
        if (issueType !== 2 /* IssueType.FeatureRequest */) {
            sourceSelect.append(this.makeOption(IssueSource.Unknown, localize(11758, null), false));
        }
    ...
    async renderBlocks() {
        ...
        if (selectedExtension && this.nonGitHubIssueUrl) {
            ...
            reset(descriptionTitle, localize(11760, null));
            reset(descriptionSubtitle, localize(11761, null, selectedExtension.displayName));
            this.publicGithubButton.label = localize(11762, null);
            return;
        }
        ...
        if (issueType === 0 /* IssueType.Bug */) {
            ...
            reset(descriptionTitle, localize(11763, null) + ' ', $('span.required-input', undefined, '*'));
            reset(descriptionSubtitle, localize(11764, null));
        }
            ...
            reset(descriptionTitle, localize(11765, null) + ' ', $('span.required-input', undefined, '*'));
            reset(descriptionSubtitle, localize(11766, null));
        }
        else if (issueType === 2 /* IssueType.FeatureRequest */) {
            reset(descriptionTitle, localize(11767, null) + ' ', $('span.required-input', undefined, '*'));
            reset(descriptionSubtitle, localize(11768, null));
        }
    }
    ...
    async writeToClipboard(baseUrl, issueBody) {
        ...
        return baseUrl + `&body=${encodeURIComponent(localize(11769, null))}`;
    }
    ...
    updateExtensionTable(extensions, numThemeExtensions) {
        ...
        if (target) {
            if (this.disableExtensions) {
                reset(target, localize(11770, null));
                return;
            }
