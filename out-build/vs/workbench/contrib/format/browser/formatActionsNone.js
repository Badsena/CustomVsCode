...
registerEditorAction(class FormatDocumentMultipleAction extends EditorAction {
    constructor() {
        super({
            ...
            label: nls.localize2(11576, "Format Document"),
    ...
    async run(accessor, editor) {
        ...
        if (formatterCount > 1) {
            ...
            notificationService.warn(nls.localize(11573, null));
        }
        else {
            const langName = model.getLanguageId();
            const message = nls.localize(11574, null, langName);
            const { confirmed } = await dialogService.confirm({
                message,
                primaryButton: nls.localize(11575, null)
            });
            if (confirmed) {
