...
let DefaultFormatter = class DefaultFormatter extends Disposable {
    ...
    async _updateConfigValues() {
        ...
        DefaultFormatter_1.extensionItemLabels.push(nls.localize(11553, null));
        DefaultFormatter_1.extensionDescriptions.push(nls.localize(11554, null));
    ...
    async _analyzeFormatter(kind, formatter, document) {
        ...
        if (defaultFormatterId) {
            ...
            if (extension && this._extensionEnablementService.isEnabled(toExtension(extension))) {
                ...
                const detail = kind === 1 /* FormattingKind.File */
                    ? nls.localize(11555, null, extension.displayName || extension.name, langName)
                    : nls.localize(11556, null, extension.displayName || extension.name, langName);
                return detail;
            }
        }
        ...
        const message = !defaultFormatterId
            ? nls.localize(11557, null, DefaultFormatter_1._maybeQuotes(langName))
            : nls.localize(11558, null, defaultFormatterId);
        return message;
    }
    async _selectFormatter(formatter, document, mode, kind) {
        ...
        if (mode !== 2 /* FormattingMode.Silent */) {
            ...
            const { confirmed } = await this._dialogService.confirm({
                message: nls.localize(11559, null),
                detail: formatterOrMessage,
                primaryButton: nls.localize(11560, null)
            });
            if (confirmed) {
            ...
            this._notificationService.prompt(Severity.Info, formatterOrMessage, [{ label: nls.localize(11561, null), run: () => this._pickAndPersistDefaultFormatter(formatter, document) }], { priority: NotificationPriority.SILENT });
        }
        return undefined;
    }
    async _pickAndPersistDefaultFormatter(formatter, document) {
        ...
        const langName = this._languageService.getLanguageName(document.getLanguageId()) || document.getLanguageId();
        const pick = await this._quickInputService.pick(picks, { placeHolder: nls.localize(11562, null, DefaultFormatter_1._maybeQuotes(langName)) });
    ...
    _updateStatus() {
        ...
        this._analyzeFormatter(1 /* FormattingKind.File */, formatter, document).then(result => {
            ...
            const command = { id: `formatter/configure/dfl/${generateUuid()}`, title: nls.localize(11563, null) };
...
Registry.as(ConfigurationExtensions.Configuration).registerConfiguration({
    ...
    properties: {
        [DefaultFormatter.configName]: {
            description: nls.localize(11566, null),
            type: ['string', 'null'],
            default: null,
...
async function showFormatterPick(accessor, model, formatters) {
    ...
    const picks = formatters.map((provider, index) => {
        ...
        const pick = {
            ...
            description: isDefault ? nls.localize(11567, null) : undefined,
        };
        if (isDefault) {
    ...
    if (!pick) {
        ...
        const pick = await quickPickService.pick(picks, { placeHolder: nls.localize(11570, null, DefaultFormatter._maybeQuotes(langName)) });
