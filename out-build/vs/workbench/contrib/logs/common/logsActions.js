...
let SetLogLevelAction = class SetLogLevelAction extends Action {
    ...
    static { this.TITLE = nls.localize2(11955, "Set Log Level..."); }
    ...
    async setLogLevelForChannel(logChannel) {
        ...
        return new Promise((resolve, reject) => {
            ...
            quickPick.placeholder = logChannel ? nls.localize(11948, null, logChannel?.label) : nls.localize(11949, null);
            quickPick.items = entries;
    ...
    getLogLevelEntries(defaultLogLevel, currentLogLevel, canSetDefaultLogLevel) {
        const button = canSetDefaultLogLevel ? { iconClass: ThemeIcon.asClassName(Codicon.checkAll), tooltip: nls.localize(11950, null) } : undefined;
        return [
    ...
    getDescription(level, defaultLogLevel) {
        return defaultLogLevel === level ? nls.localize(11951, null) : undefined;
    }
};
...
let OpenWindowSessionLogFileAction = class OpenWindowSessionLogFileAction extends Action {
    ...
    static { this.TITLE = nls.localize2(11956, "Open Window Log File (Session)..."); }
    ...
    async run() {
        const sessionResult = await this.quickInputService.pick(this.getSessions().then(sessions => sessions.map((s, index) => ({
            id: s.toString(),
            label: basename(s),
            description: index === 0 ? nls.localize(11952, null) : undefined
        }))), {
            canPickMany: false,
            placeHolder: nls.localize(11953, null)
        });
        if (sessionResult) {
            const logFileResult = await this.quickInputService.pick(this.getLogFiles(URI.parse(sessionResult.id)).then(logFiles => logFiles.map((s) => ({
                ...
                canPickMany: false,
                placeHolder: nls.localize(11954, null)
            });
            if (logFileResult) {
