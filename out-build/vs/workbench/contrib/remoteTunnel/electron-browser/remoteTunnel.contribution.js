...
export const REMOTE_TUNNEL_CATEGORY = localize2(14138, 'Remote Tunnels');
...
(function (RemoteTunnelCommandLabels) {
    RemoteTunnelCommandLabels.turnOn = localize(14098, null);
    RemoteTunnelCommandLabels.turnOff = localize(14099, null);
    RemoteTunnelCommandLabels.showLog = localize(14100, null);
    RemoteTunnelCommandLabels.configure = localize(14101, null);
    RemoteTunnelCommandLabels.copyToClipboard = localize(14102, null);
    RemoteTunnelCommandLabels.learnMore = localize(14103, null);
...
let RemoteTunnelWorkbenchContribution = class RemoteTunnelWorkbenchContribution extends Disposable {
    ...
    async recommendRemoteExtensionIfNeeded() {
        ...
        const recommed = async () => {
            ...
            this.notificationService.notify({
                ...
                message: localize(14104, null, usedOnHost, remoteExtension.friendlyName),
    ...
    async initialize() {
        ...
        if (hasUsed) {
            await this.progressService.withProgress({
                ...
                title: localize(14107, null, RemoteTunnelCommandIds.showLog),
    ...
    async startTunnel(asService) {
        ...
        for (let i = 0; i < INVALID_TOKEN_RETRIES; i++) {
            ...
            const result = await this.progressService.withProgress({
                ...
                title: localize(14108, null, RemoteTunnelCommandIds.showLog),
            }, (progress) => {
                            ...
                            case 'connected':
                                ...
                                if (status.serviceInstallFailed) {
                                    this.notificationService.notify({
                                        ...
                                        message: localize(14109, null, RemoteTunnelCommandIds.showLog),
    ...
    async getAuthenticationSession() {
        ...
        quickpick.ok = false;
        quickpick.placeholder = localize(14110, null);
    ...
    async createQuickpickItems(sessions) {
        const options = [];
        if (sessions.length) {
            options.push({ type: 'separator', label: localize(14111, null) });
            options.push(...sessions);
            options.push({ type: 'separator', label: localize(14112, null) });
        }
        for (const authenticationProvider of (await this.getAuthenticationProviders())) {
            ...
            if (!signedInForProvider || provider.supportsMultipleAccounts) {
                options.push({ label: localize(14113, null, provider.label), provider: authenticationProvider });
            }
        }
        return options;
    }
            ...
            async run(accessor) {
                ...
                if (!didNotifyPreview) {
                    const { confirmed } = await dialogService.confirm({
                        message: localize(14114, null),
                        primaryButton: localize(14115, null)
                    });
                    if (!confirmed) {
                        return;
                    }
                ...
                quickPick.placeholder = localize(14116, null);
                quickPick.items = [
                    { service: false, label: localize(14117, null), description: localize(14118, null, productService.nameShort) },
                    { service: true, label: localize(14119, null), description: localize(14120, null) }
                ];
                ...
                if (connectionInfo) {
                    ...
                    notificationService.notify({
                        severity: Severity.Info,
                        message: localize(14121, null, connectionInfo.tunnelName, connectionInfo.domain, linkToOpenForMarkdown, RemoteTunnelCommandIds.manage, RemoteTunnelCommandIds.configure, RemoteTunnelCommandIds.turnOff, remoteExtension.friendlyName, 'https://code.visualstudio.com/docs/remote/tunnels'),



                        actions: {
                            primary: [
                                toAction({ id: 'copyToClipboard', label: localize(14122, null), run: () => clipboardService.writeText(linkToOpen.toString(true)) }),
                                toAction({
                                    id: 'showExtension', label: localize(14123, null), run: () => {
    ...
    async showManageOptions() {
        ...
        return new Promise((c, e) => {
            ...
            quickPick.placeholder = localize(14129, null);
            ...
            if (this.connectionInfo) {
                quickPick.title =
                    this.connectionInfo.isAttached ?
                        localize(14130, null, this.connectionInfo.tunnelName) :
                        localize(14131, null, this.connectionInfo.tunnelName);
                ...
                quickPick.title = localize(14132, null);
            }
            items.push({ id: RemoteTunnelCommandIds.showLog, label: localize(14133, null) });
            items.push({ type: 'separator' });
            items.push({ id: RemoteTunnelCommandIds.configure, label: localize(14134, null), description: this.connectionInfo?.tunnelName });
...
Registry.as(ConfigurationExtensions.Configuration).registerConfiguration({
    ...
    properties: {
        [CONFIGURATION_KEY_HOST_NAME]: {
            description: localize(14135, null),
            type: 'string',
            scope: 1 /* ConfigurationScope.APPLICATION */,
            ignoreSync: true,
            pattern: '^(\\w[\\w-]*)?$',
            patternErrorMessage: localize(14136, null),
            maxLength: 20,
            default: ''
        },
        [CONFIGURATION_KEY_PREVENT_SLEEP]: {
            description: localize(14137, null),
            type: 'boolean',
            scope: 1 /* ConfigurationScope.APPLICATION */,
            default: false,
        }
