...
let PromptCodeActionProvider = class PromptCodeActionProvider {
    ...
    getUpdateModeCodeActions(promptFile, model, range, result) {
        ...
        result.push(this.createCodeAction(model, keyRange, localize(8551, null), [asWorkspaceTextEdit(model, { range: keyRange, text: 'agent' })]));
    }
    async getMigrateModeFileCodeActions(model, result) {
        if (model.uri.path.endsWith(LEGACY_MODE_FILE_EXTENSION)) {
            ...
            if (location && await this.fileService.canMove(model.uri, location)) {
                ...
                result.push(this.createCodeAction(model, new Range(1, 1, 1, 4), localize(8552, null), [edit]));
            }
        }
    }
            ...
            if (newNames && newNames.size > 0) {
                ...
                if (newNames.size === 1) {
                    ...
                    if (item.range.containsRange(range)) {
                        result.push(this.createCodeAction(model, item.range, localize(8553, null, newName), [asWorkspaceTextEdit(model, edit)]));
                    }
                }
                else {
                    ...
                    edits.push(edit);
                    if (item.range.containsRange(range)) {
                        result.push(this.createCodeAction(model, item.range, localize(8554, null, newNames.size), [asWorkspaceTextEdit(model, edit)]));
                    }
                }
            }
        }
        if (edits.length && result.length === 0 || edits.length > 1) {
            result.push(this.createCodeAction(model, value.range, localize(8555, null), edits.map(edit => asWorkspaceTextEdit(model, edit))));
        }
    }
};
