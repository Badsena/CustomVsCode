...
let ComputeAutomaticInstructions = class ComputeAutomaticInstructions {
    constructor(_modeKind, _enabledTools, _enabledSubagents, _sessionResource, _promptsService, _logService, _labelService, _configurationService, _contextKeyService, _workspaceService, _fileService, _remoteAgentService, _telemetryService, _languageModelToolsService) {
        ...
        for (const { uri } of instructionFiles) {
            ...
            if (match) {
                ...
                const reason = !match.file ?
                    localize(8508, null) :
                    localize(8509, null, pattern, this._labelService.getUriLabel(match.file, { relative: true }));
    ...
    async _getCustomizationsIndex(instructionFiles, _existingVariables, telemetryEvent, token) {
        ...
        if (readTool) {
            ...
            for (const { uri } of agentsMdFiles) {
                ...
                const description = folderName.trim().length === 0 ? localize(8510, null) : localize(8511, null, folderName);
                entries.push('<instruction>');
                entries.push(`<description>${description}</description>`);
                entries.push(`<file>${filePath(uri)}</file>`);
                entries.push('</instruction>');
                ...
                if (refsToCheck.length > 0) {
                    ...
                    for (let i = 0; i < stats.length; i++) {
                        ...
                        if (stat.success && stat.stat?.isFile) {
                            ...
                            const reason = localize(8512, null, basename(next));
