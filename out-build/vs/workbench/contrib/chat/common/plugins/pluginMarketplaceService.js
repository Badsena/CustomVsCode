...
let PluginMarketplaceService = class PluginMarketplaceService extends Disposable {
    constructor(_configurationService, _requestService, _fileService, _pluginRepositoryService, _logService, _storageService, _workspacePluginSettingsService, _workspaceTrustService) {
        ...
        this._installedPluginsStore = this._register(installedPluginsMemento(-1 /* StorageScope.APPLICATION */, 1 /* StorageTarget.MACHINE */, _storageService));
    ...
    setInstalledPluginEnabled(pluginUri, enabled) {
        const current = this.installedPlugins.get();
        this._installedPluginsStore.set(current.map(e => isEqual(e.pluginUri, pluginUri) ? { ...e, enabled } : e), undefined);
    }
