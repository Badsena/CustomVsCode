...
const languageModelChatProviderType = {
    ...
    properties: {
        vendor: {
            type: 'string',
            description: localize(8450, null)
        },
        displayName: {
            type: 'string',
            description: localize(8451, null)
        },
        configuration: {
            type: 'object',
            description: localize(8452, null),
            anyOf: [
                {
                    ...
                    properties: {
                        properties: {
                            ...
                            additionalProperties: {
                                ...
                                properties: {
                                    secret: {
                                        type: 'boolean',
                                        description: localize(8453, null)
                                    }
                                }
                            }
                        },
                        additionalProperties: {
                            ...
                            properties: {
                                secret: {
                                    type: 'boolean',
                                    description: localize(8454, null)
                                }
                            }
                        }
                    }
        ...
        managementCommand: {
            type: 'string',
            description: localize(8455, null),
            deprecated: true,
            deprecationMessage: localize(8456, null)
        },
        when: {
            type: 'string',
            description: localize(8457, null)
        }
    }
};
export const languageModelChatProviderExtensionPoint = ExtensionsRegistry.registerExtensionPoint({
    ...
    jsonSchema: {
        description: localize(8458, null),
        oneOf: [
            ...
            for (const extension of added) {
                for (const item of Iterable.wrap(extension.value)) {
                    if (this._vendors.has(item.vendor)) {
                        extension.collector.error(localize(8459, null, item.vendor));
                        continue;
                    }
                    if (isFalsyOrWhitespace(item.vendor)) {
                        extension.collector.error(localize(8460, null));
                        continue;
                    }
                    if (item.vendor.trim() !== item.vendor) {
                        extension.collector.error(localize(8461, null));
                        continue;
                    }
    ...
    getModelConfigurationActions(modelId) {
        ...
        for (const [key, propSchema] of Object.entries(schema.properties)) {
            ...
            const enumActions = propSchema.enum.map((value, index) => {
                ...
                const displayLabel = value === defaultValue ? localize(8462, null, itemLabel) : itemLabel;
    ...
    async promptForArray(groupName, property, propertySchema) {
        ...
        try {
            return await new Promise(resolve => {
                ...
                quickPick.placeholder = propertySchema.description ?? localize(8467, null, property);
    ...
    async promptForInput(groupName, property, propertySchema, required, existing) {
        ...
        try {
            const value = await new Promise((resolve, reject) => {
                ...
                inputBox.placeholder = localize(8468, null, property);
                ...
                disposables.add(inputBox.onDidChangeValue(value => {
                    if (!value && required) {
                        inputBox.validationMessage = localize(8469, null);
                        inputBox.severity = Severity.Error;
                        return;
                    }
                    if (propertySchema.type === 'number' || propertySchema.type === 'integer') {
                        if (isNaN(Number(value))) {
                            inputBox.validationMessage = localize(8470, null);
                            inputBox.severity = Severity.Error;
                            return;
                        }
                    }
                    if (propertySchema.type === 'boolean') {
                        if (value !== 'true' && value !== 'false') {
                            inputBox.validationMessage = localize(8471, null);
                            inputBox.severity = Severity.Error;
                            return;
                        }
                    }
                ...
                disposables.add(inputBox.onDidAccept(() => {
                    if (!inputBox.value && required) {
                        inputBox.validationMessage = localize(8472, null);
                        inputBox.severity = Severity.Error;
                        return;
                    }
