...
export async function extractImagesFromChatResponse(response, readFile) {
    ...
    const title = request ? request.messageText : localize(8429, null);
    return {
...
export function extractImagesFromToolInvocationOutputDetails(toolInvocation, sessionResource) {
    ...
    const pushImage = (mimeType, data, outputIndex) => {
        ...
        images.push({
            ...
            uri,
            name: localize(8430, null, images.length + 1),
            mimeType,
            data,
            source: localize(8431, null, toolInvocation.toolId),
            caption,
        });
    };
    ...
    for (const uriComponents of Object.values(message.uris)) {
        ...
        if (mimeType?.startsWith('image/')) {
            ...
            images.push({
                ...
                mimeType,
                data,
                source: localize(8432, null, toolInvocation.toolId),
...
export function extractImagesFromChatRequest(request) {
    ...
    for (const variable of request.variables) {
        ...
        images.push({
            ...
            source: localize(8434, null),
            caption: undefined,
        });
    }
    return images;
}
//# sourceMappingURL=chatImageExtraction.js.map