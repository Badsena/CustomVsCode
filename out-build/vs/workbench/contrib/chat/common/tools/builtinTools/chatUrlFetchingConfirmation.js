...
let ChatUrlFetchingConfirmationContribution = class ChatUrlFetchingConfirmationContribution {
    ...
    _getConfirmActions(ref, forRequest) {
        ...
        if (urlPatterns.size === 1) {
            ...
            for (const pattern of topPatterns) {
                ...
                actions.push({
                    label: forRequest
                        ? localize(8871, null, patternLabel)
                        : localize(8872, null, patternLabel),
                    select: async () => {
    ...
    async _showMoreOptions(ref, urls, forRequest) {
        return new Promise((resolve) => {
            ...
            quickTree.placeholder = localize(8875, null);
            const treeItems = [];
            const approvedUrls = this._getApprovedUrls();
            ...
            for (const { uri, patterns } of urls) {
                for (const pattern of patterns.slice().sort((a, b) => b.length - a.length)) {
                    ...
                    treeItems.push({
                        ...
                        children: [
                            {
                                label: localize(8876, null),
                                pattern,
                                approvalType: 'request',
                                checked: requestChecked
                            },
                            {
                                label: localize(8877, null),
                                pattern,
    ...
    getManageActions() {
        ...
        for (const [pattern, settings] of Object.entries(approvedUrls)) {
            ...
            if (typeof settings === 'boolean') {
                ...
                if (settings.approveRequest) {
                    parts.push(localize(8880, null));
                }
                if (settings.approveResponse) {
                    parts.push(localize(8881, null));
                }
                description = parts.length > 0
                    ? localize(8882, null, parts.join(', '))
                    : localize(8883, null);
            }
            const item = {
                label,
        ...
        items.push({
            pickable: false,
            label: localize(8884, null),
            description: localize(8885, null),
            onDidOpen: () => {
