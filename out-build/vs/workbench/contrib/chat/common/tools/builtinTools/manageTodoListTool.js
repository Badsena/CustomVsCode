...
export function createManageTodoListToolData() {
    ...
    return {
        ...
        displayName: localize(8886, null),
        userDescription: localize(8887, null),
...
let ManageTodoListTool = class ManageTodoListTool extends Disposable {
    ...
    async prepareToolInvocation(context, _token) {
        ...
        const invocationLabel = message?.replace(/^(Starting|Completed): /i, '') ?? localize(8889, null);
        const invocationMessage = new MarkdownString(invocationLabel);
        return {
            invocationMessage,
            pastTenseMessage: new MarkdownString(message ?? localize(8890, null)),
            toolSpecificData: {
                kind: 'todoList',
                todoList: todoList
            }
    ...
    generatePastTenseMessage(currentTodos, newTodos) {
        ...
        if (currentTodos.length === 0) {
            return newTodos.length === 1
                ? localize(8891, null)
                : localize(8892, null, newTodos.length);
        }
        ...
        if (startedTodos.length > 0) {
            ...
            return localize(8893, null, startedTodo.title, currentPosition, totalTodos);
        }
        ...
        if (completedTodos.length > 0) {
            ...
            return localize(8894, null, completedTodo.title, currentPosition, totalTodos);
        }
        ...
        if (addedTodos.length > 0) {
            return addedTodos.length === 1
                ? localize(8895, null)
                : localize(8896, null, addedTodos.length);
        }
        // Default message for other updates
        return localize(8897, null);
    }
