...
suite('aiCustomizationListWidget', () => {
    ...
    suite('truncateToFirstSentence', () => {
        test('keeps first sentence when within max chars', () => {
            assert.strictEqual(truncateToFirstSentence('First sentence. Second sentence.'), 'First sentence.');
        });
        test('falls back to max chars when no sentence boundary is found', () => {
            const longText = 'a'.repeat(140);
            assert.strictEqual(truncateToFirstSentence(longText), `${'a'.repeat(120)}\u2026`);
        });
    });
    suite('getCustomizationSecondaryText', () => {
        ...
        test('truncates non-hook descriptions to the first sentence', () => {
            assert.strictEqual(getCustomizationSecondaryText('Show the first sentence. Hide the rest.', 'prompt.md', PromptsType.prompt), 'Show the first sentence.');
        });
...
//# sourceMappingURL=aiCustomizationListWidget.test.js.map