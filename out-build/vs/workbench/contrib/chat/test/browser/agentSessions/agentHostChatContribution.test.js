...
import { createSessionState, ROOT_STATE_URI } from '../../../../../../platform/agentHost/common/state/sessionState.js';
...
suite('AgentHostChatContribution', () => {
    ...
    suite('session ID resolution', () => {
        test('creates new SDK session for untitled resource', async () => {
            ...
            assert.strictEqual(agentHostService.dispatchedActions[0].action.userMessage.text, 'Hello');
        ...
        test('reuses SDK session for same resource on second message', async () => {
            ...
            assert.strictEqual(agentHostService.dispatchedActions[0].action.session.toString(), agentHostService.dispatchedActions[1].action.session.toString());
        });
    ...
    suite('progress routing', () => {
        test('delta events become markdownContent progress', async () => {
            ...
            fire({ type: 'session/delta', session, turnId, content: 'hello ' });
            fire({ type: 'session/delta', session, turnId, content: 'world' });
            fire({ type: 'session/turnComplete', session, turnId });
            await turnPromise;
            assert.strictEqual(collected.length, 2);
            assert.strictEqual(collected[0][0].kind, 'markdownContent');
            assert.strictEqual(collected[0][0].content.value, 'hello ');
            assert.strictEqual(collected[1][0].kind, 'markdownContent');
            assert.strictEqual(collected[1][0].content.value, 'world');
        });
        ...
        test('events from other sessions are ignored', async () => {
            ...
            agentHostService.fireAction({
                action: { type: 'session/delta', session: AgentSession.uri('copilot', 'other-session').toString(), turnId, content: 'wrong' },
                serverSeq: 100,
                origin: undefined,
            });
            fire({ type: 'session/delta', session, turnId, content: 'right' });
            fire({ type: 'session/turnComplete', session, turnId });
            await turnPromise;
            assert.strictEqual(collected.length, 1);
    ...
    suite('permission requests', () => {
        test('permission_request event shows confirmation and responds when confirmed', async () => {
            ...
            assert.ok(agentHostService.dispatchedActions.some(a => a.action.type === 'session/permissionResolved' && a.action.requestId === 'perm-1' && a.action.approved === true));
            fire({ type: 'session/turnComplete', session, turnId });
            await turnPromise;
        });
        test('permission_request denied when user skips', async () => {
            ...
            await timeout(10);
            assert.ok(agentHostService.dispatchedActions.some(a => a.action.type === 'session/permissionResolved' && a.action.requestId === 'perm-2' && a.action.approved === false));
            fire({ type: 'session/turnComplete', session, turnId });
            await turnPromise;
        });
        ...
        test('orphaned tool_start is marked complete in history', async () => {
            ...
            agentHostService.sessionStates.set(sessionUri.toString(), {
                ...
                turns: [{
                        ...
                        responseText: '',
                        usage: undefined,
                        toolCalls: [{ status: 'completed', toolCallId: 'tc-orphan', toolName: 'read_file', displayName: 'Read File', invocationMessage: 'Reading file', confirmed: 'not-needed', success: false, pastTenseMessage: 'Reading file' }],
                    }],
            });
        ...
        test('non-terminal tool_complete sets pastTenseMessage in history', async () => {
            ...
            agentHostService.sessionStates.set(sessionUri.toString(), {
                ...
                turns: [{
                        ...
                        usage: undefined,
                        responseText: '',
                        toolCalls: [{ status: 'completed', toolCallId: 'tc-g', toolName: 'grep', displayName: 'Grep', invocationMessage: 'Searching...', confirmed: 'not-needed', success: true, pastTenseMessage: 'Searched for pattern' }],
                    }],
            });
    ...
    suite('attachment context', () => {
        test('file variable with file:// URI becomes file attachment', async () => {
            ...
            await turnPromise;
            assert.strictEqual(agentHostService.dispatchedActions.length, 1);
            const turnAction = agentHostService.dispatchedActions[0].action;
    ...
    suite('IAgentConnection config', () => {
        ...
        test('handler works with any IAgentConnection, not just IAgentHostService', async () => {
            ...
            assert.strictEqual(agentHostService.dispatchedActions.length, 1);
            assert.strictEqual(agentHostService.dispatchedActions[0].action.userMessage.text, 'Test message');
        });
    });
});
//# sourceMappingURL=agentHostChatContribution.test.js.map