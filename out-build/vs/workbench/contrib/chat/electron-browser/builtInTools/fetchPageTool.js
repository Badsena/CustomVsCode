    ...
    inputSchema: {
        ...
        properties: {
            urls: {
                ...
                description: localize(9021, null)
            }
        },
        required: ['urls']
    }
...
let FetchWebPageTool = class FetchWebPageTool {
    ...
    async invoke(invocation, _countTokens, _progress, token) {
        ...
        if (!allValidUris.length && invalidUris.size === 0) {
            return {
                content: [{ kind: 'text', value: localize(9022, null) }]
            };
        }
            ...
            try {
                ...
                if (imageMimeType) {
                    ...
                    if (detected.seemsBinary) {
                        ...
                        fileContents.push(localize(9023, null));
                    }
                    else {
    ...
    async prepareToolInvocation(context, token) {
        ...
        const pastTenseMessage = invalid.length
            ? invalid.length > 1
                ...
                ? new MarkdownString(localize(9024, null, urlsNeedingConfirmation.size, invalid.map(url => `- ${url}`).join('\n')))
                // If there is only one invalid URL, show it
                : new MarkdownString(localize(9025, null, invalid[0]))
            // No invalid URLs
            : new MarkdownString();
        const invocationMessage = new MarkdownString();
        if (urlsNeedingConfirmation.size > 1) {
            pastTenseMessage.appendMarkdown(localize(9026, null, urlsNeedingConfirmation.size));
            invocationMessage.appendMarkdown(localize(9027, null, urlsNeedingConfirmation.size));
        }
            ...
            if (url.length > 400 || validFileUris.length === 1) {
                pastTenseMessage.appendMarkdown(localize(9028, null, url));
                ...
                invocationMessage.appendMarkdown(localize(9029, null, url));
                ...
                pastTenseMessage.appendMarkdown(localize(9030, null, url));
                invocationMessage.appendMarkdown(localize(9031, null, url));
            }
        }
        ...
        if (context.chatSessionResource) {
            ...
            if (urlsMentionedInPrompt && urlsNeedingConfirmation.size === 0) {
                confirmationNotNeededReason = localize(9032, null);
            }
        }
        ...
        result.confirmationMessages = {
            ...
            disclaimer: new MarkdownString('$(info) ' + localize(9035, null), { supportThemeIcons: true }),
    ...
    _getPromptPartsForResults(urls, results) {
        return results.map((value, i) => {
            const title = results.length > 1 ? localize(9036, null, urls[i]) : undefined;
            if (!value) {
                return {
                    kind: 'text',
                    title,
                    value: localize(9037, null)
                };
            }
