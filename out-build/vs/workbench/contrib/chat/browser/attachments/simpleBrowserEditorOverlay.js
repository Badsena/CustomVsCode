...
let SimpleBrowserOverlayWidget = class SimpleBrowserOverlayWidget {
    constructor(_editor, _container, _hostService, _chatWidgetService, fileService, environmentService, logService, configurationService, _preferencesService, _browserElementsService, contextMenuService, telemetryService) {
        ...
        const startSelectionMessage = localize(6634, null);
        ...
        actions.push(toAction({
            ...
            run: async () => {
                ...
                // stop selection
                message.textContent = localize(6639, null);
                finishedSelecting();
            }
        }));
        const startButton = this._showStore.add(new ButtonWithDropdown(mainContent, {
            ...
            title: localize(6640, null),
            supportIcons: true,
            ...defaultButtonStyles
        }));
        startButton.primaryButton.label = localize(6641, null);
        startButton.element.classList.add('element-selection-start');
        const cancelButton = this._showStore.add(new Button(mainContent, { ...defaultButtonStyles, supportIcons: true, title: localize(6642, null) }));
        cancelButton.element.className = 'element-selection-cancel hidden';
        const cancelButtonLabel = localize(6643, null);
        cancelButton.label = cancelButtonLabel;
        const configure = this._showStore.add(new Button(mainContent, { supportIcons: true, title: localize(6644, null) }));
        configure.icon = Codicon.gear;
        const collapseOverlay = this._showStore.add(new Button(mainContent, { supportIcons: true, title: localize(6645, null) }));
        collapseOverlay.icon = Codicon.chevronRight;
        const nextSelection = this._showStore.add(new Button(mainContent, { supportIcons: true, title: localize(6646, null) }));
        ...
        const expandOverlay = this._showStore.add(new Button(expandContainer, { supportIcons: true, title: localize(6647, null) }));
        ...
        const startElementSelection = async () => {
            ...
            this._editor.focus();
            // start selection
            message.textContent = localize(6648, null);
            this.hideElement(startButton.element);
            this.showElement(cancelButton.element);
            await this.addElementToChat(cts);
            // stop selection
            message.textContent = localize(6649, null);
            finishedSelecting();
        };
        ...
        this._showStore.add(addDisposableListener(cancelButton.element, 'click', () => {
            cts.cancel();
            message.textContent = localize(6650, null);
            finishedSelecting();
        }));
...
let SimpleBrowserOverlayController = class SimpleBrowserOverlayController {
    constructor(container, group, instaService, configurationService, _browserElementsService, contextKeyService) {
        ...
        const show = async (locator, browserType) => {
            ...
            connectingWebviewElement.textContent = localize(6651, null);
            ...
            try {
                ...
                connectingWebviewElement.textContent = localize(6652, null);
                return;
            }
