...
let ImplicitContextAttachmentWidget = class ImplicitContextAttachmentWidget extends Disposable {
    ...
    renderMainContext(context, isSelection) {
        ...
        const attachmentTypeName = file?.scheme === Schemas.vscodeNotebookCell ? localize(6618, null) : localize(6619, null);
        const contextLabel = context.name ?? (file ? basename(file) : localize(6620, null));
        ...
        if (isSuggestedEnabled) {
            if (!isSelection) {
                const buttonMsg = context.enabled
                    ? localize(6621, null, attachmentTypeName, contextLabel)
                    : localize(6622, null, contextLabel);
                ...
                const pinButtonMsg = localize(6623, null);
            ...
            const buttonMsg = context.enabled ? localize(6624, null, attachmentTypeName) : localize(6625, null, attachmentTypeName);
            const toggleButton = this.renderDisposables.add(new Button(contextNode, { supportIcons: true, title: buttonMsg }));
        ...
        if (isStringImplicitContextValue(context.value)) {
            ...
            title = this.renderString(label, context.name, context.icon, context.value.resourceUri, markdownTooltip, localize(6626, null));
            contextNode.ariaLabel = localize(6627, null, context.name);
        }
        else {
    ...
    renderResource(attachmentValue, isSelection, enabled, label, contextNode) {
        ...
        const attachmentTypeName = file.scheme === Schemas.vscodeNotebookCell ? localize(6628, null) : localize(6629, null);
        const fileBasename = basename(file);
        const fileDirname = dirname(file);
        const friendlyName = `${fileBasename} ${fileDirname}`;
        const ariaLabel = range
            ? localize(6630, null, attachmentTypeName, friendlyName, range.startLineNumber, range.endLineNumber)
            : localize(6631, null, attachmentTypeName, friendlyName);
        const uriLabel = this.labelService.getUriLabel(file, { relative: true });
        const currentFile = localize(6632, null, attachmentTypeName);
        const inactive = localize(6633, null, attachmentTypeName);
