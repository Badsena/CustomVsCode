...
const toolSetsSchema = {
    ...
    additionalProperties: {
        ...
        properties: {
            tools: {
                description: localize(7829, null),
                type: 'array',
                minItems: 1,
                items: {
                    type: 'string',
            ...
            icon: {
                description: localize(7830, null),
                type: 'string',
                enum: Array.from(getAllCodicons(), icon => icon.id),
                markdownEnumDescriptions: Array.from(getAllCodicons(), icon => `$(${icon.id})`),
            },
            description: {
                description: localize(7831, null),
                type: 'string'
            },
        },
    }
    ...
    constructor(extensionService, lifecycleService, _languageModelToolsService, _userDataProfileService, _fileService, _logService) {
        ...
        this._store.add(autorun(r => {
            ...
            for (const item of data) {
                ...
                toolEnumDescriptions.push(localize(7832, null, item.sourceLabel, item.name, item.description));
            }
...
export class ConfigureToolSets extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: localize2(7839, 'Configure Tool Sets...'),
            shortTitle: localize(7833, null),
    ...
    async run(accessor) {
        ...
        const pick = await quickInputService.pick(picks, {
            canPickMany: false,
            placeHolder: localize(7835, null),
        });
        if (!pick) {
        ...
        if (!pick.toolset) {
            const name = await quickInputService.input({
                placeHolder: localize(7836, null),
                validateInput: async (input) => {
                    if (!input) {
                        return localize(7837, null);
                    }
                    if (!isValidBasename(input)) {
                        return localize(7838, null, input);
                    }
                    return undefined;
                }
            });
