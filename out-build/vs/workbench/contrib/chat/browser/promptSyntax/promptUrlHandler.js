...
let PromptUrlHandler = class PromptUrlHandler extends Disposable {
    ...
    async handleURL(uri) {
        ...
        try {
            ...
            if (result.res.statusCode !== 200) {
                ...
                this.notificationService.error(localize(7743, null, urlString));
                return true;
            }
    ...
    async shouldBlockInstall(promptType, url) {
        ...
        detail.appendMarkdown(localize(7744, null, `[${uriLabel}](${url.toString()})`));
        detail.appendMarkdown(localize(7745, null));
        let message;
        switch (promptType) {
            case PromptsType.prompt:
                message = localize(7746, null);
                break;
            case PromptsType.instructions:
                message = localize(7747, null);
                break;
            default:
                message = localize(7748, null);
                break;
        }
        const { confirmed } = await this.dialogService.confirm({
            type: 'warning',
            primaryButton: localize(7749, null),
            cancelButton: localize(7750, null),
            message,
            custom: {
                markdownDetails: [{
                        markdown: detail
