...
class AbstractNewPromptFileAction extends Action2 {
    ...
    async run(accessor, options) {
        ...
        notificationService.prompt(Severity.Info, localize(7657, null), [
            {
                label: localize(7658, null),
                run: () => {
...
class NewPromptFileAction extends AbstractNewPromptFileAction {
    constructor() {
        super(NEW_PROMPT_COMMAND_ID, localize(7660, null), PromptsType.prompt);
    }
}
class NewInstructionsFileAction extends AbstractNewPromptFileAction {
    constructor() {
        super(NEW_INSTRUCTIONS_COMMAND_ID, localize(7661, null), PromptsType.instructions);
    }
}
class NewAgentFileAction extends AbstractNewPromptFileAction {
    constructor() {
        super(NEW_AGENT_COMMAND_ID, localize(7662, null), PromptsType.agent);
    }
}
class NewSkillFileAction extends Action2 {
    ...
    async run(accessor, options) {
        ...
        const skillName = await quickInputService.input({
            prompt: localize(7664, null),
            placeHolder: localize(7665, null),
            validateInput: async (value) => {
                if (!value || !value.trim()) {
                    return localize(7666, null);
                }
                const name = value.trim();
                if (name.length > 64) {
                    return localize(7667, null);
                }
                // Per spec: lowercase alphanumeric and hyphens only
                if (!/^[a-z0-9-]+$/.test(name)) {
                    return localize(7668, null);
                }
                if (name.startsWith('-') || name.endsWith('-')) {
                    return localize(7669, null);
                }
                if (name.includes('--')) {
                    return localize(7670, null);
                }
                return undefined;
            }
        });
...
class NewUntitledPromptFileAction extends Action2 {
    constructor() {
        super({
            ...
            title: localize2(7671, "New Untitled Prompt File"),
            f1: true,
            precondition: ChatContextKeys.enabled,
            category: CHAT_CATEGORY,
            keybinding: {
