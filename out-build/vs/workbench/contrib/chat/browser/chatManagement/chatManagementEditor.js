...
export const chatManagementSashBorder = registerColor('chatManagement.sashBorder', PANEL_BORDER, localize(7178, null));
...
let ChatManagementEditor = class ChatManagementEditor extends EditorPane {
    ...
    renderSidebar(parent) {
        // Define sections
        this.sections = [
            { id: CHAT_MANAGEMENT_SECTION_USAGE, label: localize(7179, null) },
            { id: CHAT_MANAGEMENT_SECTION_MODELS, label: localize(7180, null) }
        ];
    ...
    renderHeader(parent) {
        ...
        tile.textContent = localize(7182, null);
        // Plan badge
    ...
    updateHeaderData() {
        ...
        if (anonymousUser || isFreePlan) {
            if (anonymousUser) {
                ...
                this.planBadge.textContent = localize(7183, null);
            }
        }
        else {
            this.planBadge.style.display = '';
        ...
        if (newUser || signedOut || disabled || shouldUpgrade) {
            ...
            if (shouldUpgrade && !isFreePlan && !anonymousUser) {
                ...
                if (this.chatEntitlementService.entitlement === ChatEntitlement.Pro) {
                    buttonLabel = localize(7184, null);
                }
                else {
                    buttonLabel = localize(7185, null);
                }
                commandId = 'workbench.action.chat.upgradePlan';
            }
            else if (shouldUpgrade && (isFreePlan || anonymousUser)) {
                // Upgrade case for free plan
                buttonLabel = localize(7186, null);
                commandId = 'workbench.action.chat.upgradePlan';
            }
            else if (newUser) {
                buttonLabel = localize(7187, null);
                ...
                buttonLabel = localize(7188, null);
                commandId = 'workbench.action.chat.triggerSetup';
            }
            else if (disabled) {
                buttonLabel = localize(7189, null);
                commandId = 'workbench.action.chat.triggerSetup';
            }
            else {
                buttonLabel = localize(7190, null);
                commandId = 'workbench.action.chat.triggerSetup';
            }
