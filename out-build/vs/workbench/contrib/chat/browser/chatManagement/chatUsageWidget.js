...
let ChatUsageWidget = class ChatUsageWidget extends Disposable {
    ...
    render() {
        ...
        if (this.chatEntitlementService.anonymous && this.chatEntitlementService.sentiment.installed && !completionsQuota && !chatQuota && !premiumChatQuota) {
            this.renderLimitedQuotaItem(this.usageSection, localize(7259, null));
            this.renderLimitedQuotaItem(this.usageSection, localize(7260, null));
        }
        // Copilot Usage section - show detailed breakdown of all quotas
        else if (completionsQuota || chatQuota || premiumChatQuota) {
            // Inline Suggestions
            if (completionsQuota) {
                this.renderQuotaItem(this.usageSection, localize(7261, null), completionsQuota);
            }
            // Chat messages
            if (chatQuota) {
                this.renderQuotaItem(this.usageSection, localize(7262, null), chatQuota);
            }
            // Premium requests
            if (premiumChatQuota) {
                const premiumLabel = premiumChatQuota.overageEnabled ? localize(7263, null) : localize(7264, null);
                ...
                if (premiumChatQuota.overageEnabled && !premiumChatQuota.unlimited) {
                    ...
                    overageMessage.append(localize(7265, null));
                    DOM.append(overageMessage, $('br'));
                    overageMessage.append(localize(7266, null));
                }
            }
            // Reset date
            if (resetDate) {
                const resetText = DOM.append(this.usageSection, $('.allowance-resets'));
                resetText.textContent = localize(7267, null, resetDateHasTime ? this.dateTimeFormatter.value.format(new Date(resetDate)) : this.dateFormatter.value.format(new Date(resetDate)));
            }
        }
        // Emit height change
    ...
    renderQuotaItem(container, label, quota, overageEnabled = false) {
        ...
        if (quota.unlimited) {
            quotaItemValue.textContent = localize(7268, null);
        }
        else {
            quotaItemValue.textContent = localize(7269, null);
        }
    ...
    renderLimitedQuotaItem(container, label) {
        ...
        quotaItemLabel.textContent = label;
        const quotaItemValue = DOM.append(quotaItemHeader, $('.quota-item-value'));
        quotaItemValue.textContent = localize(7270, null);
        // Progress bar - using same structure as chat status
        const progressBarContainer = DOM.append(quotaItem, $('.quota-bar'));
