...
class ManageLanguageModelAuthenticationAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: localize2(6011, 'Manage Language Model Access...'),
    ...
    async run(accessor) {
        ...
        if (extensionAuth.size === 0) {
            dialogService.prompt({
                type: 'info',
                message: localize(5999, null),
                detail: localize(6000, null)
            });
            return;
        }
        const items = [];
        ...
        for (const [ownerId, allowedExtensions] of extensionAuth) {
            ...
            items.push({
                type: 'separator',
                id: ownerId,
                label: localize(6001, null, extension.displayName || extension.name),
                buttons: [{
                        iconClass: ThemeIcon.asClassName(Codicon.info),
                        tooltip: localize(6002, null),
                    }]
            });
            ...
            if (allowedExtensions.length > 0) {
                ...
                items.push({
                    label: localize(6005, null),
                    description: localize(6006, null, ownerId),
                    pickable: false
                });
            }
        }
        ...
        const result = await quickInputService.pick(items, {
            ...
            title: localize(6007, null),
            placeHolder: localize(6008, null),
        });
        if (!result) {
            return;
        }
