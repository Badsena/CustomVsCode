    ...
    registerAction2(class ForkConversationAction extends Action2 {
        constructor() {
            super({
                ...
                title: localize2(5994, "Fork Conversation"),
                tooltip: localize2(5995, "Fork conversation from this point"),
                f1: false,
                ...
                menu: [
                    {
                        ...
                        group: 'navigation',
                        order: 3,
                        when: ContextKeyExpr.and(ChatContextKeys.isRequest, ContextKeyExpr.or(ChatContextKeys.lockedToCodingAgent.negate(), ChatContextKeys.chatSessionSupportsFork))
                    }
                ]
            });
        }
        async run(accessor, ...args) {
            ...
            const forkedTitlePrefix = localize(5990, null);
            ...
            if (URI.isUri(args[0])) {
                ...
                cleanData.customTitle = chatModel.title.startsWith(forkedTitlePrefix)
                    ? chatModel.title
                    : localize(5991, null, chatModel.title);
            ...
            forkedData.customTitle = chatModel.title.startsWith(forkedTitlePrefix)
                ? chatModel.title
                : localize(5992, null, chatModel.title);
...
async function forkContributedChatSession(sourceSessionResource, request, openForkedSessionImmediately, chatSessionsService, chatWidgetService, progressService) {
    const forkedItem = await progressService.withProgress({ location: 15 /* ProgressLocation.Notification */, title: localize(5993, null) }, async () => {
