...
import { parseMarketplaceReferences } from '../../common/plugins/pluginMarketplaceService.js';
...
class InstallFromSourceAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: localize2(6049, 'Install Plugin from Source'),
            category: CHAT_CATEGORY,
            icon: Codicon.add,
            precondition: ChatContextKeys.enabled,
            f1: true,
    ...
    async run(accessor) {
        ...
        inputBox.placeholder = localize(6039, null);
        inputBox.prompt = localize(6040, null);
        inputBox.show();
...
class ManagePluginMarketplacesAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: localize2(6050, 'Manage Plugin Marketplaces'),
            icon: Codicon.globe,
            category: CHAT_CATEGORY,
            precondition: ChatContextKeys.enabled,
            f1: true,
    ...
    async run(accessor) {
        ...
        if (refs.length === 0) {
            quickInputService.pick([], { placeHolder: localize(6041, null) });
            return;
        }
        ...
        const selected = await quickInputService.pick(items, {
            placeHolder: localize(6043, null),
        });
        if (!selected) {
            return;
        }
        ...
        const actionItems = [
            { id: 'showPlugins', label: localize(6044, null) },
        ];
        ...
        if (repoExists) {
            actionItems.push({ id: 'openDirectory', label: localize(6045, null) });
        }
        actionItems.push({ id: 'removeMarketplace', label: localize(6046, null) });
        const action = await quickInputService.pick(actionItems, {
            placeHolder: localize(6047, null, ref.displayLabel),
        });
        if (!action) {
            return;
        }
