...
let InsertCodeBlockOperation = class InsertCodeBlockOperation {
    ...
    async run(context) {
        ...
        if (activeEditorControl) {
            ...
            if (activeNotebookEditor) {
                ...
                this.notify(localize(6105, null));
            }
        }
    ...
    async handleNotebookEditor(notebookEditor, codeBlockContext) {
        if (notebookEditor.isReadOnly) {
            this.notify(localize(6106, null));
            return false;
        }
    ...
    async handleTextEditor(codeEditor, codeBlockContext) {
        ...
        if (isReadOnly(activeModel, this.textFileService)) {
            this.notify(localize(6107, null));
            return false;
        }
...
let ApplyCodeBlockOperation = class ApplyCodeBlockOperation {
    ...
    async run(context) {
        ...
        if (codemapperUri && !isEqual(activeEditorControl?.getModel().uri, codemapperUri) && !this.notebookService.hasSupportedNotebooks(codemapperUri)) {
            ...
            try {
                ...
                if (codeEditor && codeEditor.hasModel()) {
                    ...
                    this.notify(localize(6108, null, codemapperUri.toString()));
                    return;
                }
            }
            catch (e) {
        ...
        if (activeEditorControl && !this.notebookService.hasSupportedNotebooks(codemapperUri)) {
            ...
            if (activeNotebookEditor) {
                ...
                this.notify(localize(6109, null));
            }
        }
        if (isResponseVM(context.element)) {
            const requestId = context.element.requestId;
    ...
    async evaluateURIToUse(resource, activeEditorControl) {
        ...
        const activeEditorOption = activeEditorControl?.getModel().uri ? { label: localize(6110, null, this.labelService.getUriLabel(activeEditorControl.getModel().uri, { relative: true })), id: 'activeEditor' } : undefined;
        const untitledEditorOption = { label: localize(6111, null), id: 'newUntitledFile' };
        const options = [];
        if (resource) {
            // code block had an URI, but it doesn't exist
            options.push({ label: localize(6112, null, this.labelService.getUriLabel(resource, { relative: true })), id: 'createFile' });
        ...
        const selected = options.length > 1 ? await this.quickInputService.pick(options, { placeHolder: localize(6113, null) }) : options[0];
        if (selected) {
            switch (selected.id) {
                case 'createFile':
                    if (resource) {
                        try {
                            ...
                            this.notify(localize(6114, null, error.message));
    ...
    async handleNotebookEditor(notebookEditor, chatSessionResource, code) {
        if (notebookEditor.isReadOnly) {
            this.notify(localize(6115, null));
            return undefined;
        }
        ...
        if (!codeMapper) {
            this.notify(localize(6116, null));
            return undefined;
        }
        ...
        try {
            const iterable = await this.progressService.withProgress({ location: 15 /* ProgressLocation.Notification */, delay: 500, sticky: true, cancellable: true }, async (progress) => {
                progress.report({ message: localize(6117, null, codeMapper) });
            ...
            if (!isCancellationError(e)) {
                this.notify(localize(6118, null, e.message));
            }
        }
        finally {
    ...
    async handleTextEditor(codeEditor, chatSessionResource, code, applyCodeBlockSuggestionId) {
        ...
        try {
            const iterable = await this.progressService.withProgress({ location: 15 /* ProgressLocation.Notification */, delay: 500, sticky: true, cancellable: true }, async (progress) => {
                progress.report({ message: localize(6121, null, codeMapper) });
            ...
            if (!isCancellationError(e)) {
                this.notify(localize(6122, null, e.message));
            }
        }
        finally {
            cancellationTokenSource.dispose();
