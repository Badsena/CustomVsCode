...
export { SlashCommandCompletions };
    ...
    constructor(languageFeaturesService, chatWidgetService, chatAgentService, chatAgentNameService, chatSessionsService) {
        ...
        this._register(this.languageFeaturesService.completionProvider.register({ scheme: Schemas.vscodeChatInput, hasAccessToAllModels: true }, {
            ...
            provideCompletionItems: async (model, position, _context, token) => {
                ...
                const label = localize(8248, null);
                const item = {
                    label,
                    insertText: '',
                    range,
        ...
        try {
            ...
            try {
                ...
                if (!cts.token.isCancellationRequested) {
                    notificationService.error(localize(8249, null, String(e)));
                }
                replaceTextWith('');
                return;
            }
            ...
            const attachBlob = async (mimeType, contents, uriStr, isText = false) => {
                ...
                if (isText) {
                    ...
                    chatWidget.attachmentModel.addContext({
                        id: generateUuid(),
                        name: localize(8251, null),
                        fullName: localize(8252, null),
                        value: resized,
                        kind: 'image',
...
let BuiltinDynamicCompletions = class BuiltinDynamicCompletions extends Disposable {
    ...
    constructor(historyService, workspaceContextService, searchService, labelService, languageFeaturesService, chatWidgetService, outlineService, editorService, configurationService, codeEditorService, chatAgentService, instantiationService, chatDebugService) {
        super();
        ...
        this.registerVariableCompletions('debugEventsSnapshot', ({ widget, range }) => {
            ...
            const result = { suggestions: [] };
            result.suggestions.push({
                label: { label: text, description: localize(8253, null) },
                filterText: text,
                insertText: '',
                range,
                kind: 18 /* CompletionItemKind.Text */,
    ...
    async addFileAndFolderEntries(widget, result, info, token) {
        const makeCompletionItem = (resource, kind, description, boostPriority) => {
            ...
            const labelDescription = description
                ? localize(8254, null, uriLabel, description)
                : uriLabel;
        ...
        for (const [i, item] of this.historyService.getHistory().entries()) {
            ...
            seen.add(resource);
            const newLen = result.suggestions.push(makeCompletionItem(resource, FileKind.FILE, i === 0 ? localize(8255, null) : undefined, i === 0));
            if (newLen - len >= 5) {
                break;
            }
        }
...
export { BuiltinDynamicCompletions };
            ...
            provideCompletionItems: async (model, position, _context, _token) => {
                ...
                for (const [item, enabled] of iter) {
                    ...
                    if (isToolSet(item)) {
                        ...
                        detail = localize(8256, null, source.label, item.displayName);
