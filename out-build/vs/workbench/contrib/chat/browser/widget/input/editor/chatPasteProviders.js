...
let PasteImageProvider = class PasteImageProvider {
    ...
    async provideDocumentPasteEdits(model, ranges, dataTransfer, context, token) {
        ...
        const displayName = localize(8259, null);
        ...
        const edit = createCustomPasteEdit(model, [scaledImageContext], mimeType, this.kind, localize(8260, null), this.chatWidgetService);
...
export class PasteTextProvider {
    ...
    async provideDocumentPasteEdits(model, ranges, dataTransfer, _context, token) {
        ...
        const edit = createCustomPasteEdit(model, [copiedContext], Mimes.text, this.kind, localize(8262, null), this.chatWidgetService);
...
function getCopiedContext(code, file, language, range) {
    ...
    const pastedLines = start === end ? localize(8263, null) : localize(8264, null, end + 1 - start);
    return {
        kind: 'paste',
        value: resultText,
...
function createCustomPasteEdit(model, context, handledMimeType, kind, title, chatWidgetService) {
    const label = context.length === 1
        ? context[0].name
        : localize(8265, null, context[0].name, context.length - 1);
