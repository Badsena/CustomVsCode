...
export function getToolInvocationIcon(toolId) {
    const lowerToolId = toolId.toLowerCase();
...
let ChatThinkingContentPart = ChatThinkingContentPart_1 = class ChatThinkingContentPart extends ChatCollapsibleContentPart {
    ...
    constructor(content, context, chatContentMarkdownRenderer, streamingCompleted, instantiationService, configurationService, chatMarkdownAnchorService, languageModelsService, hoverService) {
        ...
        this.defaultTitle = localize(7999, null);
        this.workingTitle = localize(8000, null);
...
${this.hookCount > 0 ? `EXAMPLES WITH BLOCKED CONTENT (from hooks):
    ...
    setFallbackTitle() {
        const finalLabel = this.appendedItemCount > 0
            ? localize(8003, null, this.appendedItemCount, this.appendedItemCount === 1 ? '' : 's')
            : localize(8004, null);
    ...
    trackToolMetadata(toolInvocationId, toolInvocationOrMarkdown) {
        ...
        if (isToolInvocation && toolInvocationOrMarkdown.invocationMessage) {
            ...
            if (isStreamingEditTool) {
                toolCallLabel = localize(8005, null);
            }
            else {
            ...
            if (codeblockInfo?.uri) {
                ...
                toolCallLabel = localize(8006, null, filename);
            }
            else {
                toolCallLabel = localize(8007, null);
            }
        }
        else {
    ...
    setTitle(title, omitPrefix) {
        ...
        const thinkingLabel = localize(8008, null, this.defaultTitle, title);
        ...
        this.titleShimmerSpan.textContent = localize(8009, null, this.defaultTitle);
