...
class TodoListRenderer {
    ...
    renderElement(todo, index, templateData) {
        ...
        const ariaLabel = localize(8019, null, todo.title, statusText);
    ...
    getStatusText(status) {
        switch (status) {
            case 'completed':
                return localize(8020, null);
            case 'in-progress':
                return localize(8021, null);
            case 'not-started':
            default:
                return localize(8022, null);
        }
    }
...
let ChatTodoListWidget = class ChatTodoListWidget extends Disposable {
    ...
    createChatTodoWidget() {
        ...
        this.titleElement.textContent = localize(8023, null);
        ...
        if (!this._todoList) {
            this._todoList = this._register(this.instantiationService.createInstance((WorkbenchList), 'ChatTodoListRenderer', this.todoListContainer, new TodoListDelegate(), [new TodoListRenderer()], {
                ...
                accessibilityProvider: {
                    getAriaLabel: (todo) => {
                        const statusText = this.getStatusText(todo.status);
                        return localize(8025, null, todo.title, statusText);
                    },
                    getWidgetAriaLabel: () => localize(8026, null)
                }
            }));
        }
    ...
    updateClearButtonState() {
        ...
        if (shouldDisable) {
            this.clearButton.setTitle(localize(8027, null));
        }
        else {
            this.clearButton.setTitle(localize(8028, null));
        }
    }
    updateTitleElement(titleElement, todoList) {
        ...
        if (this._isExpanded) {
            ...
            titleText.textContent = totalCount > 0 ?
                localize(8031, null, currentTaskNumber, totalCount) :
                localize(8032, null);
            ...
            if (todoToShow) {
                ...
                todoText.textContent = localize(8033, null, todoToShow.title, currentTaskNumber, totalCount);
            ...
            // Show "Done" when all tasks are completed
                ...
                doneText.textContent = localize(8034, null, totalCount, totalCount);
    ...
    getStatusText(status) {
        switch (status) {
            case 'completed':
                return localize(8035, null);
            case 'in-progress':
                return localize(8036, null);
            case 'not-started':
            default:
                return localize(8037, null);
        }
    }
};
