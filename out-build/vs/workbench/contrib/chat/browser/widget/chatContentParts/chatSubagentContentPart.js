...
let ChatSubagentContentPart = ChatSubagentContentPart_1 = class ChatSubagentContentPart extends ChatCollapsibleContentPart {
    ...
    static extractSubagentInfo(toolInvocation) {
        const defaultDescription = localize(7969, null);
    ...
    constructor(subAgentInvocationId, toolInvocation, context, chatContentMarkdownRenderer, listPool, editorPool, currentWidthDelegate, codeBlockModelCollection, announcedToolProgressKeys, instantiationService, chatMarkdownAnchorService, hoverService, configurationService) {
        ...
        const prefix = agentName || localize(7970, null);
    ...
    doRenderPromptSection() {
        ...
        const rawFirstLine = lines[0] || localize(7971, null);
    ...
    updateTitle() {
        const prefix = this.agentName || localize(7972, null);
    ...
    updateHover() {
        ...
        this._hoverDisposable.value = this.hoverService.setupDelayedHover(this._collapseButton.element, {
            content: localize(7973, null, this.modelName),
        });
    }
    /**
    ...
    appendHookItem(factory, hookPart) {
        ...
        const hookMessage = hookPart.stopReason
            ? (hookPart.toolDisplayName
                ? localize(7974, null, hookPart.toolDisplayName)
                : localize(7975, null))
            : (hookPart.toolDisplayName
                ? localize(7976, null, hookPart.toolDisplayName)
                : localize(7977, null));
