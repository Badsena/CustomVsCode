...
let ChatSuggestNextWidget = class ChatSuggestNextWidget extends Disposable {
    ...
    render(mode) {
        ...
        const modeName = mode.name.get() || mode.label.get() || localize(7978, null);
        this.titleElement.textContent = localize(7979, null, modeName);
    ...
    createPromptButton(handoff) {
        ...
        button.setAttribute('aria-label', localize(7980, null, handoff.label));
        ...
        if (showContinueOn && availableContributions.length > 0) {
            ...
            dropdownContainer.setAttribute('aria-label', localize(7981, null, handoff.label));
            ...
            const showContextMenu = (e, anchor) => {
                ...
                const actions = availableContributions.map(contrib => {
                    ...
                    return new Action(contrib.type, localize(7982, null, name), ThemeIcon.isThemeIcon(icon) ? ThemeIcon.asClassName(icon) : undefined, true, () => {
    ...
    createAutopilotButton(handoff) {
        ...
        const label = localize(7983, null);
        const button = dom.$('.chat-welcome-view-suggested-prompt');
        button.setAttribute('tabindex', '0');
        button.setAttribute('role', 'button');
