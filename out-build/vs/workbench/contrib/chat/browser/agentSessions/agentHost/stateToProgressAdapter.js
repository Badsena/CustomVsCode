...
import { getToolOutputText } from '../../../../../../platform/agentHost/common/state/sessionState.js';
...
function completedToolCallToSerialized(tc) {
    ...
    if (isTerminal && tc.toolInput) {
        const toolOutput = tc.status === "completed" /* ToolCallStatus.Completed */ ? getToolOutputText(tc) : undefined;
        toolSpecificData = {
            kind: 'terminal',
...
export function toolCallStateToInvocation(tc) {
    ...
    if (getToolKind(tc) === 'terminal') {
        invocation.toolSpecificData = {
            kind: 'terminal',
            commandLine: { original: tc.status !== "streaming" /* ToolCallStatus.Streaming */ ? (tc.toolInput ?? '') : '' },
            language: getToolLanguage(tc) ?? 'shellscript',
        };
    }
    return invocation;
 ...
 */
export function permissionToConfirmation(perm) {
    let title;
    let toolSpecificData;
    switch (perm.permissionKind) {
        case "shell" /* PermissionKind.Shell */: {
        ...
        case "write" /* PermissionKind.Write */: {
            ...
            let rawInput;
            try {
                rawInput = perm.rawRequest ? JSON.parse(perm.rawRequest) : { path: perm.path };
            }
            catch {
                rawInput = { path: perm.path };
            }
            toolSpecificData = { kind: 'input', rawInput };
            break;
        }
        case "mcp" /* PermissionKind.Mcp */: {
            const toolTitle = perm.toolName ?? 'MCP Tool';
            title = perm.serverName ? `${perm.serverName}: ${toolTitle}` : toolTitle;
            let rawInput;
            try {
                rawInput = perm.rawRequest ? JSON.parse(perm.rawRequest) : { serverName: perm.serverName, toolName: perm.toolName };
            }
            catch {
                rawInput = { serverName: perm.serverName, toolName: perm.toolName };
            }
            toolSpecificData = { kind: 'input', rawInput };
            break;
        }
        case "read" /* PermissionKind.Read */: {
            ...
            let rawInput;
            try {
                rawInput = perm.rawRequest ? JSON.parse(perm.rawRequest) : { path: perm.path, intention: perm.intention };
            }
            catch {
                rawInput = { path: perm.path, intention: perm.intention };
            }
            toolSpecificData = { kind: 'input', rawInput };
            break;
        }
        default: {
            ...
            let rawInput;
            try {
                rawInput = perm.rawRequest ? JSON.parse(perm.rawRequest) : {};
            }
            catch {
                rawInput = {};
            }
...
//# sourceMappingURL=stateToProgressAdapter.js.map