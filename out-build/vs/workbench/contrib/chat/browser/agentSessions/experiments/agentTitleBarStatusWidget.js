...
const NLS_EXTENSION_HOST = localize(6289, null);
const TITLE_DIRTY = '\u25cf ';
/**
...
let AgentTitleBarStatusWidget = class AgentTitleBarStatusWidget extends BaseActionViewItem {
    ...
    _renderChatInputMode(disposables) {
        ...
        pill.setAttribute('aria-label', localize(6290, null));
        pill.tabIndex = 0;
        ...
        const hoverLabel = localize(6291, null);
    ...
    _renderSessionMode(disposables) {
        ...
        const sessionInfo = this.agentTitleBarStatusService.sessionInfo;
        titleLabel.textContent = sessionInfo?.title ?? localize(6295, null);
        ...
        disposables.add(this.hoverService.setupManagedHover(hoverDelegate, pill, () => {
            const sessionInfo = this.agentTitleBarStatusService.sessionInfo;
            return sessionInfo ? localize(6296, null, sessionInfo.title) : localize(6297, null);
        }));
    ...
    _renderSessionReadyMode(disposables) {
        ...
        const titleLabel = $('span.agent-status-title');
        const sessionInfo = this.agentTitleBarStatusService.sessionInfo;
        titleLabel.textContent = sessionInfo?.title ?? localize(6298, null);
        pill.appendChild(titleLabel);
        ...
        disposables.add(this.hoverService.setupManagedHover(hoverDelegate, pill, () => {
            const sessionInfo = this.agentTitleBarStatusService.sessionInfo;
            return sessionInfo ? localize(6299, null, sessionInfo.title) : localize(6300, null);
        }));
    ...
    _renderSearchButton(disposables, parent) {
        ...
        searchButton.setAttribute('aria-label', localize(6301, null));
    ...
    _renderStatusBadge(disposables, activeSessions, unreadSessions, attentionNeededSessions) {
        ...
        if (viewSessionsEnabled && hasUnreadSessions && this.workspaceContextService.getWorkbenchState() !== 1 /* WorkbenchState.EMPTY */) {
            ...
            const unreadTooltip = unreadSessions.length === 1
                ? localize(6308, null, unreadSessions.length)
                : localize(6309, null, unreadSessions.length);
        ...
        if (viewSessionsEnabled && hasActiveSessions) {
            ...
            const activeTooltip = hasAttentionNeeded
                ? (attentionNeededSessions.length === 1
                    ? localize(6310, null, attentionNeededSessions.length)
                    : localize(6311, null, attentionNeededSessions.length))
                : (activeSessions.length === 1
                    ? localize(6312, null, activeSessions.length)
                    : localize(6313, null, activeSessions.length));
    ...
    _openSessionsWithFilter(filterType) {
        ...
        if (filterType === 'unread') {
            ...
            if (isFilteredToInProgress) {
                ...
                this._storeFilter({
                    providers: preservedProviders,
                    states: [1 /* AgentSessionStatus.Completed */, 0 /* AgentSessionStatus.Failed */],
                    archived: true,
                    read: false
                });
                // Track that this window applied the badge filter
    ...
    _renderEscapeButton(disposables, parent) {
        ...
        escButton.setAttribute('aria-label', localize(6314, null));
        escButton.tabIndex = 0;
        parent.appendChild(escButton);
        // Setup hover
        const hoverDelegate = getDefaultHoverDelegate('mouse');
        disposables.add(this.hoverService.setupManagedHover(hoverDelegate, escButton, localize(6315, null)));
        // Click handler
    ...
    _renderEnterButton(disposables, parent) {
        ...
        enterButton.textContent = keybinding?.getLabel() ?? localize(6316, null);
        enterButton.setAttribute('role', 'button');
        enterButton.setAttribute('aria-label', localize(6317, null));
        ...
        const hoverText = keybinding
            ? localize(6318, null, keybinding.getLabel())
            : localize(6319, null);
    ...
    _getLabel() {
        ...
        if (this.editorGroupsService.partOptions.showTabs === 'none') {
            ...
            if (activeEditor) {
                ...
                label = `${dirty}${activeEditor.getTitle(0 /* Verbosity.SHORT */)}`;
            }
        }
        if (!label) {
            label = localize(6320, null);
        }
        // Apply prefix and suffix decorations
        if (prefix) {
            label = localize(6321, null, prefix, label);
        }
        if (suffix) {
            label = localize(6322, null, label, suffix);
        }
...
let AgentTitleBarStatusRendering = class AgentTitleBarStatusRendering extends Disposable {
    ...
    constructor(actionViewItemService, instantiationService, configurationService) {
        ...
        this._register(configurationService.onDidChangeConfiguration(e => {
            if (e.affectsConfiguration(ChatConfiguration.AgentStatusEnabled) || e.affectsConfiguration(ChatConfiguration.UnifiedAgentsBar) || e.affectsConfiguration("window.commandCenter" /* LayoutSettings.COMMAND_CENTER */)) {
                updateClass();
            }
        }));
    }
