...
let ChatStatusBarEntry = class ChatStatusBarEntry extends Disposable {
    ...
    getEntryProps() {
        let text = '$(copilot)';
        let ariaLabel = localize(7514, null);
        let kind;
        if (isNewUser(this.chatEntitlementService)) {
            ...
            if (this.chatEntitlementService.sentiment.later || // user skipped setup
                ...
                const finishSetup = localize(7515, null);
            ...
            if (this.chatEntitlementService.sentiment.disabled || this.chatEntitlementService.sentiment.untrusted) {
                ...
                ariaLabel = localize(7516, null);
            }
            // Sessions in progress
            else if (this.runningSessionsCount > 0) {
                text = '$(copilot-in-progress)';
                if (this.runningSessionsCount > 1) {
                    ariaLabel = localize(7517, null, this.runningSessionsCount);
                }
                else {
                    ariaLabel = localize(7518, null);
                }
            }
            // Signed out
            else if (this.chatEntitlementService.entitlement === ChatEntitlement.Unknown) {
                const signedOutWarning = localize(7519, null);
            ...
            // Free Quota Exceeded
                ...
                if (chatQuotaExceeded && !completionsQuotaExceeded) {
                    quotaWarning = localize(7520, null);
                }
                else if (completionsQuotaExceeded && !chatQuotaExceeded) {
                    quotaWarning = localize(7521, null);
                }
                else {
                    quotaWarning = localize(7522, null);
                }
            ...
            // Completions Disabled
                ...
                text = '$(copilot-unavailable)';
                ariaLabel = localize(7523, null);
            }
            // Completions Snoozed
            else if (this.completionsService.isSnoozing()) {
                text = '$(copilot-snooze)';
                ariaLabel = localize(7524, null);
            }
        }
        const baseResult = {
            name: localize(7525, null),
            text,
            ariaLabel,
