...
const extensionPoint = ExtensionsRegistry.registerExtensionPoint({
    ...
    jsonSchema: {
        description: localize(7553, null),
        type: 'array',
        items: {
            type: 'object',
            properties: {
                id: {
                    description: localize(7554, null),
                    type: 'string',
                },
                icon: {
                    description: localize(7555, null),
                    type: 'string'
                },
                displayName: {
                    description: localize(7556, null),
                    type: 'string'
                }
            },
            ...
            for (const ext of extensions) {
                ...
                for (const contribution of ext.value) {
                    ...
                    if (!icon && contribution.icon) {
                        ext.collector.error(localize(7557, null, contribution.id));
                        continue;
                    }
                    if (!icon) {
