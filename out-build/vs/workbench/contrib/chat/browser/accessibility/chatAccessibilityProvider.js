...
export const getToolConfirmationAlert = (accessor, toolInvocation) => {
    ...
    const text = toolInvocation.map(v => {
        ...
        if (v.toolSpecificData) {
            if (v.toolSpecificData.kind === 'terminal') {
                ...
                input = localize(5739, null, v.toolSpecificData.modifiedFiles.length);
            }
        }
    ...
    let message = acceptKb && cancelKb
        ? localize(5740, null, text.map(t => t.title).join(', '), acceptKb, cancelKb)
        : localize(5741, null, text.map(t => t.title).join(', '));
    if (text.some(t => t.detail)) {
        message += ' ' + localize(5742, null, text.map(t => t.detail ? t.detail : '').join(' '));
    }
    return message;
};
    ...
    _getLabelWithInfo(element) {
        ...
        switch (tableCount) {
            case 0:
                break;
            case 1:
                tableCountHint = localize(5744, null);
                break;
            default:
                tableCountHint = localize(5745, null, tableCount);
                break;
        }
        ...
        switch (fileTreeCount) {
            case 0:
                break;
            case 1:
                fileTreeCountHint = localize(5746, null);
                break;
            default:
                fileTreeCountHint = localize(5747, null, fileTreeCount);
                break;
        }
        ...
        switch (codeBlockCount) {
            case 0:
                label = accessibleViewHint
                    ? localize(5748, null, toolInvocationHint, fileTreeCountHint, elicitationHint, tableCountHint, element.response.toString(), accessibleViewHint)
                    : localize(5749, null, fileTreeCountHint, elicitationHint, tableCountHint, element.response.toString());
                break;
            case 1:
                label = accessibleViewHint
                    ? localize(5750, null, toolInvocationHint, fileTreeCountHint, elicitationHint, tableCountHint, element.response.toString(), accessibleViewHint)
                    : localize(5751, null, fileTreeCountHint, elicitationHint, tableCountHint, element.response.toString());
                break;
            default:
                label = accessibleViewHint
                    ? localize(5752, null, toolInvocationHint, fileTreeCountHint, elicitationHint, tableCountHint, codeBlockCount, element.response.toString(), accessibleViewHint)
                    : localize(5753, null, fileTreeCountHint, elicitationHint, codeBlockCount, tableCountHint, element.response.toString());
                break;
        }
        return label;
    }
