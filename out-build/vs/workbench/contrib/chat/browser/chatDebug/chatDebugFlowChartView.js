...
let ChatDebugFlowChartView = class ChatDebugFlowChartView extends Disposable {
    constructor(parent, filterState, chatService, chatDebugService, contextKeyService, instantiationService) {
        ...
        this.filterWidget = this._register(childInstantiationService.createInstance(FilterWidget, {
            placeholder: localize(6904, null),
            ariaLabel: localize(6905, null),
        }));
    ...
    updateBreadcrumb() {
        ...
        this.breadcrumbWidget.setItems([
            new TextBreadcrumbItem(localize(6906, null), true),
            new TextBreadcrumbItem(sessionTitle, true),
            new TextBreadcrumbItem(localize(6907, null)),
        ]);
    }
    load() {
        ...
        if (events.length === 0) {
            ...
            emptyMsg.textContent = localize(6908, null);
            return;
        }
        ...
        if (filtered.length === 0) {
            const emptyMsg = DOM.append(this.content, $('.chat-debug-flowchart-empty'));
            emptyMsg.textContent = localize(6909, null);
            return;
        }
        ...
        if (slice.shownCount < slice.totalCount) {
            ...
            const showMoreBtn = this.loadDisposables.add(new Button(showMoreContainer, { ...defaultButtonStyles, secondary: true, title: localize(6910, null) }));
            showMoreBtn.label = localize(6911, null, remaining);
