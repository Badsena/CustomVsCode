...
export function renderSection(parent, label, plainText, tokenizedHtml, disposables, initiallyCollapsed = false, clipboardService, scrollable) {
    ...
    if (clipboardService) {
        const copyBtn = disposables.add(new Button(header, {
            title: localize(7060, null),
            ariaLabel: localize(7061, null),
...
export async function renderToolCallContent(content, languageService, clipboardService, scrollable) {
    ...
    if (content.durationInMillis !== undefined) {
        statusParts.push(localize(7064, null, content.durationInMillis));
    }
    ...
    if (content.input) {
        ...
        renderSection(sectionsContainer, localize(7065, null), plainText, tokenizedHtml, disposables, false, clipboardService, scrollable);
    }
    if (content.output) {
        const { plainText, tokenizedHtml } = await tokenizeContent(content.output, languageService);
        renderSection(sectionsContainer, localize(7066, null), plainText, tokenizedHtml, disposables, false, clipboardService, scrollable);
    }
...
export function toolCallContentToPlainText(content) {
    const lines = [];
    lines.push(localize(7067, null, content.toolName));
    if (content.result) {
        lines.push(localize(7068, null, content.result));
    }
    if (content.durationInMillis !== undefined) {
        lines.push(localize(7069, null, content.durationInMillis));
    }
    if (content.input) {
        lines.push('');
        lines.push(`[${localize(7070, null)}]`);
        try {
