...
let ChatSetupController = class ChatSetupController extends Disposable {
    ...
    async setup(options = {}) {
        ...
        const title = localize(7377, null);
    ...
    async signIn(options) {
        ...
        if (!defaultAccount && !this.lifecycleService.willShutdown) {
            const { confirmed } = await this.dialogService.confirm({
                type: Severity.Error,
                message: localize(7378, null, this.defaultAccountService.getDefaultAccountAuthenticationProvider().name),
                detail: localize(7379, null),
                primaryButton: localize(7380, null)
            });
            if (confirmed) {
    ...
    async doInstallWithRetry() {
        ...
        if (error) {
            if (!this.lifecycleService.willShutdown) {
                const { confirmed } = await this.dialogService.confirm({
                    type: Severity.Error,
                    message: localize(7381, null),
                    detail: error && !isCancellationError(error) ? toErrorMessage(error) : undefined,
                    primaryButton: localize(7382, null)
                });
                if (confirmed) {
    ...
    async handleEnterpriseInstance() {
        ...
        const result = await this.quickInputService.input({
            prompt: localize(7383, null, defaultChat.provider.enterprise.name),
            placeHolder: localize(7384, null),
            ...
            validateInput: async (value) => {
                ...
                if (domainRegEx.test(value)) {
                    isSingleWord = true;
                    return {
                        content: localize(7385, null, `https://${value}.ghe.com`),
                        severity: Severity.Info
                    };
                }
                if (!fullUriRegEx.test(value)) {
                    return {
                        content: localize(7386, null, defaultChat.provider.enterprise.name),
