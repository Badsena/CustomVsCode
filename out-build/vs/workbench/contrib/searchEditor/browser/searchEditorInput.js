...
const SearchEditorIcon = registerIcon('search-editor-label-icon', Codicon.search, localize(14739, null));
let SearchEditorInput = class SearchEditorInput extends EditorInput {
    ...
    getName(maxLength = 12) {
        ...
        if (this.backingUri) {
            ...
            return localize(14740, null, basename((originalURI ?? this.backingUri).path, SEARCH_EDITOR_EXT));
        }
        const query = this._cachedConfigurationModel?.config?.query?.trim();
        if (query) {
            return localize(14741, null, trimToMax(query));
        }
        return localize(14742, null);
    }
    setDirty(dirty) {
