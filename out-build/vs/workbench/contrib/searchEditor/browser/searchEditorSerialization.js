        ...
        for (let i = 0; i < str.length; i++) {
            if (str[i] === '\\') {
                ...
                if (escaped === 'n') {
                    ...
                    throw Error(localize(14743, null));
                }
            }
            else {
                out += str[i];
...
export const serializeSearchResultForEditor = (searchResult, rawIncludePattern, rawExcludePattern, contextLines, labelFormatter, sortOrder, limitHit) => {
    ...
    const filecount = searchResult.fileCount() > 1 ? localize(14744, null, searchResult.fileCount()) : localize(14745, null);
    const resultcount = searchResult.count() > 1 ? localize(14746, null, searchResult.count()) : localize(14747, null);
    const info = [
        searchResult.count()
