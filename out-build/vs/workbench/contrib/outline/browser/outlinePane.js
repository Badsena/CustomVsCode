...
let OutlinePane = class OutlinePane extends ViewPane {
    ...
    async _handleEditorControlChanged(pane) {
        ...
        if (!pane || !this._outlineService.canCreateOutline(pane) || !resource) {
            return this._showMessage(localize(13348, null));
        }
        let loadingMessage;
        if (!didCapture) {
            loadingMessage = new TimeoutTimer(() => {
                this._showMessage(localize(13349, null, basename(resource)));
            }, 100);
        }
        ...
        const updateTree = () => {
            if (newOutline.isEmpty) {
                // no more elements
                this._showMessage(localize(13350, null, basename(resource)));
