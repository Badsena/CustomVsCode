...
let RunTestTool = class RunTestTool {
    ...
    static { this.DEFINITION = {
        ...
        userDescription: localize(16682, null),
    ...
    async invoke(invocation, countTokens, progress, token) {
        ...
        if (!testCases.length) {
            return {
                ...
                toolResultError: localize(16683, null),
            };
        }
        progress.report({ message: localize(16684, null) });
        ...
        if (!result) {
            return {
                ...
                toolResultError: localize(16685, null),
            };
        }
        ...
        if (token.isCancellationRequested) {
            ...
            return {
                content: [{ kind: 'text', value: localize(16686, null) }],
                toolResultMessage: localize(16687, null),
            };
        }
    ...
    async _getTestCasesToRun(params, tests, progress) {
        ...
        progress.report({ message: localize(16688, null) });
    ...
    async _getFileTestsToRun(params, progress) {
        ...
        progress.report({ message: localize(16689, null) });
    ...
    prepareToolInvocation(context, token) {
        ...
        const title = localize(16690, null);
        const inFiles = params.files?.map((f) => '`' + basename(f) + '`');
        return Promise.resolve({
            invocationMessage: localize(16691, null),
            confirmationMessages: {
                title,
                message: inFiles?.length
                    ? new MarkdownString().appendMarkdown(localize(16692, null, inFiles.join(', ')))
                    : localize(16693, null),
