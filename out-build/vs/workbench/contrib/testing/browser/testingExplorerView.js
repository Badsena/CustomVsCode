...
let TestingExplorerView = class TestingExplorerView extends ViewPane {
        ...
        for (const { profiles, controller } of this.testProfileService.all()) {
            ...
            for (const profile of profiles) {
                ...
                profileActions.push(toAction({
                    ...
                    label: defaults.includes(profile) ? localize(16544, null, profile.label) : profile.label,
                    run: () => {
    ...
    getDropdownAction() {
        return new Action('selectRunConfig', localize(16547, null), 'codicon-chevron-down', true);
    }
...
let ResultSummaryView = class ResultSummaryView extends Disposable {
    ...
    render() {
        ...
        if (!results.length) {
            ...
            this.container.innerText = localize(16548, null);
    ...
    renderActivityBadge(countSummary, isRunning) {
        if (isRunning) {
            ...
            this.lastBadge = new IconBadge(spinningLoading, () => localize(16549, null));
        }
            ...
            this.lastBadge = new IconBadge(icons.testingContinuousIsOn, () => localize(16550, null));
        }
        else {
            if (!this.lastBadge) {
                return;
    ...
    getLocalizedBadgeString(countBadgeType, count) {
        switch (countBadgeType) {
            case "passed" /* TestingCountBadge.Passed */:
                return localize(16551, null, count);
            case "skipped" /* TestingCountBadge.Skipped */:
                return localize(16552, null, count);
            default:
                return localize(16553, null, count);
        }
    }
};
...
let NoTestsForDocumentWidget = class NoTestsForDocumentWidget extends Disposable {
    constructor(container, filterState) {
        ...
        emptyParagraph.innerText = localize(16554, null);
        const buttonLabel = localize(16555, null);
...
const getLabelForTestTreeElement = (element) => {
    ...
    if (element instanceof TestItemTreeElement) {
        if (element.duration !== undefined) {
            label = localize(16556, null, label, formatDuration(element.duration));



        }
        if (element.retired) {
            label = localize(16557, null, label);
