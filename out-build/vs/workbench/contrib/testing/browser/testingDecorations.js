        ...
        get hoverMessage() {
            if (!hoverMessage) {
                ...
                if (testIdWithMessages) {
                    ...
                    building.appendMarkdown(` [${localize(16514, null)}](command:vscode.peekTestError?${args})`);
                }
            }
            return hoverMessage;
        },
...
let RunTestDecoration = class RunTestDecoration {
    ...
    getGutterLabel() {
        switch (getTestingConfiguration(this.configurationService, "testing.defaultGutterClickAction" /* TestingConfigKeys.DefaultGutterClickAction */)) {
            case "contextMenu" /* DefaultGutterClickAction.ContextMenu */:
                return localize(16517, null);
            case "debug" /* DefaultGutterClickAction.Debug */:
                return localize(16518, null);
            case "runWithCoverage" /* DefaultGutterClickAction.Coverage */:
                return localize(16519, null);
            case "run" /* DefaultGutterClickAction.Run */:
            default:
                return localize(16520, null);
        }
    }
    /**
    ...
    getTestContextMenuActions(test, resultItem) {
        ...
        [
            { bitset: 2 /* TestRunProfileBitset.Run */, label: localize(16521, null) },
            { bitset: 4 /* TestRunProfileBitset.Debug */, label: localize(16522, null) },
            { bitset: 8 /* TestRunProfileBitset.Coverage */, label: localize(16523, null) },
        ...
        if (capabilities & 16 /* TestRunProfileBitset.HasNonDefaultProfile */) {
            testActions.push(new Action('testing.runUsing', localize(16524, null), undefined, undefined, async () => {
        ...
        if (resultItem && isFailedState(resultItem.computedState)) {
            testActions.push(new Action('testing.gutter.peekFailure', localize(16525, null), undefined, undefined, () => this.commandService.executeCommand('vscode.peekTestError', test.item.extId)));
        }
        if (resultItem?.computedState === 2 /* TestResultState.Running */) {
            testActions.push(new Action('testing.gutter.cancel', localize(16526, null), undefined, undefined, () => this.commandService.executeCommand("testing.cancelRun" /* TestCommandId.CancelTestRunAction */)));
        }
        testActions.push(new Action('testing.gutter.reveal', localize(16527, null), undefined, undefined, () => this.commandService.executeCommand('_revealTestInExplorer', test.item.extId)));
...
let MultiRunTestDecoration = class MultiRunTestDecoration extends RunTestDecoration {
    ...
    getContextMenuActions() {
        ...
        const allActions = [];
        [
            { bitset: 2 /* TestRunProfileBitset.Run */, label: localize(16528, null) },
            { bitset: 8 /* TestRunProfileBitset.Coverage */, label: localize(16529, null) },
            { bitset: 4 /* TestRunProfileBitset.Debug */, label: localize(16530, null) },
        ...
        if (overflow > 0) {
            ...
            testSubmenus.push(new Action('testing.gutter.overflow', localize(16531, null, overflow), undefined, undefined, () => this.pickAndRun(testItems)));
        }
    ...
    async pickAndRun(testItems) {
        ...
        const item = await doPick(testItems.map(({ currentLabel, testItem }) => ({ label: currentLabel, test: testItem.test, result: testItem.resultItem })), localize(16532, null));
        if (!item) {
            return;
        }
