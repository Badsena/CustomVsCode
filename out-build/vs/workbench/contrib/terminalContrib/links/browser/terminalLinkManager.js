...
let TerminalLinkManager = class TerminalLinkManager extends DisposableStore {
            ...
            activate: async (event, text) => {
                ...
                if (terminalConfigurationService.config.allowedLinkSchemes.indexOf(scheme) === -1) {
                    const userAllowed = await new Promise((resolve) => {
                        notificationService.prompt(Severity.Warning, nls.localize(16151, null, scheme), [
                            {
                                label: nls.localize(16152, null, scheme),
                                run: () => {
    ...
    _getLinkHoverString(uri, label) {
        ...
        if (editorConf.multiCursorModifier === 'ctrlCmd') {
            if (isMacintosh) {
                clickLabel = nls.localize(16153, null);
            }
            else {
                clickLabel = nls.localize(16154, null);
            }
        }
        else {
            if (isMacintosh) {
                clickLabel = nls.localize(16155, null);
            }
            else {
                clickLabel = nls.localize(16156, null);
            }
        }
        let fallbackLabel = nls.localize(16157, null);
        try {
            if (this._tunnelService.canTunnel(URI.parse(uri))) {
                fallbackLabel = nls.localize(16158, null);
            }
        }
        catch {
