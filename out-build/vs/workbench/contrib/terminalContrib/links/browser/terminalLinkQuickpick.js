...
let TerminalLinkQuickpick = class TerminalLinkQuickpick extends DisposableStore {
    ...
    async show(instance, links) {
        ...
        const picks = [];
        if (webPicks) {
            picks.push({ type: 'separator', label: localize(16160, null) });
            picks.push(...webPicks);
        }
        if (filePicks) {
            picks.push({ type: 'separator', label: localize(16161, null) });
            picks.push(...filePicks);
        }
        if (folderPicks) {
            picks.push({ type: 'separator', label: localize(16162, null) });
            picks.push(...folderPicks);
        }
        if (wordPicks) {
            picks.push({ type: 'separator', label: localize(16163, null) });
        ...
        pick.items = picks;
        pick.placeholder = localize(16164, null);
        ...
        if (!usingAllLinks) {
            disposables.add(Event.once(pick.onDidChangeValue)(async () => {
                ...
                const picks = [];
                if (webPicks) {
                    picks.push({ type: 'separator', label: localize(16165, null) });
                    picks.push(...webPicks);
                }
                if (filePicks) {
                    picks.push({ type: 'separator', label: localize(16166, null) });
                    picks.push(...filePicks);
                }
                if (folderPicks) {
                    picks.push({ type: 'separator', label: localize(16167, null) });
                    picks.push(...folderPicks);
                }
                if (wordPicks) {
                    picks.push({ type: 'separator', label: localize(16168, null) });
                    picks.push(...wordPicks);
                }
                pick.items = picks;
            }));
