...
let OutputMonitor = class OutputMonitor extends Disposable {
    ...
    async _assessOutputForErrors(buffer, token) {
        ...
        const response = await this._languageModelsService.sendChatRequest(model, undefined, [{ role: 1 /* ChatMessageRole.User */, content: [{ type: 'text', value: `Evaluate this terminal output to determine if there were errors. If there are errors, return them. Otherwise, return undefined: ${buffer}.` }] }], {}, token);
        try {
    ...
    async _requestFreeFormTerminalInput(token, execution, confirmationPrompt, acceptAnyKey = false) {
        ...
        const { promise: userPrompt, part } = this._createElicitationPart(token, execution.sessionResource, new MarkdownString(localize(15917, null)), new MarkdownString(localize(15918, null, confirmationPrompt.prompt)), '', localize(15919, null), undefined, () => {
    ...
    async _confirmRunInTerminal(token, suggestedOption, execution, confirmationPrompt) {
        ...
        let inputDataDisposable = Disposable.None;
        let instanceDisposedDisposable = Disposable.None;
        const { promise: userPrompt, part } = this._createElicitationPart(token, execution.sessionResource, new MarkdownString(localize(15920, null)), new MarkdownString(localize(15921, null, confirmationPrompt.prompt, suggestedOptionValue, isString(suggestedOption) ? '' : suggestedOption.description ? ' (' + suggestedOption.description + ')' : '')), '', localize(15922, null), localize(15923, null), async (value) => {
