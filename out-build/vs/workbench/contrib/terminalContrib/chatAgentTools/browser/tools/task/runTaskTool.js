...
let RunTaskTool = class RunTaskTool {
    ...
    async invoke(invocation, _countTokens, _progress, token) {
        ...
        if (!task) {
            return { content: [{ kind: 'text', value: `Task not found: ${args.id}` }], toolResultMessage: new MarkdownString(localize(15976, null, args.id)) };
        }
        const taskLabel = task._label;
        const activeTasks = await this._tasksService.getActiveTasks();
        if (activeTasks.includes(task)) {
            return { content: [{ kind: 'text', value: `The task ${taskLabel} is already running.` }], toolResultMessage: new MarkdownString(localize(15977, null, taskLabel)) };
        }
        ...
        try {
            ...
            if (!resources || resources.length === 0) {
                return { content: [{ kind: 'text', value: `Task started but no terminal was found for: ${taskLabel}` }], toolResultMessage: new MarkdownString(localize(15978, null, taskLabel)) };
            }
            ...
            if (terminals.length === 0) {
                return { content: [{ kind: 'text', value: `Task started but no terminal was found for: ${taskLabel}` }], toolResultMessage: new MarkdownString(localize(15979, null, taskLabel)) };
            }
    ...
    async prepareToolInvocation(context, token) {
        ...
        const task = await getTaskForTool(args.id, taskDefinition, args.workspaceFolder, this._configurationService, this._tasksService, true);
        if (!task) {
            return { invocationMessage: new MarkdownString(localize(15980, null, args.id)) };
        }
        const taskLabel = task._label;
        const activeTasks = await this._tasksService.getActiveTasks();
        if (task && activeTasks.includes(task)) {
            return { invocationMessage: new MarkdownString(localize(15981, null)) };
        }
        if (await this._isTaskActive(task)) {
            return {
                invocationMessage: new MarkdownString(localize(15982, null, taskLabel)),
                pastTenseMessage: new MarkdownString(localize(15983, null, taskLabel)),
                confirmationMessages: undefined
            };
        }
        return {
            invocationMessage: new MarkdownString(localize(15984, null, taskLabel)),
            pastTenseMessage: new MarkdownString(task?.configurationProperties.isBackground
                ? localize(15985, null, taskLabel)
                : localize(15986, null, taskLabel)),
            confirmationMessages: task
                ? { title: localize(15987, null), message: localize(15988, null, taskLabel) }
                : undefined
        };
    }
};
...
export const RunTaskToolData = {
    ...
    displayName: localize(15989, null),
    ...
    userDescription: localize(15990, null),
    icon: Codicon.tools,
