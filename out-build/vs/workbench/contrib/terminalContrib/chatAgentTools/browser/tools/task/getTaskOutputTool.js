...
export const GetTaskOutputToolData = {
    ...
    displayName: localize(15969, null),
...
let GetTaskOutputTool = class GetTaskOutputTool extends Disposable {
    ...
    async prepareToolInvocation(context, token) {
        ...
        if (!task) {
            return { invocationMessage: new MarkdownString(localize(15970, null, args.id)) };
        }
        const taskLabel = task._label;
        const activeTasks = await this._tasksService.getActiveTasks();
        if (activeTasks.includes(task)) {
            return { invocationMessage: new MarkdownString(localize(15971, null, taskLabel)) };
        }
        return {
            invocationMessage: new MarkdownString(localize(15972, null, taskLabel)),
            pastTenseMessage: new MarkdownString(localize(15973, null, taskLabel)),
        };
    }
    async invoke(invocation, _countTokens, _progress, token) {
        ...
        const task = await getTaskForTool(args.id, taskDefinition, args.workspaceFolder, this._configurationService, this._tasksService, true);
        if (!task) {
            return { content: [{ kind: 'text', value: `Task not found: ${args.id}` }], toolResultMessage: new MarkdownString(localize(15974, null, args.id)) };
        }
        ...
        if (!terminals || terminals.length === 0) {
            return { content: [{ kind: 'text', value: `Terminal not found for task ${taskLabel}` }], toolResultMessage: new MarkdownString(localize(15975, null, taskLabel)) };
        }
