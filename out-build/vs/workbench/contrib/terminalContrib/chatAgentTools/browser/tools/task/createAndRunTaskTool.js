...
let CreateAndRunTaskTool = class CreateAndRunTaskTool {
    ...
    async invoke(invocation, _countTokens, _progress, token) {
        ...
        _progress.report({ message: new MarkdownString(localize(15955, null)) });
        let task;
        ...
        if (!task) {
            return { content: [{ kind: 'text', value: `Task not found: ${args.task.label}` }], toolResultMessage: new MarkdownString(localize(15956, null, args.task.label)) };
        }
        ...
        try {
            ...
            _progress.report({ message: new MarkdownString(localize(15957, null, args.task.label)) });
            ...
            if (!terminals || terminals.length === 0) {
                return { content: [{ kind: 'text', value: `Task started but no terminal was found for: ${args.task.label}` }], toolResultMessage: new MarkdownString(localize(15958, null, args.task.label)) };
            }
    ...
    async prepareToolInvocation(context, token) {
        ...
        if (allTasks?.find(t => t._label === task.label)) {
            return {
                invocationMessage: new MarkdownString(localize(15959, null, task.label)),
                pastTenseMessage: new MarkdownString(localize(15960, null, task.label)),
        ...
        if (activeTasks.find(t => t._label === task.label)) {
            return {
                invocationMessage: new MarkdownString(localize(15961, null, task.label)),
                pastTenseMessage: new MarkdownString(localize(15962, null, task.label)),
                confirmationMessages: undefined
            };
        }
        return {
            invocationMessage: new MarkdownString(localize(15963, null, task.label)),
            pastTenseMessage: new MarkdownString(localize(15964, null, task.label)),
            confirmationMessages: {
                title: localize(15965, null),
                message: new MarkdownString(localize(15966, null, task.label, task.command, task.args?.length ? ` and args \`${task.args.join(' ')}\`` : ''))
            }
        };
    }
};
