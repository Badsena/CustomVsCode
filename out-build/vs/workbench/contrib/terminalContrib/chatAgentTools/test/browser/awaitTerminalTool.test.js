...
suite('AwaitTerminalTool', () => {
    ...
    test('returns error when terminal ID does not exist', async () => {
        ...
        const result = await tool.invoke(createInvocation('invalid-id', 0), async () => 0, { report: () => { } }, cts.token);
    ...
    test('returns output and exit code when terminal completes', async () => {
        ...
        const resultPromise = tool.invoke(createInvocation('test-terminal', 0), async () => 0, { report: () => { } }, cts.token);
    ...
    test('returns timeout status when terminal times out', async () => {
        return runWithFakedTimers({}, async () => {
            ...
            const result = await tool.invoke(createInvocation('test-terminal', 100), async () => 0, { report: () => { } }, cts.token);
            assert.strictEqual(result.content.length, 1);
            assert.strictEqual(result.content[0].kind, 'text');
            const value = result.content[0].value;
            ...
            assert.strictEqual(result.toolMetadata?.exitCode, undefined);
    ...
    test('negative timeout is treated as no timeout', async () => {
        ...
        const resultPromise = tool.invoke(createInvocation('test-terminal', -100), async () => 0, { report: () => { } }, cts.token);
    ...
    test('throws CancellationError when token is cancelled with timeout', async () => {
        return runWithFakedTimers({}, async () => {
            const deferred = new DeferredPromise();
            RunInTerminalTool.getExecution = () => createMockExecution(deferred.p, 'output');
            const resultPromise = tool.invoke(createInvocation('test-terminal', 5000), async () => 0, { report: () => { } }, cts.token);
            cts.cancel();
            await assert.rejects(resultPromise, CancellationError);
        });
    });
