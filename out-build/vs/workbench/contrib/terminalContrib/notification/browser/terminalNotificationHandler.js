...
export class TerminalNotificationHandler extends Disposable {
    ...
    _showOsc99Notification(state) {
        ...
        secondaryActions.push(actionStore.add(new Action('terminal.osc99.dismiss', localize(16170, null), undefined, true, () => {
            if (activeRef.current) {
                activeRef.current.closeReason = 'secondary';
            }
            handleRef.current?.close();
        })));
        secondaryActions.push(actionStore.add(new Action('terminal.osc99.disable', localize(16171, null), undefined, true, async () => {
