...
export async function shouldPasteTerminalText(accessor, text, bracketedPasteMode) {
    ...
    let detail = localize(16062, null);
    ...
    const { result, checkboxChecked } = await dialogService.prompt({
        message: localize(16063, null, textForLines.length),
        detail,
        type: 'warning',
        buttons: [
            {
                label: localize(16064, null),
                run: () => ({ confirmed: true, singleLine: false })
            },
            {
                label: localize(16065, null),
                run: () => ({ confirmed: true, singleLine: true })
            }
        ],
        cancelButton: true,
        checkbox: {
            label: localize(16066, null)
        }
    });
    if (!result) {
        return false;
