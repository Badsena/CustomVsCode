...
let TerminalInstance = class TerminalInstance extends Disposable {
    ...
    static getXtermConstructor(keybindingService, contextKeyService) {
        ...
        xtermConstructor = Promises.withAsyncBody(async (resolve) => {
            ...
            // Localize strings
            Terminal.strings.promptLabel = nls.localize(15414, null);
            Terminal.strings.tooMuchOutput = keybinding ? nls.localize(15415, null, keybinding.getLabel()) : nls.localize(15416, null);
            resolve(Terminal);
        });
        return xtermConstructor;
    }
            ...
            this._register(xterm.raw.onBell(() => {
                if (this._configurationService.getValue("terminal.integrated.enableBell" /* TerminalSettingId.EnableBell */) || this._configurationService.getValue("terminal.integrated.enableVisualBell" /* TerminalSettingId.EnableVisualBell */)) {
                    this.statusList.add({
                        ...
                        icon: Codicon.bell,
                        tooltip: nls.localize(15417, null)
    ...
    _refreshShellIntegrationInfoStatus(instance) {
        ...
        instance.statusList.add({
            ...
            tooltip: `${nls.localize(15422, null)}: ${cmdDetectionType}`,
            detailedTooltip: `${nls.localize(15423, null)}: ${cmdDetectionType}${detailedAdditionsString}`
        });
    }
    ...
    _open() {
        ...
        xterm.raw.attachCustomKeyEventHandler((event) => {
            ...
            // shell process
            if (!this._terminalConfigurationService.config.sendKeybindingsToShell && resolveResult.kind === 2 /* ResultKind.KbFound */ && resolveResult.commandId && this._skipTerminalCommands.some(k => k === resolveResult.commandId)) {
                event.preventDefault();
                return false;
            }
    ...
    _createProcessManager() {
        ...
        this._register(processManager.onPtyDisconnect(() => {
            ...
            this.statusList.add({
                ...
                tooltip: nls.localize(15424, null)
            });
        }));
    ...
    async _createProcess() {
        ...
        if (!trusted && !this.remoteAuthority && !this._workbenchEnvironmentService.remoteAuthority) {
            this._onProcessExit({ message: nls.localize(15425, null) });
        }
            ...
            this._onProcessExit({
                message: nls.localize(15426, null, this._cwd, this._userHome)
            });
        }
    ...
    async _trust() {
        ...
        const trustRequest = await this._workspaceTrustRequestService.requestWorkspaceTrust({
            message: nls.localize(15430, null)
        });
    ...
    _setAriaLabel(xterm, terminalId, title) {
        ...
        if (xterm && xterm.textarea) {
            if (title && title.length > 0) {
                labelParts.push(nls.localize(15431, null, terminalId, title));
            }
            else {
                labelParts.push(nls.localize(15432, null, terminalId));
            }
            const screenReaderOptimized = this._accessibilityService.isScreenReaderOptimized();
            if (!screenReaderOptimized) {
                labelParts.push(nls.localize(15433, null));
            }
            ...
            if (this._configurationService.getValue("accessibility.verbosity.terminal" /* AccessibilityVerbositySettingId.Terminal */) && accessibilityHelpKeybinding) {
                labelParts.push(nls.localize(15434, null, accessibilityHelpKeybinding));
            }
    ...
    _onEnvironmentVariableInfoChanged(info) {
        if (info.requiresAction) {
            this.xterm?.raw.textarea?.setAttribute('aria-label', nls.localize(15437, null, this._instanceId));
        }
    ...
    async changeColor(color, skipQuickPick) {
        ...
        quickPick.placeholder = nls.localize(15438, null);
        quickPick.show();
    ...
    switch (typeof exitCodeOrError) {
        case 'number': {
            ...
            if (processState === 4 /* ProcessState.KilledDuringLaunch */) {
                if (commandLine) {
                    message = nls.localize(15439, null, commandLine, code);
                }
                else {
                    message = nls.localize(15440, null, code);
                }
            }
            else {
                if (commandLine) {
                    message = nls.localize(15441, null, commandLine, code);
                }
                else {
                    message = nls.localize(15442, null, code);
                }
            }
            break;
        }
        case 'object': {
            ...
            message = nls.localize(15443, null, innerMessage);
            break;
        }
    }
    return { code, message };
