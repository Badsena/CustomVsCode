...
export function getTerminalDecorationHoverContent(command, hoverMessage, showCommandActions) {
    let hoverContent = showCommandActions ? `${localize(15541, null)}\n\n---\n\n` : '';
    if (!command) {
        if (hoverMessage) {
        ...
        if (isNumber(command.duration)) {
            ...
            if (command.exitCode) {
                if (command.exitCode === -1) {
                    hoverContent += localize(15542, null, fromNow(command.timestamp, true), durationText);
                }
                else {
                    hoverContent += localize(15543, null, fromNow(command.timestamp, true), durationText, command.exitCode);
                }
            }
            else {
                hoverContent += localize(15544, null, fromNow(command.timestamp, true), durationText);
            }
        }
        else {
            if (command.exitCode) {
                if (command.exitCode === -1) {
                    hoverContent += localize(15545, null, fromNow(command.timestamp, true));
                }
                else {
                    hoverContent += localize(15546, null, fromNow(command.timestamp, true), command.exitCode);
                }
            }
            else {
                hoverContent += localize(15547, null);
            }
        }
    }
    return hoverContent;
...
const unknownText = localize(15548, null);
const runningText = localize(15549, null);
export function getTerminalCommandDecorationTooltip(command, storedState) {
    ...
    if (typeof duration === 'number') {
        ...
        if (exitCode) {
            if (exitCode === -1) {
                hoverContent += localize(15550, null, fromNowText, durationText);
            }
            else {
                hoverContent += localize(15551, null, fromNowText, durationText, exitCode);
            }
        }
        else {
            hoverContent += localize(15552, null, fromNowText, durationText);
        }
    }
    else {
        if (exitCode) {
            if (exitCode === -1) {
                hoverContent += localize(15553, null, fromNowText);
            }
            else {
                hoverContent += localize(15554, null, fromNowText, exitCode);
            }
        }
        else {
            hoverContent += localize(15555, null, fromNowText);
        }
    }
    return hoverContent;
}
