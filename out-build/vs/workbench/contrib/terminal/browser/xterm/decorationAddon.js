...
let DecorationAddon = class DecorationAddon extends Disposable {
    ...
    _getContextMenuActions() {
        const label = localize(15525, null);
        return [
            {
    ...
    async _getCommandActions(command) {
        ...
        if (command.command !== '') {
            const labelRun = localize(15526, null);
            actions.push({
                ...
                run: async () => {
                    ...
                    if (!command.isTrusted) {
                        const shouldRun = await new Promise(r => {
                            this._notificationService.prompt(Severity.Info, localize(15527, null, command.command), [{
                                    label: localize(15528, null),
                                    run: () => r(true)
                                }, {
                                    label: localize(15529, null),
                                    run: () => r(false)
                                }]);
                        });
                        if (!shouldRun) {
            ...
            const labelCopy = localize(15530, null);
            actions.push({
        ...
        if (command.hasOutput()) {
            const labelCopyCommandAndOutput = localize(15531, null);
            actions.push({
            ...
            const labelText = localize(15532, null);
            actions.push({
            ...
            const labelHtml = localize(15533, null);
            actions.push({
        ...
        const labelRunRecent = localize(15534, null);
        actions.push({
        ...
        const labelGoToRecent = localize(15535, null);
        actions.push({
        ...
        actions.push(new Separator());
        const labelAbout = localize(15536, null);
        actions.push({
    ...
    _createAttachToChatAction(command) {
        ...
        const labelAttachToChat = localize(15537, null);
        return {
    ...
    _showToggleVisibilityQuickPick() {
        ...
        quickPick.title = localize(15538, null);
