...
let TerminalProfileQuickpick = class TerminalProfileQuickpick {
    ...
    async _createAndShow(type) {
        ...
        let keyMods;
        const options = {
            placeHolder: type === 'createInstance' ? nls.localize(15486, null) : nls.localize(15487, null),
        ...
        if (configProfiles.length > 0) {
            quickPickItems.push({ type: 'separator', label: nls.localize(15490, null) });
        ...
        quickPickItems.push({ type: 'separator', label: nls.localize(15491, null) });
        ...
        if (autoDetectedProfiles.length > 0) {
            quickPickItems.push({ type: 'separator', label: nls.localize(15492, null) });
    ...
    async _isProfileSafe(profile) {
        ...
        return await new Promise(r => {
            ...
            const handle = this._notificationService.prompt(Severity.Warning, nls.localize(15493, null, `"${unsafePaths.join(',')}"`), [{
                    label: nls.localize(15494, null),
                    run: () => r(true)
                }, {
                    label: nls.localize(15495, null),
                    run: () => r(false)
                }]);
    ...
    _createProfileQuickPickItem(profile) {
        const buttons = [{
                ...
                tooltip: nls.localize(15496, null)
            }];
