...
let TerminalService = class TerminalService extends Disposable {
    ...
    async _showTerminalCloseConfirmation(singleTerminal) {
        ...
        if (foregroundInstances.length === 1 || singleTerminal) {
            message = nls.localize(15497, null);
        }
        else {
            message = nls.localize(15498, null, foregroundInstances.length);
        }
        const { confirmed } = await this._dialogService.confirm({
            type: 'warning',
            message,
            primaryButton: nls.localize(15499, null)
        });
        return !confirmed;
    }
    ...
    _evaluateLocalCwd(shellLaunchConfig) {
        ...
        if (!isString(shellLaunchConfig.cwd) && shellLaunchConfig.cwd?.scheme === Schemas.file) {
            if (VirtualWorkspaceContext.getValue(this._contextKeyService)) {
                shellLaunchConfig.initialText = formatMessageForTerminal(nls.localize(15500, null, '\x1b[3m', '\x1b[23m'), { excludeLeadingNewLine: true, loudFormatting: true });
                shellLaunchConfig.type = 'Local';
            }
            else if (this._remoteAgentService.getConnection()) {
                shellLaunchConfig.initialText = formatMessageForTerminal(nls.localize(15501, null, '\x1b[3m', '\x1b[23m'), { excludeLeadingNewLine: true, loudFormatting: true });
                shellLaunchConfig.type = 'Local';
            }
        }
    }
