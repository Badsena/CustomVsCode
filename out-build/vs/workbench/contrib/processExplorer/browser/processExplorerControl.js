...
class ProcessHeaderTreeRenderer {
    ...
    renderElement(node, index, templateData) {
        templateData.name.textContent = localize(13837, null);
        templateData.cpu.textContent = localize(13838, null);
        templateData.pid.textContent = localize(13839, null);
        templateData.memory.textContent = localize(13840, null);
    }
...
let ProcessExplorerControl = class ProcessExplorerControl extends Disposable {
    ...
    onTreeContextMenu(container, e) {
        ...
        if (typeof this.killProcess === 'function') {
            actions.push(toAction({ id: 'killProcess', label: localize(13842, null), run: () => this.killProcess?.(pid, 'SIGTERM') }));
            actions.push(toAction({ id: 'forceKillProcess', label: localize(13843, null), run: () => this.killProcess?.(pid, 'SIGKILL') }));
            actions.push(new Separator());
        }
        actions.push(toAction({
            id: 'copy',
            label: localize(13844, null),
            run: () => {
                const selectionPids = this.getSelectedPids();
        ...
        if (this.isDebuggable(item.cmd)) {
            actions.push(new Separator());
            actions.push(toAction({ id: 'debug', label: localize(13846, null), run: () => this.attachTo(item) }));
        }
