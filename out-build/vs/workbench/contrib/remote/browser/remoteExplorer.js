...
import { ForwardPortAction, TunnelPanel, TunnelPanelDescriptor, TunnelViewModel } from './tunnelView.js';
...
import { isWeb, isWindows, isMacintosh } from '../../../../base/common/platform.js';
...
let ForwardedPortsView = class ForwardedPortsView extends Disposable {
    constructor(contextKeyService, environmentService, remoteExplorerService, tunnelService, activityService, statusbarService) {
        ...
        this._register(Registry.as(Extensions.ViewsRegistry).registerViewWelcomeContent(TUNNEL_VIEW_ID, {
            content: this.environmentService.remoteAuthority ? nls.localize(13914, null, `command:${ForwardPortAction.INLINE_ID}`)
                : nls.localize(13915, null, `command:${ForwardPortAction.INLINE_ID}`),
        }));
    ...
    async getViewContainer() {
        return Registry.as(Extensions.ViewContainersRegistry).registerViewContainer({
            ...
            title: nls.localize2(13931, "Ports"),
            icon: portsViewIcon,
    ...
    async updateActivityBadge() {
        if (this.remoteExplorerService.tunnelModel.forwarded.size > 0) {
            this.activityBadge.value = this.activityService.showViewActivity(TUNNEL_VIEW_ID, {
                badge: new NumberBadge(this.remoteExplorerService.tunnelModel.forwarded.size, n => n === 1 ? nls.localize(13916, null) : nls.localize(13917, null, n))
            });
        }
        else {
    ...
    get entry() {
        ...
        const text = `${count}`;
        if (count === 0) {
            tooltip = nls.localize(13918, null);
        }
        else {
            ...
            tooltip = nls.localize(13919, null, allTunnels.map(forwarded => forwarded.remotePort).join(', '));
        }
        return {
            name: nls.localize(13920, null),
...
let AutomaticPortForwarding = class AutomaticPortForwarding extends Disposable {
    constructor(terminalService, notificationService, openerService, externalOpenerService, remoteExplorerService, environmentService, contextKeyService, configurationService, debugService, remoteAgentService, tunnelService, hostService, logService, storageService, preferencesService) {
        ...
        if (!environmentService.remoteAuthority) {
            ...
            this.configurationService.whenRemoteConfigurationLoaded().then(() => remoteAgentService.getEnvironment()).then(environment => {
                setup(environment);
            });
        }
            ...
            this.portListener = this._register(this.remoteExplorerService.tunnelModel.onForwardPort(async () => {
                ...
                if (Array.from(this.remoteExplorerService.tunnelModel.forwarded.values()).filter(tunnel => tunnel.source.source === TunnelSource.Auto).length > fallbackAt) {
                    ...
                    this.notificationService.notify({
                        message: nls.localize(13921, null),
                        severity: Severity.Warning,
                        actions: {
                            primary: [
                                toAction({
                                    id: 'switchBack',
                                    label: nls.localize(13922, null),
                                    run: async () => {
    ...
    setup(environment) {
        ...
        const os = environment?.os ?? (isWeb ? 3 /* OperatingSystem.Linux */ : (isWindows ? 1 /* OperatingSystem.Windows */ : (isMacintosh ? 2 /* OperatingSystem.Macintosh */ : 3 /* OperatingSystem.Linux */)));
        if (os !== 3 /* OperatingSystem.Linux */) {
            ...
            this.outputForwarder = this._register(new OutputAutomaticPortForwarding(this.terminalService, this.notificationService, this.openerService, this.externalOpenerService, this.remoteExplorerService, this.configurationService, this.debugService, this.tunnelService, this.hostService, this.logService, this.contextKeyService, this.environmentService, () => false));
        }
        else {
            const useProc = () => (this.configurationService.getValue(PORT_AUTO_SOURCE_SETTING) === PORT_AUTO_SOURCE_SETTING_PROCESS);
            if (useProc()) {
                this.procForwarder = this._register(new ProcAutomaticPortForwarding(false, alreadyForwarded, !isSwitch, this.configurationService, this.remoteExplorerService, this.notificationService, this.openerService, this.externalOpenerService, this.tunnelService, this.hostService, this.logService, this.contextKeyService, this.environmentService));
            }
            else if (this.configurationService.getValue(PORT_AUTO_SOURCE_SETTING) === PORT_AUTO_SOURCE_SETTING_HYBRID) {
                this.procForwarder = this._register(new ProcAutomaticPortForwarding(true, alreadyForwarded, !isSwitch, this.configurationService, this.remoteExplorerService, this.notificationService, this.openerService, this.externalOpenerService, this.tunnelService, this.hostService, this.logService, this.contextKeyService, this.environmentService));
            }
            this.outputForwarder = this._register(new OutputAutomaticPortForwarding(this.terminalService, this.notificationService, this.openerService, this.externalOpenerService, this.remoteExplorerService, this.configurationService, this.debugService, this.tunnelService, this.hostService, this.logService, this.contextKeyService, this.environmentService, useProc));
        }
        this.listenForPorts();
    }
};
...
class OnAutoForwardedAction extends Disposable {
    ...
    constructor(notificationService, remoteExplorerService, openerService, externalOpenerService, tunnelService, hostService, logService, environmentService) {
        super();
        this.notificationService = notificationService;
        this.remoteExplorerService = remoteExplorerService;
        this.tunnelService = tunnelService;
        ...
        if (tunnel) {
            ...
            switch (attributes) {
                ...
                case OnPortForward.OpenBrowser: {
                    // ✅ Disabled - user must manually open browser
                    break;
                }
    ...
    async basicMessage(tunnel) {
        ...
        return nls.localize(13924, null, label ? ` (${label})` : '', tunnel.tunnelRemotePort);
    }
    linkMessage() {
        return nls.localize(13925, null, `command:${TunnelPanel.ID}.focus`);
    }
    async showNotification(tunnel) {
        console.log('[Amypo] ==============================');
        console.log('[Amypo] Port detected!');
        console.log('[Amypo] Remote Port:', tunnel.tunnelRemotePort);
        console.log('[Amypo] Local Port:', tunnel.tunnelLocalPort);
        console.log('[Amypo] Local Address:', tunnel.localAddress);
        console.log('[Amypo] ==============================');
        const hasFocus = await this.hostService.hadLastFocus();
        console.log('[Amypo] Window has focus:', hasFocus);
        // ✅ Always show notification regardless of focus
        // if (!await this.hostService.hadLastFocus()) {
        // 	return;
        // }
        this.lastNotification?.close();
        let message = await this.basicMessage(tunnel);
        const choices = []; // ✅ Remove auto browser choice
        if ((tunnel.tunnelLocalPort !== tunnel.tunnelRemotePort) && this.tunnelService.canElevate && this.tunnelService.isPortPrivileged(tunnel.tunnelRemotePort)) {
            ...
            message += nls.localize(13926, null, tunnel.tunnelRemotePort);
        ...
        console.log('[Amypo] Showing notification...');
    ...
    elevateChoice(tunnel) {
        return {
            ...
            label: nls.localize(13928, null, tunnel.tunnelRemotePort),
            run: async () => {
                await this.remoteExplorerService.close({ host: tunnel.tunnelRemoteHost, port: tunnel.tunnelRemotePort }, TunnelCloseReason.Other);
                ...
                this.lastNotification = this.notificationService.prompt(Severity.Info, await this.basicMessage(newTunnel) + this.linkMessage(), [this.elevateChoice(newTunnel)], { neverShowAgain: { id: 'remote.tunnelsView.autoForwardNeverShow', isSecondary: true } });
                this.notificationDisposable.value = this.lastNotification.onDidClose(() => {
                    this.lastNotification = undefined;
                    this.lastShownPort = undefined;
                });
...
class OutputAutomaticPortForwarding extends Disposable {
    constructor(terminalService, notificationService, openerService, externalOpenerService, remoteExplorerService, configurationService, debugService, tunnelService, hostService, logService, contextKeyService, environmentService, privilegedOnly) {
        super();
        this.terminalService = terminalService;
        this.notificationService = notificationService;
        this.openerService = openerService;
        ...
        this.notifier = new OnAutoForwardedAction(notificationService, remoteExplorerService, openerService, externalOpenerService, tunnelService, hostService, logService, environmentService);
    ...
    startUrlFinder() {
        // ✅ Amypo - Allow local port detection even without remote features
        if (!this.urlFinder && !this.environmentService.remoteAuthority &&
            (this.remoteExplorerService.portsFeaturesEnabled !== PortsEnablement.AdditionalFeatures)) {
            // continue anyway for local machines
        }
        else if (!this.urlFinder && this.environmentService.remoteAuthority &&
            (this.remoteExplorerService.portsFeaturesEnabled !== PortsEnablement.AdditionalFeatures)) {
            return;
        }
        ...
        this._register(this.urlFinder.onDidMatchLocalUrl(async (localUrl) => {
            if (this.notifiedPorts.has(localUrl.port)) {
            ...
            const alreadyDetected = mapHasAddressLocalhostOrAllInterfaces(this.remoteExplorerService.tunnelModel.detected, localUrl.host, localUrl.port);
            console.log('[Amypo] Already detected:', alreadyDetected);
            if (alreadyDetected) {
                return;
            }
            const attributes = (await this.remoteExplorerService.tunnelModel.getAttributes([localUrl]))?.get(localUrl.port);
            console.log('[Amypo] Attributes:', attributes);
            ...
            // ✅ Fixed - show notification directly for local ports
            console.log('[Amypo] Showing notification for port:', localUrl.port);
            this.notifiedPorts.add(localUrl.port);
            this.notificationService.prompt(Severity.Info, nls.localize(13929, null, localUrl.port), [
                {
                    label: nls.localize(13930, null),
                    run: () => {
                        const url = `http://${localUrl.host}:${localUrl.port}`;
                        console.log('[Amypo] Opening browser:', url);
    ...
    stopUrlFinder() {
        if (this.urlFinder) {
            ...
            this.notifiedPorts.clear();
        }
    }
}
class ProcAutomaticPortForwarding extends Disposable {
    constructor(unforwardOnly, alreadyAutoForwarded, needsInitialCandidates, configurationService, remoteExplorerService, notificationService, openerService, externalOpenerService, tunnelService, hostService, logService, contextKeyService, environmentService) {
        super();
