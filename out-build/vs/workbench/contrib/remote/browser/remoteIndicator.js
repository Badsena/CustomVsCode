...
import { MenuId, IMenuService, MenuItemAction, registerAction2, Action2 } from '../../../../platform/actions/common/actions.js';
...
let RemoteStatusIndicator = class RemoteStatusIndicator extends Disposable {
    ...
    registerActions() {
        const category = nls.localize2(13970, "Remote");
        // Show Remote Menu
        const that = this;
        this._register(registerAction2(class extends Action2 {
            constructor() {
                super({
                    ...
                    category,
                    title: nls.localize2(13971, "Show Remote Menu"),
                    f1: true,
                    keybinding: {
            ...
            this._register(registerAction2(class extends Action2 {
                constructor() {
                    super({
                        ...
                        category,
                        title: nls.localize2(13972, "Close Remote Connection"),
                        f1: true,
            ...
            this._register(registerAction2(class extends Action2 {
                constructor() {
                    super({
                        ...
                        category,
                        title: nls.localize2(13973, "Install Remote Development Extensions"),
                        f1: true
                    });
    ...
    updateRemoteStatusIndicator() {
        ...
        if (this.remoteAuthority) {
            ...
            switch (this.connectionState) {
                case 'initializing':
                    this.renderRemoteStatusIndicator(nls.localize(13949, null), nls.localize(13950, null), undefined, true /* progress */);
                    break;
                case 'reconnecting':
                    this.renderRemoteStatusIndicator(`${nls.localize(13951, null, truncate(hostLabel, RemoteStatusIndicator_1.REMOTE_STATUS_LABEL_MAX_LENGTH))}`, undefined, undefined, true /* progress */);
                    break;
                case 'disconnected':
                    this.renderRemoteStatusIndicator(`$(alert) ${nls.localize(13952, null, truncate(hostLabel, RemoteStatusIndicator_1.REMOTE_STATUS_LABEL_MAX_LENGTH))}`);
                    break;
                default: {
                    ...
                    if (hostNameTooltip) {
                        ...
                        tooltip.appendText(nls.localize(13953, null, hostLabel));
                    }
        ...
        if (this.virtualWorkspaceLocation) {
            ...
            if (workspaceLabel) {
                ...
                if (hostNameTooltip) {
                    ...
                    tooltip.appendText(nls.localize(13954, null, workspaceLabel));
                }
                if (!isWeb || this.remoteAuthority) {
                    tooltip.appendMarkdown('\n\n');
                    tooltip.appendMarkdown(nls.localize(13955, null, `command:${LIST_WORKSPACE_UNSUPPORTED_EXTENSIONS_COMMAND_ID}`));
                }
        ...
        this.renderRemoteStatusIndicator(RemoteStatusIndicator_1.DEFAULT_REMOTE_STATUS_LABEL, nls.localize(13956, null));
        return;
    }
    ...
    withNetworkStatus(initialText, initialTooltip, showProgress) {
        ...
        switch (this.networkState) {
            case 'offline': {
                const offlineMessage = nls.localize(13958, null);
                text = textWithAlert();
            ...
            case 'high-latency':
                text = textWithAlert();
                tooltip = this.appendTooltipLine(tooltip, nls.localize(13959, null, remoteConnectionLatencyMeasurer.latency?.current?.toFixed(2), remoteConnectionLatencyMeasurer.latency?.average?.toFixed(2)));
                break;
        }
        ...
        try {
            ...
            if (!this.lifecycleService.willShutdown) {
                const { confirmed } = await this.dialogService.confirm({
                    type: Severity.Error,
                    message: nls.localize(13960, null, remoteLabel),
                    detail: error && !isCancellationError(error) ? toErrorMessage(error) : undefined,
                    primaryButton: nls.localize(13961, null)
                });
                if (confirmed) {
    ...
    showRemoteMenu() {
                ...
                for (const metadata of this.remoteExtensionMetadata) {
                    if (!metadata.installed && metadata.isPlatformCompatible) {
                        ...
                        const buttons = [{
                                ...
                                tooltip: nls.localize(13962, null)
                            }];
                        notInstalledItems.push({ type: 'item', id: metadata.id, label: label, buttons: buttons });
                    }
                }
                items.push({
                    type: 'separator', label: nls.localize(13963, null)
                });
            ...
            if (RemoteStatusIndicator_1.SHOW_CLOSE_REMOTE_COMMAND_ID) {
                if (this.remoteAuthority) {
                    items.push({
                        type: 'item',
                        id: RemoteStatusIndicator_1.CLOSE_REMOTE_COMMAND_ID,
                        label: nls.localize(13964, null)
                    });
                    if (this.connectionState === 'disconnected') {
                        items.push({
                            type: 'item',
                            id: ReloadWindowAction.ID,
                            label: nls.localize(13965, null)
                        });
                    }
                }
                else if (this.virtualWorkspaceLocation) {
        ...
        quickPick.placeholder = nls.localize(13967, null);
        ...
        disposables.add(Event.once(quickPick.onDidAccept)((async (_) => {
            ...
            if (selectedItems.length === 1) {
                ...
                if (remoteExtension) {
                    quickPick.items = [];
                    quickPick.busy = true;
                    quickPick.placeholder = nls.localize(13968, null);
                    try {
...
Registry.as(ConfigurationExtensions.Configuration)
    ...
    properties: {
        'workbench.remoteIndicator.showExtensionRecommendations': {
            type: 'boolean',
            markdownDescription: nls.localize(13969, null),
            default: true
        },
    }
});
