...
class HelpModel extends Disposable {
    ...
    updateItems() {
        ...
        if (getStarted.length) {
            ...
            const getStartedHelpItem = this.items?.find(item => item.icon === icons.getStartedIcon) ?? new GetStartedHelpItem(icons.getStartedIcon, nls.localize(13891, null), helpItemValues, this.quickInputService, this.environmentService, this.openerService, this.remoteExplorerService, this.workspaceContextService, this.commandService);
        ...
        if (documentation.length) {
            ...
            const documentationHelpItem = this.items?.find(item => item.icon === icons.documentationIcon) ?? new HelpItem(icons.documentationIcon, nls.localize(13892, null), helpItemValues, this.quickInputService, this.environmentService, this.openerService, this.remoteExplorerService, this.workspaceContextService);
        ...
        if (issues.length) {
            ...
            const reviewIssuesHelpItem = this.items?.find(item => item.icon === icons.reviewIssuesIcon) ?? new HelpItem(icons.reviewIssuesIcon, nls.localize(13893, null), helpItemValues, this.quickInputService, this.environmentService, this.openerService, this.remoteExplorerService, this.workspaceContextService);
        ...
        if (helpItems.length) {
            ...
            const issueReporterItem = this.items?.find(item => item.icon === icons.reportIssuesIcon) ?? new IssueReporterItem(icons.reportIssuesIcon, nls.localize(13894, null), helpItemValues, this.quickInputService, this.environmentService, this.commandService, this.openerService, this.remoteExplorerService, this.workspaceContextService);
...
class HelpItemBase {
    ...
    async handleClick() {
        ...
        if (this.values.length > 1) {
            ...
            if (actions.length) {
                const action = await this.quickInputService.pick(actions, { placeHolder: nls.localize(13895, null) });
                if (action) {
...
let HelpPanel = class HelpPanel extends ViewPane {
    ...
    static { this.TITLE = nls.localize2(13906, "Help and feedback"); }
    ...
    renderBody(container) {
        ...
        this.tree = this.instantiationService.createInstance((WorkbenchAsyncDataTree), 'RemoteHelp', treeContainer, new HelpTreeVirtualDelegate(), [new HelpTreeRenderer()], new HelpDataSource(), {
            accessibilityProvider: {
                ...
                getWidgetAriaLabel: () => nls.localize(13896, null)
            }
        });
...
let RemoteViewPaneContainer = class RemoteViewPaneContainer extends FilterViewPaneContainer {
    ...
    getTitle() {
        const title = nls.localize(13897, null);
        return title;
    }
};
...
Registry.as(Extensions.ViewContainersRegistry).registerViewContainer({
    id: VIEWLET_ID,
    title: nls.localize2(13907, "Remote Explorer"),
...
class ReconnectionTimer {
    ...
    _render() {
        ...
        if (remainingTime === 1) {
            this._parent.report(nls.localize(13898, null, remainingTime));
        }
        else {
            this._parent.report(nls.localize(13899, null, remainingTime));
        }
    }
}
/**
    ...
    constructor(remoteAgentService, progressService, dialogService, commandService, quickInputService, logService, environmentService, telemetryService) {
        ...
        if (connection) {
            ...
            const reconnectButton = {
                label: nls.localize(13900, null),
                callback: () => {
                    reconnectWaitEvent?.skipWait();
                }
            };
            const reloadButton = {
                label: nls.localize(13901, null),
                callback: () => {
                ...
                switch (e.type) {
                    case 0 /* PersistentConnectionEventType.ConnectionLost */:
                        ...
                        if (visibleProgress || e.millisSinceLastIncomingData > DISCONNECT_PROMPT_TIME) {
                            ...
                            visibleProgress.report(nls.localize(13902, null));
                        }
                        break;
                    ...
                    case 2 /* PersistentConnectionEventType.ReconnectionRunning */:
                        ...
                        if (visibleProgress || e.millisSinceLastIncomingData > DISCONNECT_PROMPT_TIME) {
                            ...
                            visibleProgress.report(nls.localize(13903, null));
                    ...
                    case 3 /* PersistentConnectionEventType.ReconnectionPermanentFailure */:
                        ...
                        if (e.handled) {
                            ...
                            dialogService.confirm({
                                type: Severity.Error,
                                message: nls.localize(13904, null),
                                primaryButton: nls.localize(13905, null)
                            }).then(result => {
                                if (result.confirmed) {
                                    commandService.executeCommand(ReloadWindowAction.ID);
                                }
