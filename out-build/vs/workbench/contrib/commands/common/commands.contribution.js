...
class RunCommands extends Action2 {
    constructor() {
        super({
            id: 'runCommands',
            title: nls.localize2(9333, "Run Commands"),
            f1: false,
            metadata: {
                description: nls.localize(9329, null),
                args: [
                    {
                        name: 'args',
                        schema: {
                            ...
                            properties: {
                                commands: {
                                    type: 'array',
                                    description: nls.localize(9330, null),
                                    items: {
                                        anyOf: [
                                            {
    ...
    async run(accessor, args) {
        ...
        if (!this._isCommandArgs(args)) {
            notificationService.error(nls.localize(9331, null));
            return;
        }
        if (args.commands.length === 0) {
            notificationService.warn(nls.localize(9332, null));
            return;
        }
