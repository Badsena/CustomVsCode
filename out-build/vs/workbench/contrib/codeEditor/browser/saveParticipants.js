...
let FormatOnSaveParticipant = class FormatOnSaveParticipant {
    ...
    async participate(model, context, progress, token) {
        ...
        const nestedProgress = new Progress(provider => {
            progress.report({
                message: localize(9238, null, provider.displayName || provider.extensionId && provider.extensionId.value || '???', createCommandUri('workbench.action.openSettings', 'editor.formatOnSave').toString())
            });
        });
...
let CodeActionOnSaveParticipant = class CodeActionOnSaveParticipant extends Disposable {
    ...
    async participate(model, context, progress, token) {
        ...
        progress.report({ message: localize(9239, null) });
    ...
    async applyOnSaveActions(model, codeActionsOnSave, excludes, progress, token) {
        const getActionProgress = new class {
            ...
            _report() {
                progress.report({
                    message: localize(9240, null, [...this._names].map(name => `'${name}'`).join(', '), createCommandUri('workbench.action.openSettings', 'editor.codeActionsOnSave').toString())
                });
            }
            report(provider) {
        ...
        for (const codeActionKind of codeActionsOnSave) {
            ...
            try {
                for (const action of actionsToRun.validActions) {
                    progress.report({ message: localize(9241, null, action.action.title) });
