...
const $ = DOM.$;
const searchBoxLabel = localize(13590, null);
const SEARCH_TOC_BEHAVIOR_KEY = 'workbench.settings.settingsSearchTocBehavior';
const SHOW_AI_RESULTS_ENABLED_LABEL = localize(13591, null);
const SHOW_AI_RESULTS_DISABLED_LABEL = localize(13592, null);
...
let SettingsEditor2 = class SettingsEditor2 extends EditorPane {
    ...
    createHeader(parent) {
        ...
        const clearInputAction = this._register(new Action(SETTINGS_EDITOR_COMMAND_CLEAR_SEARCH_RESULTS, localize(13593, null), ThemeIcon.asClassName(preferencesClearInputIcon), false, async () => this.clearSearchResults()));
        ...
        const filterAction = this._register(new Action(SETTINGS_EDITOR_COMMAND_SUGGEST_FILTERS, localize(13594, null), ThemeIcon.asClassName(preferencesFilterIcon)));
    ...
    toggleAiSearch() {
        if (this.searchInputActionBar && this.showAiResultsAction && this.searchInputActionBar.hasAction(this.showAiResultsAction)) {
            if (!this.showAiResultsAction.enabled) {
                aria.status(localize(13595, null));
            }
    ...
    createBody(parent) {
        ...
        this.noResultsMessage.innerText = localize(13596, null);
        this.clearFilterLinkContainer = $('span.clear-search-filters');
        this.clearFilterLinkContainer.textContent = ' - ';
        const clearFilterLink = DOM.append(this.clearFilterLinkContainer, $('a.pointer.prominent', { tabindex: 0 }, localize(13597, null)));
    ...
    createTOC(container) {
        ...
        this.tocTree = this._register(this.instantiationService.createInstance(TOCTree, DOM.append(container, $('.settings-toc-wrapper', {
            'role': 'navigation',
            'aria-label': localize(13598, null),
        })), this.viewState));
        this.tocTreeDisposed = false;
    ...
    renderResultCountMessages(showAiResultsMessage) {
        ...
        if (!this.searchResultModel) {
            ...
            if (showAiResultsMessage) {
                switch (count) {
                    case 0:
                        resultString = localize(13600, null);
                        break;
                    case 1:
                        resultString = localize(13601, null);
                        break;
                    default: resultString = localize(13602, null, count);
                }
            }
            else {
                switch (count) {
                    case 0:
                        resultString = localize(13603, null);
                        break;
                    case 1:
                        resultString = localize(13604, null);
                        break;
                    default: resultString = localize(13605, null, count);
                }
            }
...
let SyncControls = class SyncControls extends Disposable {
    constructor(window, container, commandService, userDataSyncService, userDataSyncEnablementService, telemetryService) {
        ...
        this.turnOnSyncButton.label = localize(13606, null);
