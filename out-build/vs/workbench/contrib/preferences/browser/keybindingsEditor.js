...
let KeybindingsEditor = class KeybindingsEditor extends EditorPane {
    ...
    constructor(group, telemetryService, themeService, keybindingsService, contextMenuService, keybindingEditingService, contextKeyService, notificationService, clipboardService, instantiationService, editorService, storageService, configurationService, accessibilityService) {
        ...
        this.recordKeysAction = this._register(new Action(KEYBINDINGS_EDITOR_COMMAND_RECORD_SEARCH_KEYS, localize(13442, null), ThemeIcon.asClassName(keybindingsRecordKeysIcon)));
        this.recordKeysAction.checked = false;
        this.sortByPrecedenceAction = this._register(new Action(KEYBINDINGS_EDITOR_COMMAND_SORTBY_PRECEDENCE, localize(13443, null), ThemeIcon.asClassName(keybindingsSortIcon)));
    ...
    createHeader(parent) {
        ...
        const fullTextSearchPlaceholder = localize(13444, null);
        const keybindingsSearchPlaceholder = localize(13445, null);
        const clearInputAction = this._register(new Action(KEYBINDINGS_EDITOR_COMMAND_CLEAR_SEARCH_RESULTS, localize(13446, null), ThemeIcon.asClassName(preferencesClearInputIcon), false, async () => this.clearSearchResults()));
    ...
    createRecordingBadge(container) {
        ...
        recordingBadge.textContent = localize(13447, null);
    ...
    renderKeybindingsEntries(reset, preserveFocus) {
        if (this.keybindingsEditorModel) {
            ...
            this.accessibilityService.alert(localize(13452, null, keybindingsEntries.length));
    ...
    getAriaLabel(keybindingsEntries) {
        if (this.sortByPrecedenceAction.checked) {
            return localize(13453, null, keybindingsEntries.length);
        }
        else {
            return localize(13454, null, keybindingsEntries.length);
        }
    }
    ...
    createDefineKeybindingAction(keybindingItemEntry) {
        return {
            label: keybindingItemEntry.keybindingItem.keybinding ? localize(13455, null) : localize(13456, null),
            enabled: true,
    ...
    onKeybindingEditingError(error) {
        this.notificationService.error(typeof error === 'string' ? error : localize(13465, null, `${error}`));
    }
};
...
let ActionsColumnRenderer = class ActionsColumnRenderer {
    ...
    createEditAction(keybindingItemEntry) {
        return {
            ...
            enabled: true,
            id: 'editKeybinding',
            tooltip: this.keybindingsService.appendKeybinding(localize(13466, null), KEYBINDINGS_EDITOR_COMMAND_DEFINE),
    ...
    createAddAction(keybindingItemEntry) {
        return {
            ...
            enabled: true,
            id: 'addKeybinding',
            tooltip: this.keybindingsService.appendKeybinding(localize(13467, null), KEYBINDINGS_EDITOR_COMMAND_DEFINE),
            run: () => this.keybindingsEditor.defineKeybinding(keybindingItemEntry, false)
        };
    }
...
let CommandColumnRenderer = class CommandColumnRenderer {
    ...
    renderElement(keybindingItemEntry, index, templateData) {
        ...
        const title = keybindingItem.commandLabel ? localize(13468, null, keybindingItem.commandLabel, keybindingItem.command) : keybindingItem.command;
...
let SourceColumnRenderer = class SourceColumnRenderer {
    ...
    renderElement(keybindingItemEntry, index, templateData) {
        ...
        if (isString(keybindingItemEntry.keybindingItem.source)) {
            ...
            templateData.sourceColumnHover.update(localize(13469, null, extensionLabel));
...
class AccessibilityProvider {
    ...
    getWidgetAriaLabel() {
        return localize(13470, null);
    }
    getAriaLabel({ keybindingItem }) {
        const ariaLabel = [
            keybindingItem.commandLabel ? keybindingItem.commandLabel : keybindingItem.command,
            keybindingItem.keybinding?.getAriaLabel() || localize(13471, null),
            keybindingItem.when ? keybindingItem.when : localize(13472, null),
        ...
        if (this.configurationService.getValue("accessibility.verbosity.keybindingsEditor" /* AccessibilityVerbositySettingId.KeybindingsEditor */)) {
            const kbEditorAriaLabel = localize(13473, null);
