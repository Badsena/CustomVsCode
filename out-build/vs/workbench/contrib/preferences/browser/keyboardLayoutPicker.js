...
let KeyboardLayoutPickerContribution = class KeyboardLayoutPickerContribution extends Disposable {
    ...
    constructor(keyboardLayoutService, statusbarService) {
        ...
        const name = nls.localize(13481, null);
        const layout = this.keyboardLayoutService.getCurrentKeyboardLayout();
        if (layout) {
            const layoutInfo = parseKeyboardLayoutDescription(layout);
            const text = nls.localize(13482, null, layoutInfo.label);
        ...
        this._register(this.keyboardLayoutService.onDidChangeKeyboardLayout(() => {
            ...
            if (this.pickerElement.value) {
                const text = nls.localize(13483, null, layoutInfo.label);
                ...
                const text = nls.localize(13484, null, layoutInfo.label);
                this.pickerElement.value = this.statusbarService.addEntry({
                    name,
                    text,
                    ariaLabel: text,
...
const DEFAULT_CONTENT = [
    `// ${nls.localize(13485, null)}`,
    `// ${nls.localize(13486, null)}`,
    ``,
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: nls.localize2(13492, "Change Keyboard Layout"),
            f1: true
        });
    }
    async run(accessor) {
        ...
        if (picks.length > 0) {
            ...
            picks.unshift({ type: 'separator', label: nls.localize(13487, null, platform) });
        }
        const configureKeyboardLayout = { label: nls.localize(13488, null) };
        picks.unshift(configureKeyboardLayout);
        // Offer to "Auto Detect"
        const autoDetectMode = {
            label: nls.localize(13489, null),
            description: isAutoDetect ? `Current: ${parseKeyboardLayoutDescription(currentLayout).label}` : undefined,
            picked: isAutoDetect ? true : undefined
        };
        picks.unshift(autoDetectMode);
        const pick = await quickInputService.pick(picks, { placeHolder: nls.localize(13490, null), matchOnDescription: true });
        if (!pick) {
            return;
        }
        ...
        if (pick === configureKeyboardLayout) {
            ...
            await fileService.stat(file).then(undefined, () => {
                ...
                throw new Error(nls.localize(13491, null, file.toString(), error));
            });
