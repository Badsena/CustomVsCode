...
let ConfigureLanguageBasedSettingsAction = class ConfigureLanguageBasedSettingsAction extends Action {
    ...
    static { this.LABEL = nls.localize2(13541, "Configure Language Specific Settings..."); }
    ...
    async run() {
        ...
        const picks = languages.map(({ languageName, languageId }) => {
            const description = nls.localize(13539, null, languageId);
        ...
        await this.quickInputService.pick(picks, { placeHolder: nls.localize(13540, null) })
            .then(pick => {
            if (pick) {
