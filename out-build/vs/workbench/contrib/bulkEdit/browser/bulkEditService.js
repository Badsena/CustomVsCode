...
let BulkEdit = class BulkEdit {
    ...
    ariaMessage() {
        ...
        if (this._edits.length === 0) {
            return localize(5663, null);
        }
        else if (otherResources.size === 0) {
            if (textEditCount > 1 && textEditResources.size > 1) {
                return localize(5664, null, textEditCount, textEditResources.size);
            }
            else {
                return localize(5665, null, textEditCount);
            }
        }
        else {
            return localize(5666, null, textEditCount, textEditResources.size, otherResources.size);
        }
    }
    ...
    async _performFileEdits(edits, undoRedoGroup, undoRedoSource, confirmBeforeUndo, progress) {
        ...
        const model = this._instaService.createInstance(BulkFileEdits, this._label || localize(5667, null), this._code || 'undoredo.workspaceEdit', undoRedoGroup, undoRedoSource, confirmBeforeUndo, progress, this._token, edits);
        return await model.apply();
    }
    async _performTextEdits(edits, undoRedoGroup, undoRedoSource, progress, reason) {
        this._logService.debug('_performTextEdits', JSON.stringify(edits));
        const model = this._instaService.createInstance(BulkTextEdits, this._label || localize(5668, null), this._code || 'undoredo.workspaceEdit', this._editor, undoRedoGroup, undoRedoSource, progress, this._token, edits);
...
let BulkEditService = class BulkEditService {
    ...
    async apply(editsIn, options) {
        ...
        if (edits.length === 0) {
            return { ariaSummary: localize(5669, null), isApplied: false };
        }
    ...
    async _shouldVeto(label, reason) {
        ...
        switch (reason) {
            case 1 /* ShutdownReason.CLOSE */:
                message = localize(5670, null);
                break;
            case 4 /* ShutdownReason.LOAD */:
                message = localize(5671, null);
                break;
            case 3 /* ShutdownReason.RELOAD */:
                message = localize(5672, null);
                break;
            default:
                message = isMacintosh ? localize(5673, null) : localize(5674, null);
                break;
        }
        const result = await this._dialogService.confirm({
            message,
            detail: localize(5675, null, label || localize(5676, null)),
        });
...
Registry.as(Extensions.Configuration).registerConfiguration({
    ...
    properties: {
        [autoSaveSetting]: {
            description: localize(5677, null),
            default: true,
            type: 'boolean'
        }
    }
