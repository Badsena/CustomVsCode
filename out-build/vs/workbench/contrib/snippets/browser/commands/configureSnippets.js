        ...
        if (file.isGlobalSnippets) {
            ...
            const snippet = {
                ...
                description: names.size === 0
                    ? nls.localize(14756, null)
                    : nls.localize(14757, null, [...names].join(', '))
            };
            existing.push(snippet);
            if (!source) {
                continue;
            }
            const detail = nls.localize(14758, null, source, labelService.getUriLabel(file.location, { relative: true }));
...
async function createSnippetFile(scope, defaultPath, quickInputService, fileService, textFileService, opener) {
    ...
    const input = await quickInputService.input({
        placeHolder: nls.localize(14759, null),
        async validateInput(input) {
            if (!input) {
                return nls.localize(14760, null);
            }
            if (!isValidBasename(input)) {
                return nls.localize(14761, null, input);
            }
            if (await fileService.exists(createSnippetUri(input))) {
                return nls.localize(14762, null, input);
            }
            return undefined;
        }
    });
...
export class ConfigureSnippetsAction extends SnippetsAction {
    constructor() {
        super({
            ...
            title: nls.localize2(14772, "Configure Snippets"),
            shortTitle: {
                ...nls.localize2(14773, "Snippets"),
                mnemonicTitle: nls.localize(14763, null),
            },
            f1: true,
            menu: [
    ...
    async run(accessor) {
        ...
        const globalSnippetPicks = [{
                scope: nls.localize(14764, null),
                label: nls.localize(14765, null),
                uri: userDataProfileService.currentProfile.snippetsHome
            }];
        const workspaceSnippetPicks = [];
        for (const folder of workspaceService.getWorkspace().folders) {
            workspaceSnippetPicks.push({
                scope: nls.localize(14766, null, folder.name),
                label: nls.localize(14767, null, folder.name),
                uri: folder.toResource('.vscode')
            });
        }
        if (existing.length > 0) {
            existing.unshift({ type: 'separator', label: nls.localize(14768, null) });
            existing.push({ type: 'separator', label: nls.localize(14769, null) });
        }
        else {
            existing.push({ type: 'separator', label: nls.localize(14770, null) });
        }
        const pick = await quickInputService.pick([].concat(existing, globalSnippetPicks, workspaceSnippetPicks, picks.future), {
            placeHolder: nls.localize(14771, null),
