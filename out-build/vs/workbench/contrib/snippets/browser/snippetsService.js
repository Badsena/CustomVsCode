...
(function (snippetExt) {
    function toValidSnippet(extension, snippet, languageService) {
        if (isFalsyOrWhitespace(snippet.path)) {
            extension.collector.error(localize(14807, null, extension.description.name, String(snippet.path)));
            return null;
        }
        if (isFalsyOrWhitespace(snippet.language) && !snippet.path.endsWith('.code-snippets')) {
            extension.collector.error(localize(14808, null, extension.description.name, String(snippet.path)));
            return null;
        }
        if (!isFalsyOrWhitespace(snippet.language) && !languageService.isRegisteredLanguageId(snippet.language)) {
            extension.collector.error(localize(14809, null, extension.description.name, String(snippet.language)));
            return null;
        }
        ...
        if (!resources.isEqualOrParent(snippetLocation, extensionLocation)) {
            extension.collector.error(localize(14810, null, extension.description.name, snippetLocation.path, extensionLocation.path));
            return null;
        }
        return {
    ...
    snippetExt.snippetsContribution = {
        description: localize(14811, null),
        type: 'array',
        ...
        items: {
            ...
            properties: {
                language: {
                    description: localize(14812, null),
                    type: 'string'
                },
                path: {
                        ...
                        if (this._environmentService.isExtensionDevelopment) {
                            file.load().then(file => {
                                ...
                                if (file.data.some(snippet => snippet.isBogous)) {
                                    extension.collector.warn(localize(14814, null, extension.description.name));
                                }
                            }, err => {
                                // generic error
                                extension.collector.warn(localize(14815, null, file.location.toString()));
                            });
                        }
                    }
                }
