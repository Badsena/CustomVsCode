...
export async function pickSnippet(accessor, languageIdOrSnippets, resourceUri) {
            ...
            if (snippet.snippetSource === 3 /* SnippetSource.Extension */) {
                ...
                if (isEnabled) {
                    pick.buttons = [{
                            ...
                            tooltip: nls.localize(14787, null)
                        }];
                }
                else {
                    pick.description = nls.localize(14788, null);
                    pick.buttons = [{
                            iconClass: ThemeIcon.asClassName(Codicon.eye),
                            tooltip: nls.localize(14789, null)
                        }];
                }
            }
            result.push(pick);
    ...
    picker.placeholder = nls.localize(14790, null);
    ...
    if (!picker.items.length) {
        picker.validationMessage = nls.localize(14791, null);
    }
    picker.show();
