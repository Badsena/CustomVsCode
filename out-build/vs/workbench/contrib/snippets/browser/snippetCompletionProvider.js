...
export class SnippetCompletion {
    constructor(snippet, range) {
        ...
        this.detail = localize(14782, null, snippet.description || snippet.name, snippet.source);
    ...
    _disambiguateSnippets(suggestions) {
        ...
        for (let i = 0; i < suggestions.length; i++) {
            ...
            for (; to < suggestions.length && item.label === suggestions[to].label; to++) {
                suggestions[to].label.label = localize(14783, null, suggestions[to].label.label, suggestions[to].snippet.name);
            }
            if (to > i + 1) {
                suggestions[i].label.label = localize(14784, null, suggestions[i].label.label, suggestions[i].snippet.name);
                i = to;
            }
        }
    }
