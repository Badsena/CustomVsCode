...
const createCodeActionsAutoSave = (description) => {
    return {
        ...
        enumDescriptions: [
            nls.localize(9050, null),
            nls.localize(9051, null),
            nls.localize(9052, null),
            nls.localize(9053, null),
            nls.localize(9054, null)
        ],
        default: 'explicit',
...
const createNotebookCodeActionsAutoSave = (description) => {
    return {
        ...
        enumDescriptions: [
            nls.localize(9055, null),
            nls.localize(9056, null),
            nls.localize(9057, null),
            nls.localize(9058, null)
        ],
        default: 'explicit',
        description: description
    };
...
const codeActionsOnSaveSchema = {
    ...
    markdownDescription: nls.localize(9059, null, '`#files.autoSave#`'),
    type: ['object', 'array'],
    additionalProperties: {
        type: 'string',
        enum: ['always', 'explicit', 'never', true, false],
...
const notebookCodeActionsOnSaveSchema = {
    ...
    markdownDescription: nls.localize(9060, null, '`#files.autoSave#`'),
    type: 'object',
    additionalProperties: {
        type: ['string', 'boolean'],
        enum: ['explicit', 'never', true, false],
    ...
    updateConfigurationSchema(allProvidedKinds) {
        ...
        for (const codeActionKind of allProvidedKinds) {
            if (CodeActionKind.Source.contains(codeActionKind) && !properties[codeActionKind.value]) {
                properties[codeActionKind.value] = createCodeActionsAutoSave(nls.localize(9061, null, codeActionKind.value));
                notebookProperties[codeActionKind.value] = createNotebookCodeActionsAutoSave(nls.localize(9062, null, codeActionKind.value));
            }
        }
