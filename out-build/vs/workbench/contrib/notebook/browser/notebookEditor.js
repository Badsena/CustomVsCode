    ...
    async setInput(input, options, context, token, noRetry) {
        try {
            ...
            if (model === null) {
                ...
                if (!knownProvider) {
                    throw new Error(localize(13146, null, input.viewType));
                }
                ...
                throw createEditorOpenError(new Error(localize(13147, null, input.viewType)), [
                    toAction({
                        id: 'workbench.notebook.action.installOrEnableMissing', label: extensionInfo
                            ? localize(13148, null, input.viewType)
                            : localize(13149, null, input.viewType),
                        run: async () => {
                    ...
                    toAction({
                        id: 'workbench.notebook.action.openAsText', label: localize(13150, null), run: async () => {
            ...
            if (e.fileOperationResult === 7 /* FileOperationResult.FILE_TOO_LARGE */) {
                ...
                if (e instanceof TooLargeFileOperationError) {
                    message = localize(13151, null, ByteSize.formatSize(e.size));
                }
                else {
                    message = localize(13152, null);
                }
            ...
            const error = createEditorOpenError(e instanceof Error ? e : new Error((e ? e.message : '')), [
                toAction({
                    id: 'workbench.notebook.action.openInTextEditor', label: localize(13153, null), run: async () => {
