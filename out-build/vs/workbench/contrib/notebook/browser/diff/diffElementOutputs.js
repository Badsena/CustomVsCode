...
export class OutputElement extends Disposable {
    ...
    render(index, beforeElement) {
        ...
        if (mimeTypes.length > 1) {
            ...
            mimeTypePicker.title = nls.localize(13026, null, mimeTypes.map(mimeType => mimeType.mimeType).join(', '));
    ...
    _renderMissingRenderer(viewModel, preferredMimeType) {
        if (!viewModel.model.outputs.length) {
            return this._renderMessage(viewModel, nls.localize(13027, null));
        }
        if (!preferredMimeType) {
            const mimeTypes = viewModel.model.outputs.map(op => op.mime);
            const mimeTypesMessage = mimeTypes.join(', ');
            return this._renderMessage(viewModel, nls.localize(13028, null, mimeTypesMessage));
        }
    ...
    async pickActiveMimeTypeRenderer(notebookTextModel, viewModel) {
        ...
        const items = mimeTypes.filter(mimeType => mimeType.isTrusted).map((mimeType, index) => ({
            ...
            description: index === currIndex ? nls.localize(13029, null) : undefined
        }));
        ...
        picker.placeholder = items.length !== mimeTypes.length
            ? nls.localize(13030, null)
            : nls.localize(13031, null);
    ...
    generateRendererInfo(renderId) {
        ...
        return nls.localize(13032, null);
    }
