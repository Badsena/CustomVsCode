...
registerAction2(class DeleteCellAction extends NotebookCellAction {
    ...
    async runWithContext(accessor, context) {
        ...
        if (runState === NotebookCellExecutionState.Executing && configService.getValue(NotebookSetting.confirmDeleteRunningCell)) {
            ...
            const primaryButton = localize(12911, null);
            confirmation = await dialogService.confirm({
                type: 'question',
                message: localize(12912, null),
                primaryButton: primaryButton,
                checkbox: {
                    label: localize(12913, null)
                }
            });
        }
        else {
...
registerAction2(class ChangeCellLanguageAction extends NotebookCellAction {
    constructor() {
        super({
            ...
            metadata: {
                description: localize(12917, null),
                args: [
                    {
                        name: 'range',
    ...
    async showLanguagePicker(accessor, context) {
        ...
        providerLanguages.forEach(languageId => {
            ...
            if (context.cell.cellKind === CellKind.Markup ? (languageId === 'markdown') : (languageId === context.cell.language)) {
                description = localize(12918, null, languageId);
            }
            else {
                description = localize(12919, null, languageId);
            }
        ...
        const autoDetectMode = {
            label: localize(12920, null)
        };
        const picks = [
            autoDetectMode,
            { type: 'separator', label: localize(12921, null) },
            ...topItems,
            { type: 'separator' },
            ...mainItems
        ];
        const selection = await quickInputService.pick(picks, { placeHolder: localize(12922, null) });
...
registerAction2(class DetectCellLanguageAction extends NotebookCellAction {
    constructor() {
        super({
            ...
            title: localize2(12930, "Accept Detected Language for Cell"),
            f1: true,
    ...
    async runWithContext(accessor, context) {
        ...
        if (detection) {
            ...
            notificationService.warn(localize(12923, null));
        }
    }
});
...
registerAction2(class SelectNotebookIndentation extends NotebookAction {
    constructor() {
        super({
            ...
            title: localize2(12931, 'Select Indentation'),
            f1: true,
    ...
    async showNotebookIndentationPicker(accessor, context) {
        ...
        if (!activeNotebook || activeNotebook.isDisposed) {
            return quickInputService.pick([{ label: localize(12924, null) }]);
        }
        if (activeNotebook.isReadOnly) {
            return quickInputService.pick([{ label: localize(12925, null) }]);
        }
        const picks = [
        ...
        picks.splice(3, 0, { type: 'separator', label: localize(12926, null) });
        picks.unshift({ type: 'separator', label: localize(12927, null) });
        const action = await quickInputService.pick(picks, { placeHolder: localize(12928, null), matchOnDetail: true });
        if (!action) {
            return;
        }
        action.run();
