...
let NotebookKeymapService = class NotebookKeymapService extends Disposable {
    ...
    promptForDisablingOtherKeymaps(newKeymap, oldKeymaps) {
        ...
        this.notificationService.prompt(Severity.Info, localize(13246, null, distinct(oldKeymaps.map(k => k.local.manifest.displayName)).map(name => `'${name}'`).join(', ')), [{
                label: localize(13247, null),
                run: () => onPrompt(true)
            }, {
                label: localize(13248, null),
                run: () => onPrompt(false)
            }]);
    }
};
