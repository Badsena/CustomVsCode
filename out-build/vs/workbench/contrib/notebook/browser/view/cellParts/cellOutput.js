...
let CellOutputElement = class CellOutputElement extends Disposable {
    ...
    _renderMissingRenderer(viewModel, preferredMimeType) {
        if (!viewModel.model.outputs.length) {
            return this._renderMessage(viewModel, nls.localize(13256, null));
        }
        if (!preferredMimeType) {
            const mimeTypes = viewModel.model.outputs.map(op => op.mime);
            const mimeTypesMessage = mimeTypes.join(', ');
            return this._renderMessage(viewModel, nls.localize(13257, null, mimeTypesMessage));
        }
    ...
    async _attachToolbar(outputItemDiv, notebookTextModel, kernel, index, currentMimeType, mimeTypes) {
        ...
        const pickAction = this.toolbarDisposables.add(new Action('notebook.output.pickMimetype', nls.localize(13258, null), ThemeIcon.asClassName(mimetypeIcon), undefined, async (_context) => this._pickActiveMimeTypeRenderer(outputItemDiv, notebookTextModel, kernel, this.output)));
    ...
    async _pickActiveMimeTypeRenderer(outputItemDiv, notebookTextModel, kernel, viewModel) {
        ...
        mimeTypes.forEach((mimeType, index) => {
            if (mimeType.isTrusted) {
                ...
                arr.push({
                    ...
                    description: index === currIndex ? nls.localize(13259, null) : undefined
                });
            }
        });
        if (unsupportedItems.some(m => JUPYTER_RENDERER_MIMETYPES.includes(m.id))) {
            unsupportedItems.push({
                label: nls.localize(13260, null),
                id: 'installRenderers',
        ...
        picker.placeholder = items.length !== mimeTypes.length
            ? nls.localize(13261, null)
            : nls.localize(13262, null);
    ...
    _generateRendererInfo(renderId) {
        ...
        return nls.localize(13263, null);
    }
