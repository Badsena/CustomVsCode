...
let CommentReply = class CommentReply extends Disposable {
    ...
    setCommentEditorDecorations() {
        ...
        const placeholder = hasExistingComments
            ? (this._commentOptions?.placeHolder || nls.localize(9355, null))
            : (this._commentOptions?.placeHolder || nls.localize(9356, null));
    ...
    createReplyButton(commentEditor, commentForm) {
        ...
        this._register(this.hoverService.setupManagedHover(getDefaultHoverDelegate('mouse'), this._reviewThreadReplyButton, this._commentOptions?.prompt || nls.localize(9357, null)));
        this._reviewThreadReplyButton.textContent = this._commentOptions?.prompt || nls.localize(9358, null);
