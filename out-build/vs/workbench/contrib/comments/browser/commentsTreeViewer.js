...
export const COMMENTS_VIEW_TITLE = nls.localize2(9430, "Comments");
...
let CommentNodeRenderer = class CommentNodeRenderer {
    ...
    getCountString(commentCount) {
        if (commentCount > 2) {
            return nls.localize(9421, null, commentCount - 1);
        }
        else if (commentCount === 2) {
            return nls.localize(9422, null);
        }
        else {
            return nls.localize(9423, null);
        }
    }
    getRenderedComment(commentBody) {
        ...
        for (let i = 0; i < images.length; i++) {
            ...
            textDescription.textContent = image.alt ? nls.localize(9424, null, image.alt) : nls.localize(9425, null);
    ...
    renderElement(node, index, templateData) {
        ...
        if (node.element.threadRelevance === CommentThreadApplicability.Outdated) {
            ...
            templateData.threadMetadata.relevance.innerText = nls.localize(9426, null);
        ...
        if (node.element.range) {
            if (node.element.range.startLineNumber === node.element.range.endLineNumber) {
                templateData.threadMetadata.range.textContent = nls.localize(9427, null, node.element.range.startLineNumber);
            }
            else {
                templateData.threadMetadata.range.textContent = nls.localize(9428, null, node.element.range.startLineNumber, node.element.range.endLineNumber);
            }
        }
        ...
        templateData.repliesMetadata.lastReplyDetail.textContent = nls.localize(9429, null, lastComment.userName);
