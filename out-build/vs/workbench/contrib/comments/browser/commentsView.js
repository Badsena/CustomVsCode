...
let CommentsPanel = class CommentsPanel extends FilterViewPane {
    ...
    constructor(options, instantiationService, viewDescriptorService, editorService, configurationService, contextKeyService, contextMenuService, keybindingService, openerService, themeService, commentService, hoverService, uriIdentityService, storageService, pathService) {
        ...
        super({
            ...options,
            filterOptions: {
                placeholder: nls.localize(9431, null),
                ariaLabel: nls.localize(9432, null),
    ...
    updateFilter() {
        ...
        this.filterWidget.updateBadge(total === filtered || total === 0 ? undefined : nls.localize(9433, null, filtered, total));
    ...
    makeCommentLocationLabel(file, range) {
        ...
        if (!range) {
            return nls.localize(9434, null, fileLabel);
        }
        if (range.startLineNumber === range.endLineNumber) {
            return nls.localize(9435, null, range.startLineNumber, range.startColumn, fileLabel);
        }
        else {
            return nls.localize(9436, null, range.startLineNumber, range.endLineNumber, fileLabel);
        }
    }
    ...
    getScreenReaderInfoForNode(element, forAriaLabel) {
        ...
        if (forAriaLabel && this.configurationService.getValue("accessibility.verbosity.comments" /* AccessibilityVerbositySettingId.Comments */)) {
            ...
            accessibleViewHint = kbLabel ? nls.localize(9437, null, kbLabel) : nls.localize(9438, null);
        }
        ...
        if (element.threadRelevance === CommentThreadApplicability.Outdated) {
            return nls.localize(9439, null, labelInfo.userName, labelInfo.bodyLabel, labelInfo.locationLabel, labelInfo.replyCountLabel, relevantLines) + replies + accessibleViewHint;
        }
        else {
            return nls.localize(9440, null, labelInfo.userName, labelInfo.bodyLabel, labelInfo.locationLabel, labelInfo.replyCountLabel, relevantLines) + replies + accessibleViewHint;
        }
    }
    getRepliesAsString(node, forAriaLabel) {
        ...
        return '\n' + node.replies.map(reply => nls.localize(9441, null, reply.comment.userName, (typeof reply.comment.body === 'string') ? reply.comment.body : reply.comment.body.value)).join('\n');
    }
    getReplyCountAsString(node, forAriaLabel) {
        return node.replies.length && !forAriaLabel ? nls.localize(9442, null, node.replies.length) : '';
    }
    createTree() {
            ...
            accessibilityProvider: {
                getAriaLabel: (element) => {
                    if (element instanceof CommentsModel) {
                        return nls.localize(9443, null);
                    }
                    if (element instanceof ResourceWithCommentThreads) {
                        return nls.localize(9444, null, basename(element.resource), element.resource.fsPath);
                    }
