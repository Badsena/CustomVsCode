    ...
    _findNearestCommentingRange(reverse) {
        ...
        if (this.accessibilityService.isScreenReaderOptimized()) {
            ...
            if (commentRangeStart && commentRangeEnd) {
                ...
                oneLine ? status(nls.localize(9390, null, commentRangeStart)) : status(nls.localize(9391, null, commentRangeStart, commentRangeEnd));
            }
        }
    }
    nextCommentingRange() {
            ...
            if (!this._hasRespondedToEditorChange) {
                if (this._commentInfos.some(commentInfo => commentInfo.commentingRanges.ranges.length > 0 || commentInfo.commentingRanges.fileComments)) {
                    ...
                    if (verbose) {
                        ...
                        if (keybinding) {
                            status(nls.localize(9392, null, keybinding));
                        }
                        else {
                            status(nls.localize(9393, null));
                        }
                    }
                    else {
                        status(nls.localize(9394, null));
                    }
                }
            }
        });
    ...
    addCommentAtLine(range, e) {
        ...
        if (!newCommentInfos.length || !this.editor?.hasModel()) {
            ...
            if (!newCommentInfos.length) {
                if (range) {
                    this.notificationService.error(nls.localize(9395, null));
                }
                else {
                    this.notificationService.error(nls.localize(9396, null));
                }
            }
            return Promise.resolve();
        }
        if (newCommentInfos.length > 1) {
            if (e && range) {
                ...
                return this.quickInputService.pick(picks, { placeHolder: nls.localize(9397, null), matchOnDescription: true }).then(pick => {
                    if (!pick) {
                        return;
                    }
