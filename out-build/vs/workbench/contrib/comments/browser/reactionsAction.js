...
export class ToggleReactionsAction extends Action {
    ...
    constructor(toggleDropdownMenu, title) {
        super(ToggleReactionsAction.ID, title || nls.localize(9473, null), 'toggle-reactions', true);
        this._menuActions = [];
...
export class ReactionActionViewItem extends ActionViewItem {
    ...
    getTooltip() {
        const action = this.action;
        const toggleMessage = action.enabled ? nls.localize(9474, null) : '';
        if (action.count === undefined) {
            return nls.localize(9475, null, toggleMessage, action.label);
            ...
            if (action.count === 1) {
                return nls.localize(9476, null, toggleMessage, action.label);
                ...
                return nls.localize(9477, null, toggleMessage, action.count, action.label);
            ...
            if (action.reactors.length <= 10 && action.reactors.length === action.count) {
                return nls.localize(9478, null, toggleMessage, action.reactors.join(', '), action.label);
                ...
                return nls.localize(9479, null, toggleMessage, displayedReactors.join(', '), action.count - displayedReactors.length, action.label);
