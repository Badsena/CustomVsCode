        ...
        try {
            ...
            if (command) {
                if (path.isAbsolute(command)) {
                    ...
                    if (!commandExists) {
                        throw new Error(nls.localize(10335, null, command));
                    }
                }
                else {
                    // relative path
                ...
                throw new Error(nls.localize(10336, null, this.debugType));
            }
            let env = process.env;
            ...
            if (command === 'node') {
                if (Array.isArray(args) && args.length > 0) {
                    ...
                    if (!child.pid) {
                        throw new Error(nls.localize(10337, null, args[0]));
                    }
                    this.serverProcess = child;
                }
                else {
                    throw new Error(nls.localize(10338, null));
                }
            }
            else {
