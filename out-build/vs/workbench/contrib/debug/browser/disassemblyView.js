...
const disassemblyNotAvailable = {
    ...
    instruction: {
        address: '-1',
        instruction: localize(10065, null)
    },
};
    ...
    openSourceCode(instruction) {
        if (instruction) {
            ...
            this.editorService.openEditor({
                resource: sourceURI,
                description: localize(10067, null),
                options: {
                    preserveFocus: false,
                    selection: selection,
                    revealIfOpened: true,
...
class AccessibilityProvider {
    getWidgetAriaLabel() {
        return localize(10068, null);
    }
    getAriaLabel(element) {
        let label = '';
        const instruction = element.instruction;
        if (instruction.address !== '-1') {
            label += `${localize(10069, null)}: ${instruction.address}`;
        }
        if (instruction.instructionBytes) {
            label += `, ${localize(10070, null)}: ${instruction.instructionBytes}`;
        }
        label += `, ${localize(10071, null)}: ${instruction.instruction}`;
        return label;
    }
}
