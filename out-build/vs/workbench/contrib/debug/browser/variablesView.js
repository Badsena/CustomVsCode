    ...
    renderActionBar(actionBar, expression, _data) {
        ...
        if (viz.original) {
            const action = toAction({
                id: 'debugViz', label: localize(10175, null), class: ThemeIcon.asClassName(Codicon.eye), run: () => this.debugService.getViewModel().setVisualizedExpression(viz.original, undefined)
            });
            action.checked = true;
            primary.push(action);
    ...
    renderActionBar(actionBar, expression, data) {
        ...
        this.visualization.getApplicableFor(expression, cts.token).then(result => {
            ...
            if (actions.length === 0) {
                ...
                actionBar.push(toAction({ id: 'debugViz', label: localize(10177, null), class: ThemeIcon.asClassName(Codicon.eye), run: () => this.pickVisualizer(actions, originalExpression, data) }), { icon: true, label: false });
            }
        });
    }
...
class VariablesAccessibilityProvider {
    getWidgetAriaLabel() {
        return localize(10178, null);
    }
    getAriaLabel(element) {
        if (element instanceof Scope) {
            return localize(10179, null, element.name);
        }
        if (element instanceof Variable) {
            return localize(10180, null, element.name, element.value);
        }
        return null;
    }
}
...
async function tryInstallHexEditor(extensionsWorkbenchService, notificationService) {
    try {
        await extensionsWorkbenchService.install(HEX_EDITOR_EXTENSION_ID, {
            justification: localize(10181, null),
            enable: true
