...
let DebugService = class DebugService {
    constructor(editorService, paneCompositeService, viewsService, viewDescriptorService, notificationService, dialogService, layoutService, contextService, contextKeyService, lifecycleService, instantiationService, extensionService, fileService, configurationService, extensionHostDebugService, activityService, commandService, quickInputService, workspaceTrustRequestService, uriIdentityService, testService) {
        ...
        this.disposables.add(this.model.onDidChangeCallStack(() => {
            ...
            if (numberOfSessions > 0) {
                ...
                if (viewContainer) {
                    this.activity = this.activityService.showViewContainerActivity(viewContainer.id, { badge: new NumberBadge(numberOfSessions, n => n === 1 ? nls.localize(9969, null) : nls.localize(9970, null, n)) });
                }
            }
        }));
        ...
        this.disposables.add(extensionService.onWillStop(evt => {
            evt.veto(this.model.getSessions().length > 0, nls.localize(9971, null));
        }));
    ...
    async startDebugging(launch, configOrName, options, saveBeforeStart = !options?.parentSession) {
        const message = options && options.noDebug ? nls.localize(9972, null) : nls.localize(9973, null);
        ...
        try {
            ...
            if (compound) {
                ...
                if (!compound.configurations) {
                    throw new Error(nls.localize(9974, null));
                }
                ...
                const values = await Promise.all(compound.configurations.map(configData => {
                    ...
                    if (typeof configData === 'string') {
                        ...
                        if (launchesContainingName.length === 1) {
                            ...
                            throw new Error(launchesContainingName.length === 0 ? nls.localize(9975, null, name)
                                : nls.localize(9976, null, name));
                        }
                    }
                        ...
                        if (launchesMatchingConfigData.length === 1) {
                            ...
                            throw new Error(nls.localize(9977, null, configData.folder, configData.name, compound.name));
                        }
                    }
            ...
            if (configOrName && !config) {
                const message = !!launch ? nls.localize(9978, null, typeof configOrName === 'string' ? configOrName : configOrName.name) :
                    nls.localize(9979, null);
            ...
            try {
                ...
                if (!dbg || (configByProviders.request !== 'attach' && configByProviders.request !== 'launch')) {
                    ...
                    if (configByProviders.request !== 'attach' && configByProviders.request !== 'launch') {
                        message = configByProviders.request ? nls.localize(9980, null, 'request', configByProviders.request)
                            : nls.localize(9981, null, 'request');
                    }
                    else {
                        message = resolvedConfig.type ? nls.localize(9982, null, resolvedConfig.type) :
                            nls.localize(9983, null);
                    }
                    const actionList = [];
                    actionList.push(toAction({
                        id: 'installAdditionalDebuggers',
                        label: nls.localize(9984, null, resolvedConfig.type),
                        enabled: true,
                ...
                if (err && err.message) {
                    ...
                    await this.showError(nls.localize(9985, null));
                }
    ...
    async confirmConcurrentSession(sessionLabel) {
        const result = await this.dialogService.confirm({
            message: nls.localize(9986, null, sessionLabel)
        });
        ...
        listenerDisposables.add(session.onDidEndAdapter(async (adapterExitEvent) => {
            if (adapterExitEvent) {
                if (adapterExitEvent.error) {
                    this.notificationService.error(nls.localize(9987, null, adapterExitEvent.error.message || adapterExitEvent.error.toString()));
                }
                ...
                if (editor.input === DisassemblyViewInput.instance) {
                    ...
                    if (stackFrame && isCodeEditor(control) && control.hasModel()) {
                        ...
                        if (lineNumber >= 1 && lineNumber <= model.getLineCount()) {
                            ...
                            aria.alert(nls.localize(9988, null, lineContent, thread && thread.stoppedDetails ? `, reason ${thread.stoppedDetails.reason}` : '', stackFrame.source ? stackFrame.source.name : '', stackFrame.range.startLineNumber));
                        }
                    }
                }
            }
    ...
    async addBreakpoints(uri, rawBreakpoints, ariaAnnounce = true) {
        ...
        if (ariaAnnounce) {
            breakpoints.forEach(bp => aria.status(nls.localize(9989, null, bp.lineNumber, uri.fsPath)));
        }
    ...
    async removeBreakpoints(id) {
        ...
        toRemove.forEach(bp => aria.status(nls.localize(9990, null, bp.lineNumber, bp.uri.fsPath)));
