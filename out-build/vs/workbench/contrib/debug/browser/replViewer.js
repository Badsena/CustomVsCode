...
export class ReplAccessibilityProvider {
    getWidgetAriaLabel() {
        return localize(10149, null);
    }
    getAriaLabel(element) {
        if (element instanceof Variable) {
            return localize(10150, null, element.name, element.value);
        }
        if (element instanceof ReplOutputElement || element instanceof ReplEvaluationInput || element instanceof ReplEvaluationResult) {
            return element.value + (element instanceof ReplOutputElement && element.count > 1 ? localize(10151, null, element.count) : '');
        }
        if (element instanceof RawObjectReplElement) {
            return localize(10152, null, element.name, element.value);
        }
        if (element instanceof ReplGroup) {
            return localize(10153, null, element.name);
        }
        return '';
    }
}
