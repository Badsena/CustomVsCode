...
const ABORT_LABEL = nls.localize(10040, null);
const DEBUG_ANYWAY_LABEL = nls.localize(10041, null);
const DEBUG_ANYWAY_LABEL_NO_MEMO = nls.localize(10042, null);
let DebugTaskRunner = class DebugTaskRunner {
    ...
    async runTaskAndCheckErrors(root, taskId) {
        try {
            ...
            const message = errorCount > 1
                ? nls.localize(10043, null, taskLabel)
                : errorCount === 1
                    ? nls.localize(10044, null, taskLabel)
                    : taskSummary && typeof taskSummary.exitCode === 'number'
                        ? nls.localize(10045, null, taskLabel, taskSummary.exitCode)
                        : nls.localize(10046, null, taskLabel);
            let DebugChoice;
    ...
    async runTask(root, taskId, token = this.globalCancellation.token) {
        ...
        if (!root) {
            return Promise.reject(new Error(nls.localize(10051, null, typeof taskId === 'string' ? taskId : taskId.type)));
        }
        // run a task before starting a debug session
        const task = await this.taskService.getTask(root, taskId);
        if (!task) {
            const errorMessage = typeof taskId === 'string'
                ? nls.localize(10052, null, taskId)
                : nls.localize(10053, null);
        ...
        const result = new Promise((resolve, reject) => {
            ...
            store.add(didAcquireInput.event(() => {
                ...
                store.add(disposableTimeout(() => {
                    if (!taskStarted) {
                        const errorMessage = nls.localize(10054, null, typeof taskId === 'string' ? taskId : JSON.stringify(taskId));
                ...
                if (!hideSlowPreLaunchWarning) {
                    ...
                    store.add(disposableTimeout(() => {
                        const message = nls.localize(10055, null, task.configurationProperties.name);
                        ...
                        if (canConfigure) {
                            buttons.splice(1, 0, nls.localize(10056, null));
                        }
