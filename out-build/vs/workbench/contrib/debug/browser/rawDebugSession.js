...
let RawDebugSession = class RawDebugSession {
    ...
    async start() {
        if (!this.debugAdapter) {
            return Promise.reject(new Error(nls.localize(10090, null)));
        }
            ...
            try {
                ...
                if (!result.success) {
                    const { confirmed } = await this.dialogSerivce.confirm({
                        ...
                        message: nls.localize(10091, null),
                        primaryButton: nls.localize(10092, null)
                    });
                    if (confirmed) {
        ...
        return new Promise((completeDispatch, errorDispatch) => {
            if (!this.debugAdapter) {
                if (this.inShutdown) {
                    ...
                    errorDispatch(new Error(nls.localize(10093, null, command)));
                }
                return;
            }
    ...
    handleErrorResponse(errorResponse, showErrors) {
        ...
        const url = error?.url;
        if (error && url) {
            const label = error.urlLabel ? error.urlLabel : nls.localize(10094, null);
