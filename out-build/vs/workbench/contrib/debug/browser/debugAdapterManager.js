            ...
            delta.added.forEach(added => {
                added.value.forEach(rawAdapter => {
                    if (!rawAdapter.type || (typeof rawAdapter.type !== 'string')) {
                        added.collector.error(nls.localize(9792, null));
                    }
                    if (rawAdapter.type !== '*') {
        ...
        const definitions = {
            'common': {
                properties: {
                    'name': {
                        type: 'string',
                        description: nls.localize(9793, null),
                        default: 'Launch'
                    },
                    'debugServer': {
                        type: 'number',
                        description: nls.localize(9794, null),
                        default: 4711
                    },
                    'preLaunchTask': {
                        anyOf: [taskSchema, {
                        ...
                        description: nls.localize(9795, null),
                    ...
                    'postDebugTask': {
                        ...
                        default: '',
                        defaultSnippets: [{ body: { task: '', type: '' } }],
                        description: nls.localize(9796, null),
                        examples: this.taskLabels,
                    },
                    ...
                    'suppressMultipleSessionWarning': {
                        type: 'boolean',
                        description: nls.localize(9797, null),
                        default: true
                    }
                }
            }
    ...
    async guessDebugger(gettingConfigurations) {
        ...
        if (candidates.length === 0 && languageLabel) {
            ...
            const { confirmed } = await this.dialogService.confirm({
                ...
                message: nls.localize(9798, null, languageLabel),
                primaryButton: nls.localize(9799, null, languageLabel)
            });
            if (confirmed) {
        ...
        if (suggestedCandidates.length > 0) {
            picks.push({ type: 'separator', label: nls.localize(9800, null) }, ...suggestedCandidates.map(c => ({ label: c.label, pick: () => ({ debugger: c }) })));
        }
        ...
        if (dynamic.length) {
            ...
            for (const d of dynamic) {
                picks.push({
                    label: nls.localize(9801, null, d.label),
                    pick: async () => {
        ...
        picks.push({ type: 'separator', label: '' }, { label: languageLabel ? nls.localize(9802, null, languageLabel) : nls.localize(9803, null) });
        ...
        const placeHolder = nls.localize(9804, null);
