...
let StartDebugActionViewItem = class StartDebugActionViewItem extends BaseActionViewItem {
    constructor(context, action, options, debugService, configurationService, commandService, contextService, contextViewService, keybindingService, hoverService, contextKeyService) {
        ...
        this.providers = [];
        this.toDispose = [];
        this.selectBox = new SelectBox([], -1, contextViewService, defaultSelectBoxStyles, { ariaLabel: nls.localize(9785, null), useCustomDrawn: !hasNativeContextMenu(this.configurationService) });
    ...
    updateOptions() {
        ...
        if (this.debugOptions.length === 0) {
            this.debugOptions.push({ label: nls.localize(9786, null), handler: async () => false });
        }
        this.debugOptions.push({ label: SeparatorSelectOption.text, handler: () => Promise.resolve(false) });
        disabledIdxs.push(this.debugOptions.length - 1);
        separatorIdxs.push(this.debugOptions.length - 1);
        ...
        manager.getLaunches().filter(l => !l.hidden).forEach(l => {
            const label = inWorkspace ? nls.localize(9787, null, l.name) : nls.localize(9788, null);
            this.debugOptions.push({
                label, handler: async () => {
    ...
    _setAriaLabel(title) {
        ...
        if (keybinding) {
            ariaLabel = nls.localize(9789, null, ariaLabel, keybinding);
        }
        else {
            ariaLabel = nls.localize(9790, null, ariaLabel);
        }
...
let FocusSessionActionViewItem = class FocusSessionActionViewItem extends SelectActionViewItem {
    constructor(action, session, debugService, contextViewService, configurationService) {
        super(null, action, [], -1, contextViewService, defaultSelectBoxStyles, { ariaLabel: nls.localize(9791, null), useCustomDrawn: !hasNativeContextMenu(configurationService) });
        this.debugService = debugService;
        this.configurationService = configurationService;
