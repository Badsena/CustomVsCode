...
let BreakpointWidget = class BreakpointWidget extends ZoneWidget {
    ...
    get placeholder() {
        ...
        switch (this.context) {
            case 2 /* Context.LOG_MESSAGE */:
                return nls.localize(9630, null, acceptString, closeString);
            case 1 /* Context.HIT_COUNT */:
                return nls.localize(9631, null, acceptString, closeString);
            default:
                return nls.localize(9632, null, acceptString, closeString);
        }
    }
    ...
    _fillContainer(container) {
        ...
        const selectBox = this.store.add(new SelectBox([
            { text: nls.localize(9633, null) },
            { text: nls.localize(9634, null) },
            { text: nls.localize(9635, null) },
            { text: nls.localize(9636, null) },
        ], this.context, this.contextViewService, defaultSelectBoxStyles, { ariaLabel: nls.localize(9637, null), useCustomDrawn: !hasNativeContextMenu(this._configurationService) }));
    ...
    createModesInput(container) {
        ...
        const sb = this.selectModeBox = new SelectBox([
            { text: nls.localize(9638, null), isDisabled: true },
    ...
    createTriggerBreakpointInput(container) {
        ...
        const selectBreakpointBox = this.selectBreakpointBox = this.store.add(new SelectBox(breakpointOptions, index + 1, this.contextViewService, defaultSelectBoxStyles, { ariaLabel: nls.localize(9639, null), useCustomDrawn: !hasNativeContextMenu(this._configurationService) }));
    ...
    buildBreakpointOptions() {
        ...
        for (const [i, bp] of this.availableBreakpoints.entries()) {
            this.textModelService.createModelReference(bp.uri).then(ref => {
                ...
                breakpointOptions[i + 1].description = nls.localize(9643, null);
            });
        }
