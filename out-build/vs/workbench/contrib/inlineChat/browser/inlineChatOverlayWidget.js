...
let InlineChatInputWidget = class InlineChatInputWidget extends Disposable {
    constructor(_editorObs, _contextKeyService, _menuService, instantiationService, modelService, configurationService) {
        ...
        this._layoutData = derived(r => {
            ...
            const minWidth = 220;
            const maxWidth = 600;
            const clampedWidth = this._input.getOption(149 /* EditorOption.wordWrap */) === 'on'
                ? maxWidth
        ...
        this._store.add(this._input.onKeyDown(e => {
            if (e.keyCode === 18 /* KeyCode.DownArrow */ && !actionBar.isEmpty()) {
...
let InlineChatSessionOverlayWidget = class InlineChatSessionOverlayWidget extends Disposable {
    ...
    show(session) {
        ...
        const requestMessage = derived(r => {
            ...
            if (!response) {
                return { message: localize(11632, null), icon: ThemeIcon.modify(Codicon.loading, 'spin') };
            }
            ...
            if (lastPart?.kind === 'toolInvocation') {
                ...
                return { message: localize(11638, null), icon: ThemeIcon.modify(Codicon.loading, 'spin') };
            }
        });
