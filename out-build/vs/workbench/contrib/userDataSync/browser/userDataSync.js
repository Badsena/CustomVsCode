...
const turnOffSyncCommand = { id: 'workbench.userDataSync.actions.turnOff', title: localize2(17198, 'Turn Off') };
const configureSyncCommand = { id: CONFIGURE_SYNC_COMMAND_ID, title: localize2(17199, 'Configure...') };
...
const syncNowCommand = {
    ...
    title: localize2(17200, 'Sync Now'),
    description(userDataSyncService) {
        if (userDataSyncService.status === "syncing" /* SyncStatus.Syncing */) {
            return localize(17128, null);
        }
        if (userDataSyncService.lastSyncTime) {
            return localize(17129, null, fromNow(userDataSyncService.lastSyncTime, true));
        }
        return undefined;
    }
};
const showSyncSettingsCommand = { id: 'workbench.userDataSync.actions.settings', title: localize2(17201, 'Show Settings'), };
const showSyncedDataCommand = { id: 'workbench.userDataSync.actions.showSyncedData', title: localize2(17202, 'Show Synced Data'), };
...
let UserDataSyncWorkbenchContribution = class UserDataSyncWorkbenchContribution extends Disposable {
        ...
        if (conflicts.length) {
            ...
            for (const conflict of this.userDataSyncService.conflicts) {
                ...
                if (!this.conflictsDisposables.has(key)) {
                    ...
                    const handle = this.notificationService.prompt(Severity.Warning, localize(17130, null, conflictsArea.toLowerCase()), [
                        {
                            label: localize(17131, null),
                            run: () => {
                                this.acceptLocal(conflict, conflict.conflicts[0]);
                            }
                        },
                        {
                            label: localize(17132, null),
                            run: () => {
                                this.acceptRemote(conflict, conflict.conflicts[0]);
                            }
                        },
                        {
                            label: localize(17133, null),
                            run: () => {
    ...
    async acceptRemote(syncResource, conflict) {
        try {
            ...
            this.notificationService.error(localize(17134, null, `command:${SHOW_SYNC_LOG_COMMAND_ID}`));
        }
    }
    async acceptLocal(syncResource, conflict) {
        try {
            ...
            this.notificationService.error(localize(17135, null, `command:${SHOW_SYNC_LOG_COMMAND_ID}`));
        }
    }
        ...
        switch (error.code) {
            case "SessionExpired" /* UserDataSyncErrorCode.SessionExpired */:
                this.notificationService.notify({
                    ...
                    message: localize(17136, null),
                    actions: {
                        primary: [toAction({
                                id: 'turn on sync',
                                label: localize(17137, null),
                                run: () => this.turnOn()
                            })]
                    }
                });
            ...
            case "TurnedOff" /* UserDataSyncErrorCode.TurnedOff */:
                this.notificationService.notify({
                    severity: Severity.Info,
                    message: localize(17138, null),
                    actions: {
                        primary: [toAction({
                                id: 'turn on sync',
                                label: localize(17139, null),
                                run: () => this.turnOn()
                            })]
                    }
                });
            ...
            case "TooLarge" /* UserDataSyncErrorCode.TooLarge */:
                if (error.resource === "keybindings" /* SyncResource.Keybindings */ || error.resource === "settings" /* SyncResource.Settings */ || error.resource === "tasks" /* SyncResource.Tasks */) {
                    ...
                    this.handleTooLargeError(error.resource, localize(17140, null, sourceArea.toLowerCase(), sourceArea.toLowerCase(), '100kb'), error);
                }
                break;
            case "LocalTooManyProfiles" /* UserDataSyncErrorCode.LocalTooManyProfiles */:
                this.disableSync("profiles" /* SyncResource.Profiles */);
                this.notificationService.error(localize(17141, null));
                break;
            ...
            case "UpgradeRequired" /* UserDataSyncErrorCode.UpgradeRequired */: {
                const message = localize(17142, null, this.productService.version, this.productService.commit);
                const operationId = error.operationId ? localize(17143, null, error.operationId) : undefined;
                this.notificationService.notify({
            ...
            case "MethodNotFound" /* UserDataSyncErrorCode.MethodNotFound */: {
                const message = localize(17144, null);
                const operationId = error.operationId ? localize(17145, null, error.operationId) : undefined;
                this.notificationService.notify({
                    severity: Severity.Error,
                    message: operationId ? `${message} ${operationId}` : message,
                    actions: {
            ...
            case "IncompatibleRemoteContent" /* UserDataSyncErrorCode.IncompatibleRemoteContent */:
                this.notificationService.notify({
                    severity: Severity.Error,
                    message: localize(17148, null),
                    actions: {
                        primary: [
                            toAction({
                                id: 'reset',
                                label: localize(17149, null),
                                run: () => this.userDataSyncWorkbenchService.resetSyncedData()
                            }),
                            toAction({
                                id: 'show synced data',
                                label: localize(17150, null),
            ...
            case "DefaultServiceChanged" /* UserDataSyncErrorCode.DefaultServiceChanged */:
                ...
                // If settings sync got turned off then ask user to turn on sync again.
                    ...
                    this.notificationService.notify({
                        severity: Severity.Info,
                        message: localize(17154, null, this.productService.nameLong),
                        actions: {
                            primary: [toAction({
                                    id: 'turn on sync',
                                    label: localize(17155, null),
                                    run: () => this.turnOn()
                                })]
                        }
                    });
    ...
    handleTooLargeError(resource, message, error) {
        const operationId = error.operationId ? localize(17156, null, error.operationId) : undefined;
        this.notificationService.notify({
            severity: Severity.Error,
            message: operationId ? `${message} ${operationId}` : message,
            actions: {
                primary: [toAction({
                        id: 'open sync file',
                        label: localize(17157, null, getSyncAreaLabel(resource)),
    ...
    handleInvalidContentError({ profile, syncResource: source }) {
        ...
        const handle = this.notificationService.notify({
            severity: Severity.Error,
            message: localize(17158, null, errorArea.toLowerCase()),
            actions: {
                primary: [toAction({
                        id: 'open sync file',
                        label: localize(17159, null, errorArea),
    ...
    async updateGlobalActivityBadge() {
        ...
        if (this.userDataSyncService.conflicts.length && this.userDataSyncEnablementService.isEnabled()) {
            badge = new NumberBadge(this.getConflictsCount(), () => localize(17160, null, SYNC_TITLE.value));
        }
        else if (this.turningOnSync) {
            badge = new ProgressBadge(() => localize(17161, null));
        }
        if (badge) {
    ...
    async updateAccountBadge() {
        ...
        if (this.userDataSyncService.status !== "uninitialized" /* SyncStatus.Uninitialized */ && this.userDataSyncEnablementService.isEnabled() && this.userDataSyncWorkbenchService.accountStatus === "unavailable" /* AccountStatus.Unavailable */) {
            badge = new NumberBadge(1, () => localize(17162, null));
        }
        if (badge) {
    ...
    async turnOn() {
        try {
            if (!this.userDataSyncWorkbenchService.authenticationProviders.length) {
                throw new Error(localize(17163, null));
            }
                ...
                switch (e.code) {
                    case "TooLarge" /* UserDataSyncErrorCode.TooLarge */:
                        if (e.resource === "keybindings" /* SyncResource.Keybindings */ || e.resource === "settings" /* SyncResource.Settings */ || e.resource === "tasks" /* SyncResource.Tasks */) {
                            this.handleTooLargeError(e.resource, localize(17164, null, getSyncAreaLabel(e.resource).toLowerCase(), '100kb'), e);
                            return;
                        }
                        break;
                    case "IncompatibleLocalContent" /* UserDataSyncErrorCode.IncompatibleLocalContent */:
                    case "Gone" /* UserDataSyncErrorCode.Gone */:
                    case "UpgradeRequired" /* UserDataSyncErrorCode.UpgradeRequired */: {
                        const message = localize(17165, null, this.productService.version, this.productService.commit);
                        const operationId = e.operationId ? localize(17166, null, e.operationId) : undefined;
                        this.notificationService.notify({
                            severity: Severity.Error,
                            message: operationId ? `${message} ${operationId}` : message,
                        });
                    ...
                    case "IncompatibleRemoteContent" /* UserDataSyncErrorCode.IncompatibleRemoteContent */:
                        this.notificationService.notify({
                            severity: Severity.Error,
                            message: localize(17167, null),
                            actions: {
                                primary: [
                                    toAction({
                                        id: 'reset',
                                        label: localize(17168, null),
                                        run: () => this.userDataSyncWorkbenchService.resetSyncedData()
                                    }),
                                    toAction({
                                        id: 'show synced data',
                                        label: localize(17169, null),
                                        run: () => this.userDataSyncWorkbenchService.showSyncActivity()
                                    })
                                ]
                            }
                    ...
                    case "Forbidden" /* UserDataSyncErrorCode.Forbidden */:
                        this.notificationService.error(localize(17170, null));
                        return;
                }
                this.notificationService.error(localize(17171, null, `command:${SHOW_SYNC_LOG_COMMAND_ID}`));
            }
            else {
                this.notificationService.error(localize(17172, null, getErrorMessage(e)));
            }
        }
    }
    async askToConfigure() {
        return new Promise((c, e) => {
            ...
            quickPick.customLabel = localize(17173, null);
            quickPick.description = localize(17174, null);
    ...
    async configureSyncOptions() {
        return new Promise((c, e) => {
            ...
            const quickPick = this.quickInputService.createQuickPick();
            disposables.add(quickPick);
            quickPick.title = localize(17175, null, SYNC_TITLE.value);
            quickPick.placeholder = localize(17176, null);
            quickPick.canSelectMany = true;
            quickPick.ignoreFocusOut = true;
            quickPick.ok = true;
            const items = this.getConfigureSyncQuickPickItems();
    ...
    async turnOff() {
        const result = await this.dialogService.confirm({
            message: localize(17177, null),
            detail: localize(17178, null),
            primaryButton: localize(17179, null),
            checkbox: this.userDataSyncWorkbenchService.accountStatus === "available" /* AccountStatus.Available */ ? {
                label: localize(17180, null)
            } : undefined
        });
        if (result.confirmed) {
    ...
    async selectSettingsSyncService(userDataSyncStore) {
        return new Promise((c, e) => {
            ...
            quickPick.title = localize(17181, null, SYNC_TITLE.value);
            quickPick.description = localize(17182, null);
            quickPick.hideInput = true;
            quickPick.ignoreFocusOut = true;
            const getDescription = (url) => {
                const isDefault = isEqual(url, userDataSyncStore.defaultUrl);
                if (isDefault) {
                    return localize(17183, null);
                }
                return undefined;
            };
            quickPick.items = [
        ...
        this._register(registerAction2(class TurningOnSyncAction extends Action2 {
            constructor() {
                super({
                    ...
                    title: localize2(17203, 'Backup and Sync Settings...'),
                    category: SYNC_TITLE,
                    f1: true,
                    precondition: when,
                    menu: [{
    ...
    registerSignInAction() {
        ...
        this._register(MenuRegistry.appendMenuItem(MenuId.AccountsContext, {
            ...
            command: {
                id,
                title: localize(17189, null),
            },
            when
        }));
    }
    getShowConflictsTitle() {
        return localize2(17204, "Show Conflicts ({0})", this.getConflictsCount());
    }
        ...
        this._register(registerAction2(class StopSyncAction extends Action2 {
            ...
            async run() {
                try {
                    ...
                    if (!isCancellationError(e)) {
                        that.notificationService.error(localize(17191, null, `command:${SHOW_SYNC_LOG_COMMAND_ID}`));
                    }
                }
            }
        }));
        ...
        this._register(registerAction2(class ShowSyncActivityAction extends Action2 {
            constructor() {
                super({
                    ...
                    title: localize(17193, null, SYNC_TITLE.value),
                    tooltip: localize(17194, null),
                    icon: Codicon.output,
                    menu: [{
                            id: MenuId.CommandPalette,
    ...
    registerDownloadSyncActivityAction() {
        this._register(registerAction2(class DownloadSyncActivityAction extends Action2 {
            ...
            async run(accessor) {
                ...
                if (folder) {
                    notificationService.info(localize(17196, null));
                }
            }
        }));
    }
