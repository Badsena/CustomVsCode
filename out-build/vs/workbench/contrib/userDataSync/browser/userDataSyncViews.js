...
let UserDataSyncDataViews = class UserDataSyncDataViews extends Disposable {
    ...
    registerConflictsView(container) {
        ...
        const viewName = localize2(17240, "Conflicts");
    ...
    registerMachinesView(container) {
        ...
        const name = localize2(17241, "Synced Machines");
    ...
    registerActivityView(container, remote) {
        ...
        const name = remote ? localize2(17242, "Sync Activity (Remote)") : localize2(17243, "Sync Activity (Local)");
        const treeView = this.instantiationService.createInstance(TreeView, id, name.value);
    ...
    registerExternalActivityView(container) {
        ...
        const name = localize2(17244, "Sync Activity (Developer)");
    ...
    registerDataViewActions(viewId) {
        ...
        this._register(registerAction2(class extends Action2 {
            ...
            async run(accessor, handle) {
                ...
                return commandService.executeCommand(API_OPEN_DIFF_EDITOR_COMMAND_ID, remoteResource, localResource, localize(17219, null, localize(17220, null, basename(remoteResource)), localize(17221, null, basename(localResource))), undefined);
            }
        }));
        this._register(registerAction2(class extends Action2 {
            constructor() {
            ...
            async run(accessor, handle) {
                ...
                const result = await dialogService.confirm({
                    message: localize(17223, null, getSyncAreaLabel(syncResource)),
                    type: 'info',
    ...
    registerTroubleShootView(container) {
        ...
        const name = localize2(17245, "Troubleshoot");
        const treeView = this.instantiationService.createInstance(TreeView, id, name.value);
            ...
            return {
                ...
                command: previousResource ? {
                    ...
                    arguments: [
                        previousResource,
                        resource,
                        localize(17225, null, `${basename(resource)} (${fromNow(syncResourceHandle.previous.created, true)})`, `${basename(resource)} (${fromNow(syncResourceHandle.created, true)})`),
                        undefined
                    ]
                } : {
    ...
    async getChildrenForSyncResourceTreeItem(element) {
        ...
        if (children.length) {
            ...
            if (machineId) {
                ...
                children[0].description = machine?.isCurrent ? localize(17226, null) : machine?.name;
            }
        }
        return children;
    }
    ...
    async getChildrenForSyncResourceTreeItem(element) {
        ...
        if (children.length) {
            ...
            if (machineId) {
                const machines = await this.getMachines();
                const machine = machines.find(({ id }) => id === machineId);
                children[0].description = machine?.isCurrent ? localize(17227, null) : machine?.name;
            }
        }
        return children;
    }
...
let UserDataSyncMachinesViewDataProvider = class UserDataSyncMachinesViewDataProvider {
    ...
    async getChildren(element) {
        ...
        try {
            ...
            this.treeView.message = machines.length ? undefined : localize(17228, null);
            return machines.map(({ id, name, isCurrent, platform }) => ({
                handle: id,
                collapsibleState: TreeItemCollapsibleState.None,
                label: { label: name },
                description: isCurrent ? localize(17229, null) : undefined,
    ...
    async disable(machineIds) {
        ...
        if (!machinesToDisable.length) {
            throw new Error(localize(17230, null, machineIds.join(',')));
        }
        const result = await this.dialogService.confirm({
            type: 'info',
            message: machinesToDisable.length > 1 ? localize(17231, null)
                : localize(17232, null, machinesToDisable[0].name),
            primaryButton: localize(17233, null),
        });
        if (!result.confirmed) {
            return false;
        }
    ...
    async rename(machineId) {
        ...
        inputBox.placeholder = localize(17234, null);
        inputBox.busy = true;
        inputBox.show();
        const machines = await this.getMachines();
        const machine = machines.find(({ id }) => id === machineId);
        ...
        if (!machine) {
            ...
            throw new Error(localize(17235, null, machineId));
        }
        inputBox.busy = false;
        ...
        disposableStore.add(inputBox.onDidChangeValue(() => inputBox.validationMessage = validateMachineName(inputBox.value) ? '' : localize(17236, null)));
    ...
    async getChildren(element) {
        if (!element) {
            return [{
                    handle: 'SYNC_LOGS',
                    collapsibleState: TreeItemCollapsibleState.Collapsed,
                    label: { label: localize(17237, null) },
                    themeIcon: Codicon.folder,
                }, {
                    handle: 'LAST_SYNC_STATES',
                    collapsibleState: TreeItemCollapsibleState.Collapsed,
                    label: { label: localize(17238, null) },
                    themeIcon: Codicon.folder,
                }];
        }
    ...
    async getSyncLogs() {
        ...
        for (const syncLogResource of logResources) {
            ...
            result.push({
                ...
                description: this.uriIdentityService.extUri.isEqual(logFolder, this.environmentService.logsHome) ? localize(17239, null) : undefined,
