    ...
    onAutoSyncError(error) {
        switch (error.code) {
            case "LocalTooManyRequests" /* UserDataSyncErrorCode.LocalTooManyRequests */: {
                const message = isWeb ? localize(17119, null, this.productService.nameLong)
                    : localize(17120, null, this.productService.nameLong);
                this.notificationService.notify({
                    ...
                    actions: {
                        primary: [
                            toAction({
                                id: 'Show Sync Logs',
                                label: localize(17121, null),
                                run: () => this.commandService.executeCommand(SHOW_SYNC_LOG_COMMAND_ID)
                            }),
                            toAction({
                                id: 'Restart',
                                label: isWeb ? localize(17122, null) : localize(17123, null),
            ...
            case "RemoteTooManyRequests" /* UserDataSyncErrorCode.TooManyRequests */: {
                const operationId = error.operationId ? localize(17124, null, error.operationId) : undefined;
                const message = localize(17125, null);
                this.notificationService.notify({
                    severity: Severity.Error,
                    message: operationId ? `${message} ${operationId}` : message,
                    source: error.operationId ? localize(17126, null, error.operationId) : undefined,
                    actions: {
                        primary: [
                            toAction({
                                id: 'Show Sync Logs',
                                label: localize(17127, null),
                                run: () => this.commandService.executeCommand(SHOW_SYNC_LOG_COMMAND_ID)
                            })
                        ]
                    }
