...
const LOCAL_HISTORY_CATEGORY = localize2(11898, 'Local History');
const CTX_LOCAL_HISTORY_ENABLED = ContextKeyExpr.has('config.workbench.localHistory.enabled');
//#region Compare with File
export const COMPARE_WITH_FILE_LABEL = localize2(11899, 'Compare with File');
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: localize2(11900, 'Compare with Previous'),
            menu: {
                id: MenuId.TimelineItemContext,
                group: '1_compare',
                order: 2,
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: localize2(11901, 'Select for Compare'),
            menu: {
                id: MenuId.TimelineItemContext,
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: localize2(11902, 'Compare with Selected'),
            menu: {
                id: MenuId.TimelineItemContext,
                group: '2_compare_with',
                order: 1,
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: localize2(11903, 'Show Contents'),
            menu: {
                id: MenuId.TimelineItemContext,
                group: '3_contents',
                order: 1,
...
const RESTORE_CONTENTS_LABEL = localize2(11904, 'Restore Contents');
registerAction2(class extends Action2 {
    constructor() {
        super({
...
const restoreSaveSource = SaveSourceRegistry.registerSource('localHistoryRestore.source', localize(11878, null));
async function restore(accessor, item) {
    ...
    if (entry) {
        ...
        const { confirmed } = await dialogService.confirm({
            type: 'warning',
            message: localize(11879, null, basename(entry.workingCopy.resource)),
            detail: localize(11880, null),
            primaryButton: localize(11881, null)
        });
        if (!confirmed) {
            return;
        }
        ...
        try {
            ...
            await dialogService.error(localize(11882, null, basename(entry.workingCopy.resource)), toErrorMessage(error));
            return;
        }
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: localize2(11905, 'Find Entry to Restore'),
            f1: true,
    ...
    async run(accessor) {
        ...
        resourcePicker.placeholder = localize(11883, null);
        ...
        entryPicker.placeholder = localize(11884, null);
...
MenuRegistry.appendMenuItem(MenuId.TimelineTitle, { command: { id: 'workbench.action.localHistory.restoreViaPicker', title: localize2(11906, 'Local History: Find Entry to Restore...') }, group: 'submenu', order: 1, when: CTX_LOCAL_HISTORY_ENABLED });
//#endregion
//#region Rename
registerAction2(class extends Action2 {
    constructor() {
    ...
    async run(accessor, item) {
        ...
        if (entry) {
            ...
            inputBox.title = localize(11885, null);
            inputBox.ignoreFocusOut = true;
            inputBox.placeholder = localize(11886, null);
...
registerAction2(class extends Action2 {
    ...
    async run(accessor, item) {
        ...
        if (entry) {
            ...
            const { confirmed } = await dialogService.confirm({
                type: 'warning',
                message: localize(11887, null, entry.workingCopy.name, toLocalHistoryEntryDateLabel(entry.timestamp)),
                detail: localize(11888, null),
                primaryButton: localize(11889, null),
            });
            if (!confirmed) {
                return;
            }
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: localize2(11909, 'Delete All'),
            f1: true,
            category: LOCAL_HISTORY_CATEGORY,
            precondition: CTX_LOCAL_HISTORY_ENABLED
        });
    ...
    async run(accessor) {
        ...
        const { confirmed } = await dialogService.confirm({
            type: 'warning',
            message: localize(11890, null),
            detail: localize(11891, null),
            primaryButton: localize(11892, null),
        });
        if (!confirmed) {
            return;
        }
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: localize2(11910, 'Create Entry'),
            f1: true,
            category: LOCAL_HISTORY_CATEGORY,
    ...
    async run(accessor) {
        ...
        const disposables = new DisposableStore();
        const inputBox = disposables.add(quickInputService.createInputBox());
        inputBox.title = localize(11893, null);
        inputBox.ignoreFocusOut = true;
        inputBox.placeholder = localize(11894, null, labelService.getUriBasenameLabel(resource));
        inputBox.show();
...
async function openEntry(entry, editorService, options) {
    ...
    await editorService.openEditor({
        resource,
        label: localize(11895, null, entry.workingCopy.name, SaveSourceRegistry.getSourceLabel(entry.source), toLocalHistoryEntryDateLabel(entry.timestamp)),
        options
    });
}
...
export function toDiffEditorArguments(arg1, arg2, options) {
    ...
    if (URI.isUri(arg2)) {
        ...
        label = localize(11896, null, arg1.workingCopy.name, SaveSourceRegistry.getSourceLabel(arg1.source), toLocalHistoryEntryDateLabel(arg1.timestamp), arg1.workingCopy.name);
    }
    // Compare with another entry
        ...
        label = localize(11897, null, arg1.workingCopy.name, SaveSourceRegistry.getSourceLabel(arg1.source), toLocalHistoryEntryDateLabel(arg1.timestamp), modified.workingCopy.name, SaveSourceRegistry.getSourceLabel(modified.source), toLocalHistoryEntryDateLabel(modified.timestamp));
    }
    return [
        originalResource,
        modifiedResource,
