...
const configurationExtPoint = ExtensionsRegistry.registerExtensionPoint({
    ...
    jsonSchema: {
        description: nls.localize(3578, null),
        type: 'array',
        ...
        items: {
            ...
            properties: {
                fileMatch: {
                    type: ['string', 'array'],
                    description: nls.localize(3579, null),
                    items: {
                        type: ['string']
                    }
                },
                url: {
                    description: nls.localize(3580, null),
                    type: 'string'
                }
            }
        }
                    ...
                    if (uri.startsWith('./')) {
                        try {
                            ...
                            if (!resources.isEqualOrParent(colorThemeLocation, extensionLocation)) {
                                collector.warn(nls.localize(3584, null, configurationExtPoint.name, colorThemeLocation.toString(), extensionLocation.path));
                            }
                        }
                        catch (e) {
                            collector.error(nls.localize(3585, null, e.message));
                        }
                    }
                    else if (!/^[^:/?#]+:\/\//.test(uri)) {
                        collector.error(nls.localize(3586, null));
                        return;
                    }
                });
            }
...
class JSONValidationDataRenderer extends Disposable {
    ...
    render(manifest) {
        ...
        const headers = [
            nls.localize(3587, null),
            nls.localize(3588, null),
        ];
