...
let MainThreadWorkspace = class MainThreadWorkspace extends Disposable {
    ...
    getStatusMessage(extensionName, addCount, removeCount) {
        ...
        if (wantsToAdd && !wantsToDelete) {
            if (addCount === 1) {
                message = localize(3418, null, extensionName);
            }
            else {
                message = localize(3419, null, extensionName, addCount);
            }
        }
        // Delete Folders
        else if (wantsToDelete && !wantsToAdd) {
            if (removeCount === 1) {
                message = localize(3420, null, extensionName);
            }
            else {
                message = localize(3421, null, extensionName, removeCount);
            }
        }
        // Change Folders
        else {
            message = localize(3422, null, extensionName);
        }
        return message;
    }
