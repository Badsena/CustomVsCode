...
const statusBarItemSchema = {
    ...
    properties: {
        ...
        name: {
            type: 'string',
            description: localize(3424, null)
        },
        text: {
            type: 'string',
            description: localize(3425, null)
        },
        tooltip: {
            type: 'string',
            description: localize(3426, null)
        },
        command: {
            type: 'string',
            description: localize(3427, null)
        },
        alignment: {
            type: 'string',
            enum: ['left', 'right'],
            description: localize(3428, null)
        },
        priority: {
            type: 'number',
            description: localize(3429, null)
        },
        accessibilityInformation: {
            type: 'object',
            description: localize(3430, null),
            required: ['label'],
            properties: {
                role: {
                    type: 'string',
                    description: localize(3431, null)
                },
                label: {
                    type: 'string',
                    description: localize(3432, null)
                }
            }
        }
    }
...
const statusBarItemsSchema = {
    description: localize(3433, null),
    oneOf: [
        statusBarItemSchema,
        {
            type: 'array',
            ...
            for (const entry of extensions) {
                ...
                for (const candidate of Iterable.wrap(value)) {
                    if (!isUserFriendlyStatusItemEntry(candidate)) {
                        collector.error(localize(3434, null));
                        continue;
                    }
