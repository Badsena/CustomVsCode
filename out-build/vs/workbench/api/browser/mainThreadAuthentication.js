...
let MainThreadAuthentication = class MainThreadAuthentication extends Disposable {
    ...
    $showContinueNotification(message) {
        const yes = nls.localize(3329, null);
        const no = nls.localize(3330, null);
        const deferredPromise = new DeferredPromise();
        let result = false;
    ...
    async loginPrompt(provider, extensionName, recreatingSession, options) {
        ...
        if (customMessage) {
            ...
            message = recreatingSession
                ? nls.localize(3331, null, extensionName, provider.label)
                : nls.localize(3332, null, extensionName, provider.label);
        }
        const buttons = [
            {
                label: nls.localize(3333, null),
                run() {
                    return true;
                },
            }
    ...
    async continueWithIncorrectAccountPrompt(chosenAccountLabel, requestedAccountLabel) {
        const result = await this.dialogService.prompt({
            message: nls.localize(3335, null),
            detail: nls.localize(3336, null, chosenAccountLabel, requestedAccountLabel),
            type: Severity.Warning,
            cancelButton: true,
            buttons: [
                {
                    label: nls.localize(3337, null, chosenAccountLabel),
                    run: () => chosenAccountLabel
                },
                {
                    label: nls.localize(3338, null, requestedAccountLabel),
    ...
    async $showDeviceCodeModal(userCode, verificationUri) {
        const { result } = await this.dialogService.prompt({
            type: Severity.Info,
            message: nls.localize(3339, null),
            detail: nls.localize(3340, null, userCode, verificationUri),
            buttons: [
                {
                    label: nls.localize(3341, null),
                    run: () => true
                }
            ],
            cancelButton: true
        ...
        if (result) {
            ...
            try {
                ...
                this.notificationService.error(nls.localize(3342, null, verificationUri));
            }
        }
        return false;
    }
    async $promptForClientRegistration(authorizationServerUrl) {
        ...
        const result = await this.dialogService.prompt({
            type: Severity.Info,
            message: nls.localize(3343, null),
            detail: nls.localize(3344, null, authorizationServerUrl, redirectUrls),
            buttons: [
                {
                    label: nls.localize(3345, null),
                    run: async () => {
                        try {
                            await this.clipboardService.writeText(redirectUrls);
                        }
                        catch (error) {
                            this.notificationService.error(nls.localize(3346, null));
                        }
                        return true;
                    }
                },
        ...
        const sharedTitle = nls.localize(3348, null);
        const clientId = await this.quickInputService.input({
            title: sharedTitle,
            prompt: nls.localize(3349, null),
            placeHolder: nls.localize(3350, null),
            ignoreFocusLost: true,
            validateInput: async (value) => {
                if (!value || value.trim().length === 0) {
                    return nls.localize(3351, null);
                }
                return undefined;
            }
        });
        ...
        const clientSecret = await this.quickInputService.input({
            title: sharedTitle,
            prompt: nls.localize(3352, null, clientId),
            placeHolder: nls.localize(3353, null),
            password: true,
            ignoreFocusLost: true
        });
        return {
