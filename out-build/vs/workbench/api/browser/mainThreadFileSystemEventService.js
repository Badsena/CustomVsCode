                ...
                if (showPreview === undefined) {
                    ...
                    if (data.extensionNames.length === 1) {
                        if (operation === 0 /* FileOperation.CREATE */) {
                            message = localize(3372, null, data.extensionNames[0]);
                        }
                        else if (operation === 3 /* FileOperation.COPY */) {
                            message = localize(3373, null, data.extensionNames[0]);
                        }
                        else if (operation === 2 /* FileOperation.MOVE */) {
                            message = localize(3374, null, data.extensionNames[0]);
                        }
                        else /* if (operation === FileOperation.DELETE) */ {
                            message = localize(3375, null, data.extensionNames[0]);
                        }
                    }
                    else {
                        if (operation === 0 /* FileOperation.CREATE */) {
                            message = localize(3376, null, data.extensionNames.length);
                        }
                        else if (operation === 3 /* FileOperation.COPY */) {
                            message = localize(3377, null, data.extensionNames.length);
                        }
                        else if (operation === 2 /* FileOperation.MOVE */) {
                            message = localize(3378, null, data.extensionNames.length);
                        }
                        else /* if (operation === FileOperation.DELETE) */ {
                            message = localize(3379, null, data.extensionNames.length);
                        }
                    }
                    if (needsConfirmation) {
                        ...
                        const { confirmed } = await dialogService.confirm({
                            type: Severity.Info,
                            message,
                            primaryButton: localize(3380, null),
                            cancelButton: localize(3381, null)
                        });
                        showPreview = true;
                        if (!confirmed) {
                            // no changes wanted
                        ...
                        const { result, checkboxChecked } = await dialogService.prompt({
                            ...
                            buttons: [
                                {
                                    label: localize(3382, null),
                                    run: () => Choice.OK
                                },
                                {
                                    label: localize(3383, null),
                                    run: () => Choice.Preview
                                }
                            ],
                            cancelButton: {
                                label: localize(3384, null),
                                run: () => Choice.Cancel
                            },
                            checkbox: { label: localize(3385, null) }
                        });
            ...
            _progressLabel(operation) {
                switch (operation) {
                    case 0 /* FileOperation.CREATE */:
                        return localize(3386, null);
                    case 2 /* FileOperation.MOVE */:
                        return localize(3387, null);
                    case 3 /* FileOperation.COPY */:
                        return localize(3388, null);
                    case 1 /* FileOperation.DELETE */:
                        return localize(3389, null);
                    case 4 /* FileOperation.WRITE */:
                        return localize(3390, null);
                }
            }
        };
