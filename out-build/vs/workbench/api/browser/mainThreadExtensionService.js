...
let MainThreadExtensionService = class MainThreadExtensionService {
    ...
    async _handleMissingInstalledDependency(extension, missingInstalledDependency) {
        ...
        if (this._extensionEnablementService.isEnabled(missingInstalledDependency)) {
            this._notificationService.notify({
                ...
                message: localize(3361, null, extName, missingInstalledDependency.manifest.displayName || missingInstalledDependency.manifest.name),
                actions: {
                    primary: [toAction({ id: 'reload', label: localize(3362, null), run: () => this._hostService.reload() })]
                }
            });
        }
        else {
            ...
            if (enablementState === 5 /* EnablementState.DisabledByVirtualWorkspace */) {
                this._notificationService.notify({
                    severity: Severity.Error,
                    message: localize(3363, null, extName, missingInstalledDependency.manifest.displayName || missingInstalledDependency.manifest.name),
                });
            }
            else if (enablementState === 0 /* EnablementState.DisabledByTrustRequirement */) {
                this._notificationService.notify({
                    severity: Severity.Error,
                    message: localize(3364, null, extName, missingInstalledDependency.manifest.displayName || missingInstalledDependency.manifest.name),
                    actions: {
                        primary: [toAction({ id: 'manageWorkspaceTrust', label: localize(3365, null), run: () => this._commandService.executeCommand('workbench.trust.manage') })]
                    }
                });
            }
                ...
                this._notificationService.notify({
                    severity: Severity.Error,
                    message: localize(3366, null, extName, missingInstalledDependency.manifest.displayName || missingInstalledDependency.manifest.name),
                    actions: {
                        primary: [toAction({
                                id: 'enable', label: localize(3367, null), enabled: true,
                ...
                this._notificationService.notify({
                    severity: Severity.Error,
                    message: localize(3368, null, extName, missingInstalledDependency.manifest.displayName || missingInstalledDependency.manifest.name),
                });
            }
        }
    }
    async _handleMissingNotInstalledDependency(extension, missingDependency) {
        ...
        if (dependencyExtension) {
            this._notificationService.notify({
                severity: Severity.Error,
                message: localize(3369, null, extName, dependencyExtension.displayName, dependencyExtension.publisherDisplayName),
                actions: {
                    primary: [toAction({
                            id: 'install',
                            label: localize(3370, null),
            ...
            this._notificationService.error(localize(3371, null, extName, missingDependency));
        }
    }
