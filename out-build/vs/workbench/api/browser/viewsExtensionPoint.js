...
const viewsContainerSchema = {
    ...
    properties: {
        id: {
            description: localize(3435, null),
            type: 'string',
            pattern: '^[a-zA-Z0-9_-]+$'
        },
        title: {
            description: localize(3436, null),
            type: 'string'
        },
        icon: {
            description: localize(3437, null),
            type: 'string'
        }
    },
...
export const viewsContainersContribution = {
    description: localize(3438, null),
    type: 'object',
    properties: {
        'activitybar': {
            description: localize(3439, null),
            type: 'array',
            items: viewsContainerSchema
        },
        'panel': {
            description: localize(3440, null),
            type: 'array',
            items: viewsContainerSchema
        },
        'secondarySidebar': {
            description: localize(3441, null),
            type: 'array',
            items: viewsContainerSchema
        }
    },
...
const viewDescriptor = {
    ...
    properties: {
        type: {
            markdownDescription: localize(3442, null),
            type: 'string',
            enum: [
                'tree',
                'webview',
        ...
        id: {
            markdownDescription: localize(3445, null),
            type: 'string'
        },
        name: {
            description: localize(3446, null),
            type: 'string'
        },
        when: {
            description: localize(3447, null),
            type: 'string'
        },
        icon: {
            description: localize(3448, null),
            type: 'string'
        },
        contextualTitle: {
            description: localize(3449, null),
            type: 'string'
        },
        visibility: {
            description: localize(3450, null),
            type: 'string',
            enum: [
                'visible',
                'hidden',
        ...
        initialSize: {
            type: 'number',
            description: localize(3454, null),
        },
        accessibilityHelpContent: {
            type: 'string',
            markdownDescription: localize(3455, null)
        }
    }
};
const remoteViewDescriptor = {
    ...
    properties: {
        id: {
            description: localize(3456, null),
            type: 'string'
        },
        name: {
            description: localize(3457, null),
            type: 'string'
        },
        when: {
            description: localize(3458, null),
            type: 'string'
        },
        group: {
            description: localize(3459, null),
            type: 'string'
        },
        remoteName: {
            description: localize(3460, null),
            type: ['string', 'array'],
            items: {
                type: 'string'
            }
...
const viewsContribution = {
    description: localize(3461, null),
    type: 'object',
    properties: {
        'explorer': {
            description: localize(3462, null),
            type: 'array',
            items: viewDescriptor,
            default: []
        },
        'debug': {
            description: localize(3463, null),
            type: 'array',
            items: viewDescriptor,
            default: []
        },
        'scm': {
            description: localize(3464, null),
            type: 'array',
            items: viewDescriptor,
            default: []
        },
        'test': {
            description: localize(3465, null),
            type: 'array',
            items: viewDescriptor,
            default: []
        },
        'remote': {
            description: localize(3466, null),
            type: 'array',
    ...
    additionalProperties: {
        description: localize(3467, null),
        type: 'array',
        items: viewDescriptor,
        default: []
    }
...
let ViewsExtensionHandler = class ViewsExtensionHandler {
    ...
    isValidViewsContainer(viewsContainersDescriptors, collector) {
        if (!Array.isArray(viewsContainersDescriptors)) {
            collector.error(localize(3468, null));
            return false;
        }
        for (const descriptor of viewsContainersDescriptors) {
            if (typeof descriptor.id !== 'string' && isFalsyOrWhitespace(descriptor.id)) {
                collector.error(localize(3469, null, 'id'));
                return false;
            }
            if (!(/^[a-z0-9_-]+$/i.test(descriptor.id))) {
                collector.error(localize(3470, null, 'id'));
                return false;
            }
            if (typeof descriptor.title !== 'string') {
                collector.error(localize(3471, null, 'title'));
                return false;
            }
            if (typeof descriptor.icon !== 'string') {
                collector.error(localize(3472, null, 'icon'));
                return false;
            }
            if (isFalsyOrWhitespace(descriptor.title)) {
                collector.warn(localize(3473, null, 'title'));
                return true;
            }
        }
        return true;
        ...
        for (const extension of extensions) {
            ...
            Object.entries(value).forEach(([key, value]) => {
                ...
                if (key === 'remote' && !isProposedApiEnabled(extension.description, 'contribViewsRemote')) {
                    collector.warn(localize(3474, null, key));
                    return;
                }
                if (key === 'agentSessions' && !isProposedApiEnabled(extension.description, 'chatSessionsProvider')) {
                    collector.warn(localize(3475, null, key));
                    return;
                }
                const viewContainer = this.getViewContainer(key);
                if (!viewContainer) {
                    collector.warn(localize(3476, null, key));
                }
                ...
                for (let index = 0; index < value.length; index++) {
                    ...
                    if (viewIds.has(item.id)) {
                        collector.error(localize(3477, null, item.id));
                        continue;
                    }
                    if (this.viewsRegistry.getView(item.id) !== null) {
                        collector.error(localize(3478, null, item.id));
                        continue;
                    }
                    ...
                    if (!type) {
                        collector.error(localize(3479, null, item.type));
                        continue;
                    }
    ...
    isValidViewDescriptors(viewDescriptors, collector) {
        if (!Array.isArray(viewDescriptors)) {
            collector.error(localize(3480, null));
            return false;
        }
        for (const descriptor of viewDescriptors) {
            if (typeof descriptor.id !== 'string') {
                collector.error(localize(3481, null, 'id'));
                return false;
            }
            if (typeof descriptor.name !== 'string') {
                collector.error(localize(3482, null, 'name'));
                return false;
            }
            if (descriptor.when && typeof descriptor.when !== 'string') {
                collector.error(localize(3483, null, 'when'));
                return false;
            }
            if (descriptor.icon && typeof descriptor.icon !== 'string') {
                collector.error(localize(3484, null, 'icon'));
                return false;
            }
            if (descriptor.contextualTitle && typeof descriptor.contextualTitle !== 'string') {
                collector.error(localize(3485, null, 'contextualTitle'));
                return false;
            }
            if (descriptor.visibility && !this.convertInitialVisibility(descriptor.visibility)) {
                collector.error(localize(3486, null, 'visibility', Object.values(InitialVisibility).join(', ')));
                return false;
            }
        }
        return true;
