...
let MainThreadMcp = class MainThreadMcp extends Disposable {
    ...
    async continueWithIncorrectAccountPrompt(chosenAccountLabel, requestedAccountLabel) {
        const result = await this.dialogService.prompt({
            message: nls.localize(3394, null),
            detail: nls.localize(3395, null, chosenAccountLabel, requestedAccountLabel),
            type: Severity.Warning,
            cancelButton: true,
            buttons: [
                {
                    label: nls.localize(3396, null, chosenAccountLabel),
                    run: () => chosenAccountLabel
                },
                {
                    label: nls.localize(3397, null, requestedAccountLabel),
        ...
        for (const { serverId, scopes } of serversUsingProvider) {
            ...
            try {
                ...
                if (UserInteractionRequiredError.is(e)) {
                    ...
                    server.pushLog(LogLevel.Warning, nls.localize(3398, null, providerLabel));
                    server.stop();
                }
    ...
    async loginPrompt(mcpLabel, providerLabel, recreatingSession) {
        const message = recreatingSession
            ? nls.localize(3399, null, mcpLabel, providerLabel)
            : nls.localize(3400, null, mcpLabel, providerLabel);
        const buttons = [
            {
                label: nls.localize(3401, null),
                run() {
                    return true;
                },
            }
