...
suite('ObservableChatSession', function () {
    ...
    test('initialization sets forkSession and revives forked items', async function () {
        ...
        proxy.$forkChatSession.resolves(forkedItem);
        ...
        assert.ok(proxy.$forkChatSession.calledOnceWithExactly(1, session.sessionResource, expectedRequestDto, CancellationToken.None));
        assert.ok(result);
    ...
    test('initialization forwards initial session options context', async function () {
        ...
        proxy.$provideChatSessionContent.resolves(sessionContent);
        await session.initialize(CancellationToken.None, { initialSessionOptions });
        assert.ok(proxy.$provideChatSessionContent.calledOnceWith(1, resource, { initialSessionOptions }, CancellationToken.None));
    });
...
suite('MainThreadChatSessions', function () {
    ...
    test('$onDidChangeChatSessionProviderOptions refreshes option groups', async function () {
        ...
        const provideOptionsStub = proxy.$provideChatSessionProviderOptions;
    ...
    test('option change notifications are sent to the extension', async function () {
        ...
        proxy.$provideHandleOptionsChange.resetHistory();
        ...
        assert.ok(proxy.$provideHandleOptionsChange.calledOnce);
        const call = proxy.$provideHandleOptionsChange.firstCall;
    ...
    test('option change notifications fail silently when provider not registered', async function () {
        ...
        assert.strictEqual(proxy.$provideHandleOptionsChange.callCount, 0);
    });
    ...
    test('hasAnySessionOptions returns correct values', async function () {
        ...
        proxy.$provideChatSessionContent
...
//# sourceMappingURL=mainThreadChatSessions.test.js.map