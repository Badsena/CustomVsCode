...
const shellCommandCategory = localize2(17746, 'Shell Command');
export class InstallShellScriptAction extends Action2 {
    constructor() {
        super({
            id: 'workbench.action.installCommandLine',
            title: localize2(17747, "Install '{0}' command in PATH", product.applicationName),
    ...
    async run(accessor) {
        ...
        try {
            ...
            dialogService.info(localize(17744, null, productService.applicationName));
        }
        catch (error) {
...
export class UninstallShellScriptAction extends Action2 {
    constructor() {
        super({
            ...
            title: localize2(17748, "Uninstall '{0}' command from PATH", product.applicationName),
            category: shellCommandCategory,
            f1: true
        });
    }
    async run(accessor) {
        ...
        try {
            ...
            dialogService.info(localize(17745, null, productService.applicationName));
        }
        catch (error) {
            if (isCancellationError(error)) {
                return;
