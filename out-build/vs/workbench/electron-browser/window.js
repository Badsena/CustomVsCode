...
let NativeWindow = NativeWindow_1 = class NativeWindow extends BaseWindow {
    constructor(editorService, editorGroupService, configurationService, titleService, themeService, notificationService, commandService, keybindingService, telemetryService, workspaceEditingService, fileService, menuService, lifecycleService, integrityService, nativeEnvironmentService, accessibilityService, contextService, openerService, nativeHostService, tunnelService, layoutService, workingCopyService, filesConfigurationService, productService, remoteAuthorityResolverService, dialogService, storageService, logService, instantiationService, sharedProcessService, progressService, labelService, bannerService, uriIdentityService, preferencesService, utilityProcessWorkerWorkbenchService, hostService, contextMenuService) {
        ...
        console.log("Welcome to Amypo Coder - Ready!");
    ...
    registerListeners() {
        ...
        ipcRenderer.on('vscode:reportSharedProcessCrash', (event, ...argsRaw) => {
            this.notificationService.prompt(Severity.Error, localize(17851, null), [{
                    label: localize(17852, null),
        ...
        ipcRenderer.on('vscode:showResolveShellEnvError', (event, ...argsRaw) => {
            ...
            this.notificationService.prompt(Severity.Error, message, [{
                    label: localize(17853, null),
                    run: () => this.nativeHostService.relaunch()
                },
                {
                    label: localize(17854, null),
                    run: () => this.preferencesService.openUserSettings({ query: 'application.shellEnvironmentResolutionTimeout' })
                },
                {
                    label: localize(17855, null),
                    run: () => this.openerService.open('https://go.microsoft.com/fwlink/?linkid=2149667')
                }]);
        });
        ipcRenderer.on('vscode:showCredentialsError', (event, ...argsRaw) => {
            const message = argsRaw[0];
            this.notificationService.prompt(Severity.Error, localize(17856, null, message), [{
                    label: localize(17857, null),
                    run: () => this.openerService.open('https://go.microsoft.com/fwlink/?linkid=2190713')
                }]);
        });
        ipcRenderer.on('vscode:showTranslatedBuildWarning', () => {
            this.notificationService.prompt(Severity.Warning, localize(17858, null, this.productService.nameLong), [{
                    label: localize(17859, null),
                    run: () => {
        ...
        ipcRenderer.on('vscode:showArgvParseWarning', () => {
            this.notificationService.prompt(Severity.Warning, localize(17860, null), [{
                    label: localize(17861, null),
        ...
        ipcRenderer.on('vscode:openProxyAuthenticationDialog', async (event, ...argsRaw) => {
            ...
            const result = await this.dialogService.input({
                type: 'warning',
                message: localize(17862, null),
                primaryButton: localize(17863, null),
                inputs: [
                    { placeholder: localize(17864, null), value: payload.username },
                    { placeholder: localize(17865, null), type: 'password', value: payload.password }
                ],
                detail: localize(17866, null, `${payload.authInfo.host}:${payload.authInfo.port}`),
                checkbox: {
                    label: localize(17867, null),
    ...
    onBeforeShutdownError({ error, reason }) {
        this.dialogService.error(this.toShutdownLabel(reason, true), localize(17868, null, toErrorMessage(error)));
    }
    onWillShutdown({ reason, force, joiners }) {
        ...
        const shutdownDialogScheduler = new RunOnceScheduler(() => {
            ...
            this.progressService.withProgress({
                ...
                title: this.toShutdownLabel(reason, false),
                detail: pendingJoiners.length > 0 ? localize(17869, null, pendingJoiners.map(joiner => `- ${joiner.label}`).join('\n')) : undefined
            }, () => {
        ...
        if (isError) {
            switch (reason) {
                case 1 /* ShutdownReason.CLOSE */:
                    return localize(17870, null);
                case 2 /* ShutdownReason.QUIT */:
                    return localize(17871, null);
                case 3 /* ShutdownReason.RELOAD */:
                    return localize(17872, null);
                case 4 /* ShutdownReason.LOAD */:
                    return localize(17873, null);
            }
        }
        switch (reason) {
            case 1 /* ShutdownReason.CLOSE */:
                return localize(17874, null);
            case 2 /* ShutdownReason.QUIT */:
                return localize(17875, null);
            case 3 /* ShutdownReason.RELOAD */:
                return localize(17876, null);
            case 4 /* ShutdownReason.LOAD */:
                return localize(17877, null);
        }
    }
    toForceShutdownLabel(reason) {
        switch (reason) {
            case 1 /* ShutdownReason.CLOSE */:
                return localize(17878, null);
            case 2 /* ShutdownReason.QUIT */:
                return localize(17879, null);
            case 3 /* ShutdownReason.RELOAD */:
                return localize(17880, null);
            case 4 /* ShutdownReason.LOAD */:
                return localize(17881, null);
        }
    }
    //#endregion
    ...
    async handleWarnings() {
        ...
        (async () => {
            ...
            if (isAdmin && !isWindows) {
                this.notificationService.warn(localize(17882, null, this.productService.nameShort));
            }
        })();
            ...
            for (const folder of this.contextService.getWorkspace().folders) {
                if (this.uriIdentityService.extUri.isEqualOrParent(folder.uri, installLocationUri)) {
                    this.bannerService.show({
                        ...
                        message: localize(17883, null, this.labelService.getUriLabel(installLocationUri)),
        ...
        if (isMacintosh) {
            ...
            if (eolReleases.has(majorVersion)) {
                const message = localize(17884, null, this.productService.nameLong, eolReleases.get(majorVersion));
                this.notificationService.prompt(Severity.Warning, message, [{
                        label: localize(17885, null),
        ...
        this.progressService.withProgress({
            title: localize(17886, null),
            location: 10 /* ProgressLocation.Window */,
            delay: 1600,
            buttons: [localize(17887, null)]
...
let ZoomStatusEntry = class ZoomStatusEntry extends Disposable {
    ...
    createZoomEntry(visibleOrText) {
        ...
        const zoomOutAction = disposables.add(new Action('workbench.action.zoomOut', localize(17888, null), ThemeIcon.asClassName(Codicon.remove), true, () => this.commandService.executeCommand(zoomOutAction.id)));
        const zoomInAction = disposables.add(new Action('workbench.action.zoomIn', localize(17889, null), ThemeIcon.asClassName(Codicon.plus), true, () => this.commandService.executeCommand(zoomInAction.id)));
        const zoomResetAction = disposables.add(new Action('workbench.action.zoomReset', localize(17890, null), undefined, true, () => this.commandService.executeCommand(zoomResetAction.id)));
        zoomResetAction.tooltip = this.keybindingService.appendKeybinding(zoomResetAction.label, zoomResetAction.id);
        const zoomSettingsAction = disposables.add(new Action('workbench.action.openSettings', localize(17891, null), ThemeIcon.asClassName(Codicon.settingsGear), true, () => this.commandService.executeCommand(zoomSettingsAction.id, 'window.zoom')));
        ...
        const name = localize(17892, null);
    ...
    updateZoomLevelLabel(targetWindowId) {
        if (this.zoomLevelLabel) {
            ...
            this.zoomLevelLabel.tooltip = localize(17893, null, zoomLevel, zoomFactor);
        }
    }
};
