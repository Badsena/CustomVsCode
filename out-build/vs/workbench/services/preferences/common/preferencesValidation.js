    ...
    return value => {
        ...
        if (prop.type === 'string') {
            if (prop.enum && !isStringArray(prop.enum)) {
                errors.push(nls.localize(18625, null));
            }
            else if (!isString(value)) {
                errors.push(nls.localize(18626, null));
            }
            else {
...
export function getInvalidTypeError(value, type) {
    ...
    if (!typeArr.some(_type => valueValidatesAsType(value, _type))) {
        return nls.localize(18627, null, JSON.stringify(type));
    }
    return;
}
...
function toRegExp(pattern) {
    try {
        ...
        try {
            ...
            console.error(nls.localize(18628, null), pattern);
            return /.*/;
        }
    }
}
function getStringValidators(prop) {
    ...
    return [
        ...
        },
        {
            ...
            message: nls.localize(18635, null)
        },
        {
            ...
            message: nls.localize(18636, null, prop.enum ? prop.enum.map(key => `"${key}"`).join(', ') : '[]')
        }
        ...
        if (propItems && !Array.isArray(propItems.type)) {
            ...
            return value => {
                ...
                if (!Array.isArray(value)) {
                    message += nls.localize(18643, null);
                    message += '\n';
                    return message;
                }
                ...
                if (prop.uniqueItems) {
                    if (new Set(arrayValue).size < arrayValue.length) {
                        message += nls.localize(18644, null);
                        message += '\n';
                    }
                }
                if (prop.minItems && arrayValue.length < prop.minItems) {
                    message += nls.localize(18645, null, prop.minItems);
                    message += '\n';
                }
                if (prop.maxItems && arrayValue.length > prop.maxItems) {
                    message += nls.localize(18646, null, prop.maxItems);
                    message += '\n';
                }
                if (propItems.type === 'string') {
                    if (!isStringArray(arrayValue)) {
                        message += nls.localize(18647, null);
                        message += '\n';
                        return message;
                    }
                            ...
                            if (!patternRegex.test(v)) {
                                message +=
                                    propItems.patternErrorMessage ||
                                        nls.localize(18648, null, withQuotes(v), withQuotes(propItems.pattern));
                            }
                        });
                    }
                    ...
                    if (propItemsEnum) {
                        arrayValue.forEach(v => {
                            if (propItemsEnum.indexOf(v) === -1) {
                                message += nls.localize(18649, null, withQuotes(v), '[' + propItemsEnum.map(withQuotes).join(', ') + ']');
                                message += '\n';
                            }
                        });
                    }
...
function getObjectValidator(prop) {
    if (prop.type === 'object') {
        ...
        return value => {
            ...
            if (!isObject(value)) {
                errors.push(nls.localize(18650, null));
            }
            else {
                Object.keys(value).forEach((key) => {
                    ...
                    if (propertyNames?.pattern && !propertyNamesErrorShown) {
                        ...
                        if (!patternRegex.test(key)) {
                            const errorMessage = propertyNames.patternErrorMessage ||
                                nls.localize(18651, null, propertyNames.pattern);
                    ...
                    if (additionalProperties === false) {
                        errors.push(nls.localize(18652, null, key));
                    }
