...
(function (OpenLocalFileCommand) {
    ...
    OpenLocalFileCommand.LABEL = nls.localize(18210, null);
    function handler() {
        return accessor => {
...
(function (SaveLocalFileCommand) {
    ...
    SaveLocalFileCommand.LABEL = nls.localize(18211, null);
    function handler() {
        return accessor => {
...
(function (OpenLocalFolderCommand) {
    ...
    OpenLocalFolderCommand.LABEL = nls.localize(18212, null);
    function handler() {
        return accessor => {
            const dialogService = accessor.get(IFileDialogService);
...
(function (OpenLocalFileFolderCommand) {
    ...
    OpenLocalFileFolderCommand.LABEL = nls.localize(18213, null);
    function handler() {
        return accessor => {
            const dialogService = accessor.get(IFileDialogService);
...
let SimpleFileDialog = class SimpleFileDialog extends Disposable {
    ...
    getOptions(options, isSave = false) {
        ...
        if ((this.scheme !== Schemas.file) && !this.fileService.hasProvider(defaultUri)) {
            this.notificationService.info(nls.localize(18214, null, defaultUri.toString()));
            return undefined;
        }
    ...
    async pickResource(isSave = false) {
        ...
        return new Promise((resolve) => {
            ...
            this.filePickBox.placeholder = nls.localize(18215, null);
            this.filePickBox.ok = true;
            this.filePickBox.okLabel = typeof this.options.openLabel === 'string' ? this.options.openLabel : this.options.openLabel?.withoutMnemonic;
            if ((this.scheme !== Schemas.file) && this.options && this.options.availableFileSystems && (this.options.availableFileSystems.length > 1) && (this.options.availableFileSystems.indexOf(Schemas.file) > -1)) {
                this.filePickBox.customButton = true;
                this.filePickBox.customLabel = nls.localize(18216, null);
    ...
    setButtons() {
        this.filePickBox.buttons = [{
                ...
                tooltip: this._showDotFiles ? nls.localize(18217, null) : nls.localize(18218, null),
                alwaysVisible: true
            }];
    }
    isBadSubpath(value) {
        ...
        if ((value.length > 0) && ((value[0] === '~') || this.canTildaEscapeHatch(value))) {
            ...
            if ((!newFolderIsOldFolder && (this.endsWithSlash(value) || newFolderIsParent || newFolderIsUnrelated)) || reset) {
                ...
                if (stat?.isDirectory && (resources.basename(valueUri) !== '.') && this.endsWithSlash(value)) {
                    ...
                    this.filePickBox.validationMessage = nls.localize(18219, null);
    ...
    yesNoPrompt(uri, message) {
        ...
        prompt.customLabel = nls.localize(18220, null);
    ...
    async validate(uri) {
        if (uri === undefined) {
            this.filePickBox.validationMessage = nls.localize(18221, null);
        ...
        if (this.requiresTrailing) { // save
            if (stat?.isDirectory) {
                // Can't do this
                this.filePickBox.validationMessage = nls.localize(18222, null);
                return Promise.resolve(false);
            }
            else if (stat) {
                // Replacing a file.
                // Show a yes/no prompt
                const message = nls.localize(18223, null, resources.basename(uri));
                return this.yesNoPrompt(uri, message);
            }
            else if (!(isValidBasename(resources.basename(uri), this.isWindows))) {
                // Filename not allowed
                this.filePickBox.validationMessage = nls.localize(18224, null);
                return Promise.resolve(false);
            }
            else if (!statDirname) {
                // Folder to save in doesn't exist
                const message = nls.localize(18225, null, resources.basename(resources.dirname(uri)));
                return this.yesNoPrompt(uri, message);
            }
            else if (!statDirname.isDirectory) {
                this.filePickBox.validationMessage = nls.localize(18226, null);
                return Promise.resolve(false);
            }
            else if (statDirname.readonly) {
                this.filePickBox.validationMessage = nls.localize(18227, null);
                return Promise.resolve(false);
            }
        }
        else { // open
            if (!stat) {
                ...
                this.filePickBox.validationMessage = nls.localize(18228, null);
                return Promise.resolve(false);
            }
            else if (uri.path === '/' && this.isWindows) {
                this.filePickBox.validationMessage = nls.localize(18229, null);
                return Promise.resolve(false);
            }
            else if (stat.isDirectory && !this.allowFolderSelection) {
                // Folder selected when folder selection not permitted
                this.filePickBox.validationMessage = nls.localize(18230, null);
                return Promise.resolve(false);
            }
            else if (!stat.isDirectory && !this.allowFileSelection) {
                // File selected when file selection not permitted
                this.filePickBox.validationMessage = nls.localize(18231, null);
                return Promise.resolve(false);
            }
        }
