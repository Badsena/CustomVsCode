...
let WebLocaleService = class WebLocaleService {
    ...
    async setLocale(languagePackItem, _skipDialog = false) {
        ...
        const restartDialog = await this.dialogService.confirm({
            type: 'info',
            message: localize(18588, null, this.productService.nameLong),
            detail: localize(18589, null, languagePackItem.label),
            primaryButton: localize(18590, null),
        });
    ...
    async clearLocalePreference() {
        ...
        const restartDialog = await this.dialogService.confirm({
            type: 'info',
            message: localize(18591, null, this.productService.nameLong),
            detail: localize(18592, null),
            primaryButton: localize(18593, null),
        });
        if (restartDialog.confirmed) {
            this.hostService.restart();
        }
