...
const authenticationDefinitionSchema = {
    ...
    properties: {
        id: {
            type: 'string',
            description: localize(18063, null)
        },
        label: {
            type: 'string',
            description: localize(18064, null),
        },
        authorizationServerGlobs: {
            type: 'array',
            items: {
                type: 'string',
                description: localize(18065, null),
            },
            description: localize(18066, null)
        }
    }
};
const authenticationExtPoint = ExtensionsRegistry.registerExtensionPoint({
    ...
    jsonSchema: {
        description: localize(18067, null),
        type: 'array',
...
let AuthenticationService = class AuthenticationService extends Disposable {
            ...
            added.forEach(point => {
                for (const provider of point.value) {
                    if (isFalsyOrWhitespace(provider.id)) {
                        point.collector.error(localize(18068, null));
                        continue;
                    }
                    if (isFalsyOrWhitespace(provider.label)) {
                        point.collector.error(localize(18069, null));
                        continue;
                    }
                    if (!this.declaredProviders.some(p => p.id === provider.id)) {
                        ...
                        point.collector.error(localize(18070, null, provider.id));
                    }
                }
            });
    ...
    registerDeclaredAuthenticationProvider(provider) {
        if (isFalsyOrWhitespace(provider.id)) {
            throw new Error(localize(18071, null));
        }
        if (isFalsyOrWhitespace(provider.label)) {
            throw new Error(localize(18072, null));
        }
        if (this.declaredProviders.some(p => p.id === provider.id)) {
            throw new Error(localize(18073, null, provider.id));
        }
