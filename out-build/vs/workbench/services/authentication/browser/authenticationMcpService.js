...
let AuthenticationMcpService = class AuthenticationMcpService extends Disposable {
    ...
    updateBadgeCount() {
        ...
        if (numberOfRequests > 0) {
            const badge = new NumberBadge(numberOfRequests, () => nls.localize(18054, null));
    ...
    async showGetSessionPrompt(provider, accountName, mcpServerId, mcpServerName) {
        ...
        const { result } = await this.dialogService.prompt({
            type: Severity.Info,
            message: nls.localize(18055, null, mcpServerName, provider.label, accountName),
            buttons: [
                {
                    label: nls.localize(18056, null),
                    run: () => SessionPromptChoice.Allow
                },
                {
                    label: nls.localize(18057, null),
    ...
    async selectSession(providerId, mcpServerId, mcpServerName, scopes, availableSessions) {
        ...
        items.push({ label: nls.localize(18058, null) });
        quickPick.items = items;
        quickPick.title = nls.localize(18059, null, mcpServerName, this._authenticationService.getProvider(providerId).label);



        quickPick.placeholder = nls.localize(18060, null, mcpServerName);
    ...
    requestSessionAccess(providerId, mcpServerId, mcpServerName, scopes, possibleSessions) {
        ...
        const menuItem = MenuRegistry.appendMenuItem(MenuId.AccountsContext, {
            ...
            command: {
                ...
                title: nls.localize(18061, null, provider.label, mcpServerName)
    ...
    async requestNewSession(providerId, scopes, mcpServerId, mcpServerName) {
        ...
        const menuItem = MenuRegistry.appendMenuItem(MenuId.AccountsContext, {
            ...
            command: {
                id: commandId,
                title: nls.localize(18062, null, provider.label, mcpServerName)
