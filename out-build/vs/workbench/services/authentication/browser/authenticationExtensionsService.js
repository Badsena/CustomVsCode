...
let AuthenticationExtensionsService = class AuthenticationExtensionsService extends Disposable {
    ...
    updateBadgeCount() {
        ...
        if (numberOfRequests > 0) {
            const badge = new NumberBadge(numberOfRequests, () => nls.localize(18045, null));
    ...
    async showGetSessionPrompt(provider, accountName, extensionId, extensionName) {
        ...
        const { result } = await this.dialogService.prompt({
            type: Severity.Info,
            message: nls.localize(18046, null, extensionName, provider.label, accountName),
            buttons: [
                {
                    label: nls.localize(18047, null),
                    run: () => SessionPromptChoice.Allow
                },
                {
                    label: nls.localize(18048, null),
    ...
    async selectSession(providerId, extensionId, extensionName, scopeListOrRequest, availableSessions) {
        ...
        items.push({ label: nls.localize(18049, null) });
        quickPick.items = items;
        quickPick.title = nls.localize(18050, null, extensionName, this._authenticationService.getProvider(providerId).label);



        quickPick.placeholder = nls.localize(18051, null, extensionName);
    ...
    requestSessionAccess(providerId, extensionId, extensionName, scopeListOrRequest, possibleSessions) {
        ...
        const menuItem = MenuRegistry.appendMenuItem(MenuId.AccountsContext, {
            ...
            command: {
                ...
                title: nls.localize(18052, null, provider.label, extensionName)
    ...
    async requestNewSession(providerId, scopeListOrRequest, extensionId, extensionName) {
        ...
        const menuItem = MenuRegistry.appendMenuItem(MenuId.AccountsContext, {
            ...
            command: {
                id: commandId,
                title: nls.localize(18053, null, provider.label, extensionName)
