...
let NativeExtensionService = class NativeExtensionService extends AbstractExtensionService {
    ...
    _onExtensionHostCrashed(extensionHost, code, signal) {
        ...
        if (extensionHost.kind === 1 /* ExtensionHostKind.LocalProcess */) {
            if (code === 55 /* ExtensionHostExitCode.VersionMismatch */) {
                this._notificationService.prompt(Severity.Error, nls.localize(18475, null), [{
                        label: nls.localize(18476, null),
                        run: () => {
            ...
            if (this._localCrashTracker.shouldAutomaticallyRestart()) {
                ...
                this._notificationService.status(nls.localize(18477, null), { hideAfter: 5000 });
                ...
                if (this._environmentService.isBuilt) {
                    ...
                    choices.push({
                        label: nls.localize(18479, null),
                        run: () => this._nativeHostService.openDevTools()
                    });
                }
                choices.push({
                    label: nls.localize(18480, null),
                    run: () => this.startExtensionHosts()
                });
                if (this._environmentService.isBuilt) {
                    choices.push({
                        label: nls.localize(18481, null),
                        run: () => {
                            this._instantiationService.invokeFunction(accessor => {
                ...
                this._notificationService.prompt(Severity.Error, nls.localize(18482, null), choices);
            }
        }
    }
    ...
    async _doResolveExtensions(emitter) {
        ...
        if (remoteAuthority) {
            ...
            if (!remoteEnv) {
                this._notificationService.notify({ severity: Severity.Error, message: nls.localize(18483, null) });
                // Proceed with the local extension host
                return this._startLocalExtensionHost(emitter);
            }
    ...
    async _handleNoResolverFound(remoteAuthority) {
        ...
        if (extension) {
            if (!extensionIsEnabled(this._logService, this._extensionEnablementService, extension, false)) {
                const message = nls.localize(18484, null, recommendation.friendlyName);
                this._notificationService.prompt(Severity.Info, message, [{
                        label: nls.localize(18485, null),
                        run: async () => {
            ...
            const message = nls.localize(18486, null, recommendation.friendlyName);
            this._notificationService.prompt(Severity.Info, message, [{
                    label: nls.localize(18487, null),
                    run: async () => {
                        ...
                        if (galleryExtension) {
                            ...
                            this._notificationService.error(nls.localize(18488, null));
                        }
                    }
                }], {
                sticky: true,
...
class RestartExtensionHostAction extends Action2 {
    constructor() {
        super({
            ...
            title: nls.localize2(18490, "Restart Extension Host"),
    ...
    async run(accessor) {
        ...
        const stopped = await extensionService.stopExtensionHosts(nls.localize(18489, null));
        if (stopped) {
