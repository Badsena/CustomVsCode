    ...
    async handleURL(uri, options) {
        ...
        if (!trusted) {
            ...
            const result = await this.dialogService.confirm({
                message: localize(18351, null, extensionDisplayName),
                checkbox: {
                    label: localize(18352, null),
                },
                primaryButton: localize(18353, null),
                custom: {
                    markdownDetails: [{
    ...
    async handleUnhandledURL(uri, extensionId, options) {
        try {
            await this.commandService.executeCommand('workbench.extensions.installExtension', extensionId, {
                justification: {
                    reason: `${localize(18354, null)}\n${uri.toString()}`,
                    action: localize(18355, null)
                },
                enable: true,
        ...
        /* Extension cannot be added and require window reload */
            ...
            const result = await this.dialogService.confirm({
                message: localize(18356, null, extensionId),
                primaryButton: localize(18357, null)
            });
            if (!result.confirmed) {
                return;
            }
...
class ManageAuthorizedExtensionURIsAction extends Action2 {
    constructor() {
        super({
            ...
            title: localize2(18359, 'Manage Authorized Extension URIs...'),
            category: localize2(18360, 'Extensions'),
            menu: {
    ...
    async run(accessor) {
        ...
        if (items.length === 0) {
            await quickInputService.pick([{ label: localize(18358, null) }]);
            return;
        }
