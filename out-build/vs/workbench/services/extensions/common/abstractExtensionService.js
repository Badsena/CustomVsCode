...
let AbstractExtensionService = AbstractExtensionService_1 = class AbstractExtensionService extends Disposable {
    ...
    async _deltaExtensions(lock, _toAdd, _toRemove) {
        ...
        if (result.removedDueToLooping.length > 0) {
            this._notificationService.notify({
                ...
                message: nls.localize(18363, null, result.removedDueToLooping.map(e => `'${e.identifier.value}'`).join(', '))
            });
        }
    ...
    async _resolveAndProcessExtensions(lock) {
        ...
        if (result.removedDueToLooping.length > 0) {
            this._notificationService.notify({
                severity: Severity.Error,
                message: nls.localize(18364, null, result.removedDueToLooping.map(e => `'${e.identifier.value}'`).join(', '))
            });
        }
    ...
    async _handleExtensionTests() {
        ...
        if (!extensionHostManager) {
            const msg = nls.localize(18365, null, this._environmentService.extensionTestsLocationURI.toString());
            console.error(msg);
            ...
            veto(value, reason) {
                ...
                if (typeof value === 'boolean') {
                    ...
                    value.then(value => {
                        ...
                        vetoReasons.add(nls.localize(18366, null, reason, toErrorMessage(error)));
                    });
                }
            }
        });
        ...
        if (!veto) {
            ...
            if (!auto) {
                ...
                const { confirmed } = await this._dialogService.confirm({
                    ...
                    message: nls.localize(18367, null),
                    detail: vetoReasonsArray.length === 1 ?
                        vetoReasonsArray[0] :
                        vetoReasonsArray.join('\n -'),
                    primaryButton: nls.localize(18368, null)
                });
                if (confirmed) {
                    return true;
                }
    ...
    async _onRemoteExtensionHostCrashed(extensionHost, reconnectionToken) {
        try {
            ...
            if (this._remoteCrashTracker.shouldAutomaticallyRestart()) {
                ...
                this._notificationService.status(nls.localize(18369, null), { hideAfter: 5000 });
                this._startExtensionHostsIfNecessary(false, Array.from(this._allRequestedActivateEvents.keys()));
            }
            else {
                this._notificationService.prompt(Severity.Error, nls.localize(18370, null), [{
                        label: nls.localize(18371, null),
                        run: () => {
                            this._startExtensionHostsIfNecessary(false, Array.from(this._allRequestedActivateEvents.keys()));
                        }
                    }]);
