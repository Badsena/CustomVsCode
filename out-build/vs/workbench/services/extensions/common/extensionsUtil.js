...
export function dedupExtensions(system, user, workspace, development, logService) {
    ...
    system.forEach((systemExtension) => {
        ...
        if (extension) {
            logService.warn(localize(18463, null, extension.extensionLocation.fsPath, systemExtension.extensionLocation.fsPath));
        }
    ...
    user.forEach((userExtension) => {
        ...
        if (extension) {
            if (extension.isBuiltin) {
                ...
                logService.warn(localize(18464, null, extension.extensionLocation.fsPath, userExtension.extensionLocation.fsPath));
            }
        }
    ...
    workspace.forEach(workspaceExtension => {
        ...
        if (extension) {
            logService.warn(localize(18465, null, extension.extensionLocation.fsPath, workspaceExtension.extensionLocation.fsPath));
        }
        result.set(workspaceExtension.identifier, workspaceExtension);
    });
    development.forEach(developedExtension => {
        logService.info(localize(18466, null, developedExtension.extensionLocation.fsPath));
