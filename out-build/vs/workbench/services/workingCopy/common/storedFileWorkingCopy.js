...
let StoredFileWorkingCopy = class StoredFileWorkingCopy extends ResourceWorkingCopy {
    ...
    async doSave(options) {
        ...
        return this.progressService.withProgress({
            title: localize(19195, null, this.name),
    ...
    doSaveSequential(versionId, options, progress, saveCancellation) {
        return this.saveSequentializer.run(versionId, (async () => {
            ...
            progress.report({ message: localize(19196, null) });
    ...
    doHandleSaveError(error, options) {
        ...
        if (fileOperationError.fileOperationResult === 3 /* FileOperationResult.FILE_MODIFIED_SINCE */) {
            message = localize(19197, null, this.name);
            primaryActions.push(toAction({ id: 'fileWorkingCopy.overwrite', label: localize(19198, null), run: () => this.save({ ...options, ignoreModifiedSince: true, reason: 1 /* SaveReason.EXPLICIT */ }) }));
            primaryActions.push(toAction({ id: 'fileWorkingCopy.revert', label: localize(19199, null), run: () => this.revert() }));
        }
        // Any other save error
            ...
            if (canSaveElevated && (isPermissionDenied || triedToUnlock)) {
                primaryActions.push(toAction({
                    ...
                    label: triedToUnlock ?
                        isWindows ? localize(19200, null) : localize(19201, null) :
                        isWindows ? localize(19202, null) : localize(19203, null),
                    run: () => {
            ...
            // Unlock
                ...
                primaryActions.push(toAction({ id: 'fileWorkingCopy.unlock', label: localize(19204, null), run: () => this.save({ ...options, writeUnlock: true, reason: 1 /* SaveReason.EXPLICIT */ }) }));
            }
            // Retry
            else {
                primaryActions.push(toAction({ id: 'fileWorkingCopy.retry', label: localize(19205, null), run: () => this.save({ ...options, reason: 1 /* SaveReason.EXPLICIT */ }) }));
            }
            // Save As
            primaryActions.push(toAction({
                id: 'fileWorkingCopy.saveAs',
                label: localize(19206, null),
                run: async () => {
            ...
            // Revert
            primaryActions.push(toAction({ id: 'fileWorkingCopy.revert', label: localize(19207, null), run: () => this.revert() }));
            // Message
            if (isWriteLocked) {
                if (triedToUnlock && canSaveElevated) {
                    message = isWindows ?
                        localize(19208, null, this.name) :
                        localize(19209, null, this.name);
                }
                else {
                    message = localize(19210, null, this.name);
                }
            }
            else if (canSaveElevated && isPermissionDenied) {
                message = isWindows ?
                    localize(19211, null, this.name) :
                    localize(19212, null, this.name);
            }
            else {
                message = localize(19213, null, this.name, toErrorMessage(error, false));
            }
        }
