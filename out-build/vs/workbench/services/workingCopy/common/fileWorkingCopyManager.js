...
let FileWorkingCopyManager = class FileWorkingCopyManager extends Disposable {
    ...
    static { this.FILE_WORKING_COPY_SAVE_CREATE_SOURCE = SaveSourceRegistry.registerSource('fileWorkingCopyCreate.source', localize(19183, null)); }
    static { this.FILE_WORKING_COPY_SAVE_REPLACE_SOURCE = SaveSourceRegistry.registerSource('fileWorkingCopyReplace.source', localize(19184, null)); }
    ...
    provideDecorations() {
        ...
        const provider = this._register(new class extends Disposable {
            constructor(stored) {
                super();
                this.stored = stored;
                this.label = localize(19185, null);
    ...
    async confirmOverwrite(resource) {
        const { confirmed } = await this.dialogService.confirm({
            type: 'warning',
            message: localize(19189, null, basename(resource)),
            detail: localize(19190, null, basename(resource), basename(dirname(resource))),
            primaryButton: localize(19191, null)
        });
        return confirmed;
    }
    async confirmMakeWriteable(resource) {
        const { confirmed } = await this.dialogService.confirm({
            type: 'warning',
            message: localize(19192, null, basename(resource)),
            detail: localize(19193, null),
            primaryButton: localize(19194, null)
        });
        return confirmed;
    }
