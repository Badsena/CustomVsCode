...
let ExtensionManagementService = class ExtensionManagementService extends CommontExtensionManagementService {
    ...
    getDependentsErrorMessage(extension, dependents) {
        if (dependents.length === 1) {
            return localize(18287, null, extension.manifest.displayName || extension.manifest.name, dependents[0].manifest.displayName || dependents[0].manifest.name);
        }
        if (dependents.length === 2) {
            return localize(18288, null, extension.manifest.displayName || extension.manifest.name, dependents[0].manifest.displayName || dependents[0].manifest.name, dependents[1].manifest.displayName || dependents[1].manifest.name);
        }
        return localize(18289, null, extension.manifest.displayName || extension.manifest.name, dependents[0].manifest.displayName || dependents[0].manifest.name, dependents[1].manifest.displayName || dependents[1].manifest.name);
    }
    ...
    async canInstallGalleryExtension(gallery) {
        ...
        if (!manifest) {
            return new MarkdownString().appendText(localize(18290, null));
        }
        if (this.extensionManagementServerService.remoteExtensionManagementServer
        ...
        return new MarkdownString().appendText(localize(18291, null, gallery.displayName || gallery.name));
    }
    async canInstallResourceExtension(extension) {
        ...
        return new MarkdownString().appendText(localize(18292, null, extension.manifest.displayName ?? extension.identifier.id));
    }
    ...
    async installGalleryExtensions(extensions) {
        ...
        const manifests = await Promise.all(extensions.map(async ({ extension }) => {
            ...
            if (!manifest) {
                throw new Error(localize(18293, null, extension.displayName || extension.name));
            }
            return manifest;
        }));
        ...
        await Promise.all(extensions.map(async ({ extension, options }) => {
            try {
                const manifest = await this.extensionGalleryService.getManifest(extension, CancellationToken.None);
                if (!manifest) {
                    throw new Error(localize(18294, null, extension.displayName || extension.name));
                }
    ...
    async installFromGallery(gallery, installOptions, servers) {
        const manifest = await this.extensionGalleryService.getManifest(gallery, CancellationToken.None);
        if (!manifest) {
            throw new Error(localize(18295, null, gallery.displayName || gallery.name));
        }
    ...
    async getInstallableServers(gallery) {
        const manifest = await this.extensionGalleryService.getManifest(gallery, CancellationToken.None);
        if (!manifest) {
            return Promise.reject(localize(18296, null, gallery.displayName || gallery.name));
        }
    ...
    validServers(gallery, manifest, servers) {
        ...
        for (const server of servers) {
            if (!installableServers.includes(server)) {
                const error = new Error(localize(18297, null, gallery.displayName || gallery.name, server.label));
    ...
    async getExtensionManagementServersToInstall(gallery, manifest) {
        ...
        if (!servers.length) {
            const error = new Error(localize(18298, null, gallery.displayName || gallery.name));
            error.name = "Unsupported" /* ExtensionManagementErrorCode.Unsupported */;
            throw error;
        }
        return servers;
    ...
    async hasToFlagExtensionsMachineScoped(extensions) {
        if (this.isExtensionsSyncEnabled()) {
            const { result } = await this.dialogService.prompt({
                type: Severity.Info,
                message: extensions.length === 1 ? localize(18299, null) : localize(18300, null),
                detail: extensions.length === 1
                    ? localize(18301, null, extensions[0].displayName)
                    : localize(18302, null),
                buttons: [
                    {
                        label: localize(18303, null),
                        run: () => false
                    },
                    {
                        label: localize(18304, null),
                        run: () => true
                    }
                ],
                cancelButton: {
    ...
    async requestPublisherTrust(extensions) {
        const manifests = await Promise.all(extensions.map(async ({ extension }) => {
            const manifest = await this.extensionGalleryService.getManifest(extension, CancellationToken.None);
            if (!manifest) {
                throw new Error(localize(18305, null, extension.displayName || extension.name));
            }
            return manifest;
        }));
        await this.checkForTrustedPublishers(extensions.map((e, index) => ({ extension: e.extension, manifest: manifests[index], checkForPackAndDependencies: !e.options?.donotIncludePackAndDependencies })));
    ...
    async checkForTrustedPublishers(extensions) {
        ...
        const installButton = {
            label: allPublishers.length > 1 ? localize(18306, null) : localize(18307, null),
            run: () => {
        ...
        const title = allPublishers.length === 1
            ? localize(18309, null, allPublishers[0].publisherDisplayName)
            : allPublishers.length === 2
                ? localize(18310, null, allPublishers[0].publisherDisplayName, allPublishers[1].publisherDisplayName)
                : localize(18311, null, allPublishers[0].publisherDisplayName, allPublishers.length - 1);
        ...
        if (untrustedExtensions.length === 1) {
            ...
            if (otherUntrustedPublishers.length) {
                customMessage.appendMarkdown(localize(18312, null, `[${extension.displayName}](${extension.detailsLink})`, getPublisherLink(extension)));
                ...
                if (otherUntrustedPublishers.length === 1) {
                    customMessage.appendMarkdown(localize(18313, null, commandUri, getPublisherLink(otherUntrustedPublishers[0])));
                }
                else {
                    customMessage.appendMarkdown(localize(18314, null, commandUri, otherUntrustedPublishers.slice(0, otherUntrustedPublishers.length - 1).map(p => getPublisherLink(p)).join(', '), getPublisherLink(otherUntrustedPublishers[otherUntrustedPublishers.length - 1])));
                }
                customMessage.appendMarkdown('&nbsp;');
                customMessage.appendMarkdown(localize(18315, null));
            }
            else {
                customMessage.appendMarkdown(localize(18316, null, `[${extension.displayName}](${extension.detailsLink})`, getPublisherLink(extension)));
            }
        }
        else {
            customMessage.appendMarkdown(localize(18317, null, getPublisherLink(allPublishers[0]), getPublisherLink(allPublishers[allPublishers.length - 1])));
        }
        if (verifiedPublishers.length || unverfiiedPublishers.length === 1) {
            for (const publisher of verifiedPublishers) {
                ...
                const publisherVerifiedMessage = localize(18318, null, getPublisherLink(publisher), `[$(link-external) ${URI.parse(publisher.publisherDomain.link).authority}](${publisher.publisherDomain.link})`);
            ...
            if (unverfiiedPublishers.length) {
                ...
                if (unverfiiedPublishers.length === 1) {
                    customMessage.appendMarkdown(`$(${Codicon.unverified.id})&nbsp;${localize(18319, null, getPublisherLink(unverfiiedPublishers[0]), unverifiedLink)}`);
                }
                else {
                    customMessage.appendMarkdown(`$(${Codicon.unverified.id})&nbsp;${localize(18320, null, unverfiiedPublishers.slice(0, unverfiiedPublishers.length - 1).map(p => getPublisherLink(p)).join(', '), getPublisherLink(unverfiiedPublishers[unverfiiedPublishers.length - 1]), unverifiedLink)}`);
                }
            }
        }
        else {
            customMessage.appendText('\n');
            customMessage.appendMarkdown(`$(${Codicon.unverified.id})&nbsp;${localize(18321, null, unverifiedLink)}`);
        }
        customMessage.appendText('\n');
        if (allPublishers.length > 1) {
            customMessage.appendMarkdown(localize(18322, null, this.productService.nameLong));
        }
        else {
            customMessage.appendMarkdown(localize(18323, null, this.productService.nameLong));
        }
    ...
    async checkForWorkspaceTrust(manifest, requireTrust) {
        if (requireTrust || this.extensionManifestPropertiesService.getExtensionUntrustedWorkspaceSupportType(manifest) === false) {
            const buttons = [];
            buttons.push({ label: localize(18324, null), type: 'ContinueWithTrust' });
            if (!requireTrust) {
                buttons.push({ label: localize(18325, null), type: 'ContinueWithoutTrust' });
            }
            buttons.push({ label: localize(18326, null), type: 'Manage' });
            const trustState = await this.workspaceTrustRequestService.requestWorkspaceTrust({
                message: localize(18327, null),
                buttons
            });
    ...
    async checkInstallingExtensionOnWeb(extension, manifest) {
        ...
        const productName = localize(18328, null, this.productService.nameLong);
        ...
        const limitedSupportMessage = localize(18329, null, extension.displayName || extension.identifier.id, productName);
        let message;
        let buttons = [];
        let detail;
        const installAnywayButton = {
            label: localize(18330, null),
            run: () => { }
        };
        const showExtensionsButton = {
            label: localize(18331, null),
        ...
        if (nonWebExtensions.length && hasLimitedSupport) {
            ...
            message = localize(18333, null, extension.displayName || extension.identifier.id, productName);
            buttons = [
                installAnywayButton,
                showExtensionsButton
            ];
    ...
    async toLocalWorkspaceExtension(extension) {
        ...
        if (extension.manifest.main) {
            if (!(await this.fileService.exists(this.uriIdentityService.extUri.joinPath(extension.location, extension.manifest.main)))) {
                isValid = false;
                validations.push([Severity.Error, localize(18334, null, extension.manifest.main)]);
            }
        }
        return {
            identifier: extension.identifier,
