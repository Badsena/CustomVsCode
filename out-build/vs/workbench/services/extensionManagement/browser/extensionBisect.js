...
let ExtensionBisectUi = class ExtensionBisectUi {
    ...
    _showBisectPrompt() {
        ...
        const message = this._extensionBisectService.disabledCount === 1
            ? localize(18244, null)
            : localize(18245, null, this._extensionBisectService.disabledCount);
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: localize2(18262, 'Start Extension Bisect'),
    ...
    async run(accessor) {
        ...
        const res = await dialogService.confirm({
            message: localize(18246, null),
            detail: localize(18247, null, 2 + Math.log2(extensions.length) | 0),
            primaryButton: localize(18248, null)
        });
        if (res.confirmed) {
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: localize2(18263, 'Continue Extension Bisect'),
            category: Categories.Help,
            f1: true,
    ...
    async run(accessor, seeingBad) {
        ...
        if (done.bad) {
            ...
            await dialogService.info(localize(18249, null), localize(18250, null, productService.nameShort));
        }
        else {
            // DONE and identified extension
            const res = await dialogService.confirm({
                type: Severity.Info,
                message: localize(18251, null),
                primaryButton: localize(18252, null),
                cancelButton: localize(18253, null),
                detail: localize(18254, null, done.id),
                checkbox: { label: localize(18255, null), checked: true }
            });
    ...
    async _checkForBad(dialogService, bisectService) {
        const { result } = await dialogService.prompt({
            type: Severity.Info,
            message: localize(18256, null),
            detail: localize(18257, null, bisectService.disabledCount),
            buttons: [
                {
                    label: localize(18258, null),
                    run: () => false // good now
                },
                {
                    label: localize(18259, null),
                    run: () => true // bad
                },
                {
                    label: localize(18260, null),
                    run: () => undefined // stop
                }
            ],
            cancelButton: {
                label: localize(18261, null),
                run: () => null // cancel
            }
        });
        return result;
...
registerAction2(class extends Action2 {
    constructor() {
        super({
            ...
            title: localize2(18264, 'Stop Extension Bisect'),
            category: Categories.Help,
            f1: true,
            precondition: ExtensionBisectUi.ctxIsBisectActive
        });
