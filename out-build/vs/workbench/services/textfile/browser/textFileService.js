...
let AbstractTextFileService = class AbstractTextFileService extends Disposable {
    ...
    static { this.TEXTFILE_SAVE_CREATE_SOURCE = SaveSourceRegistry.registerSource('textFileCreate.source', localize(18706, null)); }
    static { this.TEXTFILE_SAVE_REPLACE_SOURCE = SaveSourceRegistry.registerSource('textFileOverwrite.source', localize(18707, null)); }
    ...
    provideDecorations() {
        ...
        const provider = this._register(new class extends Disposable {
            constructor(files) {
                super();
                this.files = files;
                this.label = localize(18708, null);
    ...
    async doRead(resource, options) {
        ...
        try {
            ...
            if (error.decodeStreamErrorKind === 1 /* DecodeStreamErrorKind.STREAM_IS_BINARY */) {
                throw new TextFileOperationError(localize(18712, null), 0 /* TextFileOperationResult.FILE_IS_BINARY */, options);
            }
    ...
    async confirmOverwrite(resource) {
        const { confirmed } = await this.dialogService.confirm({
            type: 'warning',
            message: localize(18713, null, basename(resource)),
            detail: localize(18714, null, basename(resource), basename(dirname(resource))),
            primaryButton: localize(18715, null),
        });
        return confirmed;
    }
    async confirmMakeWriteable(resource) {
        const { confirmed } = await this.dialogService.confirm({
            type: 'warning',
            message: localize(18716, null, basename(resource)),
            detail: localize(18717, null),
            primaryButton: localize(18718, null)
        });
        return confirmed;
    }
