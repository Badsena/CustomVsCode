...
suite('WorkbenchThemeService', () => {
    ...
    suite('migrateThemeSettingsId', () => {
        ...
        test('migrates Experimental theme IDs to VS Code themes', () => {
            assert.deepStrictEqual(['Experimental Dark', 'Experimental Light'].map(migrateThemeSettingsId), ['VS Code Dark', 'VS Code Light']);
        });
        test('returns unknown IDs unchanged', () => {
            assert.deepStrictEqual(['Dark Modern', 'VS Code Dark', 'Some Custom Theme', ''].map(migrateThemeSettingsId), ['Dark Modern', 'VS Code Dark', 'Some Custom Theme', '']);
        });
    });
    suite('ThemeConfiguration', () => {
        function createHostColorSchemeService(dark, highContrast = false) {
            return {
         ...
         */
        function createConfigServiceWithDefaults(userConfig, defaults) {
            ...
            configService.inspect = (key, overrides) => {
            ...
            configService.getValue = (arg1, arg2) => {
...
//# sourceMappingURL=workbenchThemeService.test.js.map