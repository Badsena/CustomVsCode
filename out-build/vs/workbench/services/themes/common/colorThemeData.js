...
async function _loadColorTheme(extensionResourceLoaderService, themeLocation, result) {
    if (resources.extname(themeLocation) === '.json') {
        ...
        if (errors.length > 0) {
            return Promise.reject(new Error(nls.localize(18790, null, errors.map(e => getParseErrorMessage(e.error)).join(', '))));
        }
        else if (Json.getNodeType(contentValue) !== 'object') {
            return Promise.reject(new Error(nls.localize(18791, null)));
        }
        ...
        if (colors) {
            if (typeof colors !== 'object') {
                return Promise.reject(new Error(nls.localize(18792, null, themeLocation.toString())));
            }
        ...
        if (tokenColors) {
            if (Array.isArray(tokenColors)) {
                ...
                return Promise.reject(new Error(nls.localize(18793, null, themeLocation.toString())));
            }
        }
        ...
        if (semanticTokenColors && typeof semanticTokenColors === 'object') {
            for (const key in semanticTokenColors) {
                try {
                    ...
                    return Promise.reject(new Error(nls.localize(18794, null, themeLocation.toString())));
                }
            }
        }
    }
    ...
    return extensionResourceLoaderService.readExtensionResource(themeLocation).then(content => {
        try {
            ...
            if (!Array.isArray(settings)) {
                return Promise.reject(new Error(nls.localize(18795, null)));
            }
            convertSettings(settings, result);
            return Promise.resolve(null);
        }
        catch (e) {
            return Promise.reject(new Error(nls.localize(18796, null, e.message)));
        }
    }, error => {
        return Promise.reject(new Error(nls.localize(18797, null, themeLocation.toString(), error.message)));
    });
}
