...
const tokenTypeExtPoint = ExtensionsRegistry.registerExtensionPoint({
    ...
    jsonSchema: {
        description: nls.localize(18929, null),
        type: 'array',
        items: {
            type: 'object',
            properties: {
                id: {
                    type: 'string',
                    description: nls.localize(18930, null),
                    pattern: typeAndModifierIdPattern,
                    patternErrorMessage: nls.localize(18931, null),
                },
                superType: {
                    type: 'string',
                    description: nls.localize(18932, null),
                    pattern: typeAndModifierIdPattern,
                    patternErrorMessage: nls.localize(18933, null),
                },
                description: {
                    type: 'string',
                    description: nls.localize(18934, null),
                }
            }
        }
    }
...
const tokenModifierExtPoint = ExtensionsRegistry.registerExtensionPoint({
    ...
    jsonSchema: {
        description: nls.localize(18935, null),
        type: 'array',
        items: {
            type: 'object',
            properties: {
                id: {
                    type: 'string',
                    description: nls.localize(18936, null),
                    pattern: typeAndModifierIdPattern,
                    patternErrorMessage: nls.localize(18937, null)
                },
                description: {
                    description: nls.localize(18938, null)
                }
            }
        }
    }
...
const tokenStyleDefaultsExtPoint = ExtensionsRegistry.registerExtensionPoint({
    ...
    jsonSchema: {
        description: nls.localize(18939, null),
        type: 'array',
        items: {
            type: 'object',
            properties: {
                language: {
                    description: nls.localize(18940, null),
                    type: 'string'
                },
                scopes: {
                    description: nls.localize(18941, null),
                    type: 'object',
    ...
    constructor() {
        function validateTypeOrModifier(contribution, extensionPoint, collector) {
            if (typeof contribution.id !== 'string' || contribution.id.length === 0) {
                collector.error(nls.localize(18942, null, extensionPoint));
                return false;
            }
            if (!contribution.id.match(typeAndModifierIdPattern)) {
                collector.error(nls.localize(18943, null, extensionPoint));
                return false;
            }
            const superType = contribution.superType;
            if (superType && !superType.match(typeAndModifierIdPattern)) {
                collector.error(nls.localize(18944, null, extensionPoint));
                return false;
            }
            if (typeof contribution.description !== 'string' || contribution.id.length === 0) {
                collector.error(nls.localize(18945, null, extensionPoint));
                return false;
            }
            return true;
        }
        tokenTypeExtPoint.setHandler((extensions, delta) => {
            for (const extension of delta.added) {
                ...
                if (!extensionValue || !Array.isArray(extensionValue)) {
                    collector.error(nls.localize(18946, null));
                    return;
                }
        ...
        tokenModifierExtPoint.setHandler((extensions, delta) => {
            for (const extension of delta.added) {
                ...
                const collector = extension.collector;
                if (!extensionValue || !Array.isArray(extensionValue)) {
                    collector.error(nls.localize(18947, null));
                    return;
                }
                for (const contribution of extensionValue) {
        ...
        tokenStyleDefaultsExtPoint.setHandler((extensions, delta) => {
            for (const extension of delta.added) {
                ...
                const collector = extension.collector;
                if (!extensionValue || !Array.isArray(extensionValue)) {
                    collector.error(nls.localize(18948, null));
                    return;
                }
                for (const contribution of extensionValue) {
                    if (contribution.language && typeof contribution.language !== 'string') {
                        collector.error(nls.localize(18949, null));
                        continue;
                    }
                    if (!contribution.scopes || typeof contribution.scopes !== 'object') {
                        collector.error(nls.localize(18950, null));
                        continue;
                    }
                    for (const selectorString in contribution.scopes) {
                        ...
                        if (!Array.isArray(tmScopes) || tmScopes.some(l => typeof l !== 'string')) {
                            collector.error(nls.localize(18951, null));
                            continue;
                        }
                        try {
                            ...
                            collector.error(nls.localize(18952, null, selectorString));
