...
export class ProductIconThemeData {
    ...
    async load(fileService, logService) {
        ...
        this.isLoaded = true;
        if (warnings.length) {
            logService.error(nls.localize(18753, null, location.toString(), warnings.join('\n')));
        }
    ...
    static get defaultTheme() {
        ...
        if (!themeData) {
            themeData = ProductIconThemeData._defaultProductIconTheme = new ProductIconThemeData(DEFAULT_PRODUCT_ICON_THEME_ID, nls.localize(18754, null), ThemeSettingDefaults.PRODUCT_ICON_THEME);
...
function _loadProductIconThemeDocument(fileService, location, warnings) {
    return fileService.readExtensionResource(location).then((content) => {
        ...
        if (parseErrors.length > 0) {
            return Promise.reject(new Error(nls.localize(18755, null, parseErrors.map(e => getParseErrorMessage(e.error)).join(', '))));
        }
        else if (Json.getNodeType(contentValue) !== 'object') {
            return Promise.reject(new Error(nls.localize(18756, null)));
        }
        else if (!contentValue.iconDefinitions || !Array.isArray(contentValue.fonts) || !contentValue.fonts.length) {
            return Promise.reject(new Error(nls.localize(18757, null)));
        }
        ...
        for (const font of contentValue.fonts) {
            ...
            if (isString(fontId) && fontId.match(fontIdRegex)) {
                ...
                if (isString(font.weight) && font.weight.match(fontWeightRegex)) {
                    ...
                    warnings.push(nls.localize(18758, null, font.id));
                }
                ...
                if (isString(font.style) && font.style.match(fontStyleRegex)) {
                    ...
                    warnings.push(nls.localize(18759, null, font.id));
                }
                const sanitizedSrc = [];
                if (Array.isArray(font.src)) {
                    for (const s of font.src) {
                        if (isString(s.path) && isString(s.format) && s.format.match(fontFormatRegex)) {
                            ...
                            warnings.push(nls.localize(18760, null, font.id));
                        }
                    }
                }
                if (sanitizedSrc.length) {
                    ...
                    warnings.push(nls.localize(18761, null, font.id));
                }
            }
            else {
                warnings.push(nls.localize(18762, null, font.id));
            }
        }
        ...
        for (const iconId in contentValue.iconDefinitions) {
            ...
            if (isString(definition.fontCharacter)) {
                ...
                if (fontDefinition) {
                    ...
                    warnings.push(nls.localize(18763, null, iconId));
                }
            }
            else {
                warnings.push(nls.localize(18764, null, iconId));
            }
        }
