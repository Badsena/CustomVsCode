...
let WorkbenchThemeService = class WorkbenchThemeService extends Disposable {
    ...
    constructor(extensionService, storageService, configurationService, telemetryService, environmentService, fileService, extensionResourceLoaderService, layoutService, logService, hostColorService, userDataInitializationService, languageService, notificationService, commandService) {
        super();
        ...
        const isNewUser = this.storageService.isNew(-1 /* StorageScope.APPLICATION */);
    ...
    showNewDefaultThemeNotification() {
        ...
        const handle = this.notificationService.prompt(Severity.Info, nls.localize(18765, null), [{
                label: nls.localize(18766, null),
    ...
    async migrateAutoDetectColorScheme() {
        ...
        if (inspection.userValue === undefined
            ...
            await this.configurationService.updateValue(ThemeSettings.DETECT_COLOR_SCHEME, true, 2 /* ConfigurationTarget.USER */);
        }
    }
    ...
    async internalSetColorTheme(themeIdOrTheme, settingsTarget) {
        ...
        try {
            ...
            throw new Error(nls.localize(18767, null, themeData.location?.toString(), error.message));
        }
    }
