...
export class FileIconThemeLoader {
    ...
    loadIconThemeDocument(location) {
        return this.fileService.readExtensionResource(location).then((content) => {
            ...
            if (errors.length > 0) {
                return Promise.reject(new Error(nls.localize(18751, null, errors.map(e => getParseErrorMessage(e.error)).join(', '))));
            }
            else if (Json.getNodeType(contentValue) !== 'object') {
                return Promise.reject(new Error(nls.localize(18752, null)));
            }
