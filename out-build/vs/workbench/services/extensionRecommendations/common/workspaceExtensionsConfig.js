...
let WorkspaceExtensionsConfigService = class WorkspaceExtensionsConfigService extends Disposable {
    ...
    async toggleRecommendation(extensionId) {
        ...
        const workspaceOrFolders = isRecommended
            ? await this.pickWorkspaceOrFolders(recommendedWorksapceFolders, isWorkspaceRecommended ? workspace : undefined, localize(18345, null))
            : await this.pickWorkspaceOrFolders(workspace.folders, workspace.configuration ? workspace : undefined, localize(18346, null));
    ...
    async toggleUnwantedRecommendation(extensionId) {
        ...
        const workspaceOrFolders = isUnwanted
            ? await this.pickWorkspaceOrFolders(unWantedWorksapceFolders, isWorkspaceUnwanted ? workspace : undefined, localize(18347, null))
            : await this.pickWorkspaceOrFolders(workspace.folders, workspace.configuration ? workspace : undefined, localize(18348, null));
        for (const workspaceOrWorkspaceFolder of workspaceOrFolders) {
            if (isWorkspace(workspaceOrWorkspaceFolder)) {
    ...
    async pickWorkspaceOrFolders(workspaceFolders, workspace, placeHolder) {
        ...
        const folderPicks = workspaceFolders.map(workspaceFolder => {
            return {
                ...
                description: localize(18349, null),
