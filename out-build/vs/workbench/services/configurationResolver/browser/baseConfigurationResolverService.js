...
export class BaseConfigurationResolverService extends AbstractVariableResolverService {
            ...
            if (variable.name === 'command') {
                ...
                if (!Types.isUndefinedOrNull(value)) {
                    if (typeof value !== 'string') {
                        throw new VariableError(VariableKind.Command, localize(18146, null, commandId));
                    }
                    result = { value };
                }
            }
    ...
    async showUserInput(section, variable, inputInfos, variableToCommandMap) {
        if (!inputInfos) {
            throw new VariableError(VariableKind.Input, localize(18147, null, variable, 'inputs'));
        }
        // Find info for the given input variable
        const info = inputInfos.filter(item => item.id === variable).pop();
        if (info) {
            const missingAttribute = (attrName) => {
                throw new VariableError(VariableKind.Input, localize(18148, null, variable, info.type, attrName));
            };
            ...
            switch (info.type) {
                ...
                case 'pickString': {
                    ...
                    for (const pickOption of info.options) {
                        ...
                        if (value === info.default) {
                            item.description = localize(18149, null);
                            picks.unshift(item);
                        }
                ...
                case 'command': {
                    ...
                    return this.userInputAccessQueue.queue(() => this.commandService.executeCommand(info.command, info.args)).then(result => {
                        ...
                        throw new VariableError(VariableKind.Input, localize(18150, null, variable, info.command));
                    });
                }
                default:
                    throw new VariableError(VariableKind.Input, localize(18151, null, variable));
            }
        }
        throw new VariableError(VariableKind.Input, localize(18152, null, variable));
    }
}
