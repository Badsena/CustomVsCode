...
export class AbstractVariableResolverService {
    ...
    async evaluateSingleVariable(replacement, folderUri, processEnvironment, commandValueMapping) {
        ...
        const getFilePath = (variableKind) => {
            ...
            throw new VariableError(variableKind, (localize(18170, null, replacement.id)));
        };
        // common error handling for all variables that require an open editor
        const getFolderPathForFile = (variableKind) => {
            ...
            throw new VariableError(variableKind, localize(18171, null, replacement.id, paths.basename(filePath)));
        };
        ...
        const getFolderUri = (variableKind) => {
            if (argument) {
                ...
                throw new VariableError(variableKind, localize(18172, null, variableKind, argument));
            }
            if (folderUri) {
                return folderUri;
            }
            if (this._context.getWorkspaceFolderCount() > 1) {
                throw new VariableError(variableKind, localize(18173, null, variableKind));
            }
            throw new VariableError(variableKind, localize(18174, null, variableKind));
        };
        switch (variable) {
            case 'env':
                if (argument) {
                ...
                throw new VariableError(VariableKind.Env, localize(18175, null, replacement.id));
            case 'config':
                if (argument) {
                    const config = this._context.getConfigurationValue(folderUri, argument);
                    if (types.isUndefinedOrNull(config)) {
                        throw new VariableError(VariableKind.Config, localize(18176, null, replacement.id, argument));
                    }
                    if (types.isObject(config)) {
                        throw new VariableError(VariableKind.Config, localize(18177, null, replacement.id, argument));
                    }
                    return config;
                }
                throw new VariableError(VariableKind.Config, localize(18178, null, replacement.id));
            case 'command':
            ...
            case 'extensionInstallFolder':
                if (argument) {
                    ...
                    if (!ext) {
                        throw new VariableError(VariableKind.ExtensionInstallFolder, localize(18179, null, replacement.id, argument));
                    }
                    return this.fsPath(ext.extensionLocation);
                }
                throw new VariableError(VariableKind.ExtensionInstallFolder, localize(18180, null, replacement.id));
            default: {
                switch (variable) {
                    case 'workspaceRoot':
                    ...
                    case 'userHome':
                        ...
                        throw new VariableError(VariableKind.UserHome, localize(18181, null, replacement.id));
                    case 'lineNumber': {
                        const lineNumber = this._context.getLineNumber();
                        if (lineNumber) {
                            return lineNumber;
                        }
                        throw new VariableError(VariableKind.LineNumber, localize(18182, null, replacement.id));
                    }
                    case 'columnNumber': {
                        ...
                        throw new Error(localize(18183, null, replacement.id));
                    }
                    case 'selectedText': {
                        ...
                        throw new VariableError(VariableKind.SelectedText, localize(18184, null, replacement.id));
                    }
                    case 'file':
    ...
    resolveFromMap(variableKind, match, argument, commandValueMapping, prefix) {
        if (argument && commandValueMapping) {
            ...
            throw new VariableError(variableKind, localize(18185, null, match));
        }
        return match;
    }
}
