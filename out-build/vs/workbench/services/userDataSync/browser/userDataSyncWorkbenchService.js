...
let UserDataSyncWorkbenchService = class UserDataSyncWorkbenchService extends Disposable {
    ...
    async turnOn() {
        if (!this.authenticationProviders.length) {
            throw new Error(localize(19021, null));
        }
        ...
        if (this.accountStatus !== "available" /* AccountStatus.Available */) {
            throw new Error(localize(19022, null));
        }
        ...
        const disposable = isWeb ? Disposable.None : this.lifecycleService.onBeforeShutdown(e => e.veto((async () => {
            const { confirmed } = await this.dialogService.confirm({
                type: 'warning',
                message: localize(19023, null),
                title: localize(19024, null),
                primaryButton: localize(19025, null),
                cancelButton: localize(19026, null)
            });
            if (confirmed) {
        ...
        this.notificationService.info(localize(19027, null, SYNC_TITLE.value));
    ...
    async doTurnOnSync(token) {
        ...
        try {
            await this.progressService.withProgress({
                ...
                progress.report({ message: localize(19028, null) });
                disposables.add(this.userDataSyncService.onDidChangeStatus(status => {
                    if (status === "hasConflicts" /* SyncStatus.HasConflicts */) {
                        progress.report({ message: localize(19029, null) });
                    }
                    else {
                        progress.report({ message: localize(19030, null) });
                    }
                }));
    ...
    async handleConflictsWhileTurningOn(token) {
        ...
        const andSeparator = localize(19031, null);
        let conflictsText = '';
        ...
        await this.dialogService.prompt({
            ...
            message: localize(19032, null, conflictsText),
            detail: localize(19033, null),
            buttons: [
                {
                    label: localize(19034, null),
                    run: async () => {
                ...
                {
                    label: singleConflictResource ? localize(19035, null, singleConflictResource) : localize(19036, null),
                    run: async () => this.replace(true)
                },
                {
                    label: singleConflictResource ? localize(19037, null, singleConflictResource) : localize(19038, null),
    ...
    async resetSyncedData() {
        const { confirmed } = await this.dialogService.confirm({
            type: 'info',
            message: localize(19039, null),
            title: localize(19040, null),
            primaryButton: localize(19041, null),
        });
        if (confirmed) {
    ...
    async downloadSyncActivity() {
        const result = await this.fileDialogService.showOpenDialog({
            title: localize(19042, null),
            canSelectFiles: false,
            canSelectFolders: true,
            canSelectMany: false,
            openLabel: localize(19043, null),
        });
        if (!result?.[0]) {
            return;
        }
    ...
    async signIn() {
        ...
        if (authenticationProvider) {
            ...
            if (!this.authenticationProviders.length) {
                throw new Error(localize(19044, null));
            }
            await this.pick();
        }
    }
    ...
    async doPick() {
        ...
        quickPick.placeholder = localize(19045, null);
        quickPick.show();
        if (authenticationProviders.length > 1) {
            quickPick.busy = true;
            for (const { id, scopes } of authenticationProviders) {
    ...
    createQuickpickItems(authenticationProviders, allAccounts) {
        ...
        // Signed in Accounts
        if (allAccounts.size) {
            quickPickItems.push({ type: 'separator', label: localize(19046, null) });
            for (const authenticationProvider of authenticationProviders) {
                ...
                for (const account of accounts) {
                    quickPickItems.push({
                        ...
                        description: account.sessionId === this.current?.sessionId ? localize(19047, null) : undefined,
                        account,
                        authenticationProvider,
                    });
                }
            }
            quickPickItems.push({ type: 'separator', label: localize(19048, null) });
        }
        // Account Providers
        for (const authenticationProvider of authenticationProviders) {
            ...
            if (!allAccounts.has(authenticationProvider.id) || provider.supportsMultipleAccounts) {
                ...
                quickPickItems.push({ label: localize(19049, null, providerName), authenticationProvider });
            }
        }
