    ...
    updateWindowProgress(idx = 0) {
        ...
        if (idx < this.windowProgressStack.length) {
            ...
            if (progressTitle && progressMessage) {
                // <title>: <message>
                text = localize(18653, null, progressTitle, progressMessage);
                title = source ? localize(18654, null, source, progressTitle, progressMessage) : text;
            }
            else if (progressTitle) {
                // <title>
                text = progressTitle;
                title = source ? localize(18655, null, source, progressTitle) : text;
            }
            else if (progressMessage) {
                // <message>
                text = progressMessage;
                title = source ? localize(18656, null, source, progressMessage) : text;
            }
            else {
        ...
        const createNotification = (message, priority, increment) => {
            ...
            if (options.cancellable) {
                const cancelAction = new class extends Action {
                    constructor() {
                        super('progress.cancel', typeof options.cancellable === 'string' ? options.cancellable : localize(18658, null), undefined, true);
                    }
                    async run() {
        ...
        const createDialog = (message) => {
            ...
            if (!options.sticky) {
                buttons.push(options.cancellable
                    ? (typeof options.cancellable === 'boolean' ? localize(18659, null) : options.cancellable)
                    : localize(18660, null));
            }
