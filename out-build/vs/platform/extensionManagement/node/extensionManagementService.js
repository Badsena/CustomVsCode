    ...
    async install(vsix, options = {}) {
        ...
        try {
            ...
            if (manifest.engines && manifest.engines.vscode && !isEngineValid(manifest.engines.vscode, this.productService.version, this.productService.date)) {
                throw new Error(nls.localize(2141, null, extensionId, this.productService.version));
            }
            ...
            if (allowedToInstall !== true) {
                throw new Error(nls.localize(2142, null, allowedToInstall.value));
            }
    ...
    async downloadAndExtractGalleryExtension(extensionKey, gallery, operation, options, token) {
        ...
        try {
            ...
            if (!new ExtensionKey(gallery.identifier, gallery.version).equals(new ExtensionKey({ id: getGalleryExtensionId(manifest.publisher, manifest.name) }, manifest.version))) {
                throw new ExtensionManagementError(nls.localize(2143, null, gallery.identifier.id), "Invalid" /* ExtensionManagementErrorCode.Invalid */);
            }
    ...
    async downloadExtension(extension, operation, verifySignature, clientTargetPlatform) {
        ...
        if (verificationStatus !== ExtensionSignatureVerificationCode.Success
            ...
            if (!verificationStatus) {
                throw new ExtensionManagementError(nls.localize(2144, null), "SignatureVerificationInternal" /* ExtensionManagementErrorCode.SignatureVerificationInternal */);
            }
            switch (verificationStatus) {
                ...
                case ExtensionSignatureVerificationCode.NotSigned:
                    throw new ExtensionManagementError(nls.localize(2145, null, verificationStatus), "SignatureVerificationFailed" /* ExtensionManagementErrorCode.SignatureVerificationFailed */);
            }
            throw new ExtensionManagementError(nls.localize(2146, null, verificationStatus), "SignatureVerificationInternal" /* ExtensionManagementErrorCode.SignatureVerificationInternal */);
        }
...
let ExtensionsScanner = class ExtensionsScanner extends Disposable {
    ...
    async extractUserExtension(extensionKey, zipPath, removeIfExists, token) {
        ...
        if (await this.fileService.exists(extensionLocation)) {
            ...
            try {
                ...
                throw new ExtensionManagementError(nls.localize(2147, null, extensionLocation.fsPath, extensionKey.id), "Delete" /* ExtensionManagementErrorCode.Delete */);
            }
        }
        try {
            if (token.isCancellationRequested) {
    ...
    async scanLocalExtension(location, type, profileLocation) {
        try {
            ...
            throw new ExtensionManagementError(nls.localize(2148, null, location.path), "ScanningExtension" /* ExtensionManagementErrorCode.ScanningExtension */);
        }
        catch (error) {
        ...
        if (this.source instanceof URI) {
            if (existingExtension) {
                if (this.extensionKey.equals(new ExtensionKey(existingExtension.identifier, existingExtension.manifest.version))) {
                    try {
                        ...
                        throw new Error(nls.localize(2149, null, this.manifest.displayName || this.manifest.name));
                    }
                }
            }
            ...
            if (existingWithSameVersion) {
                try {
                    ...
                    throw new Error(nls.localize(2150, null, this.manifest.displayName || this.manifest.name));
                }
            }
        }
        // Gallery
