...
let ExtensionsScanner = class ExtensionsScanner extends Disposable {
    ...
    async scanExtensionManifest(extensionLocation) {
        ...
        try {
            ...
            if (toFileOperationResult(error) !== 1 /* FileOperationResult.FILE_NOT_FOUND */) {
                this.logService.error(this.formatMessage(extensionLocation, localize(2134, null, manifestLocation.path, error.message)));
            }
            throw error;
        }
        let manifest;
        try {
            ...
            for (const e of errors) {
                this.logService.error(this.formatMessage(extensionLocation, localize(2135, null, manifestLocation.path, e.offset, e.length, getParseErrorMessage(e.error))));
            }
            throw err;
        }
        if (getNodeType(manifest) !== 'object') {
            const errorMessage = this.formatMessage(extensionLocation, localize(2136, null, manifestLocation.path));
            ...
            try {
                ...
                if (errors.length > 0) {
                    errors.forEach((error) => {
                        this.logService.error(this.formatMessage(extensionLocation, localize(2137, null, localizedMessages.default?.path, getParseErrorMessage(error.error))));
                    });
                    return extensionManifest;
                }
                else if (getNodeType(localizedMessages) !== 'object') {
                    this.logService.error(this.formatMessage(extensionLocation, localize(2138, null, localizedMessages.default?.path)));
                    return extensionManifest;
                }
    ...
    async getLocalizedMessages(extensionLocation, extensionManifest, nlsConfiguration) {
        ...
        const reportErrors = (localized, errors) => {
            errors.forEach((error) => {
                this.logService.error(this.formatMessage(extensionLocation, localize(2139, null, localized?.path, getParseErrorMessage(error.error))));
            });
        };
        const reportInvalidFormat = (localized) => {
            this.logService.error(this.formatMessage(extensionLocation, localize(2140, null, localized?.path)));
        };
