...
let AllowedExtensionsService = class AllowedExtensionsService extends Disposable {
    ...
    isAllowed(extension) {
        ...
        const extensionReason = new MarkdownString(nls.localize(2075, null, settingsCommandLink));
        if (!isUndefined(extensionValue)) {
            ...
            if (extensionValue === 'stable' && prerelease) {
                return new MarkdownString(nls.localize(2076, null, settingsCommandLink));
            }
            if (version !== '*' && Array.isArray(extensionValue) && !extensionValue.some(v => {
                ...
                return new MarkdownString(nls.localize(2077, null, version, settingsCommandLink));
            }
            return true;
        }
        ...
        if (!isUndefined(publisherValue)) {
            if (isBoolean(publisherValue)) {
                return publisherValue ? true : new MarkdownString(nls.localize(2078, null, publisherKey, settingsCommandLink));
            }
            if (publisherValue === 'stable' && prerelease) {
                return new MarkdownString(nls.localize(2079, null, publisherKey, settingsCommandLink));
            }
            return true;
        }
