...
let WindowsMainService = class WindowsMainService extends Disposable {
    ...
    async doExtractPathsFromAPI(openConfig) {
        ...
        const pathsToOpen = await Promise.all(coalesce(openConfig.urisToOpen || []).map(async (pathToOpen) => {
            ...
            this.dialogMainService.showMessageBox({
                type: 'info',
                buttons: [localize(2881, null)],
                message: uri.scheme === Schemas.file ? localize(2882, null) : localize(2883, null),
                detail: uri.scheme === Schemas.file ?
                    localize(2884, null, getPathLabel(uri, { os: OS, tildify: this.environmentMainService })) :
                    localize(2885, null, uri.toString(true))
    ...
    async onUNCHostNotAllowed(path, options) {
        ...
        const { response, checkboxChecked } = await this.dialogMainService.showMessageBox({
            ...
            message: localize(2889, null, uri.authority),
            detail: localize(2890, null, getPathLabel(uri, { os: OS, tildify: this.environmentMainService })),
            checkboxLabel: localize(2891, null, uri.authority),
            cancelId: 1
        });
        if (response === 0) {
