...
let CodeWindow = class CodeWindow extends BaseWindow {
            ...
            case 2 /* WindowError.PROCESS_GONE */:
                ...
                if (type === 1 /* WindowError.UNRESPONSIVE */) {
                    ...
                    const { response, checkboxChecked } = await this.dialogMainService.showMessageBox({
                        ...
                        message: localize(2869, null),
                        detail: localize(2870, null),
                        checkboxLabel: this._config?.workspace ? localize(2871, null) : undefined
                    }, this._win);
                    // Handle choice
                ...
                // Process gone
                    ...
                    let message;
                    if (!details) {
                        message = localize(2872, null);
                    }
                    else {
                        message = localize(2873, null, details.reason, details.exitCode ?? '<unknown>');
                    }
                    // Show Dialog
                    const { response, checkboxChecked } = await this.dialogMainService.showMessageBox({
                        type: 'warning',
                        buttons: [
                            this._config?.workspace ? localize(2874, null) : localize(2875, null),
                            localize(2876, null)
                        ],
                        message,
                        detail: this._config?.workspace ?
                            localize(2877, null) :
                            localize(2878, null),
                        checkboxLabel: this._config?.workspace ? localize(2879, null) : undefined
                    }, this._win);
                    // Handle choice
                    const reopen = response === 0;
                    await this.destroyWindow(reopen, checkboxChecked);
    ...
    setMenuBarVisibility(visibility, notify = true) {
        ...
        if (visibility === 'toggle') {
            if (notify) {
                this.send('vscode:showInfoMessage', localize(2880, null));
            }
        }
