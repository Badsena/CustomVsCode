...
export class AbstractKeybindingService extends Disposable {
    ...
    _expectAnotherChord(firstChord, keypressLabel) {
        ...
        switch (this._currentChords.length) {
            ...
            case 1:
                ...
                this._currentChordStatusMessage = this._notificationService.status(nls.localize(2233, null, keypressLabel));
                break;
            default: {
                const fullKeypressLabel = this._currentChords.map(({ label }) => label).join(', ');
                this._currentChordStatusMessage = this._notificationService.status(nls.localize(2234, null, fullKeypressLabel));
            }
        }
        ...
        switch (resolveResult.kind) {
            case 0 /* ResultKind.NoMatchingKb */: {
                ...
                if (this.inChordMode) {
                    ...
                    this._notificationService.status(nls.localize(2235, null, currentChordsLabel, keypressLabel), { hideAfter: 10 * 1000 /* 10s */ });
                    this._leaveChordMode();
            ...
            case 2 /* ResultKind.KbFound */: {
                ...
                if (resolveResult.commandId === null || resolveResult.commandId === '') {
                    if (this.inChordMode) {
                        ...
                        this._notificationService.status(nls.localize(2236, null, currentChordsLabel, keypressLabel), { hideAfter: 10 * 1000 /* 10s */ });
                        this._leaveChordMode();
                        shouldPreventDefault = true;
                    }
                }
    ...
    appendKeybinding(label, commandId, context, enforceContextCheck) {
        if (commandId) {
            ...
            if (keybindingLabel) {
                return nls.localize(2237, null, label, keybindingLabel);
            }
        }
        return label;
    }
