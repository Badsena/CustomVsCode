...
let NativeHostMainService = class NativeHostMainService extends Disposable {
    ...
    async openSessionsWindow(windowId) {
    ...
    async installShellCommandWithPrivileges(windowId, source, target) {
        const { response } = await this.showMessageBox(windowId, {
            type: 'info',
            message: localize(2333, null, this.productService.nameShort),
            buttons: [
                localize(2334, null),
                localize(2335, null)
            ]
        ...
        try {
            ...
            throw new Error(localize(2336, null, source));
        }
    }
        ...
        try {
            ...
            switch (error.code) {
                case 'EACCES': {
                    const { response } = await this.showMessageBox(windowId, {
                        type: 'info',
                        message: localize(2337, null, this.productService.nameShort),
                        buttons: [
                            localize(2338, null),
                            localize(2339, null)
                        ]
                    ...
                    try {
                        ...
                        throw new Error(localize(2340, null, source));
                    }
                    break;
                }
                case 'ENOENT':
    ...
    async getShellCommandLink() {
        ...
        if (!sourceExists) {
            throw new Error(localize(2341, null, target));
        }
    ...
    async doOpenShellExternal(windowId, url) {
        try {
            ...
            if (matchesSomeScheme(url, Schemas.http, Schemas.https)) {
                isLink = true;
                message = localize(2342, null);
            }
            else {
                isLink = false;
                message = localize(2343, null);
            }
    ...
    async stopTracing(windowId) {
        ...
        await this.dialogMainService.showMessageBox({
            type: 'info',
            message: localize(2347, null),
            detail: localize(2348, null, path),
            buttons: [localize(2349, null)],
