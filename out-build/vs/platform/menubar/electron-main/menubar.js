...
let Menubar = class Menubar extends Disposable {
    ...
    install() {
        ...
        if (isMacintosh && !this.appMenuInstalled) {
            ...
            dockMenu.append(new MenuItem({ label: this.mnemonicLabel(nls.localize(2290, null)), click: () => this.windowsMainService.openEmptyWindow({ context: 1 /* OpenContext.DOCK */ }) }));
            app.dock.setMenu(dockMenu);
        }
        // File
        if (this.shouldDrawMenu('File')) {
            const fileMenu = new Menu();
            const fileMenuItem = new MenuItem({ label: this.mnemonicLabel(nls.localize(2291, null)), submenu: fileMenu });
        ...
        if (this.shouldDrawMenu('Edit')) {
            ...
            const editMenuItem = new MenuItem({ label: this.mnemonicLabel(nls.localize(2292, null)), submenu: editMenu });
        ...
        if (this.shouldDrawMenu('Selection')) {
            ...
            const selectionMenuItem = new MenuItem({ label: this.mnemonicLabel(nls.localize(2293, null)), submenu: selectionMenu });
        ...
        if (this.shouldDrawMenu('View')) {
            ...
            const viewMenuItem = new MenuItem({ label: this.mnemonicLabel(nls.localize(2294, null)), submenu: viewMenu });
        ...
        if (this.shouldDrawMenu('Go')) {
            ...
            const gotoMenuItem = new MenuItem({ label: this.mnemonicLabel(nls.localize(2295, null)), submenu: gotoMenu });
        ...
        if (this.shouldDrawMenu('Run')) {
            ...
            const debugMenuItem = new MenuItem({ label: this.mnemonicLabel(nls.localize(2296, null)), submenu: debugMenu });
        ...
        if (this.shouldDrawMenu('Terminal')) {
            ...
            const terminalMenuItem = new MenuItem({ label: this.mnemonicLabel(nls.localize(2297, null)), submenu: terminalMenu });
        ...
        if (this.shouldDrawMenu('Window')) {
            ...
            macWindowMenuItem = new MenuItem({ label: this.mnemonicLabel(nls.localize(2298, null)), submenu: windowMenu, role: 'window' });
        ...
        if (this.shouldDrawMenu('Help')) {
            ...
            const helpMenuItem = new MenuItem({ label: this.mnemonicLabel(nls.localize(2299, null)), submenu: helpMenu, role: 'help' });
    ...
    setMacApplicationMenu(macApplicationMenu) {
        const about = this.createMenuItem(nls.localize(2300, null, this.productService.nameLong), 'workbench.action.showAboutDialog');
        const checkForUpdates = this.getUpdateMenuItems();
        let preferences;
        if (this.shouldDrawMenu('Preferences')) {
            const preferencesMenu = new Menu();
            this.setMenuById(preferencesMenu, 'Preferences');
            preferences = new MenuItem({ label: this.mnemonicLabel(nls.localize(2301, null)), submenu: preferencesMenu });
        }
        const servicesMenu = new Menu();
        const services = new MenuItem({ label: nls.localize(2302, null), role: 'services', submenu: servicesMenu });
        const hide = new MenuItem({ label: nls.localize(2303, null, this.productService.nameLong), role: 'hide', accelerator: 'Command+H' });
        const hideOthers = new MenuItem({ label: nls.localize(2304, null), role: 'hideOthers', accelerator: 'Command+Alt+H' });
        const showAll = new MenuItem({ label: nls.localize(2305, null), role: 'unhide' });
        const quit = new MenuItem(this.likeAction('workbench.action.quit', {
            label: nls.localize(2306, null, this.productService.nameLong), click: async (item, window, event) => {
    ...
    async confirmBeforeQuit(event) {
        ...
        if (confirmBeforeClose === 'always' || (confirmBeforeClose === 'keyboardOnly' && this.isKeyboardEvent(event))) {
            const { response } = await this.nativeHostMainService.showMessageBox(this.windowsMainService.getFocusedWindow()?.id, {
                type: 'question',
                buttons: [
                    isMacintosh ? nls.localize(2307, null) : nls.localize(2308, null),
                    nls.localize(2309, null)
                ],
                message: isMacintosh ? nls.localize(2310, null) : nls.localize(2311, null)
            });
            return response === 0;
        }
        return true;
    ...
    setMacWindowMenu(macWindowMenu) {
        const minimize = new MenuItem({ label: nls.localize(2312, null), role: 'minimize', accelerator: 'Command+M', enabled: this.windowsMainService.getWindowCount() > 0 });
        const zoom = new MenuItem({ label: nls.localize(2313, null), role: 'zoom', enabled: this.windowsMainService.getWindowCount() > 0 });
        const bringAllToFront = new MenuItem({ label: nls.localize(2314, null), role: 'front', enabled: this.windowsMainService.getWindowCount() > 0 });
        const switchWindow = this.createMenuItem(nls.localize(2315, null), 'workbench.action.switchWindow');
        const nativeTabMenuItems = [];
        if (this.currentEnableNativeTabs) {
            nativeTabMenuItems.push(__separator__());
            nativeTabMenuItems.push(this.createMenuItem(nls.localize(2316, null), 'workbench.action.newWindowTab'));
            nativeTabMenuItems.push(this.createRoleMenuItem(nls.localize(2317, null), 'workbench.action.showPreviousWindowTab', 'selectPreviousTab'));
            nativeTabMenuItems.push(this.createRoleMenuItem(nls.localize(2318, null), 'workbench.action.showNextWindowTab', 'selectNextTab'));
            nativeTabMenuItems.push(this.createRoleMenuItem(nls.localize(2319, null), 'workbench.action.moveWindowTabToNewWindow', 'moveTabToNewWindow'));
            nativeTabMenuItems.push(this.createRoleMenuItem(nls.localize(2320, null), 'workbench.action.mergeAllWindowTabs', 'mergeAllWindows'));
        }
        [
            minimize,
            zoom,
    ...
    getUpdateMenuItems() {
        ...
        switch (state.type) {
            case "idle" /* StateType.Idle */:
                return [new MenuItem({
                        label: this.mnemonicLabel(nls.localize(2321, null)), click: () => setTimeout(() => {
                            this.reportMenuActionTelemetry('CheckForUpdate');
                            this.updateService.checkForUpdates(true);
                        }, 0)
                    })];
            case "checking for updates" /* StateType.CheckingForUpdates */:
                return [new MenuItem({ label: nls.localize(2322, null), enabled: false })];
            case "available for download" /* StateType.AvailableForDownload */:
                return [new MenuItem({
                        label: this.mnemonicLabel(nls.localize(2323, null)), click: () => {
                            this.updateService.downloadUpdate(true);
                        }
                    })];
            case "downloading" /* StateType.Downloading */:
            case "overwriting" /* StateType.Overwriting */:
                return [new MenuItem({ label: nls.localize(2324, null), enabled: false })];
            case "downloaded" /* StateType.Downloaded */:
                return isMacintosh ? [] : [new MenuItem({
                        label: this.mnemonicLabel(nls.localize(2325, null)), click: () => {
                            this.reportMenuActionTelemetry('InstallUpdate');
                            this.updateService.applyUpdate();
                        }
                    })];
            case "updating" /* StateType.Updating */:
                return [new MenuItem({ label: nls.localize(2326, null), enabled: false })];
            case "ready" /* StateType.Ready */:
                return [new MenuItem({
                        label: this.mnemonicLabel(nls.localize(2327, null)), click: () => {
