...
export class DiskFileSystemProvider extends AbstractDiskFileSystemProvider {
    ...
    async doWriteFile(resource, content, opts, disableWriteLock) {
        ...
        try {
            ...
            if (!opts.create || !opts.overwrite) {
                ...
                if (fileExists) {
                    if (!opts.overwrite) {
                        throw createFileSystemProviderError(localize(2223, null), FileSystemProviderErrorCode.FileExists);
                    }
                }
                else {
                    if (!opts.create) {
                        throw createFileSystemProviderError(localize(2224, null), FileSystemProviderErrorCode.FileNotFound);
                    }
                }
            }
            // Open
    ...
    async rename(from, to, opts) {
        ...
        try {
            ...
            if (error.code === 'EINVAL' || error.code === 'EBUSY' || error.code === 'ENAMETOOLONG') {
                error = new Error(localize(2225, null, basename(fromFilePath), basename(dirname(toFilePath)), error.toString()));
            }
            throw this.toFileSystemProviderError(error);
        }
    }
    async copy(from, to, opts) {
        ...
        try {
            ...
            // to something the user can better understand
            if (error.code === 'EINVAL' || error.code === 'EBUSY' || error.code === 'ENAMETOOLONG') {
                error = new Error(localize(2226, null, basename(fromFilePath), basename(dirname(toFilePath)), error.toString()));
            }
            throw this.toFileSystemProviderError(error);
        }
    }
    async validateMoveCopy(from, to, mode, overwrite) {
        ...
        if (isSameResourceWithDifferentPathCase) {
            ...
            if (mode === 'copy') {
                throw createFileSystemProviderError(localize(2227, null), FileSystemProviderErrorCode.FileExists);
            }
        ...
        if (!fromStat) {
            throw createFileSystemProviderError(localize(2228, null), FileSystemProviderErrorCode.FileNotFound);
        }
        ...
        if (!overwrite) {
            throw createFileSystemProviderError(localize(2229, null), FileSystemProviderErrorCode.FileExists);
        }
