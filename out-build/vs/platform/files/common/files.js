...
export function ensureFileSystemProviderError(error) {
    if (!error) {
        return createFileSystemProviderError(localize(2186, null), FileSystemProviderErrorCode.Unknown); // https://github.com/microsoft/vscode/issues/72798
    }
    return error;
}
...
export class ByteSize {
    ...
    static formatSize(size) {
        ...
        if (size < ByteSize.KB) {
            return localize(2187, null, size.toFixed(0));
        }
        if (size < ByteSize.MB) {
            return localize(2188, null, (size / ByteSize.KB).toFixed(2));
        }
        if (size < ByteSize.GB) {
            return localize(2189, null, (size / ByteSize.MB).toFixed(2));
        }
        if (size < ByteSize.TB) {
            return localize(2190, null, (size / ByteSize.GB).toFixed(2));
        }
        return localize(2191, null, (size / ByteSize.TB).toFixed(2));
    }
}
