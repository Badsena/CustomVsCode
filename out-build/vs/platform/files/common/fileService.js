...
let FileService = class FileService extends Disposable {
    ...
    async withProvider(resource) {
        ...
        if (!isAbsolutePath(resource)) {
            throw new FileOperationError(localize(2192, null, this.resourceForError(resource)), 8 /* FileOperationResult.FILE_INVALID_PATH */);
        }
        // Activate provider
        await this.activateProvider(resource.scheme);
        // Assert provider
        ...
        if (!provider) {
            ...
            error.message = localize(2193, null, resource.toString());
            throw error;
        }
        return provider;
    }
    ...
    async resolve(resource, options) {
        try {
            ...
            if (toFileSystemProviderErrorCode(error) === FileSystemProviderErrorCode.FileNotFound) {
                throw new FileOperationError(localize(2194, null, this.resourceForError(resource)), 1 /* FileOperationResult.FILE_NOT_FOUND */);
            }
    ...
    async doValidateCreateFile(resource, options) {
        ...
        if (!options?.overwrite && await this.exists(resource)) {
            throw new FileOperationError(localize(2195, null, this.resourceForError(resource)), 3 /* FileOperationResult.FILE_MODIFIED_SINCE */, options);
        }
    }
    ...
    async writeFile(resource, bufferOrReadableOrStream, options) {
        ...
        try {
            ...
            throw new FileOperationError(localize(2196, null, this.resourceForError(resource), ensureFileSystemProviderError(error).toString()), toFileOperationResult(error), writeFileOptions);
        }
    ...
    async validateWriteFile(provider, resource, bufferOrReadableOrStream, options) {
        ...
        if (unlock && !(provider.capabilities & 8192 /* FileSystemProviderCapabilities.FileWriteUnlock */)) {
            throw new Error(localize(2197, null, this.resourceForError(resource)));
        }
        // Validate append support
        if (options?.append && !hasFileAppendCapability(provider)) {
            throw new FileOperationError(localize(2198, null, this.resourceForError(resource)), 6 /* FileOperationResult.FILE_PERMISSION_DENIED */);
        }
        // Validate atomic support
        const atomic = !!options?.atomic;
        if (atomic) {
            if (!(provider.capabilities & 32768 /* FileSystemProviderCapabilities.FileAtomicWrite */)) {
                throw new Error(localize(2199, null, this.resourceForError(resource)));
            }
            if (!(provider.capabilities & 2 /* FileSystemProviderCapabilities.FileReadWrite */)) {
                throw new Error(localize(2200, null, this.resourceForError(resource)));
            }
            if (unlock) {
                throw new Error(localize(2201, null, this.resourceForError(resource)));
            }
        }
        ...
        if ((stat.type & FileType.Directory) !== 0) {
            throw new FileOperationError(localize(2202, null, this.resourceForError(resource)), 0 /* FileOperationResult.FILE_IS_DIRECTORY */, options);
        }
        ...
        if (typeof options?.mtime === 'number' && typeof options.etag === 'string' && options.etag !== ETAG_DISABLED &&
            ...
            throw new FileOperationError(localize(2203, null), 3 /* FileOperationResult.FILE_MODIFIED_SINCE */, options);
        }
        return { stat, buffer };
    }
    ...
    restoreReadError(error, resource, options) {
        const message = localize(2204, null, this.resourceForError(resource), ensureFileSystemProviderError(error).toString());
    ...
    async validateReadFile(resource, options) {
        ...
        if (stat.isDirectory) {
            throw new FileOperationError(localize(2205, null, this.resourceForError(resource)), 0 /* FileOperationResult.FILE_IS_DIRECTORY */, options);
        }
        // Throw if file not modified since (unless disabled)
        if (typeof options?.etag === 'string' && options.etag !== ETAG_DISABLED && options.etag === stat.etag) {
            throw new NotModifiedSinceFileOperationError(localize(2206, null), stat, options);
        }
        // Throw if file is too large to load
    ...
    validateReadFileLimits(resource, size, options) {
        if (typeof options?.limits?.size === 'number' && size > options.limits.size) {
            throw new TooLargeFileOperationError(localize(2207, null, this.resourceForError(resource)), 7 /* FileOperationResult.FILE_TOO_LARGE */, size, options);
        }
    }
    //#endregion
    ...
    async doValidateMoveCopy(sourceProvider, source, targetProvider, target, mode, overwrite) {
        ...
        if (sourceProvider === targetProvider) {
            ...
            if (isSameResourceWithDifferentPathCase && mode === 'copy') {
                throw new Error(localize(2208, null, this.resourceForError(source), this.resourceForError(target)));
            }
            if (!isSameResourceWithDifferentPathCase && providerExtUri.isEqualOrParent(target, source)) {
                throw new Error(localize(2209, null, this.resourceForError(source), this.resourceForError(target)));
            }
        }
        ...
        if (exists && !isSameResourceWithDifferentPathCase) {
            ...
            if (!overwrite) {
                throw new FileOperationError(localize(2210, null, this.resourceForError(source), this.resourceForError(target)), 4 /* FileOperationResult.FILE_MOVE_CONFLICT */);
            }
            ...
            if (sourceProvider === targetProvider) {
                ...
                if (providerExtUri.isEqualOrParent(source, target)) {
                    throw new Error(localize(2211, null, this.resourceForError(source), this.resourceForError(target)));
                }
            }
        }
    ...
    async mkdirp(provider, directory) {
        ...
        while (!providerExtUri.isEqual(directory, providerExtUri.dirname(directory))) {
            try {
                ...
                if ((stat.type & FileType.Directory) === 0) {
                    throw new Error(localize(2212, null, this.resourceForError(directory)));
                }
    ...
    async doValidateDelete(resource, options) {
        ...
        if (useTrash && !(provider.capabilities & 4096 /* FileSystemProviderCapabilities.Trash */)) {
            throw new Error(localize(2213, null, this.resourceForError(resource)));
        }
        // Validate atomic support
        const atomic = options?.atomic;
        if (atomic && !(provider.capabilities & 65536 /* FileSystemProviderCapabilities.FileAtomicDelete */)) {
            throw new Error(localize(2214, null, this.resourceForError(resource)));
        }
        if (useTrash && atomic) {
            throw new Error(localize(2215, null, this.resourceForError(resource)));
        }
        // Validate delete
        let stat = undefined;
        try {
        ...
        if (stat) {
            ...
            throw new FileOperationError(localize(2216, null, this.resourceForError(resource)), 1 /* FileOperationResult.FILE_NOT_FOUND */);
        }
        // Validate recursive
        ...
        if (!recursive) {
            ...
            if (stat.isDirectory && Array.isArray(stat.children) && stat.children.length > 0) {
                throw new Error(localize(2217, null, this.resourceForError(resource)));
            }
        }
        return provider;
    }
    ...
    throwIfFileSystemIsReadonly(provider, resource) {
        if (provider.capabilities & 2048 /* FileSystemProviderCapabilities.Readonly */) {
            throw new FileOperationError(localize(2218, null, this.resourceForError(resource)), 6 /* FileOperationResult.FILE_PERMISSION_DENIED */);
        }
        return provider;
    }
    throwIfFileIsReadonly(resource, stat) {
        if ((stat.permissions ?? 0) & FilePermission.Readonly) {
            throw new FileOperationError(localize(2219, null, this.resourceForError(resource)), 6 /* FileOperationResult.FILE_PERMISSION_DENIED */);
        }
    }
