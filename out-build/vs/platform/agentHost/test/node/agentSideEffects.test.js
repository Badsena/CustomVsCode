...
suite('AgentSideEffects', () => {
    ...
    suite('registerProgressListener', () => {
        test('maps agent progress events to state actions', () => {
            ...
            assert.ok(envelopes.some(e => e.action.type === "session/delta" /* ActionType.SessionDelta */));
        });
        test('returns a disposable that stops listening', () => {
            ...
            assert.strictEqual(envelopes.filter(e => e.action.type === "session/delta" /* ActionType.SessionDelta */).length, 1);
            listener.dispose();
    ...
    suite('handleCreateSession', () => {
        test('creates a session and dispatches session/ready', async () => {
            ...
            const ready = envelopes.find(e => e.action.type === "session/ready" /* ActionType.SessionReady */);
            assert.ok(ready, 'should dispatch session/ready');
        });
        test('throws when no provider is specified', async () => {
            await assert.rejects(() => sideEffects.handleCreateSession({ session: sessionUri.toString() }), /No provider specified/);
        });
        test('throws when no agent matches provider', async () => {
            ...
            await assert.rejects(() => noAgentSideEffects.handleCreateSession({ session: sessionUri.toString(), provider: 'nonexistent' }), /No agent registered/);
        });
    });
    ...
    suite('handleBrowseDirectory', () => {
        test('throws when the directory does not exist', async () => {
            await assert.rejects(() => sideEffects.handleBrowseDirectory(URI.from({ scheme: Schemas.inMemory, path: '/nonexistent' }).toString()), /Directory not found/);
        });
        test('throws when the target is not a directory', async () => {
            await assert.rejects(() => sideEffects.handleBrowseDirectory(URI.from({ scheme: Schemas.inMemory, path: '/testDir/file.txt' }).toString()), /Not a directory/);
        });
    });
    // ---- agents observable --------------------------------------------------
...
//# sourceMappingURL=agentSideEffects.test.js.map