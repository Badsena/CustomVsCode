...
let TerminalProcess = class TerminalProcess extends Disposable {
    ...
    async start() {
        ...
        try {
            ...
            if (errorMessage?.includes('Cannot launch conpty')) {
                return { message: localize(2483, null, 'https://code.visualstudio.com/updates/v1_109#_removal-of-winpty-support', 'terminal.integrated.windowsUseConptyDll') };
            }
    ...
    async _validateCwd() {
        try {
            ...
            if (!result.isDirectory()) {
                return { message: localize(2484, null, this._initialCwd.toString()) };
            }
        }
        catch (err) {
            if (err?.code === 'ENOENT') {
                return { message: localize(2485, null, this._initialCwd.toString()) };
            }
        }
    ...
    async _validateExecutable() {
        ...
        if (!executable) {
            return { message: localize(2486, null, slc.executable) };
        }
        try {
            const result = await fs.promises.stat(executable);
            if (!result.isFile() && !result.isSymbolicLink()) {
                return { message: localize(2487, null, slc.executable) };
            }
