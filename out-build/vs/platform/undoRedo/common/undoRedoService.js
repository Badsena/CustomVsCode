...
class RemovedResources {
    ...
    createMessage() {
        ...
        if (externalRemoval.length > 0) {
            messages.push(nls.localize(2800, null, externalRemoval.join(', ')));
        }
        if (noParallelUniverses.length > 0) {
            messages.push(nls.localize(2801, null, noParallelUniverses.join(', ')));
        }
...
let UndoRedoService = class UndoRedoService {
    ...
    _checkWorkspaceUndo(strResource, element, editStackSnapshot, checkInvalidatedResources) {
        if (element.removedResources) {
            return this._tryToSplitAndUndo(strResource, element, element.removedResources, nls.localize(2802, null, element.label, element.removedResources.createMessage()));
        }
        if (checkInvalidatedResources && element.invalidatedResources) {
            return this._tryToSplitAndUndo(strResource, element, element.invalidatedResources, nls.localize(2803, null, element.label, element.invalidatedResources.createMessage()));
        }
        ...
        if (cannotUndoDueToResources.length > 0) {
            return this._tryToSplitAndUndo(strResource, element, null, nls.localize(2804, null, element.label, cannotUndoDueToResources.join(', ')));
        }
        ...
        if (cannotLockDueToResources.length > 0) {
            return this._tryToSplitAndUndo(strResource, element, null, nls.localize(2805, null, element.label, cannotLockDueToResources.join(', ')));
        }
        // check if new stack elements were added in the meantime...
        if (!editStackSnapshot.isValid()) {
            return this._tryToSplitAndUndo(strResource, element, null, nls.localize(2806, null, element.label));
        }
        return null;
    }
    ...
    async _confirmAndExecuteWorkspaceUndo(strResource, element, editStackSnapshot, undoConfirmed) {
        if (element.canSplit() && !this._isPartOfUndoGroup(element)) {
            ...
            const { result } = await this._dialogService.prompt({
                type: Severity.Info,
                message: nls.localize(2807, null, element.label),
                buttons: [
                    {
                        label: nls.localize(2808, null, editStackSnapshot.editStacks.length),
                        run: () => UndoChoice.All
                    },
                    {
                        label: nls.localize(2809, null),
                        run: () => UndoChoice.This
                    }
                ],
                cancelButton: {
    ...
    _resourceUndo(editStack, element, undoConfirmed) {
        ...
        if (editStack.locked) {
            const message = nls.localize(2810, null, element.label);
            this._notificationService.warn(message);
            return;
        }
    ...
    async _confirmAndContinueUndo(strResource, sourceId, element) {
        const result = await this._dialogService.confirm({
            message: nls.localize(2811, null, element.label),
            primaryButton: nls.localize(2812, null),
            cancelButton: nls.localize(2813, null)
        });
        if (!result.confirmed) {
            return;
        }
    ...
    _checkWorkspaceRedo(strResource, element, editStackSnapshot, checkInvalidatedResources) {
        if (element.removedResources) {
            return this._tryToSplitAndRedo(strResource, element, element.removedResources, nls.localize(2814, null, element.label, element.removedResources.createMessage()));
        }
        if (checkInvalidatedResources && element.invalidatedResources) {
            return this._tryToSplitAndRedo(strResource, element, element.invalidatedResources, nls.localize(2815, null, element.label, element.invalidatedResources.createMessage()));
        }
        ...
        if (cannotRedoDueToResources.length > 0) {
            return this._tryToSplitAndRedo(strResource, element, null, nls.localize(2816, null, element.label, cannotRedoDueToResources.join(', ')));
        }
        const cannotLockDueToResources = [];
        for (const editStack of editStackSnapshot.editStacks) {
            if (editStack.locked) {
        ...
        if (cannotLockDueToResources.length > 0) {
            return this._tryToSplitAndRedo(strResource, element, null, nls.localize(2817, null, element.label, cannotLockDueToResources.join(', ')));
        }
        // check if new stack elements were added in the meantime...
        if (!editStackSnapshot.isValid()) {
            return this._tryToSplitAndRedo(strResource, element, null, nls.localize(2818, null, element.label));
        }
        return null;
    }
    ...
    _resourceRedo(editStack, element) {
        ...
        if (editStack.locked) {
            const message = nls.localize(2819, null, element.label);
            this._notificationService.warn(message);
            return;
        }
        return this._invokeResourcePrepare(element, (cleanup) => {
