...
let WorkspacesHistoryMainService = class WorkspacesHistoryMainService extends Disposable {
    ...
    async clearRecentlyOpened(options) {
        if (options?.confirm) {
            const { response } = await this.dialogMainService.showMessageBox({
                ...
                message: localize(2895, null),
                detail: localize(2896, null),
                cancelId: 1
            });
            if (response !== 0) {
                return;
    ...
    async updateWindowsJumpList() {
        ...
        jumpList.push({
            ...
            items: [
                {
                    type: 'task',
                    title: localize(2897, null),
                    description: localize(2898, null),
        ...
        if ((await this.getRecentlyOpened()).workspaces.length > 0) {
            ...
            if (items.length > 0) {
                jumpList.push({
                    type: 'custom',
                    name: hasWorkspaces ? localize(2899, null) : localize(2900, null),
                    items
                });
            }
        }
    ...
    getWindowsJumpListLabel(workspace, recentLabel) {
        ...
        if (this.workspacesManagementMainService.isUntitledWorkspace(workspace)) {
            return { title: localize(2901, null), description: '' };
        }
        // Workspace: normal
        ...
        return { title: localize(2902, null, filename), description: this.renderJumpListPathDescription(workspace.configPath) };
    }
