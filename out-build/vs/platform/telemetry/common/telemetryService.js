...
function getTelemetryLevelSettingDescription() {
    const telemetryText = localize(2423, null, product.nameLong);
    const externalLinksStatement = !product.privacyStatementUrl ?
        localize(2424, null, 'https://aka.ms/vscode-telemetry') :
        localize(2425, null, 'https://aka.ms/vscode-telemetry', product.privacyStatementUrl);
    const restartString = !isWeb ? localize(2426, null) : '';
    const crashReportsHeader = localize(2427, null);
    const errorsHeader = localize(2428, null);
    const usageHeader = localize(2429, null);
    const telemetryTableDescription = localize(2430, null);
...
| off   |       -       |        -        |     -      |
`;
    const deprecatedSettingNote = localize(2431, null);
...
configurationRegistry.registerConfiguration({
    ...
    'properties': {
        ...
        'telemetry.feedback.enabled': {
            type: 'boolean',
            default: true,
            description: localize(2442, null),
            policy: {
                name: 'EnableFeedback',
                category: PolicyCategory.Telemetry,
                minimumVersion: '1.99',
                localization: { description: { key: 'telemetry.feedback.enabled', value: localize(2443, null) } },
            }
        },
        ...
        [TELEMETRY_OLD_SETTING_ID]: {
            ...
            'markdownDescription': !product.privacyStatementUrl ?
                localize(2444, null, product.nameLong) :
                localize(2445, null, product.nameLong, product.privacyStatementUrl),
            'default': true,
            'restricted': true,
            'markdownDeprecationMessage': localize(2446, null, `\`#${TELEMETRY_SETTING_ID}#\``),
            'scope': 1 /* ConfigurationScope.APPLICATION */,
            'tags': ['usesOnlineServices', 'telemetry']
        }
    },
