...
class ConfigurationRegistry extends Disposable {
    ...
    updateDefaultOverrideProperty(key, newDefaultOverride, source) {
        const property = {
            ...
            description: nls.localize(1939, null, getLanguageTagSettingPlainKey(key)),
    ...
    updateOverridePropertyPatternKey() {
        for (const overrideIdentifier of this.overrideIdentifiers.values()) {
            ...
            const resourceLanguagePropertiesSchema = {
                type: 'object',
                description: nls.localize(1940, null),
                errorMessage: nls.localize(1941, null),
                $ref: resourceLanguageSettingsSchemaId,
            };
    ...
    registerOverridePropertyPatternKey() {
        const resourceLanguagePropertiesSchema = {
            type: 'object',
            description: nls.localize(1942, null),
            errorMessage: nls.localize(1943, null),
            $ref: resourceLanguageSettingsSchemaId,
        };
...
export function validateProperty(property, schema, extensionId) {
    if (!property.trim()) {
        return nls.localize(1944, null);
    }
    if (OVERRIDE_PROPERTY_REGEX.test(property)) {
        return nls.localize(1945, null, property);
    }
    if (configurationRegistry.getConfigurationProperties()[property] !== undefined && (!extensionId || !EXTENSION_UNIFICATION_EXTENSION_IDS.has(extensionId.toLowerCase()))) {
        return nls.localize(1946, null, property);
    }
    if (schema.policy?.name && configurationRegistry.getPolicyConfigurations().get(schema.policy?.name) !== undefined) {
        return nls.localize(1947, null, property, schema.policy?.name, configurationRegistry.getPolicyConfigurations().get(schema.policy?.name));
    }
    return null;
}
