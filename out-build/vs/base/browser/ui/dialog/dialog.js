...
export class Dialog extends Disposable {
    ...
    getIconAriaLabel() {
        let typeLabel = localize(3, null);
        switch (this.options.type) {
            case 'error':
                typeLabel = localize(4, null);
                break;
            case 'warning':
                typeLabel = localize(5, null);
                break;
            case 'pending':
                typeLabel = localize(6, null);
                break;
            case 'none':
            case 'info':
            case 'question':
    ...
    async show() {
        ...
        return new Promise(resolve => {
            ...
            if (!this.options.disableCloseAction && !this.options.disableCloseButton) {
                ...
                const action = this._register(new Action('dialog.close', localize(7, null), ThemeIcon.asClassName(Codicon.dialogClose), true, async () => {
                    resolve({
                        button: this.options.cancelId || 0,
                        checkboxChecked: this.checkbox ? this.checkbox.checked : undefined
                    });
