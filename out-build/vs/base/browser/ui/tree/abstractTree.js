...
class FindWidget extends Disposable {
    ...
    constructor(container, tree, contextViewProvider, placeholder, toggleContributions = [], options) {
        ...
        const closeAction = this._register(new Action('close', localize(34, null), 'codicon codicon-close', true, () => this.dispose()));
...
export class AbstractFindController {
    ...
    constructor(tree, filter, contextViewProvider, options = {}) {
        ...
        this._placeholder = options.placeholder ?? localize(35, null);
    }
    isOpened() {
        return !!this.widget;
    }
    ...
    renderMessage(showNotFound, warningMessage) {
        if (showNotFound) {
            if (this.tree.options.showNotFoundMessage ?? true) {
                this.widget?.showMessage({ type: 2 /* MessageType.WARNING */, content: warningMessage ?? localize(36, null) });
            }
            else {
    ...
    alertResults(results) {
        if (!results) {
            alert(localize(37, null));
        }
        else {
            alert(localize(38, null, results));
        }
    }
    dispose() {
...
export class FindController extends AbstractFindController {
    ...
    set mode(mode) {
        ...
        this.placeholder = isFilterMode ? localize(39, null) : localize(40, null);
        ...
        if (isMacintosh) {
            this.modelDisposables.add(model.onDidChangeCollapseState(e => {
                ...
                if (node.collapsible && !deep && this.isDOMFocused()) {
                    alert(node.collapsed ? localize(43, null) : localize(44, null));
                }
            }));
        }
    }
