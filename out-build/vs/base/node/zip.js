...
function extractEntry(stream, fileName, mode, targetPath, options, token) {
    ...
    if (!targetDirName.startsWith(targetPath)) {
        return Promise.reject(new Error(nls.localize(167, null, fileName)));
    }
    ...
    return new Promise((c, e) => {
        ...
        zipfile.once('close', () => last.then(() => {
            if (token.isCancellationRequested || zipfile.entryCount === extractedEntriesCount) {
                ...
                e(new ExtractError('Incomplete', new Error(nls.localize(168, null, extractedEntriesCount, zipfile.entryCount))));
            }
        }, e));
        zipfile.readEntry();
...
function read(zipPath, filePath) {
    return openZip(zipPath).then(zipfile => {
        return new Promise((c, e) => {
            ...
            zipfile.once('close', () => e(new Error(nls.localize(169, null, filePath))));
        });
    });
}
