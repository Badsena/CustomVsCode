...
class YamlParser {
    ...
    parseBlockMapping(baseIndent, inlineFirstEntry = false) {
        ...
        while (this.currentToken().type !== 13 /* TokenType.EOF */) {
            ...
            if (indent !== baseIndent) {
                if (indent > baseIndent) {
                    this.emitError(localize(155, null, baseIndent, indent), this.currentToken().startOffset, this.currentToken().endOffset, 'unexpected-indentation');
                }
                else {
                    break;
                }
            ...
            if (!this.options.allowDuplicateKeys && seenKeys.has(entry.key.value)) {
                this.emitError(localize(156, null, entry.key.value), entry.key.startOffset, entry.key.endOffset, 'duplicate-key');
            }
    ...
    parseMappingEntry(baseIndent) {
        ...
        if (colon.type !== 1 /* TokenType.Colon */) {
            this.emitError(localize(157, null), colon.startOffset, colon.endOffset, 'expected-colon');
            return undefined;
        }
    ...
    parseMappingValue(baseIndent, colonToken) {
        ...
        if (afterNewline.type === 13 /* TokenType.EOF */) {
            ...
            this.emitError(localize(158, null), colonToken.startOffset, colonToken.endOffset, 'missing-value');
        ...
        if (nextIndent <= baseIndent) {
            ...
            this.emitError(localize(159, null), colonToken.startOffset, colonToken.endOffset, 'missing-value');
            return this.makeEmptyScalar(colonToken.endOffset);
        }
    ...
    parseBlockSequence(baseIndent) {
        ...
        while (this.currentToken().type !== 13 /* TokenType.EOF */) {
            ...
            if (indent !== baseIndent) {
                if (indent > baseIndent) {
                    this.emitError(localize(160, null, baseIndent, indent), this.currentToken().startOffset, this.currentToken().endOffset, 'unexpected-indentation');
                }
                else {
                    break;
                }
    ...
    parseSequenceItemValue(baseIndent, dashToken) {
        ...
        this.skipNewlinesAndComments();
        if (this.currentToken().type === 13 /* TokenType.EOF */) {
            this.emitError(localize(161, null), dashToken.startOffset, dashToken.endOffset, 'missing-value');
            return this.makeEmptyScalar(dashToken.endOffset);
        }
        const nextIndent = this.currentIndent();
        if (nextIndent <= baseIndent) {
            // Empty item (just a dash)
            this.emitError(localize(162, null), dashToken.startOffset, dashToken.endOffset, 'missing-value');
            return this.makeEmptyScalar(dashToken.endOffset);
        }
    ...
    parseFlowMap() {
        ...
        while (this.currentToken().type !== 5 /* TokenType.FlowMapEnd */ && this.currentToken().type !== 13 /* TokenType.EOF */) {
            ...
            if (this.currentToken().type === 0 /* TokenType.Scalar */) {
                ...
                this.emitError(localize(163, null), this.currentToken().startOffset, this.currentToken().endOffset, 'expected-key');
                break;
            }
            this.skipFlowWhitespace();
        ...
        if (endToken.type === 5 /* TokenType.FlowMapEnd */) {
            ...
            this.emitError(localize(164, null), endToken.startOffset, endToken.endOffset, 'expected-flow-map-end');
        }
        return {
            type: 'map',
            properties,
    ...
    parseFlowSeq() {
        ...
        while (this.currentToken().type !== 7 /* TokenType.FlowSeqEnd */ && this.currentToken().type !== 13 /* TokenType.EOF */) {
            ...
            if (this.currentToken().type === 4 /* TokenType.FlowMapStart */) {
                ...
                this.emitError(localize(165, null), this.currentToken().startOffset, this.currentToken().endOffset, 'unexpected-token');
                this.advance();
                continue;
            }
            items.push(item);
        ...
        if (endToken.type === 7 /* TokenType.FlowSeqEnd */) {
            ...
            this.emitError(localize(166, null), endToken.startOffset, endToken.endOffset, 'expected-flow-seq-end');
        }
        return {
            type: 'sequence',
            items,
