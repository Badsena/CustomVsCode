...
import { app, BrowserWindow, powerMonitor, protocol, session, systemPreferences } from 'electron';
...
let CodeApplication = class CodeApplication extends Disposable {
    ...
    registerListeners() {
        ...
        app.on('activate', async (event, hasVisibleWindows) => {
            ...
            if (!hasVisibleWindows) {
                if (process.isEmbeddedApp || (this.environmentMainService.args['sessions'] && this.productService.quality !== 'stable')) {
                    await this.windowsMainService?.openSessionsWindow({ context: 1 /* OpenContext.DOCK */ });
                }
                else {
    ...
    async checkAmypoAuthorization() {
        ...
        const hasPortalUrl = process.argv.some(arg => arg.startsWith('amypocoder://'));
        if (hasPortalUrl) {
    ...
    async shouldBlockOpenable(openable, windowsMainService, dialogMainService) {
        ...
        if (isWorkspaceToOpen(openable)) {
            ...
            message = localize(170, null, openableUri.scheme === Schemas.file ? getPathLabel(openableUri, { os: OS, tildify: this.environmentMainService }) : openableUri.toString(true), this.productService.nameShort);
        }
        else if (isFolderToOpen(openable)) {
            openableUri = openable.folderUri;
            message = localize(171, null, openableUri.scheme === Schemas.file ? getPathLabel(openableUri, { os: OS, tildify: this.environmentMainService }) : openableUri.toString(true), this.productService.nameShort);
        }
        else {
            openableUri = openable.fileUri;
            message = localize(172, null, openableUri.scheme === Schemas.file ? getPathLabel(openableUri, { os: OS, tildify: this.environmentMainService }) : openableUri.toString(true), this.productService.nameShort);
        }
        ...
        const { response, checkboxChecked } = await dialogMainService.showMessageBox({
            ...
            message,
            detail: localize(175, null),
            checkboxLabel: openableUri.scheme === Schemas.file ? localize(176, null) : localize(177, null),
            cancelId: 1
        });
        if (response !== 0) {
    ...
    async handleProtocolUrl(windowsMainService, dialogMainService, urlService, uri, options) {
        ...
        if (process.isEmbeddedApp) {
            ...
            await windowsMainService.openSessionsWindow({ context: 6 /* OpenContext.LINK */, contextWindowId: undefined });
            return true;
        }
