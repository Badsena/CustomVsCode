...
class CodeMain {
    ...
    async startup() {
        ...
        try {
            ...
            await instantiationService.invokeFunction(async (accessor) => {
                ...
                bufferLogger.logger = loggerService.createLogger('main', { name: localize(178, null) });
                // Lifecycle
    ...
    async claimInstance(logService, environmentMainService, lifecycleMainService, instantiationService, productService, retry) {
        ...
        try {
            ...
            try {
                ...
                if (!retry || isWindows || error.code !== 'ECONNREFUSED') {
                    if (error.code === 'EPERM') {
                        this.showStartupWarningDialog(localize(179, null, productService.nameShort), localize(180, null), productService);
                    }
                    throw error;
                }
            ...
            if (!environmentMainService.args.wait && !environmentMainService.args.status) {
                startupWarningDialogHandle = setTimeout(() => {
                    this.showStartupWarningDialog(localize(181, null, productService.nameShort), localize(182, null), productService);
                }, 10000);
            }
        ...
        if (environmentMainService.args.status) {
            console.log(localize(183, null, productService.nameShort));
    ...
    handleStartupDataDirError(environmentMainService, productService, error) {
        if (error.code === 'EACCES' || error.code === 'EPERM') {
            ...
            this.showStartupWarningDialog(localize(184, null), localize(185, null, toErrorMessage(error), directories.join('\n')), productService);
        }
    }
