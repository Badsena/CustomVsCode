...
const accessibleDiffViewerInsertIcon = registerIcon('diff-review-insert', Codicon.add, localize(228, null));
const accessibleDiffViewerRemoveIcon = registerIcon('diff-review-remove', Codicon.remove, localize(229, null));
const accessibleDiffViewerCloseIcon = registerIcon('diff-review-close', Codicon.close, localize(230, null));
...
let View = class View extends Disposable {
    ...
    _render(store) {
        ...
        container.setAttribute('aria-label', localize(232, null));
        ...
        for (const viewItem of group?.lines || []) {
            ...
            if (viewItem.type === LineType.Header) {
                ...
                const getAriaLines = (lines) => lines === 0 ? localize(233, null)
                    : lines === 1 ? localize(234, null)
                        : localize(235, null, lines);
                ...
                header.setAttribute('aria-label', localize(236, null, (diffIndex + 1), diffsLength, r.original.startLineNumber, originalChangedLinesCntAria, r.modified.startLineNumber, modifiedChangedLinesCntAria));
    ...
    _createRow(item, lineHeight, width, originalOptions, originalModel, originalModelOpts, modifiedOptions, modifiedModel, modifiedModelOpts) {
        ...
        if (lineContent.length === 0) {
            lineContent = localize(237, null);
        }
        let ariaLabel = '';
        switch (item.type) {
            case LineType.Unchanged:
                if (item.originalLineNumber === item.modifiedLineNumber) {
                    ariaLabel = localize(238, null, lineContent, item.originalLineNumber);
                }
                else {
                    ariaLabel = localize(239, null, lineContent, item.originalLineNumber, item.modifiedLineNumber);
                }
                break;
            case LineType.Added:
                ariaLabel = localize(240, null, lineContent, item.modifiedLineNumber);
                break;
            case LineType.Deleted:
                ariaLabel = localize(241, null, lineContent, item.originalLineNumber);
                break;
        }
