...
suite('TextAreaInput', () => {
    ...
    async function simulateInteraction(recorded) {
        ...
        const wrapper = disposables.add(new class extends Disposable {
            ...
            getIgnoreSelectionChangeTime() { return Date.now(); }
    ...
    function interpretTypeEvents(OS, browser, initialState, events) {
        ...
        for (const event of events) {
            if (event.type === 'type') {
                text = (text.substring(0, selectionStart - event.replacePrevCharCnt)
                    + event.text
        ...
        return {
            ...
            selectionDirection: (browser.isFirefox || OS === 1 /* OperatingSystem.Windows */ || OS === 3 /* OperatingSystem.Linux */) ? 'forward' : 'none'
        };
    }
    test('macOS - Chrome - Korean using 2-Set Korean (1)', async () => {
        // macOS, 2-Set Korean, type 'dkrk' and click
        const recorded = {
            env: { OS: 2 /* OperatingSystem.Macintosh */, browser: { isAndroid: false, isFirefox: false, isChrome: true, isSafari: false } },
    ...
    test('macOS - Firefox - long press with mouse', async () => {
        ...
        const recorded = {
            env: { OS: 2 /* OperatingSystem.Macintosh */, browser: { isAndroid: false, isFirefox: true, isChrome: false, isSafari: false } },
    ...
    test('macOS - Safari - Chinese - issue #119469', async () => {
        const recorded = {
            env: { 'OS': 2 /* OperatingSystem.Macintosh */, 'browser': { 'isAndroid': false, 'isFirefox': false, 'isChrome': false, 'isSafari': true } },
            initial: { value: 'aaaa', selectionStart: 2, selectionEnd: 2, selectionDirection: 'none' },
            events: [
    ...
    test('Windows - Chrome - Japanese using Hiragana', async () => {
        ...
        const recorded = {
            env: { OS: 1 /* OperatingSystem.Windows */, browser: { isAndroid: false, isFirefox: false, isChrome: true, isSafari: false } },
            initial: { value: 'aaaa', selectionStart: 2, selectionEnd: 2, selectionDirection: 'forward' },
            events: [
    ...
    test('Linux - Chrome - Korean', async () => {
        ...
        const recorded = {
            env: { OS: 3 /* OperatingSystem.Linux */, browser: { isAndroid: false, isFirefox: false, isChrome: true, isSafari: false } },
            initial: { value: 'aaaa', selectionStart: 2, selectionEnd: 2, selectionDirection: 'forward' },
            events: [
