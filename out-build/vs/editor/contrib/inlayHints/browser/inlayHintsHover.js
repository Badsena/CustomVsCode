    ...
    computeAsync(anchor, _lineDecorations, source, token) {
        ...
        return new AsyncIterableProducer(async (executor) => {
            ...
            if (isNonEmptyArray(part.item.hint.textEdits)) {
                executor.emitOne(new MarkdownHover(this, anchor.range, [new MarkdownString().appendText(localize(1362, null))], false, 10001));
            }
            ...
            if (part.part.location || part.part.command) {
                ...
                const kb = useMetaKey
                    ...
                    : platform.isMacintosh
                        ...
                        : localize(1366, null);
                if (part.part.location && part.part.command) {
                    linkHint = new MarkdownString().appendText(localize(1367, null, kb));
                }
                else if (part.part.location) {
                    linkHint = new MarkdownString().appendText(localize(1368, null, kb));
                }
                else if (part.part.command) {
                    linkHint = new MarkdownString(`[${localize(1369, null)}](${asCommandLink(part.part.command)} "${part.part.command.title}") (${kb})`, { isTrusted: true });
                }
                if (linkHint) {
