    ...
    openLinkOccurrence(occurrence, openToSide, fromUserGesture = false) {
        ...
        link.resolve(CancellationToken.None).then(uri => {
            ...
            if (messageOrError === 'invalid') {
                this.notificationService.warn(nls.localize(1480, null, link.url.toString()));
            }
            else if (messageOrError === 'missing') {
                this.notificationService.warn(nls.localize(1481, null));
            }
            else {
                onUnexpectedError(err);
            }
...
function getHoverMessage(link, useMetaKey) {
    ...
    const label = link.tooltip
        ? link.tooltip
        : executeCmd
            ? nls.localize(1482, null)
            : nls.localize(1483, null);
    const kb = useMetaKey
        ? platform.isMacintosh
            ? nls.localize(1484, null)
            : nls.localize(1485, null)
        : platform.isMacintosh
            ? nls.localize(1486, null)
            : nls.localize(1487, null);
    if (link.url) {
        let nativeLabel = '';
        if (/^command:/i.test(link.url.toString())) {
            ...
            if (match) {
                ...
                nativeLabel = nls.localize(1488, null, commandId);
            }
        }
...
class OpenLinkAction extends EditorAction {
    constructor() {
        super({
            ...
            label: nls.localize2(1489, "Open Link"),
