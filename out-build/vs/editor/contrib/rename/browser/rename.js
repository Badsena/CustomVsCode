...
class RenameSkeleton {
    ...
    async _provideRenameEdits(newName, i, rejects, token) {
        ...
        if (!result) {
            return this._provideRenameEdits(newName, i + 1, rejects.concat(nls.localize(1586, null)), token);
        }
    ...
    async run() {
        ...
        try {
            ...
            if (e instanceof CancellationError) {
                ...
                if (typeof e === 'string' || isMarkdownString(e)) {
                    MessageController.get(this.editor)?.showMessage(e || nls.localize(1587, null), position);
                }
            }
            return undefined;
        }
        ...
        const renameOperation = raceCancellation(skeleton.provideRenameEdits(inputFieldResult.newName, cts2.token), cts2.token).then(async (renameResult) => {
            ...
            this._bulkEditService.apply(renameResult, {
                ...
                label: nls.localize(1588, null, loc?.text, inputFieldResult.newName),
                code: 'undoredo.rename',
                quotableLabel: nls.localize(1589, null, loc?.text, inputFieldResult.newName),
                respectAutoSaveConfig: true,
                reason: EditSources.rename(loc?.text, inputFieldResult.newName),
            }).then(result => {
                trace('edits applied');
                if (result.ariaSummary) {
                    alert(nls.localize(1590, null, loc.text, inputFieldResult.newName, result.ariaSummary));
                }
            }).catch(err => {
                trace(`error when applying edits ${JSON.stringify(err, null, '\t')}`);
                this._notificationService.error(nls.localize(1591, null));
                this._logService.error(err);
            });
        }, err => {
            trace('error when providing rename edits', JSON.stringify(err, null, '\t'));
            this._notificationService.error(nls.localize(1592, null));
            this._logService.error(err);
        }).finally(() => {
            cts2.dispose();
        });
...
export class RenameAction extends EditorAction {
    constructor() {
        super({
            ...
            label: nls.localize2(1594, "Rename Symbol"),
...
registerAction2(class FocusNextRenameSuggestion extends Action2 {
    constructor() {
        super({
            ...
            title: {
                ...nls.localize2(1595, "Focus Next Rename Suggestion"),
            },
            precondition: CONTEXT_RENAME_INPUT_VISIBLE,
            keybinding: [
                {
...
registerAction2(class FocusPreviousRenameSuggestion extends Action2 {
    constructor() {
        super({
            ...
            title: {
                ...nls.localize2(1596, "Focus Previous Rename Suggestion"),
            },
            precondition: CONTEXT_RENAME_INPUT_VISIBLE,
            keybinding: [
                {
...
Registry.as(Extensions.Configuration).registerConfiguration({
    ...
    properties: {
        'editor.rename.enablePreview': {
            ...
            description: nls.localize(1593, null),
            default: true,
            type: 'boolean'
        }
    }
