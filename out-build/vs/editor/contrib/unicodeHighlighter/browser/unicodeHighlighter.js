...
export const warningIcon = registerIcon('extensions-warning-message', Codicon.warning, nls.localize(1742, null));
        ...
        this._updateState = (state) => {
            if (state && state.hasMore) {
                ...
                if (state.nonBasicAsciiCharacterCount >= max) {
                    data = {
                        message: nls.localize(1743, null),
                        command: new DisableHighlightingOfNonBasicAsciiCharactersAction(),
                    };
                }
                else if (state.ambiguousCharacterCount >= max) {
                    data = {
                        message: nls.localize(1744, null),
                        command: new DisableHighlightingOfAmbiguousCharactersAction(),
                    };
                }
                else if (state.invisibleCharacterCount >= max) {
                    data = {
                        message: nls.localize(1745, null),
...
const configureUnicodeHighlightOptionsStr = nls.localize(1746, null);
let UnicodeHighlighterHoverParticipant = class UnicodeHighlighterHoverParticipant {
    ...
    computeSync(anchor, lineDecorations) {
        ...
        for (const d of lineDecorations) {
            ...
            switch (highlightInfo.reason.kind) {
                case 0 /* UnicodeHighlighterReasonKind.Ambiguous */: {
                    if (isBasicASCII(highlightInfo.reason.confusableWith)) {
                        reason = nls.localize(1747, null, codePointStr, formatCodePointMarkdown(highlightInfo.reason.confusableWith.codePointAt(0)));
                    }
                    else {
                        reason = nls.localize(1748, null, codePointStr, formatCodePointMarkdown(highlightInfo.reason.confusableWith.codePointAt(0)));
                    }
                    break;
                }
                case 1 /* UnicodeHighlighterReasonKind.Invisible */:
                    reason = nls.localize(1749, null, codePointStr);
                    break;
                case 2 /* UnicodeHighlighterReasonKind.NonBasicAscii */:
                    reason = nls.localize(1750, null, codePointStr);
                    break;
            }
            ...
            const adjustSettings = nls.localize(1751, null);
...
export class DisableHighlightingInCommentsAction extends EditorAction {
    ...
    constructor() {
        super({
            ...
            label: nls.localize2(1760, "Disable highlighting of characters in comments"),
            precondition: undefined
        });
        this.shortLabel = nls.localize(1752, null);
    }
...
export class DisableHighlightingInStringsAction extends EditorAction {
    ...
    constructor() {
        super({
            id: DisableHighlightingOfAmbiguousCharactersAction.ID,
            label: nls.localize2(1761, "Disable highlighting of characters in strings"),
            precondition: undefined
        });
        this.shortLabel = nls.localize(1753, null);
    }
    async run(accessor, editor) {
        const configurationService = accessor.get(IConfigurationService);
        if (configurationService) {
...
export class DisableHighlightingOfAmbiguousCharactersAction extends Action2 {
    ...
    constructor() {
        super({
            id: DisableHighlightingOfAmbiguousCharactersAction.ID,
            title: nls.localize2(1762, "Disable highlighting of ambiguous characters"),
            precondition: undefined,
            f1: false,
        });
        this.shortLabel = nls.localize(1754, null);
    }
    async run(accessor, editor) {
        const configurationService = accessor.get(IConfigurationService);
        if (configurationService) {
...
export class DisableHighlightingOfInvisibleCharactersAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: nls.localize2(1763, "Disable highlighting of invisible characters"),
            precondition: undefined,
            f1: false,
        });
        this.shortLabel = nls.localize(1755, null);
    }
    async run(accessor, editor) {
        const configurationService = accessor.get(IConfigurationService);
        if (configurationService) {
...
export class DisableHighlightingOfNonBasicAsciiCharactersAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: nls.localize2(1764, "Disable highlighting of non basic ASCII characters"),
            precondition: undefined,
            f1: false,
        });
        this.shortLabel = nls.localize(1756, null);
    }
    async run(accessor, editor) {
        const configurationService = accessor.get(IConfigurationService);
        if (configurationService) {
...
export class ShowExcludeOptions extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: nls.localize2(1765, "Show Exclude Options"),
            precondition: undefined,
            f1: false,
        });
    }
    async run(accessor, args) {
        ...
        function getExcludeCharFromBeingHighlightedLabel(codePoint) {
            if (InvisibleCharacters.isInvisibleCharacter(codePoint)) {
                return nls.localize(1757, null, codePointToHex(codePoint));
            }
            return nls.localize(1758, null, `${codePointToHex(codePoint)} "${char}"`);
        }
        const options = [];
        if (reason.kind === 0 /* UnicodeHighlighterReasonKind.Ambiguous */) {
            for (const locale of reason.notAmbiguousInLocales) {
                options.push({
                    label: nls.localize(1759, null, locale),
                    run: async () => {
