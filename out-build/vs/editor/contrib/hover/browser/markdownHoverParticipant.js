...
const $ = dom.$;
const increaseHoverVerbosityIcon = registerIcon('hover-increase-verbosity', Codicon.addSmall, nls.localize(1330, null));
const decreaseHoverVerbosityIcon = registerIcon('hover-decrease-verbosity', Codicon.removeSmall, nls.localize(1331, null));
...
let MarkdownHoverParticipant = class MarkdownHoverParticipant {
    ...
    createLoadingMessage(anchor) {
        return new MarkdownHover(this, anchor.range, [new MarkdownString().appendText(nls.localize(1332, null))], false, 2000);
    }
    computeSync(anchor, lineDecorations) {
        ...
        if (stopRenderingLineAfter >= 0 && lineLength > stopRenderingLineAfter && anchor.range.startColumn >= stopRenderingLineAfter) {
            ...
            if (showLongLineWarning) {
                result.push(new MarkdownHover(this, anchor.range, [{
                        value: nls.localize(1333, null, HIDE_LONG_LINE_WARNING_HOVER_ACTION_ID),
                        isTrusted: true
                    }], false, index++));
            }
        }
        if (!stopRenderingMessage && typeof maxTokenizationLineLength === 'number' && lineLength >= maxTokenizationLineLength) {
            if (showLongLineWarning) {
                result.push(new MarkdownHover(this, anchor.range, [{
                        value: nls.localize(1334, null, HIDE_LONG_LINE_WARNING_HOVER_ACTION_ID),
                        isTrusted: true
                    }], false, index++));
            }
        }
...
export function labelForHoverVerbosityAction(keybindingService, action) {
    switch (action) {
        case HoverVerbosityAction.Increase:
            return keybindingService.appendKeybinding(nls.localize(1335, null), INCREASE_HOVER_VERBOSITY_ACTION_ID);
        case HoverVerbosityAction.Decrease:
            return keybindingService.appendKeybinding(nls.localize(1336, null), DECREASE_HOVER_VERBOSITY_ACTION_ID);
    }
}
