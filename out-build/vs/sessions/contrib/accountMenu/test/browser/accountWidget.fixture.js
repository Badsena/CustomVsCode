...
export default defineThemedFixtureGroup({ path: 'sessions/' }, {
    LoadingSignedOutNoUpdate: defineComponentFixture({
        ...
        render: ctx => renderAccountWidget(ctx, State.Idle(0 /* UpdateType.Setup */), new Promise(() => { })),
    }),
    SignedOutNoUpdate: defineComponentFixture({
        labels: { kind: 'screenshot' },
        render: ctx => renderAccountWidget(ctx, State.Idle(0 /* UpdateType.Setup */), Promise.resolve(null)),
    }),
    SignedInNoUpdate: defineComponentFixture({
        labels: { kind: 'screenshot' },
        render: ctx => renderAccountWidget(ctx, State.Idle(0 /* UpdateType.Setup */), Promise.resolve(signedInAccount)),
    }),
