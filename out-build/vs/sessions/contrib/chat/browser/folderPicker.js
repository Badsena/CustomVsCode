...
let FolderPicker = class FolderPicker extends Disposable {
    get selectedFolderUri() {
        return this._selectedFolderUri;
    }
    constructor(_options, actionWidgetService, storageService, fileDialogService, commandService) {
        super();
        ...
        const lastFolder = this.storageService.get(this._storageKeyLastFolder, 0 /* StorageScope.PROFILE */);
        if (lastFolder) {
            try {
        ...
        try {
            const stored = this.storageService.get(this._storageKeyRecentFolders, 0 /* StorageScope.PROFILE */);
            if (stored) {
                this._recentlyPickedFolders = JSON.parse(stored)
                    .map((s) => URI.parse(s))
                    .filter((u) => allowedSchemes.has(u.scheme));
            }
        }
        catch { /* ignore */ }
    }
    ...
    showPicker() {
        ...
        const listOptions = showFilter ? { showFilter: true, filterPlaceholder: localize(3131, null) } : undefined;
        this.actionWidgetService.show('folderPicker', false, items, delegate, this._triggerElement, undefined, [], {
            getAriaLabel: (item) => item.label ?? '',
            getWidgetAriaLabel: () => localize(3132, null),
        }, listOptions);
    }
    /**
    ...
    _selectFolder(folderUri) {
        ...
        this.storageService.store(this._storageKeyLastFolder, folderUri.toString(), 0 /* StorageScope.PROFILE */, 1 /* StorageTarget.MACHINE */);
        this._updateTriggerLabel(this._triggerElement);
    ...
    async _cloneRepository() {
        try {
            const clonedPath = await this.commandService.executeCommand('git.clone', undefined, undefined, { postCloneAction: 'none' });
            if (clonedPath) {
    ...
    _addToRecentlyPickedFolders(folderUri) {
        ...
        this.storageService.store(this._storageKeyRecentFolders, JSON.stringify(this._recentlyPickedFolders.map(f => f.toString())), 0 /* StorageScope.PROFILE */, 1 /* StorageTarget.MACHINE */);
    }
    _buildItems(currentFolderUri) {
        ...
        items.push({
            kind: "action" /* ActionListItemKind.Action */,
            label: localize(3134, null),
            group: { title: '', icon: Codicon.search },
            item: { uri: URI.from({ scheme: 'command', path: 'browse' }), label: localize(3135, null) },
        });
        if (!this._options?.availableFileSystems?.length) {
            items.push({
                kind: "action" /* ActionListItemKind.Action */,
                label: localize(3136, null),
                group: { title: '', icon: Codicon.repoClone },
                item: { uri: URI.from({ scheme: 'command', path: 'clone' }), label: localize(3137, null) },
            });
        }
        return items;
    }
    ...
    removeFromRecents(folderUri) {
        ...
        if (this._selectedFolderUri && isEqual(this._selectedFolderUri, folderUri)) {
            this._selectedFolderUri = undefined;
            this.storageService.remove(this._storageKeyLastFolder, 0 /* StorageScope.PROFILE */);
            this._updateTriggerLabel(this._triggerElement);
        }
    }
    ...
    _updateTriggerLabel(trigger) {
        ...
        const label = folderUri ? basename(folderUri) : localize(3138, null);
