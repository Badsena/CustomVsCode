...
class UnifiedAICustomizationDataSource {
    ...
    createGroupItem(promptType, storage, count) {
        const storageLabels = {
            [PromptsStorage.local]: localize(3070, null, count),
            [PromptsStorage.user]: localize(3071, null, count),
            [PromptsStorage.extension]: localize(3072, null, count),
            [PromptsStorage.plugin]: localize(3073, null, count),
            [BUILTIN_STORAGE]: localize(3074, null, count),
        };
        const storageIcons = {
            ...
            accessibilityProvider: {
                getAriaLabel: (element) => {
                    ...
                    const nameAndDesc = element.description
                        ? localize(3075, null, element.name, element.description)
                        : element.name;
                    return element.disabled
                        ? localize(3076, null, nameAndDesc)
                        : nameAndDesc;
                },
                getWidgetAriaLabel: () => localize(3077, null),
            },
