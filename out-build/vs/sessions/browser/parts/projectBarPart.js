    ...
    createWorkspaceEntry(container, entry, index) {
        ...
        entryDisposables.add(addDisposableListener(entryElement, EventType.CONTEXT_MENU, (e) => {
            ...
            this.contextMenuService.showContextMenu({
                getAnchor: () => event,
                getActions: () => [
                    new Action('projectbar.customize', localize(2945, null), undefined, true, () => this.showCustomizeQuickPick(index)),
                    new Separator(),
                    new Action('projectbar.removeFolder', localize(2946, null), undefined, true, () => this.removeFolderEntry(index))
                ]
            });
        }));
    }
        ...
        const items = [
            {
                ...
                description: localize(2948, null)
            },
            {
                customType: 'icon',
                label: localize(2949, null),
                description: localize(2950, null)
            }
        ];
        const picked = await this.quickInputService.pick(items, {
            placeHolder: localize(2951, null),
            title: localize(2952, null)
        });
        if (!picked) {
            return;
        }
